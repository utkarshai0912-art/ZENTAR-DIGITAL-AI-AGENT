"""
Lazy dependency installer for opt-in ZENTAR DIGITAL AI AGENT backends.

Many Zentar features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``zentar-digital-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``zentar tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ZENTAR_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/zentar`` read-only), setting
  ``ZENTAR_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Zentar, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (ZENTAR_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (zentar trace upload / /upload-trace).
...
_LAZY_TARGET_ENV = "ZENTAR_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ZENTAR_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ZENTAR_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``zentar update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Zentar under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``zentar update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="zentar-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``zentar_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = zentar_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``zentar update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``zentar update``. Never raises; lazy-install failures
