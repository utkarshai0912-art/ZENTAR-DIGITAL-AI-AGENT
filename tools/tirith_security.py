...
it is automatically downloaded from GitHub releases to $ZENTAR_HOME/bin/tirith.
...
def _get_zentar_home() -> str:
    """Return the Zentar home directory, respecting ZENTAR_HOME env var."""
    return str(get_zentar_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zentar_home(), ".tirith-install-failed")
...
def _zentar_bin_dir() -> str:
    """Return $ZENTAR_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zentar_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZENTAR_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zentar_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZENTAR_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZENTAR_HOME/bin/tirith
    ...
    zentar_bin = os.path.join(_zentar_bin_dir(), "tirith")
    if os.path.isfile(zentar_bin) and os.access(zentar_bin, os.X_OK):
        _resolved_path = zentar_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zentar_bin
