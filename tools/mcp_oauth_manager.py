...
`mcp_tool.py`, and `zentar_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ZentarMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_zentar_provider_class() -> Optional[type]:
    ...
    class ZentarMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._zentar_server_name = server_name
            ...
            self._zentar_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ZentarTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ZentarTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._zentar_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ZentarTokenStorage
                        if isinstance(storage, ZentarTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._zentar_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ZentarTokenStorage
            if not isinstance(storage, ZentarTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``zentar mcp reauth``.
            ...
            back to ``zentar mcp reauth``.
            """
            try:
                if self._zentar_preregistered:
                    return
    ...
    return ZentarMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ZENTAR_PROVIDER_CLS: Optional[type] = _make_zentar_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ZentarMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _ZENTAR_PROVIDER_CLS is None:
            logger.warning(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `zentar mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``zentar mcp remove <name>`` and (indirectly) by
        ``zentar mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
