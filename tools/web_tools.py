...
Backend is selected during ``zentar tools`` setup (web.backend in config.yaml).
When available, Zentar can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Zentar config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Zentar' config/.env layer (``zentar config set``, ``zentar tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.zentar/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``zentar tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `zentar tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_zentar_dir("cache/web", "web_cache")
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `zentar plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `zentar tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`zentar plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from zentar_cli.config import get_env_value as _gev
