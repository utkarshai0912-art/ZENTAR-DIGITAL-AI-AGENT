...
* Never raises. Any I/O error (disk full, permission denied, missing
  ZENTAR_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_zentar_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: ZENTAR_HOME env var, then ~/.zentar
            home = os.environ.get("ZENTAR_HOME") or os.path.expanduser("~/.zentar")
