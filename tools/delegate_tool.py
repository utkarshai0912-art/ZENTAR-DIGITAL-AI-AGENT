...
# Must match zentar_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``zentar-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. zentar-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.zentar/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_zentar_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("ZENTAR_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("ZENTAR_UI_SESSION_ID", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'zentar auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Zentar config.

    Prefer the shared persistent loader because it follows the active
    ZENTAR_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ZENTAR_IGNORE_USER_CONFIG=1`` (``zentar chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ZENTAR_IGNORE_USER_CONFIG") == "1"
    if not prefer_legacy:
        try:
            from zentar_cli.config import load_config_readonly
