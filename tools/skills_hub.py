#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Zentar Skills Hub.
...
Used by zentar_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _zentar_home() -> Path:
    return get_zentar_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _zentar_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zentar_meta = metadata.get("zentar", {})
            if isinstance(zentar_meta, dict):
                tags = zentar_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by ZENTAR DIGITAL) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.zentar/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/zentar-digital-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                zentar_meta = meta_block.get("zentar", {})
                if isinstance(zentar_meta, dict):
                    tags = zentar_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Zentar centralized index source
# ---------------------------------------------------------------------------

ZENTAR_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
ZENTAR_INDEX_TTL = 6 * 3600  # 6 hours


def _zentar_index_cache_file() -> Path:
    return _index_cache_dir() / "zentar-index.json"


def _load_zentar_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZENTAR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    zentar_index_cache_file = _zentar_index_cache_file()
    if zentar_index_cache_file.exists():
        try:
            age = time.time() - zentar_index_cache_file.stat().st_mtime
            if age < ZENTAR_INDEX_TTL:
                return json.loads(zentar_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Zentar index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Zentar index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Zentar index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        zentar_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        zentar_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZentarIndexSource(SkillSource):
    """Skill source backed by the centralized Zentar Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_zentar_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zentar-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zentar-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZentarIndexSource(auth=auth), # Centralized index (search + resolved install paths)
