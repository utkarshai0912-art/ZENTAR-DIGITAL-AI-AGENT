...
One ``CDPSupervisor`` runs per Zentar ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Zentar logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "zentar-dialog-bridge.invalid"
...
(() => {
  if (window.__zentarDialogBridgeInstalled) return;
  window.__zentarDialogBridgeInstalled = true;
  const ENDPOINT = "http://zentar-dialog-bridge.invalid/";
