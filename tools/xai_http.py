...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``zentar tools`` repaint, tool-registration scans,
    ...
    2. ``~/.zentar/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_zentar_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.zentar/.env`` first, then ``os.environ``.

    Wraps :func:`zentar_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from zentar_cli.config import get_env_value as _zentar_get_env_value

        value = _zentar_get_env_value(name)
...
def zentar_xai_user_agent() -> str:
    """Return a stable Zentar-specific User-Agent for xAI HTTP calls."""
    try:
        from zentar_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Zentar-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Zentar config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Zentar home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_zentar_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Zentar-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``zentar_cli.config.get_env_value`` so keys
    stored in ``~/.zentar/.env`` (the standard Zentar location) are honored —
