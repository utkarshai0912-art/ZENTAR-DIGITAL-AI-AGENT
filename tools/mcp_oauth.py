...
This module provides the glue:
    - ``ZentarTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "ZENTAR DIGITAL AI AGENT"
"""
...
from zentar_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__zentar_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ZENTAR_HOME so each profile gets its own OAuth tokens.
    Layout: ``ZENTAR_HOME/mcp-tokens/``
    """
    try:
        from zentar_constants import get_zentar_home
        base = Path(get_zentar_home())
    except ImportError:
        base = Path(os.environ.get("ZENTAR_HOME", str(Path.home() / ".zentar")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``zentar mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `zentar mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# ZentarTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZentarTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZENTAR_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZENTAR_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ZENTAR_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Zentar records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Zentar.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `zentar mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ZentarTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "ZENTAR DIGITAL AI AGENT")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`zentar mcp login {server_name}` interactively first to complete "
