"""
Cron job management tools for ZENTAR DIGITAL AI AGENT.
...
#      saying "cat ~/.zentar/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `zentar-digital-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.zentar/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("ZENTAR_SESSION_PLATFORM")
    origin_chat_id = get_session_env("ZENTAR_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("ZENTAR_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes ZENTAR_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``ZENTAR_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via zentar model.
    ...
    if provider_name == "custom":
        try:
            from zentar_cli.runtime_provider import has_named_custom_provider
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within ZENTAR_HOME/scripts/.
    ...
    # Only relative paths within ~/.zentar/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.zentar/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.zentar/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_zentar_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_zentar_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("ZENTAR_INTERACTIVE")
        or env_var_enabled("ZENTAR_GATEWAY_SESSION")
        or env_var_enabled("ZENTAR_EXEC_ASK")
    )
