...
provider** — turn on a backend by enabling a plugin (``zentar plugins
enable video_gen/<name>``) and selecting it in ``zentar tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/zentar-digital-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `zentar tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `zentar tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `zentar tools` → "
...
# `zentar tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`zentar tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `zentar tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
