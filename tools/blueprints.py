...
`zentar skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.zentar.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.zentar.blueprint`` is a mapping containing
    ...
    zentar = meta.get("zentar") if isinstance(meta, dict) else None
    blueprint = zentar.get("blueprint") if isinstance(zentar, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.zentar.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.zentar.blueprint`` block) they can hand
    to ``zentar skills publish`` to share. ``body`` is the plain-language
