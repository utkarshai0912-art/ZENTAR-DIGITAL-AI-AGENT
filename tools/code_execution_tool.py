...
Lets the LLM write a Python script that calls Zentar tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `zentar_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `zentar_tools.py` with file-based RPC stubs
...
# available on every platform Zentar itself runs on.
...
# HERMES_*-named config that lacks a secret substring (e.g. ZENTAR_BASE_URL,
# ZENTAR_KANBAN_DB, HERMES_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ZENTAR_CHILD_ALLOWED below; ZENTAR_RPC_SOCKET /
# ZENTAR_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational HERMES_* vars (_ZENTAR_CHILD_ALLOWED) pass by exact name.
    ...
    # config like ZENTAR_KANBAN_DB / ZENTAR_BASE_URL), but a sandbox script
...
# ---------------------------------------------------------------------------
# zentar_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Zentar-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Zentar-profile soft guard."""',
...
def generate_zentar_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the zentar_tools.py stub module.
...
"""Auto-generated Zentar tools RPC stubs."""
...
def _connect():
    ...
    ZENTAR_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ZENTAR_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ZENTAR_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Zentar tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ZENTAR_RPC_DIR") or os.path.join(tempfile.gettempdir(), "zentar_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated zentar_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/zentar_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_zentar_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/zentar_tools.py", tools_src)
        ...
        env_prefix = (
            f"ZENTAR_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"ZENTAR_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ZENTAR_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with zentar_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="zentar_sandbox_")
    ...
    # selector from ZENTAR_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"zentar_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated zentar_tools module.
        ...
        tools_src = generate_zentar_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "zentar_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ZENTAR_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ZENTAR_RPC_SOCKET"] = rpc_endpoint
        child_env["ZENTAR_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the zentar-digital-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from zentar_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _zentar_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _zentar_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ZENTAR_TIMEZONE is an internal Zentar setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ZENTAR_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ZENTAR_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.zentar/.env')).
...
def _load_config() -> dict:
    ...
    ``zentar --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (zentar-digital-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``zentar tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and zentar-digital-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.zentar/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Zentar tools programmatically. "
        ...
        f"Available via `from zentar_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into zentar_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from zentar_tools import {import_str}` "
