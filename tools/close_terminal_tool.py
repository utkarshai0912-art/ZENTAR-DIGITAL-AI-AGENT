#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Zentar desktop GUI.
...
``read_terminal`` it is gated on ``ZENTAR_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — ZENTAR_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("ZENTAR_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Zentar desktop GUI (the tabs mirroring terminal(background=true) runs). "
