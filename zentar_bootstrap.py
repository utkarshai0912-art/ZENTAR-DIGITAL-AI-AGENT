"""Windows UTF-8 bootstrap for Zentar entry points.
...
should be imported at the very top of every Zentar entry point
(``zentar``, ``zentar-digital-agent``, ``zentar-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Zentar modules.

    Zentar ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Zentar source root
    ...
    """
    root = src_root or os.environ.get("ZENTAR_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ZENTAR_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ZENTAR_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import zentar_bootstrap``
# (or ``from zentar_bootstrap import apply_windows_utf8_bootstrap``) at
