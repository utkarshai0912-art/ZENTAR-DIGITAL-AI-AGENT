...
interface ZentarConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ZentarConsoleModal({ open, onClose }: ZentarConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("zentar> ");
  const inputPromptRef = useRef("zentar> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "zentar> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Zentar Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="zentar-console-title"
    >
      <div
        className={cn(
          themedBody,
