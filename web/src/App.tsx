...
const SIDEBAR_COLLAPSED_KEY = "zentar-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="zentar-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="zentar-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateZentar = status?.can_update_hermes === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'zentar update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "zentar update";
    return (
      t.status.updateZentarConfirmMessage ??
      `This will run 'zentar update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateZentarConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateZentar) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateZentar,
      runningLabel: t.status.updatingZentar,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Zentar gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateZentarConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateZentarConfirmTitle ?? `${t.status.updateZentar}?`}
    />
    </>
  );
}
