import { buildZentarWebSocketUrl } from "@zentar/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/zentar/). The Python backend
// injects ``window.__ZENTAR_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ZENTAR_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ZENTAR_BASE_PATH = readBasePath();
const BASE = ZENTAR_BASE_PATH;
...
declare global {
  interface Window {
    __ZENTAR_SESSION_TOKEN__?: string;
    __ZENTAR_BASE_PATH__?: string;
    ...
    __ZENTAR_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Zentar-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ZENTAR_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``zentar update``, ``zentar gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ZENTAR_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ZENTAR_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("zentar.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("zentar.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ZENTAR_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("zentar.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``zentar_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ZENTAR_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Zentar-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``zentar_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ZENTAR_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildZentarWebSocketUrl({
...
export const api = {
  ...
  updateZentar: () =>
    fetchJSON<ActionResponse>("/api/zentar/update", { method: "POST" }),
  checkZentarUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/zentar/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** zentar-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``zentar update``. */
