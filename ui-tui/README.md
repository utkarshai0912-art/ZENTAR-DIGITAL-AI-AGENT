...
React + Ink terminal UI for Zentar. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZENTAR_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `zentar setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.zentar/.zentar_history` or under `ZENTAR_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `zentar setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/zentar-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external zentar setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.zentar/tui-parent.log
    ...
    types/
      zentar-ink.d.ts            type declarations for @zentar/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZentarCLI subprocess for slash commands
```