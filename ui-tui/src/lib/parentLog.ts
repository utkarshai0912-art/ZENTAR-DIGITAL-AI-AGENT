...
const logDir = join(process.env.ZENTAR_HOME?.trim() || join(homedir(), '.zentar'), 'logs')
...
// a real ~/.zentar (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('zentar-tui: parent lifecycle log unavailable\n')
    }
  }
}
