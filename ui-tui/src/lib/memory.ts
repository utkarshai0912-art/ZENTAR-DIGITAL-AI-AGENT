...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because ZENTAR_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.ZENTAR_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.zentar', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `zentar-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.ZENTAR_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.ZENTAR_HEAPDUMP_MAX_BYTES?.trim()
