...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ZENTAR_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Zentar on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ZENTAR_TUI_TRUECOLOR=1.
