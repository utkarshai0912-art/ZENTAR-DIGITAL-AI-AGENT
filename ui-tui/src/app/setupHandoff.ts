import type { RunExternalProcess } from '@zentar/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zentar ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching zentar: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zentar ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
