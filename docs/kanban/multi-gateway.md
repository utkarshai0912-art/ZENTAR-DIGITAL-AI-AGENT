...
Zentar supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.zentar/config.yaml`:
...
Or set the env var: `ZENTAR_KANBAN_DISPATCH_IN_GATEWAY=false`
