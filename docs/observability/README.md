# Zentar Observer Hooks

Zentar observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Zentar' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Zentar catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Zentar only builds
