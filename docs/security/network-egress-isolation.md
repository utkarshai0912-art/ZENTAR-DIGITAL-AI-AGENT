...
When running Zentar inside Docker, the default `network_mode: host` gives the
...
The Zentar [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ zentar-digital-agent │   │ zentar-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   ZENTAR_UID=$(id -u) ZENTAR_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=zentar,zentar-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://zentar-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Zentar trust model and vulnerability reporting
