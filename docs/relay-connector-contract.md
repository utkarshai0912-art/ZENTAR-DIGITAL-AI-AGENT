...
This document is the formal interface between the **Zentar gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `zentar gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`zentar gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `zentar` CLI.
