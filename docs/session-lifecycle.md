...
The canonical transcript store is SQLite via `SessionDB` (from `zentar_state`). The
...
Counts consecutive restarts via a JSON file (`{ZENTAR_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `zentar update`, `zentar gateway restart`,
or `/restart`.
