...
Zentar middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Zentar'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Zentar calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Zentar retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Zentar guardrails,
...
- `telemetry_schema_version`: currently `zentar.observer.v1`
- `middleware_schema_version`: currently `zentar.middleware.v1`
...
Zentar stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Zentar runs
them as a nested chain in registration order. Middleware failures are fail-open:
Zentar logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Zentar applies middleware in this order:
...
For each tool call, Zentar applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Zentar pre-execution path against the effective arguments:
...
```bash
zentar plugins enable <plugin-name>
```

For isolated local testing, use one `ZENTAR_HOME` for plugin enablement and the
agent run:

```bash
export ZENTAR_HOME=/tmp/zentar-middleware-test
mkdir -p "$ZENTAR_HOME"
zentar plugins enable <plugin-name>
zentar chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run zentar plugins enable <plugin-name>
uv run zentar chat --query 'Reply exactly ok'
```
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/zentar-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Zentar expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Zentar treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Zentar preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Zentar does not convert downstream
