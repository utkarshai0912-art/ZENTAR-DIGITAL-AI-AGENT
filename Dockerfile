...
# Do not write .pyc files at runtime: /opt/zentar is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/zentar/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when zentar
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# zentar process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main zentar process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Zentar WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via ZENTAR_UID at runtime
RUN useradd -u 10000 -m -d /opt/data zentar
...
#
# ui-tui/packages/zentar-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/zentar-ink/ ui-tui/packages/zentar-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as zentar-ink above).
...
# lock on the @zentar/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/zentar/.venv at first
...
# gives the non-root zentar user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link zentar-digital-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/zentar are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the zentar user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/zentar/bin && \
    cp /opt/zentar/docker/zentar-exec-shim.sh /opt/zentar/bin/zentar && \
    chmod 0755 /opt/zentar/bin/zentar && \
    printf 'docker\n' > /opt/zentar/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $ZENTAR_HOME. $ZENTAR_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``zentar update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the zentar user via
# `s6-setuidgid zentar` in its run script. If ZENTAR_UID is unset, services
# run as the default zentar user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `zentar dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the ZENTAR_GIT_SHA build-arg to
# /opt/zentar/.zentar_build_sha at build time, and have
# zentar_cli/build_info.py read it at runtime.  Both `zentar dump` and
...
ARG ZENTAR_GIT_SHA=
RUN if [ -n "${ZENTAR_GIT_SHA}" ]; then \
        printf '%s\n' "${ZENTAR_GIT_SHA}" > /opt/zentar/.zentar_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-zentar + dashboard.
...
# `exec zentar`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $ZENTAR_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/zentar/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-zentar-setup && \
    chmod +x /etc/cont-init.d/01-zentar-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV ZENTAR_WEB_DIST=/opt/zentar/zentar_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/zentar/ui-tui/dist/entry.js`) and skip the
...
ENV ZENTAR_TUI_DIR=/opt/zentar/ui-tui
ENV ZENTAR_HOME=/opt/data
ENV ZENTAR_WRITE_SAFE_ROOT=/opt/data
ENV ZENTAR_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/zentar (root-owned, read-only) so a runtime
...
# is seeded + chowned to the zentar user by docker/stage2-hook.sh and lives
...
ENV ZENTAR_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> zentar ...` they default to root, and any file the
# command writes under $ZENTAR_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/zentar/bin/zentar, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid zentar`
# when invoked as root. Non-root callers (supervised processes,
# `--user zentar`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/zentar/.venv/bin/zentar). See the shim source for
# the opt-out env var (ZENTAR_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> zentar ...` and any
# subprocess that doesn't activate the venv first still find zentar.
#
# /opt/zentar/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/zentar/bin:/opt/zentar/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. zentar
# subcommand vs. no-args), drops to the zentar user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/zentar/docker/main-wrapper.sh" ]
CMD [ ]
