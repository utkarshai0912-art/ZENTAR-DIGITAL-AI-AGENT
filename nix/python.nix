...
let
  ...
  editableOverlay = workspace.mkEditablePyprojectOverlay {
    root = "$ZENTAR_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        zentar-digital-agent = prev.zentar-digital-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "zentar-digital-agent-env" {
    zentar-digital-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "zentar-digital-agent-editable-env" {
    zentar-digital-agent = dependency-groups;
  };
}
