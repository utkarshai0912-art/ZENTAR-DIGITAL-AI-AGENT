# nix/desktop.nix — Zentar Desktop (Electron) app build + wrapper
#
# `zentarAgent` is the fully-built `.#default` package — it ships the
# `zentar` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ZENTAR_DESKTOP_HERMES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Zentar CLI").
...
let
  npm = zentarNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "zentar-desktop";
  };
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "zentar-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "zentar-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "zentar-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/zentar-desktop $out/bin
    cp -r ${renderer}/* $out/share/zentar-desktop/
    ...
    substituteInPlace $out/share/zentar-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/zentar-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ZENTAR_DESKTOP_HERMES to the absolute path of the nix-built `zentar`
    # binary so the desktop's resolver step 4 ("existing Zentar CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/zentar-desktop \
      --add-flags "$out/share/zentar-desktop" \
      --set ZENTAR_DESKTOP_HERMES "${lib.getExe zentarAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for ZENTAR DIGITAL AI AGENT";
    homepage = "https://github.com/NousResearch/hermes-digital-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "zentar-desktop";
  };
}
