# nix/nixosModules.nix — NixOS module for zentar-digital-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: zentar runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZENTAR_HOME/.env
# and read by zentar at startup — no container recreation needed for env changes.
#
# Tool resolution: the zentar wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.zentar-digital-agent;
    effectivePackage =
    ...
    zentar-digital-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-zentar-digital-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "zentar-config-attrs";
      description = "Zentar YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "zentar-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ZENTAR_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "zentar-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ZENTAR_DOC_EOF'\n${value}\nZENTAR_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "zentar-digital-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/zentar";
    ...
    # zentar user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "zentar-container-entrypoint" ''
      set -eu

      ZENTAR_UID="''${ZENTAR_UID:?ZENTAR_UID must be set}"
      ZENTAR_GID="''${ZENTAR_GID:?ZENTAR_GID must be set}"

      # ── Group: ensure a group with GID=$ZENTAR_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ZENTAR_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="zentar"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ZENTAR_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ZENTAR_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ZENTAR_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ZENTAR_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="zentar"
        TARGET_HOME="/home/zentar"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ZENTAR_UID" -g "$ZENTAR_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ZENTAR_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ZENTAR_UID:$ZENTAR_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ZENTAR_HOME is owned by the target user.
      ...
      if [ -n "''${ZENTAR_HOME:-}" ] && [ -d "$ZENTAR_HOME" ]; then
        find "$ZENTAR_HOME" \! -user "$ZENTAR_UID" -exec chown "$ZENTAR_UID:$ZENTAR_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/zentar-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/zentar-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/zentar ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/zentar
        chmod 0440 /etc/sudoers.d/zentar
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ZENTAR_UID" --regid="$ZENTAR_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ZENTAR_HOME/.env.
    ...
    # Default: /var/lib/zentar/workspace → /data/workspace.
  ...
  in {
    options.services.zentar-digital-agent = with lib; {
      enable = mkEnableOption "ZENTAR DIGITAL AI AGENT gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = zentar-digital-agent;
        description = "The zentar-digital-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/zentar";
        description = "State directory. Contains .zentar/ subdir (ZENTAR_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Zentar config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ZENTAR_HOME/.env at activation time.
          Zentar reads this file on every startup via load_zentar_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ZENTAR_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ZENTAR_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `zentar gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the zentar user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the zentar plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Zentar discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zentar_digital_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as zentar (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in zentar-digital-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the zentar CLI to environment.systemPackages and export
          ZENTAR_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.zentar symlink to the service
            stateDir. These users are automatically added to the zentar group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.zentar-digital-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the zentar CLI to system PATH and export ZENTAR_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.zentar/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ZENTAR_HOME = "${cfg.stateDir}/.zentar";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.zentar-digital-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the zentar user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.zentar-digital-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed zentar
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure zentar is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.zentar        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zentar/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zentar/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zentar/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zentar/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zentar/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."zentar-digital-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.zentar
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.zentar ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.zentar ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.zentar -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.zentar/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.zentar/$_subdir"
            chmod 2770 "${cfg.stateDir}/.zentar/$_subdir"
            find "${cfg.stateDir}/.zentar/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # zentar-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.zentar/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.zentar/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zentar/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.zentar/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.zentar/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zentar/.managed
          chmod 0644 ${cfg.stateDir}/.zentar/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.zentar/.container-mode <<'ZENTAR_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    zentar_bin=${containerDataDir}/current-package/bin/zentar
    ZENTAR_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zentar/.container-mode
            chmod 0644 ${cfg.stateDir}/.zentar/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.zentar/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zentar";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.zentar" ]; then
                  rm -f "${symlinkPath}"
                  echo "zentar-digital-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.zentar -> stateDir/.zentar for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zentar";
                target = "${cfg.stateDir}/.zentar";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "zentar-digital-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zentar/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.zentar/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zentar/auth.json
              fi
            ''}
          ''}
          ...
          # Zentar reads $ZENTAR_HOME/.env at startup via load_zentar_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.zentar/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ZENTAR_NIX_ENV_EOF'
    ${envFileContent}
    ZENTAR_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.zentar/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.zentar/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zentar/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.zentar-digital-agent = {
          description = "ZENTAR DIGITAL AI AGENT Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            ZENTAR_HOME = "${cfg.stateDir}/.zentar";
            ZENTAR_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ZENTAR_HOME/.env by the activation script. load_zentar_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/zentar"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the zentar group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.zentar-digital-agent = {
          description = "ZENTAR DIGITAL AI AGENT Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ZENTAR_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ZENTAR_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ZENTAR_UID="$ZENTAR_UID" \
                --env ZENTAR_GID="$ZENTAR_GID" \
                --env ZENTAR_HOME=${containerDataDir}/.zentar \
                --env ZENTAR_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/zentar gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
