...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      zentar-digital-agent = self'.packages.default;
      zentarVenv = zentar-digital-agent.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "zentar-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${zentarVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "zentar-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "zentar-build-package" { } ''
          echo "PASS: package built at ${zentar-digital-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "zentar-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "zentar-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${zentar-digital-agent}/bin/zentar || (echo "FAIL: zentar binary missing"; exit 1)
          test -x ${zentar-digital-agent}/bin/zentar-digital-agent || (echo "FAIL: zentar-digital-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${zentar-digital-agent}/bin/zentar version 2>&1 | grep -qi "zentar" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "zentar-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in zentar zentar-digital-agent zentar-acp; do
            test -x ${zentar-digital-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "zentar-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking zentar --help ==="
          ${zentar-digital-agent}/bin/zentar --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${zentar-digital-agent}/bin/zentar --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "zentar-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${zentar-digital-agent}/share/zentar-digital-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${zentar-digital-agent}/share/zentar-digital-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZENTAR_BUNDLED_SKILLS" ${zentar-digital-agent}/bin/zentar || \
            (echo "FAIL: ZENTAR_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZENTAR_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "zentar-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${zentar-digital-agent}/share/zentar-digital-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${zentar-digital-agent}/share/zentar-digital-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ZENTAR_BUNDLED_PLUGINS" ${zentar-digital-agent}/bin/zentar || \
            (echo "FAIL: ZENTAR_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ZENTAR_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "zentar-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${zentar-digital-agent}/share/zentar-digital-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${zentar-digital-agent}/share/zentar-digital-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${zentar-digital-agent}/share/zentar-digital-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ZENTAR_BUNDLED_LOCALES" ${zentar-digital-agent}/bin/zentar || \
            (echo "FAIL: ZENTAR_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ZENTAR_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ZENTAR_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ZENTAR_BUNDLED_LOCALES=${zentar-digital-agent}/share/zentar-digital-agent/locales \
            ${zentarVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ZENTAR_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${zentarVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${zentarVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ZENTAR_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "zentar-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${zentar-digital-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${zentar-digital-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ZENTAR_TUI_DIR" ${zentar-digital-agent}/bin/zentar || \
            (echo "FAIL: ZENTAR_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZENTAR_TUI_DIR set in wrapper"
        ...
        # Verify ZENTAR_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        zentar-node = pkgs.runCommand "zentar-node-version" { } ''
          set -e
          echo "=== Checking ZENTAR_NODE in wrapper ==="
          grep -q "ZENTAR_NODE" ${zentar-digital-agent}/bin/zentar || \
            (echo "FAIL: ZENTAR_NODE not set in wrapper"; exit 1)
          echo "PASS: ZENTAR_NODE present in wrapper"

          ZENTAR_NODE=$(sed -n "s/^export ZENTAR_NODE='\(.*\)'/\1/p" ${zentar-digital-agent}/bin/zentar)
          test -x "$ZENTAR_NODE" || (echo "FAIL: ZENTAR_NODE=$ZENTAR_NODE not executable"; exit 1)
          echo "PASS: ZENTAR_NODE executable at $ZENTAR_NODE"

          NODE_MAJOR=$("$ZENTAR_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ZENTAR_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZENTAR_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "zentar-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZENTAR_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZENTAR_MANAGED guards ==="
          check_blocked "config set" ${zentar-digital-agent}/bin/zentar config set model foo
          check_blocked "config edit" ${zentar-digital-agent}/bin/zentar config edit
        ...
        extra-python-packages = let
          ...
          zentarWithExtra = zentar-digital-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "zentar-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${zentarWithExtra}/bin/zentar || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${zentarWithExtra}/bin/zentar || \
          ...
          if grep -q "PYTHONPATH" ${zentar-digital-agent}/bin/zentar; then
        ...
        extra-dependency-groups = let
          zentarWithGroups = zentar-digital-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "zentar-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${zentarWithGroups}"
          echo "venv: ${zentarWithGroups.hermesVenv}"
        ...
        messaging-variant = pkgs.runCommand "zentar-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "zentar-config-roundtrip" {
          ...
          merge_and_load() {
            local zentar_home="$1"
            export ZENTAR_HOME="$zentar_home"
            ${configMergeScript} ${nixSettings} "$zentar_home/config.yaml"
            ${zentarVenv}/bin/python3 -c '
import json, sys
