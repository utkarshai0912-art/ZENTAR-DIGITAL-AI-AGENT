...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      zentarNpmLib = self'.packages.default.passthru.hermesNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. zentar-digital-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        packages =
          with pkgs;
          [
            (pkgs.runCommand "zentar" { } ''
              mkdir -p $out/bin
              install -Dm755 ${../zentar} $out/bin/zentar
            '')
            uv
          ]
        ...
        shellHook = ''
          ${combinedNonNpm}
          ${zentarNpmLib.mkNpmDevShellHook npmPackageJsonPaths}

          # for the devshell to pick up the src
          export ZENTAR_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          echo "ZENTAR DIGITAL AI AGENT dev shell in $ZENTAR_PYTHON_SRC_ROOT"
          echo "Ready. Run 'zentar' to start."
        '';
      };
    };
}
