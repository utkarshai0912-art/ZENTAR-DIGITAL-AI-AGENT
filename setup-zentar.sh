#!/bin/bash
# ============================================================================
# ZENTAR DIGITAL AI AGENT Setup Script
# ============================================================================
...
# 5. Symlinks the 'zentar' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ ZENTAR DIGITAL AI AGENT Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/zentar-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink zentar into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up zentar command..."

ZENTAR_BIN="$SCRIPT_DIR/venv/bin/zentar"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZENTAR_BIN" "$COMMAND_LINK_DIR/zentar"
echo -e "${GREEN}✓${NC} Symlinked zentar → $COMMAND_LINK_DISPLAY_DIR/zentar"
...
# ============================================================================
# Seed bundled skills into ~/.zentar/skills/
# ============================================================================

ZENTAR_SKILLS_DIR="${ZENTAR_HOME:-$HOME/.zentar}/skills"
mkdir -p "$ZENTAR_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zentar/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZENTAR_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  zentar status        # Check configuration"
if is_termux; then
    echo "  zentar gateway       # Run gateway in foreground"
else
    echo "  zentar gateway install # Install gateway service (messaging + cron)"
fi
echo "  zentar cron list     # View scheduled jobs"
echo "  zentar doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zentar_cli.main setup
fi
