# ZENTAR DIGITAL AI AGENT - Development Guide

Instructions for AI coding assistants and developers working on the zentar-digital-agent codebase.

**Never give up on the right solution.**

## What Zentar Is

Zentar is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Zentar ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`zentar tools`, `zentar setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ZENTAR_HOME`. Mocks hide integration bugs.
...
- **New `ZENTAR_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `zentar tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.zentar/plugins/` (or via a
  pip entry point), and promote them in the ZENTAR DIGITAL Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `zentar <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `zentar webhook`, `zentar cron`, `zentar tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.zentar/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.zentar/zentar-digital-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _ZENTAR_CORE_TOOLS list
├── cli.py                # ZentarCLI class — interactive CLI orchestrator (~11k LOC)
├── zentar_state.py       # SessionDB — SQLite session store (FTS5 search)
├── zentar_constants.py   # get_zentar_home(), display_zentar_home() — profile-aware paths
├── zentar_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── zentar_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
├── ui-tui/               # Ink (React) terminal UI — `zentar --tui`
...
**User config:** `~/.zentar/config.yaml` (settings), `~/.zentar/.env` (API keys only).
**Logs:** `~/.zentar/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_zentar_home()`.
Browse with `zentar logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Zentar: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`zentar_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `ZentarCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.zentar/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`zentar_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/zentar` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `zentar_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `ZentarCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `zentar --tui` or `ZENTAR_TUI=1`.
...
npm run dev       # watch mode (rebuilds zentar-ink + tsx --watch)
npm start         # production
npm run build     # full build (zentar-ink + tsc)
...
### TUI in the Dashboard (`zentar dashboard` → `/chat`)

The dashboard embeds the real `zentar --tui` — **not** a rewrite.  See `zentar_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `zentar_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `zentar --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `zentar --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@zentar/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `zentar serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `ZENTAR_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.cjs` + `backendSupportsServe()` in `main.cjs`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `zentar` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `zentar --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `zentar-desktop-app-work` skill, not `zentar-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Zentar built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Zentar core.
Use the plugin route instead: create `~/.zentar/plugins/<name>/plugin.yaml`
and `~/.zentar/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Zentar tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_ZENTAR_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_ZENTAR_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_zentar_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `ZENTAR_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_zentar_home()` for the base directory — never `Path.home() / ".zentar"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `zentar_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `zentar_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `zentar tools`, `zentar setup`, most CLI subcommands | `zentar_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`zentar_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
zentar_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.zentar/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Zentar gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `zentar_cli/skin_engine.py`:
...
Users create `~/.zentar/skins/<name>.yaml`:
...
Zentar has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.zentar/plugins/` and pip-installed entry points.

### General plugins (`zentar_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.zentar/plugins/`, `./.zentar/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `zentar` at startup so
  `zentar <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(zentar_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `zentar <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `zentar --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `zentar_cli/main.py`, etc.).
...
into `~/.zentar/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `zentar memory setup` / `post_setup()` without
...
`~/.zentar/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the ZENTAR DIGITAL Discord
...
2. User: `$ZENTAR_HOME/plugins/model-providers/<name>/`
...
[`zentar-example-plugins`](https://github.com/NousResearch/hermes-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `zentar skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.zentar.tags`, `metadata.zentar.category`,
`metadata.zentar.related_skills`, `metadata.zentar.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.zentar.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Zentar tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "ZENTAR DIGITAL AI AGENT" is the secondary collaborator. If the
   contributor's commit shows "ZENTAR DIGITAL AI AGENT" as author (because they
   used Zentar to draft the skill), replace it with their actual name
...
lives in the `zentar-digital-agent-dev` skill at
...
`"messaging"`); `_ZENTAR_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `zentar tools` (the curses UI) or the
...
Zentar returns a delegation id immediately and the result re-enters the
...
go to `~/.zentar/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `zentar_cli/curator.py` wires `zentar curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.zentar/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `zentar cron <verb>`
...
- File lock at `~/.zentar/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `zentar kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `zentar_cli/kanban.py` wires `zentar kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`zentar-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `ZENTAR_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Zentar-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `ZENTAR_BACKGROUND_NOTIFICATIONS` env var):
...
Zentar supports **profiles** — multiple fully isolated instances, each with its own
`ZENTAR_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `zentar_cli/main.py` sets
`ZENTAR_HOME` before any module imports. All `get_zentar_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_zentar_home()` for all ZENTAR_HOME paths.** Import from `zentar_constants`.
   NEVER hardcode `~/.zentar` or `Path.home() / ".zentar"` in code that reads/writes state.
   ```python
   # GOOD
   from zentar_constants import get_zentar_home
   config_path = get_zentar_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".zentar" / "config.yaml"
   ```

2. **Use `display_zentar_home()` for user-facing messages.** Import from `zentar_constants`.
   This returns `~/.zentar` for default or `~/.zentar/profiles/<name>` for profiles.
   ```python
   # GOOD
   from zentar_constants import display_zentar_home
   print(f"Config saved to {display_zentar_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.zentar/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_zentar_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_zentar_home()`,
   not `Path.home() / ".zentar"`.

4. **Tests that mock `Path.home()` must also set `ZENTAR_HOME`** — since code now uses
   `get_zentar_home()` (reads env var), not `Path.home() / ".zentar"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"ZENTAR_HOME": str(tmp_path / ".zentar")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not ZENTAR_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".zentar" / "profiles"`, NOT `get_zentar_home() / "profiles"`.
   This is intentional — it lets `zentar -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.zentar` paths
Use `get_zentar_home()` from `zentar_constants` for code paths. Use `display_zentar_home()`
for user-facing print/log messages. Hardcoding `~/.zentar` breaks profiles — each profile
has its own `ZENTAR_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `zentar_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `zentar_cli/curses_ui.py` — see
`zentar_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `ZENTAR_HOME`.

### Tests must not write to `~/.zentar/`
The `_isolate_zentar_home` autouse fixture in `tests/conftest.py` redirects `ZENTAR_HOME` to a temp dir. Never hardcode `~/.zentar/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_zentar_home()` resolve within the temp dir.
Use the pattern from `tests/zentar_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZENTAR_HOME", str(home))
    return home
```
...
| HOME / `~/.zentar/` | Your real config+auth.json                  | Temp dir per test                         |
