...
description: 通过 Home Assistant 集成，使用 ZENTAR DIGITAL AI AGENT 控制您的智能家居。
...
ZENTAR DIGITAL AI AGENT 通过以下两种方式与 [Home Assistant](https://www.home-assistant.io/) 集成：
...
3. 滚动至**长期访问令牌**
4. 点击**创建令牌**，命名为"ZENTAR DIGITAL AI AGENT"
5. 复制令牌
...
ZENTAR DIGITAL AI AGENT 注册了四个智能家居控制工具：
...
在 `~/.zentar/config.yaml` 中，于 Home Assistant 平台的 `extra` 部分配置 agent 接收的事件：
...
Agent 发出的消息将以 **Home Assistant 持久通知**的形式推送（通过 `persistent_notification.create`），标题为"ZENTAR DIGITAL AI AGENT"，显示在 HA 通知面板中。
