#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Zentar docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Zentar as a Python Library", None),
        ("guides/use-mcp-with-zentar", "Use MCP with Zentar", None),
        ("guides/use-voice-mode-with-zentar", "Use Voice Mode with Zentar", None),
        ("guides/use-soul-with-zentar", "Use SOUL.md with Zentar", None),
        ("guides/build-a-zentar-plugin", "Build a Zentar Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Zentar"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# ZENTAR DIGITAL AI AGENT")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by ZENTAR DIGITAL. A terminal-native "
    ...
    lines.append(
        ...
        "zentar-digital-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/hermes-digital-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# ZENTAR DIGITAL AI AGENT — Full Documentation\n",
        (
            "This file is the entire ZENTAR DIGITAL AI AGENT documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Zentar, Features, Messaging, Integrations, Guides, "
