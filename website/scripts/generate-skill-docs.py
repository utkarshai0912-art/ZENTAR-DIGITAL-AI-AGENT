...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/hermes-digital-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zentar_meta = (fm.get("metadata") or {}).get("zentar") or {}
    tags = zentar_meta.get("tags") or []
    related = zentar_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `zentar skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ZENTAR DIGITAL AI AGENT"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Zentar ships with a large built-in skill library copied into `~/.zentar/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Zentar also syncs bundled skills on `zentar update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.zentar/skills/` tree, it is still shipped with Zentar; restore it with `zentar skills reset <name> --restore`.",
        "",
