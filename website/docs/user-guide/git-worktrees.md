...
title: "Git Worktrees"
description: "Run multiple ZENTAR DIGITAL AI AGENTs safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ZENTAR DIGITAL AI AGENT is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Zentar so each session has a clean, isolated working directory.

## Why Use Worktrees with Zentar?

Zentar treats the **current working directory** as the project root:

- CLI: the directory where you run `zentar` or `zentar chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.zentar/config.yaml`
...
git worktree add ../repo-feature feature/zentar-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zentar-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Zentar there:

```bash
cd ../repo-feature

# Start Zentar in the worktree
zentar
```
...
git worktree add ../repo-experiment-a feature/zentar-a
git worktree add ../repo-experiment-b feature/zentar-b
```
...
- Works on its own branch (`feature/zentar-a` vs `feature/zentar-b`).
...
- Zentar checkpoint data under `~/.zentar/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Zentar experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zentar-checkpoints-docs`, `feature/zentar-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Zentar from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zentar -w` (Automatic Worktree Mode)

Zentar has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zentar/zentar-<hash>`).
...
```bash
zentar -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zentar -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Zentar session its own clean checkout.
