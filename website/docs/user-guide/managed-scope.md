...
`~/.zentar/config.yaml`, `~/.zentar/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/zentar`:

```text
/etc/zentar/
├── config.yaml     # managed config layer (wins over ~/.zentar/config.yaml)
└── .env            # managed env layer (wins over ~/.zentar/.env + shell)
```
...
The location can be relocated with the `ZENTAR_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ZENTAR_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Zentar.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ZENTAR_MANAGED_DIR=/opt/org/zentar-policy
```

:::warning
A user who can set `ZENTAR_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `zentar doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/zentar/config.yaml` (managed) | `/etc/zentar/.env` (managed) |
| 2 | `~/.zentar/config.yaml` (user) | `~/.zentar/.env` (user) |
...
```bash
zentar config        # shows a header naming the managed source + the pinned keys
zentar doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Zentar refuses and names the source:

```bash
$ zentar config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/zentar/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `zentar config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/zentar

# Pin some config values for every user on this machine
sudo tee /etc/zentar/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/zentar/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/zentar
sudo chmod 0644 /etc/zentar/config.yaml /etc/zentar/.env
```

Changes take effect on the next Zentar start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`zentar doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Zentar as `root`), managed scope is advisory.
