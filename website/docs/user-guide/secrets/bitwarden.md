...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.zentar/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Zentar stores that single token in `~/.zentar/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `zentar` (or the gateway, or a cron job) starts, after `~/.zentar/.env` has loaded, Zentar calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Zentar **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Zentar process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.zentar/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Zentar keys").
...
4. **Machine accounts → New machine account → My Zentar machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.zentar/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.zentar/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
zentar secrets bitwarden status
```

From now on, every `zentar` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `zentar secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `zentar secrets bitwarden status` | Show config + binary version + token presence |
| `zentar secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `zentar secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `zentar secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `zentar secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `zentar` invocations start fresh. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.zentar/bin/` on first use. |

## Failure modes

Bitwarden never blocks Zentar startup. If anything goes wrong, you'll see a one-line warning in stderr and Zentar continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `zentar secrets bitwarden setup` |
...
- Zentar will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Zentar does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.zentar/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Zentar installations.
