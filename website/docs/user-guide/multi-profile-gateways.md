...
If you only run one ZENTAR DIGITAL AI AGENT, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more ZENTAR DIGITAL AI AGENTs that should all
...
(`ai.zentar.gateway-<name>.plist`) or systemd user service
(`zentar-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
zentar profile create coder
zentar profile create personal-bot
zentar profile create research
...
```bash
zentar config set gateway.multiplex_profiles true
zentar gateway restart
```

Equivalently, in the default profile's `~/.zentar/config.yaml`:
...
You do **not** run `zentar gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `zentar gateway start` / `run` is a
...
home). `zentar status` reports the multiplexer and the profiles it serves;
`zentar status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/zentar-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: zentar-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    zentar gateway "$action"
  else
    zentar -p "$profile" gateway "$action"
  fi
}
...
```bash
zentar-gateways start      # start every configured profile
zentar-gateways stop       # stop every configured profile
zentar-gateways restart    # restart all
zentar-gateways status     # status across all
zentar-gateways list       # delegates to `zentar gateway list`
```

:::tip
The `default` profile is targeted with `zentar gateway <action>` (no `-p`),
not `zentar -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `zentar -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.zentar.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/zentar-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.zentar.gateway.plist` /
`zentar-gateway.service`.
...
```bash
# Default profile
tail -f ~/.zentar/logs/gateway.log
tail -f ~/.zentar/logs/gateway.error.log

# Named profile
tail -f ~/.zentar/profiles/<name>/logs/gateway.log
tail -f ~/.zentar/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.zentar/logs/gateway.log ~/.zentar/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
zentar logs -f                  # follow default profile
zentar -p coder logs -f         # follow one profile
zentar logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
zentar profile list             # profiles + model + gateway state
zentar-gateways status          # full status across every profile
launchctl list | grep zentar    # macOS — PIDs and labels
systemctl --user list-units 'zentar-gateway-*'   # Linux — units
```
...
```
~/.zentar/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.zentar/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
zentar config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.zentar/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=zentar --why="gateways running" \
  sleep infinity &
...
`zentar-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.zentar/.env ~/.zentar/profiles/*/.env
```

## Updating the code

`zentar update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `zentar gateway start` after a previous `zentar gateway stop`. The
...
```bash
ps -ef | grep "zentar_cli.*-p <profile>"
cat ~/.zentar/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.zentar.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.zentar.gateway-<profile>.plist

# Linux
systemctl --user restart zentar-gateway-<profile>.service
```

### Health check

```bash
zentar doctor                  # default profile
zentar -p <profile> doctor     # one profile
```
