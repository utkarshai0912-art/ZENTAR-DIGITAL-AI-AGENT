...
Zentar runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Zentar
...
1. Zentar captures a baseline of current diagnostics for the file.
...
…). Zentar auto-detects the binary on PATH or in
`<ZENTAR_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Zentar at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ZENTAR_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`zentar lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Zentar installs both packages together when you
run `zentar lsp install typescript` or auto-install fires on first
use.

## CLI

```
zentar lsp status          # service state + per-server install status
zentar lsp list            # registry, optionally --installed-only
zentar lsp install <id>    # eagerly install one server
zentar lsp install-all     # try every server with a known recipe
zentar lsp restart         # tear down running clients
zentar lsp which <id>      # print resolved binary path
```

`zentar lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ZENTAR_HOME>/lsp/bin
...
When `install_strategy: auto`, Zentar installs binaries into
`<ZENTAR_HOME>/lsp/bin/`. NPM packages land in
`<ZENTAR_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Zentar-owned and is
...
Servers are kept alive for the life of the Zentar process. There's
...
**`zentar lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ZENTAR_HOME>/lsp/bin/`. Run
`zentar lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `zentar lsp status`**
...
When `zentar lsp status` shows a `Backend warnings` section, install
...
`~/.zentar/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.zentar/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `zentar lsp restart` to clear the set;
