...
ZENTAR DIGITAL AI AGENT can drive your desktop — clicking, typing, scrolling,
...
```
zentar computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `zentar computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `zentar tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Zentar app). `zentar computer-use doctor` will tell you which permission is missing. |
...
```
zentar -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.zentar/config.yaml`.

## `zentar computer-use doctor` — your first triage stop

`zentar computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ zentar computer-use doctor
...
agent is acting here." Each Zentar run declares its own cua-driver
**session id** (something like `zentar-3a7b9c14d2e8`); the cursor's
...
Zentar intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Zentar-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Zentar skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Zentar
...
Zentar applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.zentar/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Zentar applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  ZENTAR DIGITAL AI AGENT) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the ZENTAR DIGITAL AI AGENT itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Zentar warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Zentar from inside the RDP / console
...
```
ZENTAR_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ZENTAR_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Zentar disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Zentar sets
...
When it's on, `zentar computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Zentar at a binary you built from source instead
of the published release. Zentar resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ZENTAR_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Zentar testing (see notes).
...
### Option B — point Zentar straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ZENTAR_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ZENTAR_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ZENTAR_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Zentar is using your build

- `zentar computer-use status` prints the resolved binary path and
  version.
- `zentar computer-use doctor` confirms the binary is reachable and
...
- **Zentar spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Zentar
  change needed either way.
- **Local builds skip the version check.** Zentar warns when the
...
**First action when anything's off: run `zentar computer-use doctor`.**
...
Run `zentar computer-use install` to fetch the cua-driver binary, or
run `zentar tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `zentar computer-use doctor` will
...
- **Zentar-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Zentar `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Zentar (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Zentar among them)
