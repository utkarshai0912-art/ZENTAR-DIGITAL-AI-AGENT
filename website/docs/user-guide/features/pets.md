...
Zentar can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ZENTAR_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Zentar renders the sprite at full fidelity when your
...
zentar pets list
zentar pets list cat

# Install a pet and make it active in one step
zentar pets install boba --select
...
## `zentar pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `zentar pets list [query] [--limit N]` |
| List installed pets | `zentar pets list --installed` |
| Install a pet | `zentar pets install <slug> [--select] [--force]` |
...
| Preview/animate | `zentar pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `zentar pets off` |
| Remove an installed pet | `zentar pets remove <slug>` |
| Diagnose setup | `zentar pets doctor` |

`zentar pets show` flags:
...
Beyond installing pre-made pets from the gallery, Zentar can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `zentar pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `zentar tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ZENTAR_PET_IMAGE_PROVIDER` env var (e.g. `ZENTAR_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Zentar is
...
Run `zentar pets doctor` — it reports:
