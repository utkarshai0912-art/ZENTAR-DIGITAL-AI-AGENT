...
description: "Customize ZENTAR DIGITAL AI AGENT's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

ZENTAR DIGITAL AI AGENT's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ZENTAR_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Zentar is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Zentar now seeds a default `SOUL.md` automatically in:

```text
~/.zentar/SOUL.md
```

More precisely, it uses the current instance's `ZENTAR_HOME`, so if you run Zentar with a custom home directory, it will use:
...
- Zentar creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Zentar loads `SOUL.md` only from `ZENTAR_HOME`
- Zentar does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Zentar falls back to a built-in default identity
...
If Zentar loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZENTAR_HOME`, the personality belongs to the Zentar instance itself.

That also makes it easier to teach users:
- "Edit `~/.zentar/SOUL.md` to change Zentar' default personality."
...
- how Zentar should handle uncertainty, disagreement, or ambiguity
...
## What Zentar injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Zentar falls back to a built-in default identity ("You are ZENTAR DIGITAL AI AGENT, an intelligent AI assistant created by ZENTAR DIGITAL..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Zentar ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Zentar, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Zentar its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.zentar/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.zentar/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-zentar)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Zentar speaks
- `display.skin` and `/skin` affect how Zentar looks in the terminal
