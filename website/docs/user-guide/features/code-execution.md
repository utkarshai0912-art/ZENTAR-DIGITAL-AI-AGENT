...
The `execute_code` tool lets the agent write Python scripts that call Zentar tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Zentar over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from zentar_tools import ...`
2. Zentar generates a `zentar_tools.py` stub module with RPC functions
3. Zentar opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Zentar
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from zentar_tools import web_search, web_extract
...
```python
from zentar_tools import search_files, read_file
import json
...
```python
from zentar_tools import search_files, read_file, patch
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.zentar/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Zentar's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Zentar's own python) |
...
### `ZENTAR_*` variables in the child

The child process receives only a small, fixed set of operational `ZENTAR_*`
...
(plus `ZENTAR_RPC_DIR` / `ZENTAR_RPC_SOCKET` / `TZ` / `HOME`, which Zentar
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `ZENTAR_`
through to the child. That broad prefix was removed for security hardening: it
could leak `ZENTAR_*`-named configuration that doesn't match a secret substring
(for example `ZENTAR_BASE_URL`, `ZENTAR_KANBAN_DB`, or a `ZENTAR_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `ZENTAR_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Zentar-managed provider credentials can never
...
**Rule of thumb:** Use `execute_code` when you need to call Zentar tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
