...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Zentar's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Zentar instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
zentar memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `zentar` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Zentar syncs them back at session init.
...
The `zentar honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `zentar memory setup honcho` (or run `zentar memory setup` and pick it from the list); the `zentar honcho` subcommand then appears on the next invocation.

```bash
zentar memory setup honcho    # Configure Honcho directly (works before activation)
zentar honcho status          # Connection status, config, and key settings
zentar honcho setup           # Redirects to `zentar memory setup` (post-activation alias)
zentar honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
zentar honcho peer            # Show or update peer names + dialectic reasoning level
zentar honcho mode            # Show or set recall mode (hybrid/context/tools)
zentar honcho tokens          # Show or set token budget for context and dialectic
zentar honcho identity        # Seed or show the AI peer's Honcho identity
zentar honcho sync            # Sync Honcho config to all existing profiles
zentar honcho peers           # Show peer identities across all profiles
zentar honcho sessions        # List known Honcho session mappings
zentar honcho map             # Map current directory to a Honcho session name
zentar honcho enable          # Enable Honcho for the active profile
zentar honcho disable         # Disable Honcho for the active profile
zentar honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `zentar honcho`

If you previously used the standalone `zentar honcho setup`:
...
No re-login or re-setup needed. Run `zentar memory setup` and select "honcho" — the wizard detects your existing config.
