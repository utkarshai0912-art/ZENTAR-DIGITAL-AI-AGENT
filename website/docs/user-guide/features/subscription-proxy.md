...
chat completions — use your Zentar-managed provider subscription as their
...
| Use case | "Use Zentar as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Zentar stores
the refresh token in `~/.zentar/auth.json` — the same place all Zentar
...
Model:      Zentar-4-70B    # or Zentar-4.3-36B, Zentar-4-405B
```
...
interface in `zentar_cli/proxy/adapters/`.
...
If you see `not logged in`, run `zentar portal`. If you see
...
`zentar_cli/proxy/adapters/<provider>.py` and registering it in
