...
When ZENTAR DIGITAL AI AGENT runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.zentar/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.zentar/config.yaml` (switchable per session
via `/personality`).
...
If you use Zentar' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.zentar/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. ZENTAR DIGITAL AI AGENT's deliverable
