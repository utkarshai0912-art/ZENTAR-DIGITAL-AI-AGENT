...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _zentar_env="${ZENTAR_HOME:-$HOME/.zentar}/.env"; [ -f "$_zentar_env" ] && grep -q "^GITHUB_TOKEN=" "$_zentar_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zentar_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from ZENTAR DIGITAL AI AGENT\",
    \"comments\": [
...
```bash
source "${ZENTAR_HOME:-$HOME/.zentar}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by ZENTAR DIGITAL AI AGENT. Code looks clean — good test coverage, no security concerns."
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"body\": \"## ZENTAR DIGITAL AI AGENT Review\n\nFound 2 issues, 1 suggestion. See inline comments.\",
    \"comments\": [
...
---
*Reviewed by ZENTAR DIGITAL AI AGENT*
EOF
)"
```
