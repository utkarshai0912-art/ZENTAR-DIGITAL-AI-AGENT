...
- Give it a name like "zentar-digital-agent"
- Select scopes:
...
- Give it a title like "zentar-digital-agent-&lt;machine-name>"
...
elif _zentar_env="${ZENTAR_HOME:-$HOME/.zentar}/.env"; [ -f "$_zentar_env" ] && grep -q "^GITHUB_TOKEN=" "$_zentar_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zentar_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
