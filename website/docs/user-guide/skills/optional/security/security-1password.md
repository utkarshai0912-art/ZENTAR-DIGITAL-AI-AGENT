...
| | |
|---|---|
| Source | Optional — install with `zentar skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by ZENTAR DIGITAL AI AGENT |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Zentar terminal calls (desktop app flow only)
...
### Service Account (recommended for Zentar)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ZENTAR_HOME:-~/.zentar}/.env` (the skill will prompt for this on first load).
...
## Zentar Execution Pattern (desktop app flow)

Zentar terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zentar-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zentar-op.sock"
