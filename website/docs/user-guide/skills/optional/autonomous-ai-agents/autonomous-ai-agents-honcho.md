...
description: "Configure and use Honcho memory with Zentar -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Zentar -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zentar skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Zentar

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Zentar profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zentar#running-honcho-locally-with-zentar

### Verify

```bash
zentar honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Zentar profile's AI peer
...
Honcho models conversations as interactions between **peers**. Zentar creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Zentar instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Zentar run |
| `global` | Single session across all directories |

Manual override: `zentar honcho map my-project-name`
...
Each Zentar profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block zentar.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zentar.coder` host block in `honcho.json`
...
```bash
zentar honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Zentar when Honcho memory is active.
...
Config file: `$ZENTAR_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zentar honcho setup`. Ensure `memory.provider: honcho` is in `~/.zentar/config.yaml`.

### Memory not persisting across sessions
Check `zentar honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zentar profile create <name> --clone`. For existing profiles: `zentar honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zentar honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zentar honcho status` | Show resolved config, connection test, peer info for active profile |
| `zentar honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zentar honcho disable` | Disable Honcho for the active profile |
| `zentar honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zentar honcho peers` | Show peer identities across all profiles |
| `zentar honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zentar honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zentar honcho sessions` | List known directory-to-session-name mappings |
| `zentar honcho map <name>` | Map current working directory to a Honcho session name |
| `zentar honcho identity` | Seed AI peer identity or show both peer representations |
| `zentar honcho sync` | Create host blocks for all Zentar profiles that don't have one yet |
| `zentar honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Zentar + Honcho |
| `zentar memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zentar memory status` | Show active memory provider and config |
| `zentar memory off` | Disable external memory provider |
