---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into ZENTAR DIGITAL AI AGENT"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into ZENTAR DIGITAL AI AGENT"
---
...
Migrate a user's OpenClaw customization footprint into ZENTAR DIGITAL AI AGENT. Imports Zentar-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zentar skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | ZENTAR DIGITAL AI AGENT (ZENTAR DIGITAL) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Zentar`, `Memory`, `Persona`, `Import` |
| Related skills | [`zentar-digital-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zentar-digital-agent) |
...
# OpenClaw -> Zentar Migration

Use this skill when a user wants to move their OpenClaw setup into ZENTAR DIGITAL AI AGENT with minimal manual cleanup.
...
```bash
zentar claw migrate              # Full interactive migration
...
zentar claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `zentar setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Zentar home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Zentar memory entries
- merge OpenClaw command approval patterns into Zentar `command_allowlist`
- migrate Zentar-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Zentar working-directory configuration
- copy OpenClaw skills into `~/.zentar/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Zentar workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.zentar/tts/`
- archive non-secret docs that do not have a direct Zentar destination
...
- `~/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.zentar/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.zentar/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Zentar-compatible secrets, currently:
...
Zentar CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Zentar. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Zentar workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Zentar skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.zentar/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Zentar unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Zentar targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Zentar destination. Unsupported auth blobs must still be reported as skipped.
...
- Zentar persona state imported
- Zentar memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.zentar/skills/openclaw-imports/`
