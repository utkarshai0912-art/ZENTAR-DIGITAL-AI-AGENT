...
| | |
|---|---|
| Source | Optional — install with `zentar skills install official/productivity/shopify` |
...
4. Save to `${ZENTAR_HOME:-~/.zentar}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Zentar","productType":"Apparel","tags":["test"]}}'
```
