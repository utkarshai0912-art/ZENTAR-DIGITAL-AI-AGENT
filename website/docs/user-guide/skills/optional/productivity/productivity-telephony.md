---
title: "Telephony — Give Zentar phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Zentar phone capabilities without core tool changes"
---
...
Give Zentar phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zentar skills install official/productivity/telephony` |
| Path | `optional-skills/productivity/telephony` |
| Version | `1.0.0` |
| Author | ZENTAR DIGITAL |
| License | MIT |
...
This optional skill gives Zentar practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ZENTAR_HOME:-~/.zentar}/.env`
...
It does **not** turn Zentar into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Zentar memory
...
### 1) "I want Zentar to own a real phone number"
Use **Twilio**.
...
- pairs well with Zentar `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${ZENTAR_HOME:-~/.zentar}/.env`
...
```bash
SCRIPT="$(find ~/.zentar/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
zentar skills search telephony
zentar skills install official/productivity/telephony
```
...
Then save credentials into Zentar:
...
3. Buy it and save it into `${ZENTAR_HOME:-~/.zentar}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Zentar calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Zentar's existing `text_to_speech` support.

Use this when:
- you want the call to use Zentar's configured TTS voice rather than Twilio `<Say>`
...
Recommended Zentar TTS -> Twilio Play workflow:

1. Generate the audio with Zentar `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Zentar to create the message audio with `text_to_speech`
...
Important note:
- Zentar TTS is great for prerecorded outbound messages
...
- Zentar STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Zentar memory.
...
- Do not store arbitrary third-party phone numbers in Zentar memory.
...
3. persist that number to `${ZENTAR_HOME:-~/.zentar}/.env`
