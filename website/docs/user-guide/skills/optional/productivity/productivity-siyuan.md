...
| | |
|---|---|
| Source | Optional — install with `zentar skills install official/productivity/siyuan` |
...
3. Store it in `${ZENTAR_HOME:-~/.zentar}/.env`:
   ```
...
```yaml
# In ~/.zentar/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
