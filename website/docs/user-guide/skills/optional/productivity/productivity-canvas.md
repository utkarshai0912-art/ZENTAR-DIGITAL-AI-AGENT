...
| | |
|---|---|
| Source | Optional — install with `zentar skills install official/productivity/canvas` |
...
4. Name the token (e.g., "ZENTAR DIGITAL AI AGENT"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ZENTAR_HOME:-~/.zentar}/.env`:
...
```bash
CANVAS="python $ZENTAR_HOME/skills/productivity/canvas/scripts/canvas_api.py"
