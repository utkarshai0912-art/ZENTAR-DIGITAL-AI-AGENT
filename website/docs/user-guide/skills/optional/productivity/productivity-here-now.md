...
| | |
|---|---|
| Source | Optional — install with `zentar skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZENTAR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZENTAR_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ZENTAR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client zentar
```
...
```bash
PUBLISH="${ZENTAR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client zentar
```
...
```bash
DRIVE="${ZENTAR_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `zentar`)    |
