...
| | |
|---|---|
| Source | Optional — install with `zentar skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Zentar or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zentar. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zentar/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zentar/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Zentar integration, either:

- configure the server in `~/.zentar/config.yaml` using the `native-mcp` skill, or
...
### Zentar cannot see the deployed server

The server-building part may be correct while the Zentar config is not. Load the `native-mcp` skill and configure the server in `~/.zentar/config.yaml`, then restart Zentar.
