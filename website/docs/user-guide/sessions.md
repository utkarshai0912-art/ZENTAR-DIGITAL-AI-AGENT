...
ZENTAR DIGITAL AI AGENT automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.zentar/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Zentar stores session history so it can resume conversations, but it does not
...
Zentar explicitly injects for that turn.
...
For example, if a user sends an image and asks Zentar to make a meme from it,
Zentar may inspect that image once with vision and run an image-processing
...
`zentar sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`zentar` or `zentar chat`) |
...
zentar -c "my project"   # → resumes "my project #3"
```
...
zentar --resume 20250305_091523_a1b2c3d4
zentar -r 20250305_091523_a1b2c3d4

# Resume by title
zentar --resume "refactoring auth"

# Or with the chat subcommand
zentar chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `zentar sessions list`.

### Conversation Recap on Resume

When you resume a session, Zentar displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Zentar session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.zentar/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `zentar -r "<title>"` from the shell) and pick up where the platform left off.
...
Zentar automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `zentar sessions list` or `zentar sessions browse`.
...
```bash
zentar sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Zentar creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`zentar -c "my project"`), it automatically picks the most recent session in the lineage.
...
Zentar provides a full set of session management commands via `zentar sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
zentar sessions list

# Filter by platform
zentar sessions list --source telegram

# Show more sessions
zentar sessions list --limit 50
```
...
`zentar sessions export` is one surface for every export format, selected with `--format`:
...
zentar sessions export backup.jsonl

# Export sessions from a specific platform
zentar sessions export telegram-history.jsonl --source telegram

# Export a single session
zentar sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
zentar sessions export backup.jsonl --redact
```
...
zentar sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
zentar sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
zentar sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
zentar sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `zentar-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
zentar sessions export --format trace

# One session to a local trace file
zentar sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
zentar sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.zentar/session-exports`).

```bash
# Export one session to Markdown
zentar sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
zentar sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
zentar sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
zentar sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
zentar sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
zentar sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
zentar sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
zentar sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
zentar sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
zentar sessions prune

# Custom age threshold — bare numbers are days
zentar sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
zentar sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
zentar sessions prune --newer-than 5h

# Explicit window with absolute timestamps
zentar sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
zentar sessions prune --source telegram
zentar sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
zentar sessions prune --newer-than 5h --title "smoke test"   # title substring
zentar sessions prune --older-than 30 --max-messages 3        # tiny sessions
zentar sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
zentar sessions prune --model gpt-5 --older-than 1w           # by model (substring)
zentar sessions prune --provider openrouter --older-than 60   # by billing provider
zentar sessions prune --branch feature/old-experiment         # by git branch
zentar sessions prune --user 12345678 --chat-type group       # by messaging origin
zentar sessions prune --max-tokens 500 --older-than 7         # by token usage
zentar sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
zentar sessions prune --newer-than 5h --dry-run

# Skip confirmation
zentar sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `zentar sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `zentar sessions prune` keeps the 90-day cutoff. Every
...
`zentar sessions archive` takes the same filters as `prune` but soft-hides
...
zentar sessions archive --newer-than 5h

# Archive by title substring, preview first
zentar sessions archive --title "dry run" --dry-run
zentar sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `zentar sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`zentar sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
zentar sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`zentar insights`](/reference/cli-commands#zentar-insights).
...
When Zentar cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Zentar uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Zentar in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.zentar/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.zentar/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.zentar/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.zentar/sessions/sessions.json` is the **gateway routing index** — it maps
...
`zentar sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.zentar/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `zentar sessions list`, the cause is
`state.db` not receiving them — run `zentar sessions repair` and watch for a
...
`*.jsonl` files in `~/.zentar/sessions/`. They are no longer written or
read by Zentar. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Zentar process in the same `ZENTAR_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.zentar/config.yaml`:
...
zentar sessions delete <session_id>
