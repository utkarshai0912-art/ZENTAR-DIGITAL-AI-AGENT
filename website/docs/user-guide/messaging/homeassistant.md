...
description: Control your smart home with ZENTAR DIGITAL AI AGENT via Home Assistant integration.
...
ZENTAR DIGITAL AI AGENT integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "ZENTAR DIGITAL AI AGENT"
5. Copy the token
...
ZENTAR DIGITAL AI AGENT registers four tools for smart home control:
...
Configure which events the agent sees in `~/.zentar/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "ZENTAR DIGITAL AI AGENT".
...
The adapter reads credentials from `~/.zentar/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Zentar profile
...
running Zentar — `curl -H "Authorization: Bearer <token>" <url>/api/` should
