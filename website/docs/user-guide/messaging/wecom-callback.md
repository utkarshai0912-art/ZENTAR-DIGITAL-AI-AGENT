...
Connect Zentar to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Zentar supports two WeCom integration modes:
...
> Run `zentar gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Zentar decrypts the message, queues it for the agent
...
(Use `zentar gateway start` only after `zentar gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Zentar and the token the
...
in `~/.zentar/.env` values around `=` will also break signature checks. After
fixing, restart `zentar gateway run`.
...
Check `zentar gateway run` logs for the bound host/port. If the adapter bound to
