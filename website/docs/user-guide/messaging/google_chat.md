...
sidebar_position: 12
title: "Google Chat"
description: "Set up ZENTAR DIGITAL AI AGENT as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect ZENTAR DIGITAL AI AGENT to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Zentar
...
> Run `zentar gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Zentar can read (e.g.,
`~/.zentar/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a zentar restart)
...
- Principal: `zentar-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Zentar calls
...
- **App name**: whatever you want users to see ("Zentar" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/zentar-chat-events`.
...
`ADDED_TO_SPACE` event that Zentar uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Zentar

Add the Google Chat section to `~/.zentar/.env`:
...
Install the dependencies the Google Chat adapter needs (no Zentar extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Zentar is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Zentar detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Zentar session.
...
3. Download the JSON. Move it onto the host that runs Zentar.
4. Register the client with Zentar (run under the profile you want it scoped to):
...
zentar -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Zentar home (e.g.
`~/.zentar/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.zentar/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.zentar/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Zentar isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `zentar gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Zentar will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.zentar/google_chat_user_tokens/<sanitized_email>.json` (filesystem
