...
Connect Zentar to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Zentar — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Zentar runs it in a
...
zentar photon setup --phone +15551234567
```
...
2. **Finds or creates** the `ZENTAR DIGITAL AI AGENT` project on your account.
...
Runtime credentials are written to `~/.zentar/.env`
...
(device token, dashboard project id) lives in `~/.zentar/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Zentar
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Zentar replies with a pairing code. Approve it with:

```bash
zentar pairing approve photon <CODE>
```

Use `zentar pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.zentar/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.zentar/.env`):
...
By default Zentar responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Zentar` and
`@ZENTAR DIGITAL AI AGENT` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Zentar will reply.

## Status & troubleshooting

```bash
zentar photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Zentar uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run zentar photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `zentar photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `zentar photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `zentar photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Zentar sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Zentar wake words  | JSON list / comma / newline regex patterns for group mentions |
