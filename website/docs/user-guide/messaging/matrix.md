...
sidebar_position: 9
title: "Matrix"
description: "Set up ZENTAR DIGITAL AI AGENT as a Matrix bot"
---

# Matrix Setup

ZENTAR DIGITAL AI AGENT integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the ZENTAR DIGITAL AI AGENT pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Zentar works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Zentar behaves once it's connected.

## How Zentar Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Zentar responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Zentar requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Zentar supports Matrix threads (MSC3440). If you reply in a thread, Zentar keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Zentar auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Zentar accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Zentar normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Zentar isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
Zentar sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Zentar now includes the current Matrix room name, room ID, topic, message ID,
...
2. Choose a username like `zentar` — the full user ID will be `@zentar:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `zentar-bot`).

### Option C: Use Your Own Account

You can also run Zentar as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Zentar needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Zentar the bot's user ID and password. Zentar will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@zentar:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

ZENTAR DIGITAL AI AGENT uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure ZENTAR DIGITAL AI AGENT
...
Add the following to your `~/.zentar/.env` file:
...
# MATRIX_USER_ID=@zentar:matrix.example.org
...
Bridge and appservice deployments need extra loop protection. Zentar always
...
Optional behavior settings in `~/.zentar/config.yaml`:
...
:::tip
You can run `zentar gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Zentar supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with zentar extras
cd ~/.zentar/zentar-digital-agent && uv pip install -e ".[matrix]"
```
...
When E2EE is enabled, Zentar:

- Stores encryption keys in `~/.zentar/platforms/matrix/store/` (legacy installs: `~/.zentar/matrix/store/`)
...
In Matrix conversations, Zentar exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Zentar knows who requested the action.

### Media Limits

Zentar uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Zentar rejects arbitrary
...
Zentar includes an opt-in Synapse harness for local validation:

```bash
docker compose -f tests/e2e/matrix_synapse_gateway/docker-compose.yml up -d
ZENTAR_MATRIX_SYNAPSE_INTEGRATION=1 \
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Zentar imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Zentar bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.zentar/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Zentar detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@zentar:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@zentar:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@zentar:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@zentar:your-server';
   "
...
2. Delete the local crypto store and restart Zentar:
   ```bash
   rm -f ~/.zentar/platforms/matrix/store/crypto.db*
   # restart zentar
   ```
...
Zentar supports the same gateway commands in Matrix that it supports on other
...
Zentar only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/hermes-digital-agent/issues/12614).
...
If you previously used Zentar with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@zentar:your-server.org"},
       "password": "***",
       "initial_device_display_name": "ZENTAR DIGITAL AI AGENT"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.zentar/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.zentar/platforms/matrix/store/crypto.db
   rm -f ~/.zentar/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.zentar/.env`:
...
a working E2EE setup with a previous version of Zentar and are upgrading.
...
Matrix E2EE requires `libolm`, which doesn't compile on macOS ARM64 (Apple Silicon). The `zentar-digital-agent[matrix]` extra is gated to Linux only. If you're on macOS, proxy mode lets you run E2EE in a Docker container on a Linux VM while the actual agent runs natively on macOS with full access to your local files, memory, and skills.
...
services:
  zentar-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "ZENTAR_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.zentar/platforms/matrix/store
```
...
RUN cd ~/.zentar/zentar-digital-agent && uv pip install -e ".[matrix]"
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Zentar API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Zentar-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Zentar uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Zentar's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Zentar logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Zentar gateway isn't running, or it failed to connect.

**Fix**: Check that `zentar gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.zentar/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your ZENTAR DIGITAL AI AGENT deployment, see the [Security Guide](../security.md).
...
- **Media support**: Zentar can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
