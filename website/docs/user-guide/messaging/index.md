...
description: "Chat with Zentar from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Zentar from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Zentar](/guides/use-voice-mode-with-zentar).
...
```mermaid
flowchart TB
    subgraph Gateway["Zentar Gateway"]
...
For group chats, hooks, and automation flows, Zentar supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Zentar keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Zentar does not hide failures just because the text resembles a silence token.
...
```bash
zentar gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
zentar gateway              # Run in foreground
zentar gateway setup        # Configure messaging platforms interactively
zentar gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo zentar gateway install --system   # Linux only: install a boot-time system service
zentar gateway start        # Start the default service
zentar gateway stop         # Stop the default service
zentar gateway status       # Check default service status
zentar gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update ZENTAR DIGITAL AI AGENT to the latest version |
...
with the `session_reset` section in `~/.zentar/config.yaml`:
...
Configure per-platform overrides in `~/.zentar/gateway.json`:
...
zentar pairing approve telegram XKGH5N7P

# Other pairing commands:
zentar pairing list          # View pending + approved users
zentar pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Zentar appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.zentar/config.yaml`:
...
Off by default. When enabled, Zentar prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.zentar/config.yaml`:
...
```bash
ZENTAR_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
zentar gateway install               # Install as user service
zentar gateway start                 # Start the service
zentar gateway stop                  # Stop the service
zentar gateway status                # Check status
journalctl --user -u zentar-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo zentar gateway install --system
sudo zentar gateway start --system
sudo zentar gateway status --system
journalctl -u zentar-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Zentar installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit zentar-gateway` (or `sudo systemctl edit zentar-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `zentar update`. When `zentar update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart zentar-gateway` command (it never blocks on an interactive password prompt).
...
```bash
zentar gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `zentar update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo zentar update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/zentar-gateway`:

```
zentar ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed zentar-gateway*, /usr/bin/systemctl --no-ask-password start zentar-gateway*, /usr/bin/systemctl --no-ask-password restart zentar-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Zentar will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Zentar installations on the same machine (with different `ZENTAR_HOME` directories), each gets its own systemd service name. The default `~/.zentar` uses `zentar-gateway`; other installations use `zentar-gateway-<hash>`. The `zentar gateway` commands automatically target the correct service for your current `ZENTAR_HOME`.
:::

### macOS (launchd)

```bash
zentar gateway install               # Install as launchd agent
zentar gateway start                 # Start the service
zentar gateway stop                  # Stop the service
zentar gateway status                # Check status
tail -f ~/.zentar/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.zentar.gateway.plist`. It includes three environment variables:
...
- **ZENTAR_HOME** — scopes the gateway to your Zentar installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `zentar gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ZENTAR_HOME` directory gets its own launchd label. The default `~/.zentar` uses `ai.zentar.gateway`; other installations use `ai.zentar.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `zentar-cli` | Full access |
| Telegram | `zentar-telegram` | Full tools including terminal |
| Discord | `zentar-discord` | Full tools including terminal |
| WhatsApp | `zentar-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `zentar-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `zentar-slack` | Full tools including terminal |
| Google Chat | `zentar-google_chat` | Full tools including terminal |
| Signal | `zentar-signal` | Full tools including terminal |
| SMS | `zentar-sms` | Full tools including terminal |
| Email | `zentar-email` | Full tools including terminal |
| Home Assistant | `zentar-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `zentar-mattermost` | Full tools including terminal |
| Matrix | `zentar-matrix` | Full tools including terminal |
| DingTalk | `zentar-dingtalk` | Full tools including terminal |
| Feishu/Lark | `zentar-feishu` | Full tools including terminal |
| WeCom | `zentar-wecom` | Full tools including terminal |
| WeCom Callback | `zentar-wecom-callback` | Full tools including terminal |
| Weixin | `zentar-weixin` | Full tools including terminal |
| BlueBubbles | `zentar-bluebubbles` | Full tools including terminal |
| QQBot | `zentar-qqbot` | Full tools including terminal |
| Yuanbao | `zentar-yuanbao` | Full tools including terminal |
| Microsoft Teams | `zentar-teams` | Full tools including terminal |
| API Server | `zentar-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `zentar-webhook` | Full tools including terminal |
| Raft | `zentar-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.zentar/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
