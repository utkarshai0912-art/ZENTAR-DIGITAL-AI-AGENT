...
description: "Official optional skills shipped with zentar-digital-agent — install via zentar skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Optional skills ship with zentar-digital-agent under `optional-skills/` but are **not active by default**. Install them explicitly:

```bash
zentar skills install official/<category>/<skill>
```

For example:

```bash
zentar skills install official/blockchain/solana
zentar skills install official/mlops/flash-attention
```
...
```bash
zentar skills uninstall <skill-name>
```
...
| [**honcho**](/docs/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho) | Configure and use Honcho memory with Zentar -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshoo... |
...
| [**blender-mcp**](/docs/user-guide/skills/optional/creative/creative-blender-mcp) | Control Blender directly from Zentar via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. Use when user wants to create or modify anything in Blender. |
...
| [**kanban-video-orchestrator**](/docs/user-guide/skills/optional/creative/creative-kanban-video-orchestrator) | Plan, set up, and monitor a multi-agent video production pipeline backed by Zentar Kanban. Use when the user wants to make ANY video — narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loo... |
...
| [**zentar-s6-container-supervision**](/docs/user-guide/skills/optional/devops/devops-zentar-s6-container-supervision) | Modify, debug, or extend the s6-overlay supervision tree inside the ZENTAR DIGITAL AI AGENT Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern. |
...
| Skill | Description |
|-------|-------------|
| [**agentmail**](/docs/user-guide/skills/optional/email/email-agentmail) | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. zentar-digital-agent@agentmail.to). |
...
| [**telephony**](/docs/user-guide/skills/optional/productivity/productivity-telephony) | Give Zentar phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
