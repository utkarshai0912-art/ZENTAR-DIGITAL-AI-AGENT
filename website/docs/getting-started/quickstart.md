...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with ZENTAR DIGITAL AI AGENT — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Zentar setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [ZENTAR DIGITAL AI AGENT Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="ZENTAR DIGITAL AI AGENT Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Zentar for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Zentar working on my machine | `zentar setup` | Run a real chat and verify it responds |
| I already know my provider | `zentar model` | Save the config, then start chatting |
| I want a bot or always-on setup | `zentar gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `zentar model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `zentar model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Zentar cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install ZENTAR DIGITAL AI AGENT
### With the Zentar Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Zentar Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.

### Without Zentar Desktop:
For a command-line only install without Zentar Desktop, run:
...
The single most important setup step. Use `zentar model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `zentar setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `zentar update`. Re-enable anything later with `zentar tools`, seed skills with `zentar skills opt-in --sync`, or tune settings with `zentar setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `zentar model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `zentar model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `zentar model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `zentar_cli/models.py` may change between releases) | `zentar model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `zentar model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `zentar model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `zentar model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `zentar model` (requires `copilot` CLI + `copilot login`) |
...
ZENTAR DIGITAL AI AGENT requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `zentar model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Zentar separates secrets from normal config:

- **Secrets and tokens** → `~/.zentar/.env`
- **Non-secret settings** → `~/.zentar/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
zentar config set model anthropic/claude-opus-4.6
zentar config set terminal.backend docker
zentar config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
zentar            # classic CLI
zentar --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Zentar ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `zentar` vs `zentar --tui`.
:::
...
```bash
zentar --continue    # Resume the most recent session
zentar -c            # Short form
```
...
```bash
zentar gateway setup    # Interactive platform configuration
```
...
- `zentar tools` — tune tool access per platform
- `zentar skills` — browse and install reusable workflows
...
```bash
zentar config set terminal.backend docker    # Docker isolation
zentar config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Zentar install directory (the curl installer placed it at
# ~/.zentar/zentar-digital-agent on Linux/macOS or %LOCALAPPDATA%\zentar\zentar-digital-agent on Windows):
cd ~/.zentar/zentar-digital-agent
...
Skills are on-demand instruction documents that teach Zentar how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Zentar ships with a catalog of bundled skills already installed in `~/.zentar/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
zentar skills browse                      # list everything available
zentar skills search kubernetes           # find skills by keyword
zentar skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `zentar skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Zentar ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.zentar/zentar-digital-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Zentar opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `zentar model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `zentar gateway setup` and check `zentar gateway status` |
| `zentar --continue` can't find old session | Switched profiles or session never saved | Check `zentar sessions list` and confirm you're in the right profile |
...
| `zentar doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `zentar model`
3. `zentar setup`
4. `zentar sessions list`
5. `zentar --continue`
6. `zentar gateway status`
...
| Command | Description |
|---------|-------------|
| `zentar` | Start chatting |
| `zentar model` | Choose your LLM provider and model |
| `zentar tools` | Configure which tools are enabled per platform |
| `zentar setup` | Full setup wizard (configures everything at once) |
| `zentar doctor` | Diagnose issues |
| `zentar update` | Update to latest version |
| `zentar gateway` | Start the messaging gateway |
| `zentar --continue` | Resume last session |
