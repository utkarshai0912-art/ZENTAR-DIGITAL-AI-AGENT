...
description: "Run ZENTAR DIGITAL AI AGENT directly on an Android phone with Termux"
---

# Zentar on Android with Termux
...
ZENTAR DIGITAL AI AGENT can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Zentar from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Zentar now ships a Termux-aware installer path:
...
- links `zentar` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `zentar` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zentar" "$PREFIX/bin/zentar"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zentar` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zentar/.env`.
...
### `zentar doctor` says ripgrep or Node is missing
