...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `zentar send`."
---

# Pipe Script Output to Messaging Platforms

`zentar send` is a small, scriptable CLI that pushes a message to any
messaging platform Zentar is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | zentar send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `zentar
...
zentar send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
zentar send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
zentar send --list

# Filter by platform
zentar send --list telegram
```
...
Any platform Zentar ships adapters for works as a target:
...
`zentar send` resolves the message body in this order:

1. **Positional argument** — `zentar send --to telegram "hi"`
2. **`--file PATH`** — `zentar send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | zentar send --to telegram`

When stdin is a TTY (no pipe), Zentar does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  zentar send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `zentar send` reuses your Zentar config, the same script works on
any host where Zentar is installed — no need to export bot tokens into
...
`zentar send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  zentar send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | zentar send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.zentar/bin/zentar send \
...
```bash
./train.py --epochs 200 && \
  zentar send --to telegram "training done" || \
  zentar send --to telegram "training failed (exit $?)"
```
...
zentar send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(zentar send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `zentar send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `zentar send` calls
the platform's REST endpoint directly using credentials from
`~/.zentar/.env` and `~/.zentar/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `zentar gateway start` and retry.
...
# Machine-readable
zentar send --list --json
```

The listing is built from `~/.zentar/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`zentar gateway
...
| Approach | Multi-platform | Reuses Zentar creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `zentar send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`zentar send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `zentar send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `zentar send` shares with cron delivery.
