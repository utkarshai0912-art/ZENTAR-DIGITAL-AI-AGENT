---
sidebar_position: 7
title: "Use SOUL.md with Zentar"
description: "How to use SOUL.md to shape ZENTAR DIGITAL AI AGENT's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Zentar

`SOUL.md` is the **primary identity** for your Zentar instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Zentar to feel like the same assistant every time you talk to it — or if you want to replace the Zentar persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Zentar should be
- what Zentar should avoid stylistically
- how Zentar should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Zentar is and how Zentar speaks
...
Zentar now uses only the global SOUL file for the current instance:

```text
~/.zentar/SOUL.md
```

If you run Zentar with a custom home directory, it becomes:
...
Zentar automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Zentar does not overwrite it
- if the file exists but is empty, Zentar adds nothing from it to the prompt

## How Zentar uses it

When Zentar starts a session, it reads `SOUL.md` from `ZENTAR_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Zentar feels.
...
Zentar already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Zentar should sound.

# Avoid
What Zentar should not do.

# Defaults
How Zentar should behave when ambiguity appears.
```
...
Then restart Zentar or start a new session.
...
### I edited SOUL.md but Zentar still sounds the same

Check:
- you edited `~/.zentar/SOUL.md` or `$ZENTAR_HOME/SOUL.md`
...
### Zentar is ignoring parts of my SOUL.md
