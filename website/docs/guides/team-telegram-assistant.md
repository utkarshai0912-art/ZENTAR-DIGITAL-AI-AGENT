...
This tutorial walks you through setting up a Telegram bot powered by ZENTAR DIGITAL AI AGENT that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **ZENTAR DIGITAL AI AGENT installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.zentar/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Zentar itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Zentar Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_zentar_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by ZENTAR DIGITAL AI AGENT. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Zentar Gateway...
...
# View live logs
journalctl --user -u zentar-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `zentar gateway install` to update the plist.
:::
...
```bash
zentar gateway stop && zentar gateway start
```
...
zentar pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.zentar/.env`:
...
Control how much detail the bot shows when using tools. In `~/.zentar/config.yaml`:
...
Customize how the bot communicates by editing `~/.zentar/SOUL.md`:

For a full guide, see [Use SOUL.md with Zentar](/guides/use-soul-with-zentar).
...
```bash
# From the CLI
zentar cron list          # View all scheduled jobs
zentar cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u zentar-gateway` (Linux) or `~/.zentar/logs/gateway.log` (macOS) |
| Cron job output | `~/.zentar/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.zentar/cron/jobs.json` |
| Pairing data | `~/.zentar/pairing/` |
| Session history | `~/.zentar/sessions/` |
