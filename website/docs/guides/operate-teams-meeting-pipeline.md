...
```bash
zentar teams-pipeline validate
```
...
```bash
zentar teams-pipeline token-health
zentar teams-pipeline token-health --force-refresh
```
...
```bash
zentar teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
zentar teams-pipeline maintain-subscriptions
zentar teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Zentar cron (recommended if you already run the Zentar gateway)

Zentar ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.zentar/scripts/`. First create the script:

```bash
mkdir -p ~/.zentar/scripts
cat > ~/.zentar/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec zentar teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.zentar/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
zentar cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/zentar-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Zentar Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=zentar
EnvironmentFile=/etc/zentar/env
ExecStart=/usr/local/bin/zentar teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/zentar-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Zentar Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now zentar-teams-pipeline-maintain.timer
systemctl list-timers zentar-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/zentar teams-pipeline maintain-subscriptions >> /var/log/zentar/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.zentar/.env` at the top of a wrapper script that crontab calls.
...
```bash
zentar teams-pipeline subscriptions   # should show expirationDateTime advanced
zentar teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
zentar teams-pipeline list
zentar teams-pipeline list --status failed
zentar teams-pipeline show <job-id>
```

### Replay a stored job

```bash
zentar teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
zentar teams-pipeline fetch --meeting-id <meeting-id>
zentar teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `zentar teams-pipeline maintain-subscriptions --dry-run`
- inspect `zentar teams-pipeline list --status failed`
...
- run `zentar teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `zentar teams-pipeline run`
...
- [ ] `zentar teams-pipeline validate` returns an OK snapshot
- [ ] `zentar teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Zentar cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
