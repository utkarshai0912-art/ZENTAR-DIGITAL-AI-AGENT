...
description: "Diagnose and fix common Zentar cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`zentar gateway` for foreground, or `zentar gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `zentar cron tick`.
...
```bash
date
zentar cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.zentar/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.zentar/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.zentar/.env` |
...
| `local` | Write access to `~/.zentar/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `zentar cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `zentar skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `zentar skills list`.
...
2. `~/.zentar/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `zentar cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Zentar config directory). Verify:
```bash
ls ~/.zentar/scripts/your-script.py   # Must exist
zentar cron edit <job_id> --script ~/.zentar/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `zentar skills install <skill-name>`.
...
Jobs are stored in `~/.zentar/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.zentar/cron/jobs.json
chmod 600 ~/.zentar/cron/jobs.json   # Your user should own it
```
...
```bash
zentar cron list                    # Show all jobs, states, next_run times
zentar cron run <job_id>            # Schedule for next tick (for testing)
zentar cron edit <job_id>           # Fix configuration issues
zentar logs                         # View recent Zentar logs
zentar skills list                  # Verify installed skills
```
...
1. Run the job with `zentar cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.zentar/logs/agent.log` for scheduler messages and `~/.zentar/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/hermes-digital-agent](https://github.com/NousResearch/hermes-digital-agent) with:
