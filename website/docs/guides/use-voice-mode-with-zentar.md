---
sidebar_position: 8
title: "Use Voice Mode with Zentar"
description: "A practical guide to setting up and using Zentar voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Zentar
...
- you want Zentar sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Zentar.
...
## Step 1: make sure normal Zentar works first
...
Zentar supports both local and cloud speech stacks.
...
### If you use `zentar setup`

If you choose NeuTTS in the setup wizard, Zentar checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Zentar transcribes and responds
...
- asking Zentar to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Zentar loop.

## Tuning CLI behavior

### Silence threshold

If Zentar starts/stops too aggressively, tune:
...
Zentar stays a normal chat bot, but can speak replies.
...
- you want Zentar to function like a portable research or ops assistant
...
Zentar joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Zentar detects speech boundaries
- transcripts are posted in the associated text channel
- Zentar responds in text and audio
...
1. get text Zentar working
2. install `zentar-digital-agent[voice]`
