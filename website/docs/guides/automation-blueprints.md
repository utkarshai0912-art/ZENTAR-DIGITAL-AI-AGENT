...
Copy-paste blueprints for common automation patterns. Each blueprint uses Zentar's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`zentar webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `zentar webhook subscribe`) |
...
```bash
zentar cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/zentar-digital-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/zentar-digital-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
zentar webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
zentar cron create "0 9 * * 1" \
  "Scan the NousResearch/zentar-digital-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/zentar-digital-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (zentar_cli/commands.py, zentar_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (zentar_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
zentar cron create "0 6 * * *" \
  "Run a dependency security audit on the zentar-digital-agent project.

1. cd ~/.zentar/zentar-digital-agent && source .venv/bin/activate
...
```bash
zentar webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
zentar webhook subscribe alert-triage \
...
```python title="~/.zentar/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Zentar-Monitor/1.0"})
...
zentar cron create "every 30m" \
  ...
  --script ~/.zentar/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
zentar cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to ZENTAR DIGITAL AI AGENT development." \
...
```bash
zentar webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
zentar webhook subscribe stripe-payments \
...
```bash
zentar cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the zentar-digital-agent codebase.
