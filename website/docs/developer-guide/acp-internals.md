...
The ACP adapter wraps Zentar' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zentar acp / zentar-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.zentar/.env
  -> configure stderr logging
  -> construct ZentarACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'zentar-digital-agent[acp]==<version>' zentar-acp`, pointed at the `zentar-digital-agent` PyPI release.
...
- `allow_once` -> Zentar `once`
- `allow_always` -> Zentar `always`
- reject options -> Zentar `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Zentar tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zentar-acp"])
...
Instead it reuses Zentar' runtime resolver:

- `acp_adapter/auth.py`
- `zentar_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Zentar provider/credentials. It also always advertises a terminal setup auth method (`zentar-setup`, args `--setup`) so first-run registry clients can open Zentar' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.zentar/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `zentar-acp` toolset definition
- `zentar_cli/main.py` — `zentar acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zentar-acp` script
