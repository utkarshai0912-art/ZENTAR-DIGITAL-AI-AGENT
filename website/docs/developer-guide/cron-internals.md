...
description: "How Zentar stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zentar_cli/cron.py` | CLI `zentar cron` subcommands |
...
Jobs are stored in `~/.zentar/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `zentar cron` commands are run or during active CLI sessions.
...
```python
# ~/.zentar/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.zentar/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `zentar cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `zentar cron` CLI provides direct job management:

```bash
zentar cron list                    # Show all jobs
zentar cron create                  # Interactive job creation (alias: add)
zentar cron edit <job_id>           # Edit job configuration
zentar cron pause <job_id>          # Pause a running job
zentar cron resume <job_id>         # Resume a paused job
zentar cron run <job_id>            # Trigger immediate execution
zentar cron remove <job_id>         # Delete a job
```
