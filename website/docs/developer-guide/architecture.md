...
sidebar_position: 1
title: "Architecture"
description: "ZENTAR DIGITAL AI AGENT internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of ZENTAR DIGITAL AI AGENT internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ zentar_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ZentarCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── zentar_state.py           # SQLite session/state database with FTS5
├── zentar_constants.py       # ZENTAR_HOME, profile-aware paths
...
│
├── zentar_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `zentar` subcommands (large file)
...
│   ├── skills_config.py      # zentar skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # zentar tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # zentar gateway start/stop
│
...
```text
User input → ZentarCLI.process_input()
...
Three discovery sources: `~/.zentar/plugins/` (user), `.zentar/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `zentar plugins` or `config.yaml`.
...
Exposes Zentar as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`zentar -p <name>`) gets its own ZENTAR_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
