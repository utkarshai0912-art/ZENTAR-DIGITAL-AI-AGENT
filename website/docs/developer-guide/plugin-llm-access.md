...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Zentar already has applies. The plugin never sees a
...
[`zentar-example-plugins`](https://github.com/NousResearch/hermes-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
zentar-digital-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Zentar provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Zentar provider
        # (must be one Zentar already knows about — same names as
        # `zentar model` and config.yaml model.provider).
        ...
        # plugin sends — Zentar does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.zentar/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Zentar' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Zentar subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/hermes-digital-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/hermes-digital-agent/blob/main/tests/agent/test_plugin_llm.py)
