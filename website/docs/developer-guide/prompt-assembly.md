...
description: "How Zentar builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Zentar deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.zentar.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.zentar/SOUL.md)
You are Zentar, an AI assistant created by ZENTAR DIGITAL.
...
The platform hint (Layer 10 above) is the per-surface guidance Zentar
...
`SOUL.md` lives at `~/.zentar/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_zentar_md(cwd_path)       # 1. .zentar.md / ZENTAR.md (walks to git root)
    ...
    # SOUL.md from ZENTAR_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.zentar.md`, `ZENTAR.md` | CWD up to git root | Zentar-native project config |
...
- **YAML frontmatter stripped** — `.zentar.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Zentar concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.zentar.md` / `ZENTAR.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Zentar already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.zentar/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.zentar/MEMORY.md` and `~/.zentar/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.zentar.md`, `ZENTAR.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Zentar.
- Ephemeral overlays such as `ZENTAR_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Zentar assembles prompts for everyone, change Python and treat it as a code contribution
