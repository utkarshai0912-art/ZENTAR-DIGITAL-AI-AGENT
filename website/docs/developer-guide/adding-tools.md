...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to ZENTAR DIGITAL AI AGENT — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Zentar tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Zentar core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Zentar Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_ZENTAR_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `zentar_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `zentar_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `zentar chat -q "Use the weather tool for London"`
