# ZENTAR DIGITAL AI AGENT Security Policy

This document describes ZENTAR DIGITAL AI AGENT's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/hermes-digital-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **ZENTAR DIGITAL AI AGENT does not operate a bug
bounty program.**
...
- Environment details (`zentar version`, commit SHA, OS, Python
  version).
...
ZENTAR DIGITAL AI AGENT is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running ZENTAR DIGITAL AI AGENT,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted ZENTAR DIGITAL AI AGENT access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in ZENTAR DIGITAL AI AGENT's documentation
...
ZENTAR DIGITAL AI AGENT supports two OS-level isolation postures. They address
...
ZENTAR DIGITAL AI AGENT supports this in two ways:

- **ZENTAR DIGITAL AI AGENT's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, ZENTAR DIGITAL AI AGENT's in-process heuristics
...
ZENTAR DIGITAL AI AGENT filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in ZENTAR DIGITAL AI AGENT
itself. Bugs in ZENTAR DIGITAL AI AGENT's plugin-install or plugin-discovery
...
**Surfaces in ZENTAR DIGITAL AI AGENT:**
...
4. **Within the authorized set, all callers are equally trusted.**
   ZENTAR DIGITAL AI AGENT does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, ZENTAR DIGITAL AI AGENT's own documentation, or reasonable
  operator expectations would predict — including cases where
  ZENTAR DIGITAL AI AGENT has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  zentar-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not ZENTAR DIGITAL AI AGENT's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in ZENTAR DIGITAL AI AGENT. Bugs in Zentar
...
- ZENTAR DIGITAL AI AGENT includes supply-chain guards for MCP server
