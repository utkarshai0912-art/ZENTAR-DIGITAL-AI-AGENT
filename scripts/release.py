#!/usr/bin/env python3
"""ZENTAR DIGITAL AI AGENT Release Script
...
VERSION_FILE = REPO_ROOT / "zentar_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level ZENTAR_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (zentar journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "zentar.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.zentar/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows zentar-shim resolution + prefer --update on recovery; #52378)
    ...
    "zentar@marian.local": "Schrotti77",
    ...
    "274096618+zentar-digital-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "zentar@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on zentar mcp add)
    ...
    "zentar@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@zentar-digital-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ZENTAR_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ZENTAR_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: zentar-eval community link)
    ...
    "vigo@zentar": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.zentar@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Zentar runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ZENTAR_HOME state files)
    ...
    "zhchl@zentar-digital-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
}
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Zentar
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"zentar-digital-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "zentar@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hermes-digital-agent",
    ...
    lines.append(f"# ZENTAR DIGITAL AI AGENT v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for ZENTAR DIGITAL AI AGENT. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="ZENTAR DIGITAL AI AGENT Release Tool")
    ...
    print(f"{'='*60}")
    print("  ZENTAR DIGITAL AI AGENT Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"ZENTAR DIGITAL AI AGENT v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"ZENTAR DIGITAL AI AGENT v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'ZENTAR DIGITAL AI AGENT v{new_version} ({calver_date})' "
