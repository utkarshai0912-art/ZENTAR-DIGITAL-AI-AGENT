#!/usr/bin/env node
/**
 * ZENTAR DIGITAL AI AGENT WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.zentar/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.zentar', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (ZENTAR_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.ZENTAR_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.zentar', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.ZENTAR_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.zentar', 'document_cache');
const AUDIO_CACHE_DIR = process.env.ZENTAR_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.zentar', 'audio_cache');
...
// `zentar update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *ZENTAR DIGITAL AI AGENT*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Zentar itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['ZENTAR DIGITAL AI AGENT', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Zentar and trigger
      ...
      // Ignore Zentar' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `zentar_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `zentar_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
