...
#   2. ~/.zentar/node/ from a prior Zentar-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.zentar/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$ZENTAR_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   ZENTAR_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   ZENTAR_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   ZENTAR_HOME               (default: $HOME/.zentar)
# ============================================================================

ZENTAR_NODE_MIN_VERSION="${ZENTAR_NODE_MIN_VERSION:-20}"
ZENTAR_NODE_TARGET_MAJOR="${ZENTAR_NODE_TARGET_MAJOR:-22}"
ZENTAR_HOME="${ZENTAR_HOME:-$HOME/.zentar}"
ZENTAR_NODE_AVAILABLE=false
...
# Redirect a Zentar-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $ZENTAR_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$ZENTAR_HOME/node/etc/npmrc"
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$ZENTAR_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $ZENTAR_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$ZENTAR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$ZENTAR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $ZENTAR_NODE_TARGET_MAJOR..."
    proto install node "$ZENTAR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $ZENTAR_NODE_TARGET_MAJOR..."
    nvm install "$ZENTAR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$ZENTAR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${ZENTAR_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${ZENTAR_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${ZENTAR_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${ZENTAR_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${ZENTAR_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $ZENTAR_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $ZENTAR_HOME/node/..."
    ...
    mv "$extracted" "$ZENTAR_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$ZENTAR_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$ZENTAR_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$ZENTAR_HOME/node/bin/npx"  "$_link_dir/npx"
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ZENTAR_HOME/node/"
    return 0
}

# ---------------------------------------------------------------------------
# Heal a broken Zentar-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$ZENTAR_HOME/node/bin/$tool" \
        "$ZENTAR_HOME/node/${tool}.exe" \
        "$ZENTAR_HOME/node/$tool"; do
...
# absent. Used by zentar_constants.find_zentar_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$ZENTAR_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Zentar-managed Node is broken — redownloading to $ZENTAR_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        ZENTAR_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$ZENTAR_HOME/node/bin/node" ]; then
        export PATH="$ZENTAR_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Zentar-managed)"
            ZENTAR_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { ZENTAR_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { ZENTAR_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { ZENTAR_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { ZENTAR_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { ZENTAR_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { ZENTAR_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $ZENTAR_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
