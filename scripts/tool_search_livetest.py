#!/usr/bin/env python3
"""Live test harness for ZENTAR DIGITAL AI AGENT's Tool Search feature.
...
# Force-isolate the test environment BEFORE any zentar imports.
ORIGINAL_HOME = os.environ.get("ZENTAR_HOME")
ORIGINAL_AUTH = Path.home() / ".zentar" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.zentar/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.zentar/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="zentar_ts_live_"))
    zentar_home = home_dir / ".zentar"
    zentar_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, zentar_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".zentar" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, zentar_home / ".env")
        ...
        load_zentar_dotenv(zentar_home=str(Path.home() / ".zentar"))
    ...
    (zentar_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return zentar_home
...
def reset_module_state():
    """Drop cached modules so the new ZENTAR_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "zentar_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ZENTAR_HOME"] = str(home)
...
def main():
    ...
    # Restore original ZENTAR_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ZENTAR_HOME"] = ORIGINAL_HOME
    else:
