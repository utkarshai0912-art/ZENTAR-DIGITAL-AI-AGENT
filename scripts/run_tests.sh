#!/usr/bin/env bash
# Canonical test runner for zentar-digital-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.zentar/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.zentar/zentar-digital-agent/venv"; do
...
if [ -f "$HOME/.zentar/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.zentar"
...
exec env -i \
  ...
  ${ZENTAR_RUN_SLOW_PET_TESTS:+ZENTAR_RUN_SLOW_PET_TESTS="$ZENTAR_RUN_SLOW_PET_TESTS"} \
