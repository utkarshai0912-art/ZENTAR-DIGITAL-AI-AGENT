...
Auto-tracks and cleans up ephemeral files created during ZENTAR DIGITAL AI AGENT
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZENTAR_HOME` and `/tmp/zentar-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZENTAR_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZENTAR_HOME` or `/tmp/zentar-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZENTAR_HOME/disk-cleanup/` is itself excluded
- `$ZENTAR_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
