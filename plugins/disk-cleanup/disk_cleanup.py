"""disk_cleanup — ephemeral file cleanup for ZENTAR DIGITAL AI AGENT.
...
Rules:
  ...
  - empty dirs    → always delete (under ZENTAR_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ZENTAR_HOME and /tmp/zentar-*
Never touches: ~/.zentar/logs/ or any system directory.
"""
...
try:
    ...
    def get_zentar_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZENTAR_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".zentar").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ZENTAR_HOME/logs/``."""
    return get_zentar_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ZENTAR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZENTAR_HOME or ``/tmp/zentar-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zentar_home = get_zentar_home()
    try:
        path.resolve().relative_to(zentar_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zentar-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zentar-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "zentar-digital-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ZENTAR_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ZENTAR_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        zentar_home = get_zentar_home()
        for parent in ("cron", "cronjobs"):
            base = zentar_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZENTAR_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ZENTAR_HOME, but never recurse into known
    # durable state trees.  Some installs place the Zentar checkout, venv,
    # and desktop build under ZENTAR_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    zentar_home = get_zentar_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in zentar_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zentar_home = get_zentar_home()
    try:
        rel = path.resolve().relative_to(zentar_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "zentar-digital-agent",
        }:
            return None
        ...
        # Path isn't under ZENTAR_HOME (e.g. /tmp/zentar-*) — fall through.
        pass
