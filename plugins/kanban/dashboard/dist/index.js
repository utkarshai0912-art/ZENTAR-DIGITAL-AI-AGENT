/**
 * Zentar Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zentar/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZENTAR_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ZENTAR_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "zentar-kanban-dot-triage",
    todo: "zentar-kanban-dot-todo",
    ready: "zentar-kanban-dot-ready",
    running: "zentar-kanban-dot-running",
    blocked: "zentar-kanban-dot-blocked",
    done: "zentar-kanban-dot-done",
    archived: "zentar-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zentar-task";
  ...
  const LS_BOARD_KEY = "zentar.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zentar-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])zentar-kanban-md-code\1/i.test(attrs)) {
      return ' class="zentar-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zentar-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zentar-kanban-md",
  ...
  // standard `drop` event and our `zentar-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zentar-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zentar-kanban-column--drop");
          if (target) target.classList.add("zentar-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zentar-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("zentar-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("zentar-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ZENTAR_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zentar-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zentar-kanban-attention",
        "zentar-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zentar-kanban-attention-bar" },
        h("span", { className: "zentar-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zentar-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zentar-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zentar-kanban-attention-row",
                  "zentar-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zentar-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zentar-kanban-attention-row-id" }, task.id),
                h("span", { className: "zentar-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zentar-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zentar-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zentar-kanban-diag-action-btn",
      action.suggested ? "zentar-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zentar-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zentar-kanban-drawer-comment-row input, .zentar-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zentar-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zentar-kanban-diag", sevClass) },
      h("div", { className: "zentar-kanban-diag-header" },
        h("span", { className: "zentar-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zentar-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zentar-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zentar-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zentar-kanban-diag-data-row" },
                  h("span", { className: "zentar-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zentar-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zentar-kanban-diag-data-row" },
                h("span", { className: "zentar-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zentar-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zentar-kanban-diag-reassign-row" },
            h("span", { className: "zentar-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zentar-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zentar-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zentar-kanban-diag-msg",
              msg.ok ? "zentar-kanban-diag-msg--ok" : "zentar-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zentar-kanban-section" },
      h("div", { className: "zentar-kanban-section-head-row" },
        h("span", { className: "zentar-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zentar-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zentar-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zentar-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zentar-kanban-docs-link",
      title: "Open Zentar Kanban docs in a new tab",
      "aria-label": "Zentar Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `zentar kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "zentar-kanban-msg-ok" : "zentar-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zentar-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zentar-kanban-boardswitcher" },
      h("div", { className: "zentar-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zentar-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zentar-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zentar-kanban-dialog-title" },
        ...
        h("div", { className: "zentar-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Zentar profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zentar-kanban-bulk" },
      h("span", { className: "zentar-kanban-bulk-count" },
      ...
      h("div", { className: "zentar-kanban-bulk-priority",
      ...
      h("div", { className: "zentar-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Zentar profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "zentar-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zentar-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("zentar-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "zentar-kanban-trash",
        dragOver ? "zentar-kanban-trash--drop" : "",
        props.draggingTaskId ? "zentar-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "zentar-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "zentar-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".zentar-kanban-card")) return true;
      if (target.closest && target.closest(".zentar-kanban-column-add")) return true;
      if (target.closest && target.closest(".zentar-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("zentar-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("zentar-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zentar-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "zentar-kanban-columns",
        isScrollable ? "zentar-kanban-columns--scrollable" : "",
        isPanning ? "zentar-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zentar-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zentar-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zentar-kanban-column",
        dragOver ? "zentar-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zentar-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "zentar-kanban-col-check",
        ...
        h("span", { className: cn("zentar-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zentar-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zentar-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zentar-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zentar-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zentar-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zentar-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zentar-kanban-lane" },
                  h("div", { className: "zentar-kanban-lane-head" },
                    h("span", { className: "zentar-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zentar-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zentar-kanban-card--stale-red";
    if (age >= tier.amber) return "zentar-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zentar-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zentar-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "zentar-kanban-card",
        props.selected ? "zentar-kanban-card--selected" : "",
        props.failed ? "zentar-kanban-card--failed" : "",
        props.draggingSource ? "zentar-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zentar-kanban-card-content" },
          h("div", { className: "zentar-kanban-card-row" },
            h("label", {
              className: "zentar-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "zentar-kanban-card-check",
            ...
            h("span", { className: "zentar-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zentar kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zentar-kanban-warning-badge",
                    "zentar-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "zentar-kanban-needs-assignee",
          ...
          h("div", { className: "zentar-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zentar-kanban-card-row zentar-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zentar-kanban-assignee",
                            title: `Assigned to Zentar profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zentar-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "zentar-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zentar-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "zentar-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Zentar profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Zentar profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zentar-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zentar-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zentar-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zentar-kanban-drawer-close",
        ...
        data ? h("div", { className: "zentar-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "zentar-kanban-section" },
      h("div", { className: "zentar-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "zentar-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zentar-kanban-drawer-body" },
      h("div", { className: "zentar-kanban-drawer-title" },
        h("span", { className: cn("zentar-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "zentar-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "zentar-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "zentar-kanban-section" },
        h("div", { className: "zentar-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "zentar-kanban-section" },
        h("div", { className: "zentar-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zentar-kanban-comment" },
                h("div", { className: "zentar-kanban-comment-head" },
                  h("span", { className: "zentar-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zentar-kanban-comment-ago" },
      ...
      h("div", { className: "zentar-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "zentar-kanban-event",
              isDiag ? "zentar-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zentar-kanban-event-header" },
                  h("span", { className: "zentar-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zentar-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zentar-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zentar-kanban-event-header-plain" },
                  h("span", { className: "zentar-kanban-event-kind" }, e.kind),
                  h("span", { className: "zentar-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zentar-kanban-event-phantom-row" },
                  h("span", { className: "zentar-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zentar-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zentar-kanban-event-payload" },
    ...
    return h("div", { className: "zentar-kanban-section" },
      h("div", { className: "zentar-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "zentar-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zentar-kanban-run--${r.outcome || r.status || "ended"}`
          : "zentar-kanban-run--active";
        return h("div", { key: r.id, className: cn("zentar-kanban-run", outcomeClass) },
          h("div", { className: "zentar-kanban-run-head" },
            h("span", { className: "zentar-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zentar-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zentar-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zentar-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zentar-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zentar-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zentar-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zentar-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zentar-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zentar-kanban-pre zentar-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zentar-kanban-meta-row" },
      h("span", { className: "zentar-kanban-meta-label" }, props.label),
      h("span", { className: "zentar-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zentar-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zentar-kanban-meta-row" },
        h("span", { className: "zentar-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zentar-kanban-meta-value zentar-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zentar-kanban-meta-row" },
        h("span", { className: "zentar-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zentar-kanban-meta-value zentar-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zentar-kanban-section" },
      h("div", { className: "zentar-kanban-section-head-row" },
        h("span", { className: "zentar-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "zentar-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zentar-kanban-section" },
      h("div", { className: "zentar-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zentar-kanban-deps-row" },
        h("span", { className: "zentar-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zentar-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zentar-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zentar-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zentar-kanban-dep-chip-x",
      ...
      h("div", { className: "zentar-kanban-deps-row" },
        h("span", { className: "zentar-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zentar-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zentar-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "zentar-kanban-msg-ok"
          : "zentar-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zentar-kanban-section" },
      ...
      h("div", { className: "zentar-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zentar-kanban-home-sub zentar-kanban-home-sub--on"
              : "zentar-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ZENTAR_PLUGINS__ && typeof window.__ZENTAR_PLUGINS__.register === "function") {
    window.__ZENTAR_PLUGINS__.register("kanban", KanbanPage);
  }
})();
