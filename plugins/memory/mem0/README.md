...
```bash
zentar memory setup    # select "mem0"
```
...
Behavioral settings live in `$ZENTAR_HOME/mem0.json` (set them via `zentar memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.zentar/.env`.
...
| `user_id` | `zentar-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Zentar session and call `mem0_search` — it connects to your server.
...
```bash
zentar memory setup mem0 --mode oss \
...
```bash
zentar memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ZENTAR_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.zentar/mem0_qdrant"}}
  }
}
```
...
```bash
zentar memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ZENTAR_HOME/mem0.json`).
