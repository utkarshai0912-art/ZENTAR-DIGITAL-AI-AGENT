...
Zentar spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.zentar/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p zentar ui start
```
...
Config file: `~/.zentar/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `zentar` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `zentar-{profile}` isolates memory per active Zentar profile. Empty placeholders collapse cleanly (e.g. `zentar-{user}` with no user becomes `zentar`). |
...
The LLM API key is stored in `~/.zentar/.env` as `HINDSIGHT_LLM_API_KEY`.
