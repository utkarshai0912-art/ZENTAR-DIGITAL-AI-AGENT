...
- Supermemory API key from [app.supermemory.ai/integrations?connect=zentar](http://app.supermemory.ai/integrations?connect=zentar)
...
Config file: `$ZENTAR_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zentar` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zentar-{identity}` → `zentar-coder`). |
...
All Supermemory API calls send `x-sm-source: zentar`, and document writes stamp
`metadata.sm_source: zentar`. This is a **functional routing key, not telemetry**:
it groups Zentar-written memories into a dedicated "Zentar" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Zentar profile:

```json
{
  "container_tag": "zentar-{identity}"
}
```

For a profile named `coder`, this resolves to `zentar-coder`. The default profile resolves to `zentar-default`. Without `{identity}`, all profiles share the same container.
