...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=zentar"
...
def _load_supermemory_config(zentar_home: str) -> dict:
    config = _default_config()
    config_path = Path(zentar_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zentar_home: str) -> None:
    config_path = Path(zentar_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "zentar"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Zentar writes into the "Zentar" Space in the Supermemory
        ...
        merged = {"sm_source": "zentar", **(metadata or {})}
...
def _resolve_container_tag_for_setup(zentar_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(zentar_home)
...
def _probe_supermemory_connection(api_key: str, zentar_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(zentar_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(zentar_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zentar memory setup`.
        # All other options are documented for $ZENTAR_HOME/supermemory.json
    ...
    def save_config(self, values, zentar_home):
        ...
        _save_supermemory_config(sanitized, zentar_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, zentar_home)
    ...
    def post_setup(self, zentar_home: str, config: dict) -> None:
        ...
        from zentar_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(zentar_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from zentar_constants import get_zentar_home
        self._zentar_home = kwargs.get("zentar_home") or str(get_zentar_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zentar_home)
