...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zentar>
...
```bash
zentar memory setup honcho   # configure Honcho directly (works on a fresh install)
zentar memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
zentar config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.zentar/.env
```

> `zentar honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `zentar memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZENTAR_HOME/honcho.json` | Profile-local (isolated Zentar instances) |
| 2 | `~/.zentar/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Zentar profile: `zentar` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `zentar honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Zentar session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`zentar-digital-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`zentar`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-zentar-digital-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening zentar in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Zentar session ID (timestamp + hex). Every `zentar` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Zentar profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`zentar`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Zentar profile: `zentar` (default) or `hermes_<profile>` (e.g. `zentar -p coder` -> host key `zentar_coder`). Older `zentar.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ZENTAR_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `zentar-digital-agent`) |
...
|---------|-------------|
| `zentar memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `zentar honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `zentar memory setup`) |
| `zentar honcho status` | Show resolved config for active profile |
| `zentar honcho enable` / `disable` | Toggle Honcho for active profile |
| `zentar honcho mode <mode>` | Change recall or observation mode |
| `zentar honcho peer --user <name>` | Update user peer name |
| `zentar honcho peer --ai <name>` | Update AI peer name |
| `zentar honcho tokens --context <N>` | Set context token budget |
| `zentar honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zentar honcho map <name>` | Map current directory to a session name |
| `zentar honcho sync` | Create host blocks for all Zentar profiles |
