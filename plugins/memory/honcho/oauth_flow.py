...
``zentar://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Zentar OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "zentar-digital-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``zentar-desktop`` / ``zentar-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Zentar.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ZENTAR_HOME override can't reach.
