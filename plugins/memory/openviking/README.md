...
connection values into Zentar, or create a minimal `ovcli.conf` when one does
not exist.
...
| `OPENVIKING_AGENT` | `zentar` | Zentar peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Zentar lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Zentar sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Zentar built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Zentar action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Zentar
...
file URIs, such as
`viking://user/peers/zentar/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/zentar/memories/preferences/mem_abc123.md`. Files
