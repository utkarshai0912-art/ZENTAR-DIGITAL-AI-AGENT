...
request omits ``thinking``.  Zentar' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Zentar reasoning
...
(per Z.AI / BigModel docs).  Zentar' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Zentar reasoning effort onto GLM-5.2's native ``high``/``max``.
