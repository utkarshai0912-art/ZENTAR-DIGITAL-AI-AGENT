"""
Microsoft Teams platform adapter for ZENTAR DIGITAL AI AGENT.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Zentar"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; ZentarAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        zentar_action = data.get("zentar_action", "")
        session_key = data.get("session_key", "")

        if not zentar_action or not session_key:
        ...
        choice = choice_map.get(zentar_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="zentar_approve",
                    data={**btn_data_base, "zentar_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="zentar_approve",
                    data={**btn_data_base, "zentar_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="zentar_approve",
                    data={**btn_data_base, "zentar_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="zentar_approve",
                    data={**btn_data_base, "zentar_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Zentar\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.zentar/.env")
