"""SimpleX Chat platform adapter (Zentar plugin).
...
This adapter ships as a Zentar platform plugin under
``plugins/platforms/simplex/``. The Zentar plugin loader scans the
...
Optional environment variables:
    ...
    ZENTAR_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``zentar setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Zentar — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ZENTAR_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zentar setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.zentar/.env`` via
    ``zentar_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "zentar_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.zentar/.env"
        )
        return
