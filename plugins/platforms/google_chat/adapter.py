...
# ``model_tools`` import time, so ``zentar status``, ``zentar chat``, etc.
...
# ``zentar_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Zentar is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a zentar MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ZENTAR_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _zentar_home = _get_zentar_home()
        except (ModuleNotFoundError, ImportError):
            _zentar_home = _Path.home() / ".zentar"
        self._thread_count_store = _ThreadCountStore(
            _zentar_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Zentar is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ZENTAR_HOME", str(_Path.home() / ".zentar"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ZENTAR_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a zentar MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Zentar is thinking…' marker message.
        ...
        "Zentar is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Zentar is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Zentar is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Zentar is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Zentar is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``zentar setup``.

    The setup wizard at ``zentar_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.zentar/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. zentar-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.zentar/.env")
    print_info("Restart the gateway: zentar gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``zentar cron`` running as a
    separate process from ``zentar gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Zentar plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zentar-digital-agent[google_chat]'",
        ...
        # the "Zentar is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Zentar is thinking…' marker message "
