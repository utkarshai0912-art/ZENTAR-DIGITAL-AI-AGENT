"""
Photon Spectrum (iMessage) platform adapter for ZENTAR DIGITAL AI AGENT.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Zentar dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `zentar photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `zentar setup` / status.
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `zentar update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``zentar photon install-sidecar``: ``npm ci`` for an exact,
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Zentar defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Zentar-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Zentar checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `zentar photon setup`)"
            )
        # A `zentar update` that bumps the spectrum-ts pin rewrites
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Zentar-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Zentar plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `zentar photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: zentar photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `zentar gateway setup` alongside every other
    ...
    # Register CLI subcommands — `zentar photon ...`
