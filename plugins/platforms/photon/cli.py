"""
``zentar photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Zentar
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `zentar photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'ZENTAR DIGITAL AI AGENT')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``zentar photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "ZENTAR DIGITAL AI AGENT" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.zentar/.env,
    ...
    print("  Start the gateway:  zentar gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `zentar photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`zentar photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.zentar/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `zentar photon telemetry on` / `zentar photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.zentar/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  zentar gateway restart")
    return 0
...
#
# `zentar gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``zentar photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `zentar gateway setup` wizard."""
