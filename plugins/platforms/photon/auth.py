...
Credential storage mirrors every other Zentar channel:

    * runtime SDK creds  -> ``~/.zentar/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.zentar/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a zentar dependency
...
# registers Zentar as its own client_id.
...
# Default name of the project Zentar provisions for the operator.
DEFAULT_PROJECT_NAME = "ZENTAR DIGITAL AI AGENT"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of zentar-digital-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.zentar/auth.json`` honouring the active Zentar profile."""
    try:
        from zentar_constants import get_zentar_home
        return Path(get_zentar_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.zentar")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.zentar/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.zentar/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.zentar/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: zentar_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `zentar photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `zentar photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `zentar photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `zentar photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `zentar photon setup`)"
