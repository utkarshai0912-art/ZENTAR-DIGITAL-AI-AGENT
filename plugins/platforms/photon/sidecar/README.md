...
Small Node helper that bridges ZENTAR DIGITAL AI AGENT to Photon's Spectrum SDK
(`spectrum-ts`).  Zentar is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zentar-Sidecar-Token`)
...
The Zentar plugin's `zentar photon setup` command runs `npm install`
here automatically.
