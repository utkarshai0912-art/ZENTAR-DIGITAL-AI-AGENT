// Zentar Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Zentar): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Zentar -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Zentar-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `zentar photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Zentar can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Zentar'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Zentar allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Zentar, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-zentar-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Zentar supplied them so a known-good
