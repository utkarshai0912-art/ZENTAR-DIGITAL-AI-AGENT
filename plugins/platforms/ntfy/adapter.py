"""ntfy platform adapter (Zentar plugin).
...
``plugins/platforms/ntfy/``. The Zentar plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "zentar-in"              # subscribe topic (incoming)
          publish_topic: "zentar-out"     # optional — defaults to topic
...
_ECHO_TAG = "zentar-digital-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``zentar cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Zentar dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `zentar gateway status` without
