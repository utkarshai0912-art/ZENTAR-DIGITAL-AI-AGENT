"""
Email platform adapter for the Zentar gateway.

Allows users to interact with Zentar by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "zentar-digital-agent" "version" "{_zentar_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `zentar config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `zentar gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "ZENTAR DIGITAL AI AGENT")
        ...
        msg_id = f"<zentar-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# zentar_cli/gateway.py, and the _send_email dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        msg["From"] = address
        msg["To"] = chat_id
        msg["Subject"] = "ZENTAR DIGITAL AI AGENT"
        msg["Date"] = formatdate(localtime=True)
