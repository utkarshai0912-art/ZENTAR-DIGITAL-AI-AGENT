...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `zentar slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_zentar_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Zentar does not currently need to route them into the agent loop.
            ...
            # /model work directly without /zentar prefix). A single regex
            ...
            # manifest (see `zentar slack manifest`). In Socket Mode, Slack
            ...
            from zentar_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zentar$")

            @self._app.command(_slash_pattern)
            async def handle_zentar_command(ack, command):
            ...
            for _action_id in (
                "zentar_approve_once",
                "zentar_approve_session",
                "zentar_approve_always",
                "zentar_deny",
            ):
            ...
            for _action_id in (
                "zentar_confirm_once",
                "zentar_confirm_always",
                "zentar_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Zentar handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Zentar session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from zentar_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@zentar`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zentar_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "zentar_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "zentar_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zentar_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "zentar_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "zentar_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zentar_confirm_once": "once",
            "zentar_confirm_always": "always",
            "zentar_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zentar_approve_once": "once",
            "zentar_approve_session": "session",
            "zentar_approve_always": "always",
            "zentar_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zentar <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zentar
        ...
        if slash_name in {"zentar", ""}:
            # Legacy /zentar <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zentar_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/zentar   ``).
        ...
        # "Running /cmd…" ack shown by handle_zentar_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/zentar <question>" must produce public replies so
...
# ``zentar_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``zentar_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ZENTAR_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from zentar_cli.slack_cli import _build_full_manifest
            from zentar_constants import get_zentar_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Zentar",
                bot_description="Your ZENTAR DIGITAL AI AGENT on Slack",
            )
            target = Path(get_zentar_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `zentar slack manifest --write` anytime to refresh after "
                "Zentar adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zentar update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``zentar_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zentar-digital-agent[slack]'",
        # Interactive setup wizard — replaces zentar_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in zentar_cli/gateway.py.
