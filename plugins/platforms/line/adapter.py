"""
LINE Messaging API platform adapter for ZENTAR DIGITAL AI AGENT.
...
to ZENTAR DIGITAL AI AGENT. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ZENTAR_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            zentar_home = Path(get_zentar_home()).resolve()
        except Exception:
            zentar_home = Path.home().joinpath(".zentar").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``zentar status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``zentar status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zentar setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.zentar/.env`` via ``zentar_cli.config``.
    """
    print()
    ...
    try:
        from zentar_cli.config import get_env_var, set_env_var
    except ImportError:
        print("zentar_cli.config not available; set LINE_* vars manually in ~/.zentar/.env")
        return
