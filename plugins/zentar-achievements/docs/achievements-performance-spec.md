# Zentar Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: zentar-achievements plugin
...
### `GET /api/plugins/zentar-achievements/achievements`
...
### `POST /api/plugins/zentar-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/zentar-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/zentar-achievements/overview`
...
- Persist plugin data under:
  - `~/.zentar/plugins/zentar-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.zentar/plugins/zentar-achievements/scan_checkpoint.json`
...
- `~/.zentar/plugins/zentar-achievements/`
