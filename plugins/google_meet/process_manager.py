...
session-scoped state file under ``$ZENTAR_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ZENTAR_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_zentar_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "ZENTAR DIGITAL AI AGENT",
    ...
    If a bot is already running for this zentar install, leave it first —
    ...
    env = os.environ.copy()
    env["ZENTAR_MEET_URL"] = url
    env["ZENTAR_MEET_OUT_DIR"] = str(out)
    env["ZENTAR_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ZENTAR_MEET_HEADED"] = "1"
    if auth_state:
        env["ZENTAR_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ZENTAR_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ZENTAR_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ZENTAR_MEET_MODE"] = mode
    if realtime_model:
        env["ZENTAR_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ZENTAR_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ZENTAR_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ZENTAR_MEET_REALTIME_KEY"] = realtime_api_key
