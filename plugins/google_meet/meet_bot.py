...
``$ZENTAR_HOME/workspace/meetings/<meeting-id>/``. The main zentar process
...
Run standalone for debugging::
    ...
    ZENTAR_MEET_URL=https://meet.google.com/abc-defg-hij \\
    ZENTAR_MEET_OUT_DIR=/tmp/meet-debug \\
    ZENTAR_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``ZENTAR_MEET_OUT_DIR``.
...
# and exposes ``window.__zentarMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__zentarMeetInstalled) return;
  window.__zentarMeetInstalled = true;
  window.__zentarMeetQueue = [];
  ...
  window.__zentarMeetDrain = () => {
    const out = window.__zentarMeetQueue.slice();
    window.__zentarMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "zentar_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("ZENTAR_MEET_URL", "").strip()
    out_dir_env = os.environ.get("ZENTAR_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("ZENTAR_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("ZENTAR_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("ZENTAR_MEET_GUEST_NAME", "ZENTAR DIGITAL AI AGENT")
    duration_s = _parse_duration(os.environ.get("ZENTAR_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when ZENTAR_MEET_MODE=realtime.
    mode = os.environ.get("ZENTAR_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("ZENTAR_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("ZENTAR_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("ZENTAR_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("ZENTAR_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — ZENTAR_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: ZENTAR_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in ZENTAR_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("ZENTAR_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__zentarMeetDrain && window.__zentarMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__zentarMeetInstalled) {
