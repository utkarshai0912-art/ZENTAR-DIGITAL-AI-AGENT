...
Let the zentar agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zentar runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zentar meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zentar meet` CLI |
...
| `cli.py` | `zentar meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZENTAR_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zentar meet node {run,list,approve,remove,status,ping}` |
...
zentar meet install                                      # pip + Chromium
zentar meet setup                                        # preflight
zentar meet auth                                         # optional
zentar meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zentar meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.zentar/.env
zentar meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zentar meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zentar meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.zentar/.env
zentar meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zentar will **not** switch your system audio input automatically — the
...
zentar plugins enable google_meet
zentar meet node run --display-name my-mac --host 0.0.0.0 --port 18789
