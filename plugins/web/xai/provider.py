...
``{title, url, description, position}`` rows every other Zentar web
provider produces.
...
prefers Zentar-managed xAI Grok OAuth (via ``zentar auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.zentar/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``zentar tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `zentar auth` to sign in with "
