#!/usr/bin/env bash
# GitHub environment detection helper for ZENTAR DIGITAL AI AGENT skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _zentar_env="${ZENTAR_HOME:-$HOME/.zentar}/.env"; [ -f "$_zentar_env" ] && grep -q "^GITHUB_TOKEN=" "$_zentar_env" 2>/dev/null; then
