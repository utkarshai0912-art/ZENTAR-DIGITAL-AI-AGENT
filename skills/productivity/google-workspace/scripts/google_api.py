#!/usr/bin/env python3
"""Google Workspace API CLI for ZENTAR DIGITAL AI AGENT.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zentar-facing JSON contract and falls back to the Python client
...
from _zentar_home import get_zentar_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZENTAR_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ZENTAR DIGITAL AI AGENT")
