#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for ZENTAR DIGITAL AI AGENT.
...
# Ensure sibling modules (_zentar_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _zentar_home import display_zentar_home, get_zentar_home

ZENTAR_HOME = get_zentar_home()
TOKEN_PATH = ZENTAR_HOME / "google_token.json"
CLIENT_SECRET_PATH = ZENTAR_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ZENTAR_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Zentar profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Zentar Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Zentar Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'zentar-digital-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Zentar home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_zentar_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Zentar")
