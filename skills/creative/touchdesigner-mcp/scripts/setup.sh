...
ZENTAR_HOME_DIR="${ZENTAR_HOME:-$HOME/.zentar}"
ZENTAR_CFG="${ZENTAR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Zentar config has twozero_td MCP entry ──
if [[ ! -f "$ZENTAR_CFG" ]]; then
    echo -e " ${FAIL} Zentar config not found at ${ZENTAR_CFG}"
    manual_steps+=("Create ${ZENTAR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZENTAR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Zentar config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Zentar config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZENTAR_CFG} manually"); }
    manual_steps+=("Restart Zentar session to pick up config change")
fi
