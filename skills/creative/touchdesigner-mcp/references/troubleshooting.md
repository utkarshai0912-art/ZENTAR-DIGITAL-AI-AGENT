...
## 5. Zentar Configuration

### Config location

`$ZENTAR_HOME/config.yaml` (defaults to `~/.zentar/config.yaml` when `ZENTAR_HOME` is unset)
...
Restart the Zentar session for changes to take effect. The MCP connection is
