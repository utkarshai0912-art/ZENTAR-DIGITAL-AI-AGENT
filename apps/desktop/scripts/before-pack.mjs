 ...
 * product name (`Zentar`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Zentar'
 *
 * This is a hard failure with no obvious cause for the user — `zentar desktop`
 ...
 * (the mac-unpacked Zentar.app bundle) and Windows (win-unpacked), so we
