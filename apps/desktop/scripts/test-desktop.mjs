...
// no longer bundles the Zentar Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Zentar.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Zentar'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Zentar.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'zentar'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ZENTAR_HOME for non-sandboxed runs -- matches main.ts's
...
// ZENTAR_HOME and never touches this.
const DEFAULT_ZENTAR_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'zentar')
  }
  return path.join(os.homedir(), '.zentar')
})()
const VENV_ROOT = path.join(DEFAULT_ZENTAR_HOME, 'zentar-digital-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'zentar-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ZENTAR_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Zentar-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Zentar-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Zentar-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Zentar-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ZENTAR_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ZENTAR_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const zentarHome = path.join(sandbox, 'zentar-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(zentarHome, { recursive: true })
  ...
  env.ZENTAR_DESKTOP_CWD = cwd
  env.ZENTAR_DESKTOP_IGNORE_EXISTING = '1'
  env.ZENTAR_DESKTOP_TEST_MODE = 'fresh-install'
  env.ZENTAR_DESKTOP_USER_DATA_DIR = userDataDir
  env.ZENTAR_HOME = zentarHome
  delete env.ZENTAR_DESKTOP_HERMES
  delete env.ZENTAR_DESKTOP_ZENTAR_ROOT
  ...
  console.log(`  ZENTAR_HOME: ${zentarHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(zentarHome, 'zentar-digital-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Zentar Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of zentar_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'hermes-agent', 'zentar_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Zentar
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ZENTAR_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ZENTAR_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
