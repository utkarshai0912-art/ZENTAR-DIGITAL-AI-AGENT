...
export function markerPath(zentarHome) {
  return path.join(zentarHome, '.zentar-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(zentarHome)
  let raw
 ...
 *
 * The Tauri-based zentar-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(zentarHome, pid, { now = Date.now } = {}) {
  const file = markerPath(zentarHome)
