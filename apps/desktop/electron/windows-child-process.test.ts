...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /zentarProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'zentar_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvZentarCommand\(command, backendArgs\)/)
  assert.match(source, /function getVenvSitePackagesEntries\(venvRoot\)/)
  assert.match(source, /path\.join\(venvRoot, 'Lib', 'site-packages'\)/)
  assert.match(source, /args: \['-m', 'zentar_cli\.main', \.\.\.backendArgs\]/)
})
...
test('desktop backend launches console python so child consoles are inherited, not pythonw', () => {
  ...
  // ZENTAR_DASHBOARD_READY stdout line — no ready-file side channel is set.
...
test('desktop backend teardown tree-kills Windows backend descendants', () => {
  ...
  const resetIndex = source.indexOf('function resetZentarConnection()')
  assert.notEqual(resetIndex, -1, 'missing resetZentarConnection')
  const resetSnippet = source.slice(resetIndex, resetIndex + 300)
  assert.match(resetSnippet, /stopBackendChild\(zentarProcess\)/)
  assert.doesNotMatch(resetSnippet, /zentarProcess\.kill\('SIGTERM'\)/)
  ...
  assert.match(quitSnippet, /stopBackendChild\(zentarProcess\)/)
  assert.doesNotMatch(quitSnippet, /zentarProcess\.kill\('SIGTERM'\)/)
})
