...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zentar-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/zentar-runner-test',
    sourceRepoRoot: null,
    zentarHome: '/tmp/zentar-runner-test',
    logRoot: '/tmp/zentar-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'zentar-digital-agent', 'scripts')
