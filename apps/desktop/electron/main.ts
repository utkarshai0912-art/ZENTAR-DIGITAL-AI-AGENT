...
import { buildDesktopBackendEnv, normalizeZentarHomeRoot } from './backend-env'
import { canImportZentarCli, verifyZentarCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.ZENTAR_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ZENTAR_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ZENTAR_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ZENTAR_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[zentar] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[zentar] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('zentar:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[zentar] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[zentar] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[zentar] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[zentar] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ZENTAR_HOME — the user-facing root for everything Zentar-related. Mirrors
// scripts/install.ps1's $ZentarHome and scripts/install.sh's $ZENTAR_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\zentar (matches install.ps1)
//   macOS / Linux: ~/.zentar (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.zentar directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\zentar yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ZENTAR_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ZENTAR_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.zentar / %LOCALAPPDATA%\zentar.
function resolveZentarHome() {
  if (process.env.ZENTAR_HOME) {
    return normalizeZentarHomeRoot(process.env.ZENTAR_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'zentar-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ZENTAR_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\zentar and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ZENTAR_HOME')

    if (fromRegistry) {
      return normalizeZentarHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'zentar')
    const legacy = path.join(app.getPath('home'), '.zentar')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.zentar setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.zentar')
}

const ZENTAR_HOME = resolveZentarHome()

function zentarManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_zentar_node_dirs() in
  // zentar_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ZENTAR_HOME, 'node')
...
function pathWithZentarManagedNode(...entries) {
  return [...zentarManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ZENTAR_ROOT — the canonical mutable Zentar install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ZENTAR_ROOT = path.join(ZENTAR_HOME, 'zentar-digital-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ZENTAR_ROOT, 'venv')
...
// boots, going straight to `resolveZentarBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ZENTAR_ROOT (not alongside)
...
// active-profile.json records which Zentar profile the desktop launches its
// local backend as. When set, startZentar() passes `zentar --profile <name>
// dashboard …`, which deterministically pins ZENTAR_HOME (see
// _apply_profile_override in zentar_cli/main.py) and bypasses the sticky
// ~/.zentar/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors zentar_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// zentarDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ZENTAR_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by zentar_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ZENTAR_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (zentar_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ZENTAR_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ZENTAR_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'zentar:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.zentar) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.zentar')
}

// Seed the native About panel with the live Zentar version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `zentar update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveZentarVersion(),
  copyright: 'Copyright © 2026 ZENTAR DIGITAL'
})
...
const MEDIA_PROTOCOL = 'zentar-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(zentar-media://…)`.
...
let mainWindow = null
let zentarProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by zentarProcess +
// connectionPromise + startZentar(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ZENTAR_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ZENTAR_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startZentar() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning zentar serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Zentar backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[zentar] ${line}`)
  zentarLog.push(...lines)

  if (zentarLog.length > 300) {
    zentarLog.splice(0, zentarLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-zentar-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('zentar:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the zentar:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('zentar:bootstrap:event', ev)
}
...
// The Tauri updater writes ZENTAR_HOME/.zentar-update-in-progress for the whole
...
// (`force_kill_other_zentar`, taskkill /IM zentar.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ZENTAR_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Zentar will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(ZENTAR_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `zentar` — must not shadow `zentar.cmd`/`zentar.exe`.
...
function unwrapWindowsVenvZentarCommand(command, backendArgs) {
  ...
  if (!/^zentar(?:\.exe)?$/i.test(path.basename(resolved))) {
    return null
  }
  ...
  // died mid-`pip install` still has python.exe + zentar.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Zentar at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Zentar Python at ${python}`,
    command: python,
    args: ['-m', 'zentar_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      zentarHome: ZENTAR_HOME,
...
// spawns `zentar serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `zentar` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'zentar_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ZENTAR_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isZentarSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'zentar_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ZENTAR_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Zentar deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Zentar' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\zentar\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zentar', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'zentar', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal ZENTAR_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\zentar\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zentar', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'zentar', 'git', 'bin', 'git.exe'))
  }
...
function recentZentarLog() {
  return zentarLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's zentar root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ZENTAR_ROOT.
// ZENTAR_DESKTOP_ZENTAR_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ZENTAR_DESKTOP_ZENTAR_ROOT && path.resolve(process.env.ZENTAR_DESKTOP_ZENTAR_ROOT),
    !IS_PACKAGED && isZentarSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isZentarSourceRoot(ACTIVE_ZENTAR_ROOT) ? ACTIVE_ZENTAR_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ZENTAR_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('zentar:updates:progress', merged)
  }
}
...
// ZENTAR_HOME/zentar-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_zentar_home). That binary owns
// ALL repo mutation — running `zentar update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'zentar-setup.exe' : 'zentar-setup'
  const candidate = path.join(ZENTAR_HOME, name)
...
// Path to the venv shim whose lock decides whether `zentar update` can write
// fresh entry points. On Windows this is the file the running backend
// `zentar.exe` holds open; on POSIX it's never mandatory-locked.
function venvZentarShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'zentar.exe')
    : path.join(updateRoot, 'venv', 'bin', 'zentar')
}
...
// only signals the direct child, so on Windows a backend `zentar.exe` that
// spawned its own grandchildren (a `zentar` REPL, a pty terminal session, the
...
// `zentarProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `zentar.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a zentar
// REPL, a pty terminal, the gateway) keep `zentar.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (zentarProcess && Number.isInteger(zentarProcess.pid)) {
    pids.push(zentarProcess.pid)
  }
  ...
  if (zentarProcess && !zentarProcess.killed) {
    try {
      zentarProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvZentarShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (zentarProcess && Number.isInteger(zentarProcess.pid)) {
      stragglers.push(zentarProcess.pid)
    }
...
// `zentar update`). Instead we spawn the staged Zentar-Setup binary with
// --update and quit, so it can run `zentar update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri zentar-setup. Unlike Windows
      ...
      // whole update itself: `zentar update` (backend) + `zentar desktop
    ...
    if (!updater) {
      ...
      // `zentar desktop`, never the Tauri installer that self-copies
      // zentar-setup.exe into ZENTAR_HOME). They DO have a working `zentar`
      ...
      // checkout they're on — bare `zentar update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `zentar update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `zentar update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, zentarRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Zentar — this window will close and the updater will open. Don’t reopen Zentar yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // zentar.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running zentar, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Zentar install open ' +
        '(a second Zentar window or a terminal running zentar?). Close it and retry.'
    ...
    // `zentar update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: ZENTAR_HOME,
      env: {
        ...process.env,
        ZENTAR_HOME,
        PATH: pathWithZentarManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(ZENTAR_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvZentar = path.join(venvBin, IS_WINDOWS ? 'zentar.exe' : 'zentar')
  ...
  // not just the `zentar.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvZentar) || fileExists(path.join(updateRoot, '.zentar-bootstrap-complete'))
...
// Resolve the zentar CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `zentar` on PATH.
function resolveZentarCliBinary(updateRoot) {
  const venvZentar = path.join(updateRoot, 'venv', 'bin', 'zentar')

  if (fileExists(venvZentar)) {
    return venvZentar
  }

  return findOnPath('zentar') || null
}
...
// macOS/Linux in-app update: backend (`zentar update`) + OS-aware GUI rebuild
// (`zentar desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const zentar = resolveZentarCliBinary(updateRoot)

  if (!zentar) {
    emitUpdateProgress({ stage: 'manual', message: 'zentar update', percent: null })

    return { ok: true, manual: true, command: 'zentar update', zentarRoot: updateRoot }
  }

  // Put the Zentar-managed Node and the venv on PATH so `zentar desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\zentar\node, not node\bin.
  const env: Record<string, string> = {
    ZENTAR_HOME,
    PATH: pathWithZentarManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `zentar update` reaps stale `zentar serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ZENTAR_DESKTOP_CHILD_PID and excludes
  ...
  if (zentarProcess && Number.isInteger(zentarProcess.pid)) {
    desktopChildPids.push(zentarProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.ZENTAR_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Zentar (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(zentar, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'zentar update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'zentar update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(zentar, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Zentar to retry.',
  ...
  // Linux in-app update terminal state (#45205). `zentar desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Zentar…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `zentar-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Zentar to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Zentar desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Zentar to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Zentar.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Zentar.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Zentar to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.zentar-update-new"; then
    rm -rf "$DST.zentar-update-old" 2>/dev/null || true
    mv "$DST" "$DST.zentar-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.zentar-update-new" "$DST"
    rm -rf "$DST.zentar-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Zentar to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ZENTAR_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isZentarSourceRoot(ACTIVE_ZENTAR_ROOT) &&
    fileExists(venvPython) &&
    canImportZentarCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ZENTAR_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `zentar
...
function resolveWebDist() {
  const override = process.env.ZENTAR_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ZENTAR_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: zentar desktop --force-build`
  )
...
function resolveZentarCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Zentar.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ZENTAR_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveZentarCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_ZENTAR_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Zentar at ${ACTIVE_ZENTAR_ROOT}`,
    command,
    args: ['-m', 'zentar_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      zentarHome: ZENTAR_HOME,
      pythonPathEntries: [ACTIVE_ZENTAR_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ZENTAR_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveZentarBackend(backendArgs) {
  // 1. Explicit override -- ZENTAR_DESKTOP_ZENTAR_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ZENTAR_DESKTOP_ZENTAR_ROOT && path.resolve(process.env.ZENTAR_DESKTOP_ZENTAR_ROOT)

  if (overrideRoot && isZentarSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Zentar source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `zentar` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isZentarSourceRoot.)
  if (!IS_PACKAGED && isZentarSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Zentar source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_ZENTAR_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\zentar\zentar-digital-agent (Windows) or ~/.zentar/zentar-digital-agent.
  ...
  //    to spawning zentar. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `zentar update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `zentar` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ZENTAR_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ZENTAR_DESKTOP_IGNORE_EXISTING !== '1') {
    let zentarCommand = null
    const zentarOverride = process.env.ZENTAR_DESKTOP_HERMES

    if (zentarOverride) {
      const resolvedOverride = findOnPath(zentarOverride)

      if (resolvedOverride) {
        zentarCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(zentarOverride, { isWsl: IS_WSL })) {
        zentarCommand = zentarOverride
      } else {
        rememberLog(`Ignoring Windows Zentar override under WSL: ${zentarOverride}`)
      }
    } else {
      zentarCommand = findOnPath('zentar')
    }

    if (zentarCommand) {
      if (looksLikeDesktopAppBinary(zentarCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Zentar CLI: ${zentarCommand}`)
        zentarCommand = null
      }
    }

    if (zentarCommand) {
      const unwrapped = unwrapWindowsVenvZentarCommand(zentarCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `zentar` shim
      ...
      const shellForProbe = isCommandScript(zentarCommand)

      if (verifyZentarCli(zentarCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvZentarCommand(zentarCommand, backendArgs) || {
            label: `existing Zentar CLI at ${zentarCommand}`,
            command: zentarCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Zentar CLI at ${zentarCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed zentar_cli module via system Python.
  ...
  if (python) {
    ...
    // having zentar_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\zentar\zentar-digital-agent\venv.
    if (canImportZentarCli(python)) {
      return {
        kind: 'python',
        label: `installed zentar_cli module via ${python}`,
        command: python,
        args: ['-m', 'zentar_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: zentar_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveZentarBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'ZENTAR DIGITAL AI AGENT not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveZentarBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Zentar install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Zentar recovery was handed off to Zentar Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      zentarHome: ZENTAR_HOME,
      logRoot: path.join(ZENTAR_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Zentar install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Zentar bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ZENTAR_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startZentar() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // zentar:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveZentarBackend(backend.args))
  }
  ...
  if (!isZentarSourceRoot(ACTIVE_ZENTAR_ROOT)) {
    throw new Error(
      `Zentar install at ${ACTIVE_ZENTAR_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Zentar' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\zentar\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `zentar` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Zentar on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Zentar.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isZentarSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Zentar venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Zentar at ${ACTIVE_ZENTAR_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Zentar runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Zentar backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Zentar backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Zentar backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Zentar-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('zentar:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveZentarCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('zentar:preview-file-changed', payload)
}
...
async function waitForZentar(baseUrl, token) {
  ...
  throw new Error(`Zentar backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('zentar:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('zentar:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Zentar...".
function sendPowerResume() {
  ...
  webContents.send('zentar:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('zentar:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('zentar:window-state-changed', state)
}
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents.send('zentar:zoom:changed', { level: next, percent: zoomLevelToPercent(next) })
  window.webContents
    .executeJavaScript(
...
//
// Hosted Zentar gateways gate the dashboard behind an OAuth provider (e.g.
// ZENTAR DIGITAL) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``zentar_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / zentar #37247).
//     Both are set as HttpOnly cookies (``zentar_session_at`` ~15 min,
//     ``zentar_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:zentar-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Zentar gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Zentar backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root ZENTAR_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ZENTAR_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.ZENTAR_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by ZENTAR_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (zentar #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Zentar gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Zentar gateway is selected, but no session token is saved. ' +
...
//   2. env override (ZENTAR_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ZENTAR_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ZENTAR_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ZENTAR_DESKTOP_REMOTE_URL is set but ZENTAR_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Zentar backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.ZENTAR_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Zentar
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with ZENTAR DIGITAL" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startZentar())
  ...
  // connect to Zentar gateway". Mirror the renderer's connect here so the test
...
function resetZentarConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(zentarProcess)
...
// startZentar() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetZentarConnection() nulls zentarProcess.
  const dying = zentarProcess && !zentarProcess.killed ? zentarProcess : null
  resetZentarConnection()
...
// profile to startZentar() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startZentar() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForZentar(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: zentarLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited ZENTAR_HOME env (see _apply_profile_override
  // step 3 in zentar_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveZentarBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const zentarCwd = resolveZentarCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Zentar backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: zentarCwd,
        ZENTAR_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ZENTAR_DESKTOP: '1',
        ZENTAR_WEB_DIST: webDist,
        ...(readyFile ? { ZENTAR_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Zentar backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Zentar backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Zentar backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForZentar(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Zentar backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_zentar_home() recreates the deleted profile directory.
...
async function startZentar() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startZentar() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Zentar backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Zentar backend at ${remote.baseUrl}`, 24)
      await waitForZentar(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Zentar backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.zentar/active_profile file) and
    // resolves ZENTAR_HOME the same way `zentar -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Zentar runtime', 28)
    const backend = await ensureRuntime(resolveZentarBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const zentarCwd = resolveZentarCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Zentar backend via ${backend.label}`, 84)
    rememberLog(`Starting Zentar backend via ${backend.label}`)
    ...
    zentarProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ZENTAR_HOME for the child so Python's get_zentar_home()
          // resolves to the SAME location our resolveZentarHome() picked. Without
          // this pin, Python falls back to ~/.zentar on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\zentar, which differs from C:\Users\<u>\.zentar.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ZENTAR_HOME via setx; the desktop
    ...
    zentarProcess.stdout.on('data', rememberLog)
    zentarProcess.stderr.on('data', rememberLog)
    ...
    zentarProcess.once('error', error => {
      rememberLog(`Zentar backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Zentar backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    zentarProcess.once('exit', (code, signal) => {
      rememberLog(`Zentar backend exited (${signal || code})`)
      zentarProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Zentar backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Zentar backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentZentarLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Zentar backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(zentarProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Zentar backend to become ready', 90)
    await Promise.race([waitForZentar(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null zentarProcess when the child dies.
      childAlive: () => zentarProcess !== null && zentarProcess.exitCode === null && !zentarProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Zentar backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Zentar is
...
// pushes pet state over IPC (zentar:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via zentar:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Zentar app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Zentar icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see zentar:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Zentar in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('zentar:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startZentar().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('zentar:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Zentar…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('zentar:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetZentarConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Zentar backend failed liveness probe; dropping stale connection.')
    resetZentarConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('zentar:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('zentar:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('zentar:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('zentar:window:openNewSession', async () => {
...
ipcMain.handle('zentar:zoom:get', event => {
...
ipcMain.on('zentar:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('zentar:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('zentar:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('zentar:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('zentar:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('zentar:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('zentar:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('zentar:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('zentar:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('zentar:pet-overlay:control', payload)
})
ipcMain.handle('zentar:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startZentar() call restarts the
...
ipcMain.handle('zentar:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startZentar() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('zentar:bootstrap:cancel', async () => {
...
ipcMain.handle('zentar:boot-progress:get', async () => bootProgressState)
ipcMain.handle('zentar:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('zentar:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('zentar:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('zentar:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('zentar:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('zentar:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('zentar:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('zentar:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('zentar:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('zentar:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ZENTAR_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('zentar:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('zentar:requestMicrophoneAccess', async () => {
...
ipcMain.handle('zentar:api', async (_event, request) => {
  ...
  // backend calls ensure_zentar_home() which recreates the profile directory,
...
ipcMain.handle('zentar:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('zentar:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('zentar:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('zentar:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('zentar:readFileText', async (_event, filePath) => {
...
ipcMain.handle('zentar:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('zentar:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('zentar:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('zentar:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('zentar:saveClipboardImage', async () => {
...
ipcMain.handle('zentar:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('zentar:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('zentar:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('zentar:titlebar-theme', (_event, payload) => {
...
ipcMain.on('zentar:native-theme', (_event, mode) => {
...
ipcMain.on('zentar:translucency', (_event, payload) => {
...
ipcMain.handle('zentar:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('zentar:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveZentarCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('zentar:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveZentarCwd()
}))

ipcMain.handle('zentar:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('zentar:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('zentar:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('zentar:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('zentar:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('zentar:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: zentarLog.slice(-200) }))
...
function terminalShellCommand() {
  // ZENTAR_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ZENTAR_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Zentar' TUI into
  ...
  env.TERM_PROGRAM = 'Zentar'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a zentar/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ZENTAR_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ZENTAR_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `zentar:terminal:${id}:${suffix}`
}
...
ipcMain.handle('zentar:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('zentar:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('zentar:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('zentar:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('zentar:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('zentar:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('zentar:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('zentar:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('zentar:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('zentar:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('zentar:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('zentar:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('zentar:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('zentar:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('zentar:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('zentar:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('zentar:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('zentar:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('zentar:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('zentar:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('zentar:terminal:write', (_event, id, data) => {
...
ipcMain.handle('zentar:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('zentar:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('zentar:updates:check', async () =>
...
ipcMain.handle('zentar:updates:apply', async (_event, payload) =>
...
ipcMain.handle('zentar:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('zentar:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Zentar version (the one `release.py` bumps in
// zentar_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Zentar version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveZentarVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'zentar_cli', '__init__.py')
...
// Re-resolve the live Zentar version and push it into the native About panel
// just before showing it, so an in-place `zentar update` is reflected without
...
ipcMain.handle('zentar:version', async () => ({
  appVersion: resolveZentarVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  zentarRoot: resolveUpdateRoot()
}))
...
// the actual removal via `zentar uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (zentar_cli/uninstall.py + zentar_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ZENTAR_ROOT
  ...
  const fallback = () => ({
    zentar_home: ZENTAR_HOME,
    agent_installed: isZentarSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'zentar_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ZENTAR_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no ZENTAR DIGITAL AI AGENT venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import zentar_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ZENTAR_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `zentar-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `zentar-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('zentar:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('zentar:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('zentar:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('zentar:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// zentar:// deep links (e.g. zentar://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ZENTAR_PROTOCOL = 'zentar'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ZENTAR_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // zentar://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('zentar:deep-link', payload)
...
ipcMain.handle('zentar:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ZENTAR_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ZENTAR_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ZENTAR_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `zentar://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching zentar:// URL is in our own argv.
