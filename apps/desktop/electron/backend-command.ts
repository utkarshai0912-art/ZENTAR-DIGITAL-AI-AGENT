...
// Backend subcommand routing for the desktop-managed Zentar process.
//
// The desktop app launches its own headless backend via `zentar serve` — it
...
// app hasn't updated yet, or an older `zentar` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Zentar profile to pin via `--profile`.
 */
 ...
 * `-m zentar_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `zentar_cli/subcommands/dashboard.py` source registers
