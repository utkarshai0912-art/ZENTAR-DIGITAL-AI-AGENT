 ...
 *      internals; ZENTAR_HOME / ZENTAR_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.zentar/zentar-digital-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'zentar')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Zentar12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/zentar/zentar', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Zentar/zentar', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.zentar/zentar-digital-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'zentar'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'zentar')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'zentar://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'zentar://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves ZENTAR_HOME + ZENTAR_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    ZENTAR_HOME: '/home/u/.zentar',
    ZENTAR_DESKTOP_REMOTE_URL: 'http://box:9119',
    ZENTAR_DESKTOP_REMOTE_TOKEN: 'secret',
    ZENTAR_DESKTOP_ZENTAR_ROOT: '/home/u/dev/zentar',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.zentar/zentar-digital-agent/apps/desktop/release/linux-unpacked/Zentar',
    args: ['zentar://open/agent/42', "--note=it's fine"],
    env: { ZENTAR_HOME: '/home/u/.zentar', ZENTAR_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export ZENTAR_HOME='\/home\/u\/\.zentar'/)
  assert.match(script, /export ZENTAR_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Zentar' 'zentar:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `zentar-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Zentar/Zentar',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `zentar-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Zentar\/Zentar'\n/)
})
