 ...
 * the three user-facing uninstall modes to the `zentar uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m zentar_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `zentar_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'zentar_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Zentar.app/Contents/MacOS/Zentar  → …/Zentar.app
 *   Windows: …\Zentar\Zentar.exe                 → …\Zentar  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Zentar.app/Contents/MacOS/Zentar → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Zentar.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Zentar.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Zentar$/i.test(dir) || /[\\/]zentar-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/zentar
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import zentar_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, zentarHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export ZENTAR_HOME=${q(zentarHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import zentar_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Zentar install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ZENTAR_HOME=${String(zentarHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
