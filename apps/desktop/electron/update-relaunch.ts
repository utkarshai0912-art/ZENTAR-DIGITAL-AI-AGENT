 ...
 * Background
 * ----------
 * After `zentar update` + `zentar desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ZENTAR_DESKTOP_* is preserved
// plus ZENTAR_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ZENTAR_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ZENTAR_DESKTOP_']
