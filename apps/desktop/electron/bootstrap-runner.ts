 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of ZENTAR DIGITAL AI AGENT by spawning
...
function bootstrapCacheDir(zentarHome) {
  return path.join(zentarHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.zentar/zentar-digital-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(zentarHome) {
  if (!zentarHome) {
    return null
  }
  const candidate = path.join(zentarHome, 'zentar-digital-agent', 'scripts', installScriptName())
...
function cachedScriptPath(zentarHome, commit) {
  return path.join(bootstrapCacheDir(zentarHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/zentar-digital-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(zentarHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(zentarHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, zentarHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ZENTAR_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ZENTAR_HOME: zentarHome || process.env.ZENTAR_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, zentarHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, zentarHome }) {
  const args = ['--dir', activeRoot, '--zentar-home', zentarHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, zentarHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, zentarHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, zentarHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, zentarHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(zentarHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, zentarHome, emit })
