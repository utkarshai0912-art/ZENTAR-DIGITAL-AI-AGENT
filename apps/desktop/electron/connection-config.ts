 ...
 *     `X-Zentar-Session-Token` header; WS uses `?token=`.
...
// zentar_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - zentar_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - zentar_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / zentar #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-zentar_session_at', '__Secure-zentar_session_at', 'zentar_session_at']
const RT_COOKIE_VARIANTS = ['__Host-zentar_session_rt', '__Secure-zentar_session_rt', 'zentar_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://zentar.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a zentar session ACCESS-token cookie
