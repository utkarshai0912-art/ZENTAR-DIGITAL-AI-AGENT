...
import { formatRefValue, zentarDirectiveFormatter } from './directive-text'
...
describe('zentarDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = zentarDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = zentarDirectiveFormatter.parse('@file:src/main.tsx the entry point')
