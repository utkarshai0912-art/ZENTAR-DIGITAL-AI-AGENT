...
const ZENTAR_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ZentarRefType = (typeof ZENTAR_REF_TYPES)[number]
...
const ICON_PATHS: Record<ZentarRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ZentarRefType] ?? ICON_FALLBACK
...
const ZENTAR_DIRECTIVE_RE = new RegExp(
...
export const zentarDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZENTAR_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ZentarRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ZentarRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...zentarDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Zentar directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.zentarDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.zentarDesktop?.readFileDataUrl(id)
