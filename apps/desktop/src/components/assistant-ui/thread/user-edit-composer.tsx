...
import { zentarDirectiveFormatter } from '@/components/assistant-ui/directive-text'
import {
...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      const serialized = zentarDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ZENTAR_PATHS_MIME)) {
      return
    }
