...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.zentarDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.zentarDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.zentarDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.zentarDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.zentarDesktop && text && normalizeExternalUrl(text) === target) {
