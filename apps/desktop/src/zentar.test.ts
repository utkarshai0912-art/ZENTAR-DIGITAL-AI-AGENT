...
describe('Zentar REST session helpers', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'zentarDesktop')
  })
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getZentarConfig, '/api/config'],
      [getZentarConfigDefaults, '/api/config/defaults'],
