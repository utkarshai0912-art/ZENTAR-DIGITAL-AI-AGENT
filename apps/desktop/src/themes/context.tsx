...
const SKIN_KEY = 'zentar-desktop-theme-v2'
const MODE_KEY = 'zentar-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'zentar-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'zentar-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'zentar-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.zentarTheme = skinName
  root.dataset.zentarMode = rendered
  ...
  window.zentarDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('zentar-boot-background', chromeBg)
    window.localStorage.setItem('zentar-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.zentarThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.zentarDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Zentar backend theme on connect. */
