...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Zentar doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zentar_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `zentar model` uses — instead of
...
export type ZentarConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `zentar model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors zentar_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Zentar, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `zentar tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Zentar); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/zentar/update/check` — the backend's own update state.
