...
declare global {
  interface Window {
    zentarDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ZentarConnection>
      ...
      profile: {
        ...
        // backend under the new ZENTAR_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZentarApiRequest) => Promise<T>
      notify: (payload: ZentarNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ZentarReadFileTextResult>
      selectPaths: (options?: ZentarSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZentarPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZentarPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ZentarTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ZentarReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ZentarGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ZentarGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ZentarRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ZentarReviewScope, baseRef?: null | string) => Promise<ZentarReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ZentarReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ZentarReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ZentarTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZentarTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZentarWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ZentarPreviewFileChanged) => void) => () => void
...
export interface ZentarTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZentarTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `zentar update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  zentarRoot?: string
...
export interface ZentarConnection {
  baseUrl: string
...
export interface ZentarTitleBarTheme {
  background: string
  foreground: string
}

export interface ZentarWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface ZentarApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface ZentarNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ZentarPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZentarReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZentarPreviewWatch {
  id: string
  path: string
}
...
export interface ZentarGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ZentarGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface ZentarRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ZentarRepoStatus {
  branch: null | string
...
// turn began.
export type ZentarReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ZentarReviewFile {
  path: string
  added: number
  removed: number
...
export interface ZentarReviewList {
  files: ZentarReviewFile[]
...
export interface ZentarReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ZentarReviewShipInfo {
  ghReady: boolean
  pr: ZentarReviewPr | null
}

export interface ZentarReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZentarReadDirResult {
  entries: ZentarReadDirEntry[]
  error?: string
}

export interface ZentarPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZentarSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
