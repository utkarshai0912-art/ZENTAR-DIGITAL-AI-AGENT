...
import type { ZentarReviewFile, ZentarReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Zentar' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'zentar.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'zentar.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'zentar.desktop.reviewTreeMode'
const SELECTED_KEY = 'zentar.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ZentarReviewFile[]>([])
...
export const $reviewShipInfo = atom<ZentarReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['zentarDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ZentarReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.zentarDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.zentarDesktop?.openExternal?.(url)
    }
