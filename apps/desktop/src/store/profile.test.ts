...
const { getProfiles } = await import('@/zentar')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/zentar/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<ZentarConnection> = {}): ZentarConnection =>
  ({ baseUrl: 'https://zentar-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZentarConnection

const localConn = (over: Partial<ZentarConnection> = {}): ZentarConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZentarConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZentarConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { zentarDesktop: { getConnection } })
