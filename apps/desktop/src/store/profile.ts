...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/zentar'
...
import type { ProfileInfo } from '@/types/zentar'
...
// /api/profiles/active `current`). "default" is the root ~/.zentar. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'zentar.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'zentar.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.zentarDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ZENTAR_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.zentarDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.zentarDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'zentar.desktop.showAllProfiles'
...
// Switch to the default (root ~/.zentar) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.zentarDesktop?.touchBackend?.(target).catch(() => undefined)
}
