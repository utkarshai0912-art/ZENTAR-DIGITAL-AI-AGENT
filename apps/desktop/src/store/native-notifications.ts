...
const STORAGE_KEY = 'zentar:native-notifications'
...
// "Backgrounded" = the user isn't on Zentar. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.zentarDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.zentarDesktop
