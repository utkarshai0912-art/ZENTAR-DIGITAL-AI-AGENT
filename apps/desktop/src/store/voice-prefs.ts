...
import { getZentarConfigRecord, saveZentarConfig } from '@/zentar'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getZentarConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveZentarConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
