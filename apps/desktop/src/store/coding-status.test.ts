...
import type { ZentarRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: ZentarRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<ZentarRepoStatus | null>) {
  ;(window as unknown as { zentarDesktop?: unknown }).zentarDesktop = { git: { repoStatus: impl } }
}
