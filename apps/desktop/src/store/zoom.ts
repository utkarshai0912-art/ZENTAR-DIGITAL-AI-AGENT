...
export function setZoomPercent(percent: number): void {
  window.zentarDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.zentarDesktop?.zoom) {
  void window.zentarDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.zentarDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
