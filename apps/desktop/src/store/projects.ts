...
import type { ZentarGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/zentar'
...
const PROJECT_SCOPE_KEY = 'zentar.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Zentar gateway is not connected')
  }
...
// repo, returning where Zentar should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ZentarGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.zentarDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.zentarDesktop?.writeClipboard?.(path)
  }
}
