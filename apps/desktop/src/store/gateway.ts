import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@zentar/shared'
...
const isOpen = (gateway: ZentarGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ZentarGateway | null>(null)
...
let primaryGateway: ZentarGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ZentarGateway | null, profile = 'default'): void {
...
export function activeGateway(): ZentarGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ZentarGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
