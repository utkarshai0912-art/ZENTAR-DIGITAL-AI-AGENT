 ...
 * app's bounds and stays visible while Zentar is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'zentar.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'zentar.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.zentarDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.zentarDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.zentarDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.zentarDesktop?.petOverlay?.close()
}
