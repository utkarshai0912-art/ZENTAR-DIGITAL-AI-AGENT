...
import type { ZentarConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/zentar'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'zentar.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'zentar.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'zentar.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'zentar.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'zentar.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'zentar.desktop.lastSessionId'
...
function workspaceCwdKey(connection: ZentarConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.zentarDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.zentarDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ZentarConnection | null>(null)
...
export const setConnection = (next: Updater<ZentarConnection | null>) => updateAtom($connection, next)
