...
import type { OAuthProvider } from '@/types/zentar'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('zentar-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('zentar-desktop-onboarded-v1')).toBe('1')
  })
