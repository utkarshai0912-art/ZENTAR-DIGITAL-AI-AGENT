...
import { getZentarConfigRecord, type ZentarConfigRecord, saveZentarConfig } from '@/zentar'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ZentarConfigRecord>
  saveConfig: (config: ZentarConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.zentarDesktop?.api) {
      return Promise.resolve({})
    }

    return getZentarConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.zentarDesktop?.api) {
    ...
    return saveZentarConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ZentarConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ZentarConfigRecord, locale: Locale): ZentarConfigRecord {
