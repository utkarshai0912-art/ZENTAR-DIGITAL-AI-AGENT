...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'zentar:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.zentarDesktop?.saveImageFromUrl) {
        if (await window.zentarDesktop.saveImageFromUrl(src)) {
