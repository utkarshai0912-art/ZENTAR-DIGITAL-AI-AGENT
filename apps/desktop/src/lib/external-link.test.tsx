...
const desktopWindow = window as unknown as { zentarDesktop?: Window['zentarDesktop'] }
const initialZentarDesktop = desktopWindow.zentarDesktop

function installDesktopBridge(partial: Partial<Window['zentarDesktop']> = {}) {
  desktopWindow.zentarDesktop = {
    fetchLinkTitle: vi.fn().mockResolvedValue(''),
    openExternal: vi.fn().mockResolvedValue(undefined),
    ...partial
  } as unknown as Window['zentarDesktop']
}
...
afterEach(() => {
  ...
  if (initialZentarDesktop) {
    desktopWindow.zentarDesktop = initialZentarDesktop
  } else {
    delete desktopWindow.zentarDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('deduplicates in-flight title fetches and caches results', async () => {
    ...
    installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['zentarDesktop']['fetchLinkTitle'] })
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['zentarDesktop']['openExternal'] })
