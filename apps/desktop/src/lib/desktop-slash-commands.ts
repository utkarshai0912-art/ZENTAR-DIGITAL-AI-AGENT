...
/** Why a known Zentar command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Zentar profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show ZENTAR DIGITAL AI AGENT version', surface: exec() },
...
function isKnownZentarSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Zentar' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZentarSlashCommand(normalized)
}
