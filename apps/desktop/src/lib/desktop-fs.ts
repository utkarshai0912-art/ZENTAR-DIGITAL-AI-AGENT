import type {
  ZentarConnection,
  ZentarReadDirResult,
  ZentarReadFileTextResult,
  ZentarSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ZentarConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<ZentarReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ZentarReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ZentarReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ZentarReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: ZentarSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
