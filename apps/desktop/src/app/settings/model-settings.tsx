...
import { invalidateZentarConfig, setZentarConfigCache, useZentarConfigRecord } from '../hooks/use-config-record'
...
// Zentar' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Zentar default (medium), shown as Medium.
...
// backend now surfaces the full `zentar model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in zentar_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useZentarConfigRecord()
  const setConfig = setZentarConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `zentar model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Zentar runs the flow for you.`}
          </p>
        )}
