...
import { getElevenLabsVoices, getZentarConfigSchema, saveZentarConfig } from '@/zentar'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ZentarConfigRecord } from '@/types/zentar'

import { setZentarConfigCache, useZentarConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ZentarConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ZentarConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useZentarConfigRecord()
  ...
  const {
    ...
    queryKey: ['zentar-config-schema'],
    queryFn: getZentarConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: ZentarConfigRecord) => {
