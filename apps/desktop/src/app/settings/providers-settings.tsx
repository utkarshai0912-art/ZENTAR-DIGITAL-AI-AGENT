...
import { disconnectOAuthProvider, listOAuthProviders } from '@/zentar'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/zentar'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.zentarDesktop?.terminal)
...
//      in zentar_cli/provider_catalog.py) — the SAME provider identity
//      `zentar model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Zentar can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Zentar can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Zentar never deletes creds another tool owns behind a silent API call.
