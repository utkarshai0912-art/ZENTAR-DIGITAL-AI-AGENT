...
import type { ZentarReadDirEntry, ZentarReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ZentarReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ZentarReadDirEntry) {
...
async function filterIgnored(entries: ZentarReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZentarReadDirResult> {
  if (!window.zentarDesktop) {
