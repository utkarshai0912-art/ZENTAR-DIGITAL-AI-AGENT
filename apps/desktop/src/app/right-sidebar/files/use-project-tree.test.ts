...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ZentarReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { zentarDesktop: unknown }).zentarDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ZentarReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZentarReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { zentarDesktop: unknown }).zentarDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.zentarDesktop is missing', async () => {
    delete (window as unknown as { zentarDesktop?: unknown }).zentarDesktop
