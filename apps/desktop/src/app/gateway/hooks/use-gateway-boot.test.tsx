...
// bug that drives the REAL useGatewayBoot hook + REAL ZentarGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshZentarConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { zentarDesktop?: unknown }).zentarDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { zentarDesktop?: unknown }).zentarDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForZentar) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startZentar()'s remote branch awaits waitForZentar() for 45s before it
    ...
    ;(window as { zentarDesktop?: unknown }).zentarDesktop = desktop
    ...
    // After ~45s waitForZentar gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Zentar backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
