...
import type { ZentarGateway } from '@/zentar'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<ZentarGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.zentarDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ZentarGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ZentarGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number, signal?: AbortSignal) => {
      ...
      if (!gateway) {
        throw new Error('Zentar gateway unavailable')
      }
