import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@zentar/shared'
import { useEffect, useRef } from 'react'

import type { ZentarConnection } from '@/global'
import { ZentarGateway } from '@/zentar'
...
import type { RpcEvent } from '@/types/zentar'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.zentarDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZentarGateway | null) => void
  refreshZentarConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.zentarDesktop

    const publish = (next: ZentarConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Zentar...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Zentar…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Zentar gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshZentarConfig().catch(() => undefined)
    ...
    const gateway = new ZentarGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshZentarConfig()
