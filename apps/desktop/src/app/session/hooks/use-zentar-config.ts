...
import { getZentarConfig, getZentarConfigDefaults } from '@/zentar'
...
interface ZentarConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useZentarConfig({ activeSessionIdRef, refreshProjectBranch }: ZentarConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshZentarConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getZentarConfig(), getZentarConfigDefaults().catch(() => ({}))])
  ...
  return { refreshZentarConfig, sttEnabled, voiceMaxRecordingSeconds }
}
