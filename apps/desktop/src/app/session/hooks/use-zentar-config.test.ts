...
import { getZentarConfig } from '@/zentar'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useZentarConfig } from './use-zentar-config'

vi.mock('@/zentar', () => ({
  getZentarConfig: vi.fn(),
  getZentarConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'zentar.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getZentarConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getZentarConfig>>)

describe('useZentarConfig refreshZentarConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshZentarConfig()
    })
