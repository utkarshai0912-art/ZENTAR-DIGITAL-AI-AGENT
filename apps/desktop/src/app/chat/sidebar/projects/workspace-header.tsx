...
const branchActionLabel = (branch: ZentarGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `zentar/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<ZentarGitBranch[]>([])
  ...
  const convert = async (branch: ZentarGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
