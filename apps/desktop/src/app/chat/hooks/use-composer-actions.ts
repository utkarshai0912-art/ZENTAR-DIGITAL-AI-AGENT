...
export async function attachmentPreviewDataUrl(filePath: string): Promise<string> {
  try {
    const local = await window.zentarDesktop?.readFileDataUrl?.(filePath)
...
export const ZENTAR_PATHS_MIME = 'application/x-zentar-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Zentar sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.zentarDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(ZENTAR_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.zentarDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.zentarDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.zentarDesktop?.getPathForFile ? window.zentarDesktop.getPathForFile(file) : ''
