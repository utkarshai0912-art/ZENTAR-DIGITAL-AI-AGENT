        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.zentarDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Zentar to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.zentarDesktop?.watchPreviewFile ||
      !window.zentarDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.zentarDesktop.onPreviewFileChanged(payload => {
    ...
    void window.zentarDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.zentarDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.zentarDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:zentar-preview')
