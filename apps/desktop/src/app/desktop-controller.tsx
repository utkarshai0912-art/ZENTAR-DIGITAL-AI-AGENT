...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../zentar'
...
import { useZentarConfig } from './session/hooks/use-zentar-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.zentarDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.zentarDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.zentarDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.zentarDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // zentar:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.zentarDesktop?.onDeepLink?.(payload => {
    ...
    void window.zentarDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.zentarDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshZentarConfig, sttEnabled, voiceMaxRecordingSeconds } = useZentarConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshZentarConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshZentarConfig])
