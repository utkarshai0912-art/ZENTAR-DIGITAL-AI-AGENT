...
export const $zentarHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.zentar/logs/bootstrap-installer.log')
    $zentarHome.set('~/.zentar')
  ...
  try {
    const [logPath, zentarHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zentar_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $zentarHome.set(zentarHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Zentar" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Zentar-Setup.exe --update);
...
export async function launchZentarDesktop(): Promise<void> {
  ...
  await invoke('launch_zentar_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Zentar repository' },
