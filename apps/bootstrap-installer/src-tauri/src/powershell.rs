...
///
/// `zentar_home_override` propagates to the child as $ZENTAR_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, zentar_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = zentar_home_override {
        cmd.env("ZENTAR_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, zentar_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = zentar_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_zentar_home() {
