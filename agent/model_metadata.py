...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Zentar also honours ZENTAR_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ZENTAR_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_zentar_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run ZENTAR DIGITAL AI AGENT.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Zentar from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Zentar' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Zentar minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from zentar_constants import get_zentar_home
    return get_zentar_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Zentar grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from zentar_cli.models import get_copilot_model_context
    ...
    # ``Zentar-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Zentar sends to providers:
