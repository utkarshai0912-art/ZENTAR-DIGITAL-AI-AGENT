...
``auth.json``.  They intentionally have no dependency on ``zentar_cli.auth`` so
...
# Sources Zentar owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Zentar-owned OAuth/device-code state)
