...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ZENTAR_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Zentar runtime / deployment
    "ZENTAR_HOME", "ZENTAR_PROFILE", "ZENTAR_GATEWAY_LOCK_DIR",
    "ZENTAR_MAX_ITERATIONS", "ZENTAR_MAX_TOKENS", "ZENTAR_API_TIMEOUT",
    "ZENTAR_REDACT_SECRETS", "ZENTAR_NOUS_TIMEOUT_SECONDS",
    "_ZENTAR_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ZENTAR_KANBAN_DB", "ZENTAR_KANBAN_WORKSPACES_ROOT", "ZENTAR_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ZENTAR_KANBAN_",
    "ZENTAR_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(zentar_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(zentar_home) / ".env")
