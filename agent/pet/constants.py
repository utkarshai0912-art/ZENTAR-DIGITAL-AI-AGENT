...
row taxonomy is inferred from the atlas shape so Zentar can render both legacy
...
# rows (1536x1872), while older Zentar/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Zentar' activity state names. They are not always identical to the
...
# Legacy Zentar/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Zentar activity names -> accepted row-name aliases in descending
