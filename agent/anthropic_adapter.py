"""Anthropic Messages API adapter for ZENTAR DIGITAL AI AGENT.

Translates between Zentar's internal OpenAI-style message format and
...
# Zentar effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See zentar-digital-agent#13848, #17057.
    """
...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    See zentar-digital-agent#16748.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to zentar's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to zentar's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Zentar notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Zentar historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``zentar models``) never mutates ``~/.zentar/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Zentar)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.zentar/auth.json)
    ...
    # 1. Zentar-managed OAuth/setup token env var
    ...
    # 4. Zentar credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Zentar configs.
...
# ── Zentar-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.zentar/.anthropic_oauth.json (our own file).
...
def _get_zentar_oauth_file() -> Path:
    return get_zentar_home() / ".anthropic_oauth.json"
...
def run_zentar_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Zentar-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Zentar with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from zentar_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_zentar_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Zentar-managed OAuth credentials from ~/.zentar/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Zentar OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See zentar-digital-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See zentar-digital-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See zentar-digital-agent#13848 (Kimi) and
    zentar-digital-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("ZENTAR DIGITAL AI AGENT", "Claude Code")
                text = text.replace("ZENTAR DIGITAL AI AGENT", "Claude Code")
                text = text.replace("zentar-digital-agent", "claude-code")
                text = text.replace("ZENTAR DIGITAL", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Zentar-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Zentar surfaces in its CLI. We
