...
Every Zentar request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to ZENTAR DIGITAL AI AGENT and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=zentar-digital-agent",
        "client=zentar-client-v<__version__>",
    ]

The version is sourced live from ``zentar_cli.__version__`` so it auto-aligns
...
``zentar_cli.__version__`` is the canonical source of truth.
"""
...
def _zentar_version() -> str:
    """Return the current Zentar release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``zentar_cli`` cannot be imported (should
...
def zentar_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=zentar-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=zentar-client-v{_zentar_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=zentar-digital-agent", zentar_client_tag()]
