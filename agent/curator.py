...
def _state_file() -> Path:
    return get_zentar_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.zentar/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `zentar update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``zentar curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``zentar update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``zentar curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `zentar curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Zentar' background skill CURATOR. This is an "
    ...
    "into ~/.zentar/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: zentar-config-*, zentar-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.zentar/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.zentar/logs/curator/``)
    ...
    authored skill data in ``~/.zentar/skills/``.

    ``ensure_zentar_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_zentar_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every zentar install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: zentar curator status
        keep an umbrella stable: zentar curator pin document-tools
    ...
    lines.append("full report: zentar curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: zentar curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.zentar/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.zentar/skills/.archive/` for "
            "safety and can be restored via `zentar curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.zentar/skills/.archive/`. "
            "Restore any via `zentar curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `zentar curator restore <name>`")
    lines.append("- All archives live under `~/.zentar/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``zentar curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `zentar curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `zentar update` only if "
        ...
        # recorded in state so `zentar curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``zentar model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `zentar model` → auxiliary
