...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Zentar starts and finds ``~/.openclaw/``.

    Points users at ``zentar claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zentar claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Zentar, run "
        "`zentar claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zentar claw cleanup` (renames it to ~/.openclaw.pre-migration — "
