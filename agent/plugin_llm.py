...
Plugins built on ZENTAR DIGITAL AI AGENT often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Zentar
...
:class:`~zentar_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Zentar' main config
...
handles every provider, fallback chain, and per-task override Zentar
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`zentar_cli.plugins.PluginContext`
