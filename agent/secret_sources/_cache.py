...
cache problem must never block Zentar startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Zentar home used for cache paths.

    ``home_path`` is whatever ``load_zentar_dotenv()`` already resolved;
    falling back to ``$ZENTAR_HOME`` / ``~/.zentar`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("ZENTAR_HOME", Path.home() / ".zentar"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<zentar_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ZENTAR_HOME is ever made traversable.
            try:
