...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Zentar never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Zentar continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<zentar_home>/cache/op_cache.json``
so back-to-back short-lived ``zentar`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a ZENTAR_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Zentar to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_zentar_dotenv()`` after the .env files have loaded.
