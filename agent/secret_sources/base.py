...
``~/.zentar/.env`` has loaded and BEFORE the rest of Zentar reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per ZENTAR_HOME) by the orchestrator in
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``zentar secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``zentar secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved ZENTAR_HOME.
        """
...
# codes that must not reach Zentar' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Zentar knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Zentar
      output.
