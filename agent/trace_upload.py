"""Upload a Zentar session transcript to Hugging Face as an agent trace.

Zentar stores sessions in its own SQLite store (``zentar_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``zentar trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Zentar' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "zentar-traces"
_ZENTAR_VERSION = "zentar-digital-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Zentar OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Zentar' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Zentar conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.zentar/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `zentar trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/zentar-traces`` dataset. Returns a
