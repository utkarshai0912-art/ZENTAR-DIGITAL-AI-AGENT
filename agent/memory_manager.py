...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ZENTAR_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Zentar expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``zentar_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_zentar_home()`` themselves.
        """
        if "zentar_home" not in kwargs:
            from zentar_constants import get_zentar_home
            kwargs["zentar_home"] = str(get_zentar_home())
        for provider in self._providers:
            try:
