...
as ``kind: backend``) or ``~/.zentar/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``zentar tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ZENTAR_HOME/cache/videos/``, creating parents as needed."""
    from zentar_constants import get_zentar_home

    path = get_zentar_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ZENTAR_HOME/cache/videos/``.
