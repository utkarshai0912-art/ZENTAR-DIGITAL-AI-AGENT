"""Projects codex app-server events into Zentar' messages list.

The translator that lets Zentar' memory/skill review keep working under the
...
Zentar' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Zentar
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Zentar' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Zentar stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Zentar already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Zentar' messages store text only.
        text_parts: list[str] = []
