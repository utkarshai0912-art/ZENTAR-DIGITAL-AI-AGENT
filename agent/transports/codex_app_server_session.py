...
Owns one Codex thread per Zentar session. Drives `turn/start`, consumes
...
# Zentar' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ZENTAR_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Zentar/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Zentar session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ZENTAR_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ZENTAR_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="zentar",
            client_title="ZENTAR DIGITAL AI AGENT",
            client_version=_get_zentar_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Zentar' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Zentar' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own zentar-tools callback we
            # auto-accept — the user already approved Zentar' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Zentar approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Zentar returns 'once', 'session', 'always', or 'deny'.
...
def _get_zentar_version() -> str:
    """Best-effort Zentar version string for codex's userAgent line."""
    try:
