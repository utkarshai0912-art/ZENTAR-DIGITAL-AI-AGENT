...
projection into Zentar' display, approval bridging, transcript projection into
...
"codex_app_server"`. Zentar' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Zentar secret — gateway
        ...
        spawn_env = zentar_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ZENTAR_KANBAN_TASK"):
            kanban_db = spawn_env.get("ZENTAR_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ZENTAR_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ZENTAR_HOME", os.path.expanduser("~/.zentar")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "zentar",
        client_title: str = "ZENTAR DIGITAL AI AGENT",
