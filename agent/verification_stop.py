...
def _session_is_messaging_surface() -> bool:
    ...
    ``ZENTAR_SESSION_PLATFORM``; the CLI and TUI set ``ZENTAR_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``ZENTAR_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("ZENTAR_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``ZENTAR_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("ZENTAR_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `zentar-verify-` filename prefix, run it "
