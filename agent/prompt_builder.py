...
from zentar_constants import get_zentar_home, get_skills_dir, is_wsl
...
_ZENTAR_MD_NAMES = (".zentar.md", "ZENTAR.md")


def _find_zentar_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.zentar.md`` or ``ZENTAR.md``.
    ...
    # could pick up a .zentar.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ZENTAR_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are ZENTAR DIGITAL AI AGENT, an intelligent AI assistant created by ZENTAR DIGITAL. "
...
ZENTAR_AGENT_HELP_GUIDANCE = (
    "You run on ZENTAR DIGITAL AI AGENT (by ZENTAR DIGITAL). When the user needs help with "
    "Zentar itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://hermes-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `zentar-digital-agent` skill with skill_view(name='zentar-digital-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.zentar/kanban.db`. Your task id is in "
    "`$ZENTAR_KANBAN_TASK`; your workspace is `$ZENTAR_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ZENTAR_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ZENTAR_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ZENTAR_KANBAN_BRANCH:-wt/$ZENTAR_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ZENTAR_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`zentar profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `zentar kanban <verb>` for board operations. Use "
...
#
# The zentar-digital-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to zentar-digital-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `zentar computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Zentar "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Zentar terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Zentar desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Zentar WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Zentar itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Zentar runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Zentar itself is running. The host OS, home, and cwd "
                f"of the Zentar process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Zentar "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Zentar
    ...
    extra = (os.getenv("ZENTAR_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zentar_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ZENTAR_PLATFORM") or os.environ.get("ZENTAR_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.zentar/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ZENTAR DIGITAL AI AGENT itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `zentar-digital-agent` skill "
            "first. It has the actual commands (e.g. `zentar config set …`, `zentar tools`, "
            "`zentar setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: zentar setup, zentar setup tools, zentar setup terminal, zentar status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ZENTAR_HOME and return its content, or None.
    ...
    """
    try:
        from zentar_cli.config import ensure_zentar_home
        ensure_zentar_home()
    except Exception as e:
        logger.debug("Could not ensure ZENTAR_HOME before loading SOUL.md: %s", e)

    soul_path = get_zentar_home() / "SOUL.md"
...
def _load_zentar_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.zentar.md / ZENTAR.md — walk to git root."""
    zentar_md_path = _find_zentar_md(cwd_path)
    if not zentar_md_path:
        return ""
    try:
        content = zentar_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = zentar_md_path.name
        try:
            rel = str(zentar_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".zentar.md", context_length=context_length,
            read_path=str(zentar_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", zentar_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .zentar.md / ZENTAR.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ZENTAR_HOME is independent and always included when present.
    ...
    project_context = (
        _load_zentar_md(cwd_path, context_length)
    ...
    # SOUL.md from ZENTAR_HOME only — skip when already loaded as identity
    if not skip_soul:
