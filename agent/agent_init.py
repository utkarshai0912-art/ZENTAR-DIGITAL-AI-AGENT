...
from zentar_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: zentar config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ZENTAR_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_zentar_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ZENTAR_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .zentar.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ZENTAR_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.zentar/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from zentar_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind ZENTAR_DEV_CREDITS.
    ...
    # both live under ~/.zentar/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from zentar_logging import setup_logging, setup_verbose_logging
    setup_logging(zentar_home=_ra()._zentar_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that zentar_logging.setup_logging() does not
        ...
        # handler level inside zentar_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from zentar_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from zentar_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `zentar model` to "
                        "select a provider, or run `zentar setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ZENTAR_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["ZENTAR_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.zentar/sessions/ alongside gateway sessions
    zentar_home = get_zentar_home()
    agent.logs_dir = zentar_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.zentar/sessions/session_{sid}.json)
    ...
    try:
        from zentar_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from zentar_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "zentar"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Zentar' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "zentar", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, zentar, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from zentar_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from zentar_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by ZENTAR DIGITAL AI AGENT.  Choose a model with at least "
    ...
    # Nous Zentar 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from zentar_cli.model_switch import _check_zentar_model_warning

            _zentar_warn = _check_zentar_model_warning(agent.model or "")
            if _zentar_warn:
                _user_msg = (
                    "⚠ ZENTAR DIGITAL Zentar 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_zentar_warn)
        except Exception:
            pass
