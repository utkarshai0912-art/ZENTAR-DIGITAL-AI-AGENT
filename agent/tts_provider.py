...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Zentar with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.zentar/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Zentar
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zentar tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``zentar setup`` for
