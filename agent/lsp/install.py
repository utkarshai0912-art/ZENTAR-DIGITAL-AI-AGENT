...
appropriate.  All installs go to a Zentar-owned bin staging dir,
``<ZENTAR_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``zentar lsp
  status``.
...
# ``<ZENTAR_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `zentar lsp status` reports the host's presence.
...
def zentar_lsp_bin_dir() -> Path:
    """Return the Zentar-owned bin staging dir for LSP servers."""
    home = os.environ.get("ZENTAR_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".zentar")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(zentar_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = zentar_lsp_bin_dir().parent  # <ZENTAR_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = zentar_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a zentar-owned target dir.
    ...
    console script.
    """
    pip_target = zentar_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = zentar_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``zentar lsp status`` CLI to give users a quick
