...
_SENSITIVE_ZENTAR_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    zentar_home = get_zentar_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(zentar_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(zentar_home / rel for rel in _SENSITIVE_ZENTAR_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Zentar path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.zentar/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Zentar path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
