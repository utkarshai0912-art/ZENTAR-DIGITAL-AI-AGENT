    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `zentar setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Zentar "
...
def compress_context(
    ...
    *,
    ...
    # Zentar' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|zentar|off).
    ...
    # long-lived ``zentar_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `zentar update`) to resync.",
    ...
    try:
        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # from ``zentar_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("ZENTAR_SESSION_SOURCE", "cli"),
                            model=agent.model,
                    ...
                    try:
                        from zentar_cli.goals import migrate_goal_to_session
        ...
        # engines (e.g. zentar-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See zentar-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Zentar' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Zentar' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "zentar", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "zentar":
        logger.info(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="zentar_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
