...
Storage
-------
Bundles live in ``~/.zentar/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ZENTAR_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``zentar bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ZENTAR_HOME.

    Honors ``ZENTAR_BUNDLES_DIR`` for tests; falls back to
    ``<ZENTAR_HOME>/skill-bundles``.
    """
    override = os.environ.get("ZENTAR_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_zentar_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `zentar bundles` CLI subcommand.
# ---------------------------------------------------------------------------
