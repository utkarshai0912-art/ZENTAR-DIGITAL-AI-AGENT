...
upstream prefix cache warm.  See ``zentar-digital-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ZENTAR_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ZENTAR_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Zentar never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ZENTAR_HOME persona while keeping
    ...
    # Pointer to the zentar-digital-agent skill + docs for user questions about Zentar itself.
    stable_parts.append(ZENTAR_AGENT_HELP_GUIDANCE)
    ...
    # ZENTAR_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Zentar, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Zentar profile the agent is running
    # under so it doesn't conflate ~/.zentar/skills/ (default profile) with
    # ~/.zentar/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Zentar profile: default. Other profiles (if any) live "
            "under ~/.zentar/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Zentar profile: {active_profile}. This session reads "
            f"and writes ~/.zentar/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.zentar/skills/, ~/.zentar/plugins/, "
            f"~/.zentar/cron/, ~/.zentar/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Zentar never rebuilds or reinjects parts of it
