...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `zentar backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - zentar_home (str): The active ZENTAR_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.zentar``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "zentar").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'zentar memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], zentar_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'zentar memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``zentar_home`` is the active ZENTAR_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ZENTAR_HOME.

        ``zentar backup`` only walks ZENTAR_HOME, so any provider state kept
        ...
        of the archive, and ``zentar import`` restores them to their original
