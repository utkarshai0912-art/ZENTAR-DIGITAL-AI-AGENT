...
from zentar_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ZENTAR_KANBAN_TASK`` / ``ZENTAR_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ZENTAR_KANBAN_TASK") or os.getenv("ZENTAR_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Zentar Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``zentar_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ZENTAR_PLATFORM`` or
            ``ZENTAR_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("ZENTAR_PLATFORM")
        or get_session_env("ZENTAR_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``zentar`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.zentar/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``zentar`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ZENTAR_HOME, not cwd
        if not p.is_absolute():
            p = (zentar_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.zentar/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.zentar/skills/`` (including via symlinks) or configured
    ...
    # ~/.zentar/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Zentar can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    zentar = metadata.get("zentar") or {}
    if not isinstance(zentar, dict):
        zentar = {}
    return {
        "fallback_for_toolsets": zentar.get("fallback_for_toolsets", []),
        "requires_toolsets": zentar.get("requires_toolsets", []),
        "fallback_for_tools": zentar.get("fallback_for_tools", []),
        "requires_tools": zentar.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    zentar = metadata.get("zentar")
    if not isinstance(zentar, dict):
        return []
    raw = zentar.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Zentar metadata, VCS, virtualenv/dependency, cache, and skill
