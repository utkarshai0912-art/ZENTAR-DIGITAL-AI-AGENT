"""CLI entry point for the zentar-digital-agent ACP adapter.

Loads environment variables from ``~/.zentar/.env``, configures logging
...
try:
    ...
    # shadowing Zentar's own modules — ``zentar acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    zentar_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from ZENTAR_HOME (default ``~/.zentar``)."""
    from zentar_cli.env_loader import load_zentar_dotenv

    zentar_home = get_zentar_home()
    loaded = load_zentar_dotenv(zentar_home=zentar_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", zentar_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="zentar-acp",
        description="Run ZENTAR DIGITAL AI AGENT as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Zentar version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Zentar provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.zentar/node/ "
...
def _print_version() -> None:
    ...
    print(zentar_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ZentarACPAgent  # noqa: F401

    print("Zentar ACP check OK")


def _run_setup() -> None:
    from zentar_cli.main import main as zentar_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "zentar", "model"]
        zentar_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``zentar postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting zentar-digital-agent ACP adapter")
    ...
    import acp
    from .server import ZentarACPAgent
