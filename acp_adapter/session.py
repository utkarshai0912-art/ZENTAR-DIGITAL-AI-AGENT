"""ACP session manager — maps ACP sessions to Zentar AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.zentar/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Zentar itself is running in WSL.

    Windows ACP clients can launch ``zentar acp`` inside WSL while still sending
    ...
    """
    from zentar_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Zentar from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["zentar-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed ZENTAR DIGITAL AI AGENT."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Zentar AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Zentar runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.zentar/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ZENTAR_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_zentar_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=zentar_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Zentar daemon's process cwd.
