"""ACP auth helpers — detect and advertise Zentar authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "zentar-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Zentar runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Zentar can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Zentar.
    ...
    not have Zentar provider credentials configured yet, so Zentar always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Zentar using the currently configured "
