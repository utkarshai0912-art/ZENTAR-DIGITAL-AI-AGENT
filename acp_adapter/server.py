"""ACP agent server — exposes ZENTAR DIGITAL AI AGENT via the Agent Client Protocol."""
...
from tools.approval import (
    reset_zentar_interactive_context,
    set_zentar_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from zentar_cli import __version__ as ZENTAR_VERSION
except Exception:
    ZENTAR_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Zentar running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Zentar cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Zentar/OpenAI-compatible user content payload."""
...
class ZentarACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Zentar AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Zentar version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Zentar maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from zentar_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from zentar_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Zentar estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_zentar_session_id: str,
        previous_zentar_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.zentar.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_zentar_session_id,
                previous_zentar_session_id=previous_zentar_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_zentar_session_id: Optional[str] = None,
        previous_zentar_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Zentar changes it.

        When the internal Zentar head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_zentar_session_id`` so the
        attached ``_meta.zentar.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # zentar_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_zentar_session_id or session_id,
            previous_zentar_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["zentar-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="zentar-digital-agent", version=ZENTAR_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Zentar' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Zentar setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Zentar
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Zentar on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Zentar' local kawaii waiting/status
        ...
        # tools.approval (set_zentar_interactive_context) rather than
        # os.environ["ZENTAR_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue ZENTAR_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ZENTAR_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_zentar_interactive_context(True)
            ...
            previous_session_id = os.environ.get("ZENTAR_SESSION_ID")
            os.environ["ZENTAR_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_zentar_interactive_context(interactive_token)
                # Restore ZENTAR_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("ZENTAR_SESSION_ID", None)
                else:
                    os.environ["ZENTAR_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Zentar DB session id before the turn so we
            ...
            pre_turn_zentar_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (ZENTAR_SESSION_KEY in
        ...
        # _meta.zentar.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_zentar_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_zentar_id
            and pre_turn_zentar_id
            and post_turn_zentar_id != pre_turn_zentar_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_zentar_session_id=post_turn_zentar_id,
                    previous_zentar_session_id=pre_turn_zentar_id,
                )
            except Exception:
                logger.debug(
        ...
        # Zentar' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"ZENTAR DIGITAL AI AGENT v{ZENTAR_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Zentar has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
