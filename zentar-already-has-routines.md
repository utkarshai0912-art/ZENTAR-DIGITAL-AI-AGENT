# ZENTAR DIGITAL AI AGENT Has Had "Routines" Since March
...
Zentar equivalent — works today:
```bash
zentar webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Zentar implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | ZENTAR DIGITAL AI AGENT |
|---|---|---|
...
## Things Zentar Does That Routines Can't
...
Zentar has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
ZENTAR DIGITAL AI AGENT is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
zentar gateway setup    # enable webhooks
zentar webhook subscribe pr-review \
  --events "pull_request" \
...
GitHub: [github.com/NousResearch/hermes-digital-agent](https://github.com/NousResearch/hermes-digital-agent)

---

*ZENTAR DIGITAL AI AGENT is built by [ZENTAR DIGITAL](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
