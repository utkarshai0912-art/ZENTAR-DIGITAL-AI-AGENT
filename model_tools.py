...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_zentar_home() under the active profile.
...
#   - cli.py, zentar_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("ZENTAR_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ZENTAR_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by ZENTAR_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like zentar-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("zentar-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (zentar-*) include _ZENTAR_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("zentar-")
    ...
    # window). Core Zentar tools (toolsets._ZENTAR_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from zentar_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to zentar-digital-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from zentar_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from zentar_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        try:
            ...
            from zentar_cli.middleware import run_tool_execution_middleware
