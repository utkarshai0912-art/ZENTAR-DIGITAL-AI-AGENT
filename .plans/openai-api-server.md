# OpenAI-Compatible API Server for ZENTAR DIGITAL AI AGENT
...
SSE streaming. By exposing this endpoint, zentar-digital-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  zentar-digital-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.zentar/.env`
2. Run `zentar gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with zentar-digital-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns zentar-digital-agent as a model) |
...
Authorization: Bearer zentar-api-key-here
...
```json
{
  "object": "list",
  "data": [{
    "id": "zentar-digital-agent",
    "object": "model",
    "created": 1710000000,
    "owned_by": "zentar-digital-agent"
  }]
}
```
...
But zentar-digital-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "zentar-digital-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured zentar-digital-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of zentar-digital-agent's core prompt
...
Once implemented, zentar-digital-agent works as a drop-in backend for:
