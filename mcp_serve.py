"""
Zentar MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Zentar-specific extra)

Usage:
    zentar mcp serve
    zentar mcp serve --verbose
...
logger = logging.getLogger("zentar.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZENTAR_HOME."""
    try:
        from zentar_constants import get_zentar_home
        return get_zentar_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZENTAR_HOME", Path.home() / ".zentar")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zentar_constants import get_zentar_home
        directory_file = get_zentar_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZENTAR_HOME", Path.home() / ".zentar")
...
class EventBridge:
    ...
    This is the Zentar equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zentar_constants import get_zentar_home
            db_file = get_zentar_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZENTAR_HOME", Path.home() / ".zentar")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zentar MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zentar",
        instructions=(
            "ZENTAR DIGITAL AI AGENT messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zentar MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
