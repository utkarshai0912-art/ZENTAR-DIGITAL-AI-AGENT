"""
Status command for zentar CLI.

Shows the status of all ZENTAR DIGITAL AI AGENT components.
"""
...
from zentar_cli.auth import AuthError, resolve_provider
from zentar_cli.colors import Colors, color
from zentar_cli.config import get_env_path, get_env_value, get_zentar_home, load_config
from zentar_cli.models import provider_label
from zentar_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from zentar_cli.nous_subscription import get_nous_subscription_features
from zentar_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``zentar config``'s
...
def show_status(args):
    """Show status of all ZENTAR DIGITAL AI AGENT components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ ZENTAR DIGITAL AI AGENT Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: zentar portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: zentar model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: zentar auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: zentar auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: zentar model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from zentar_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from zentar_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   zentar gateway")
    ...
    jobs_file = get_zentar_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_zentar_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'zentar doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'zentar setup' to configure", Colors.DIM))
    print()
