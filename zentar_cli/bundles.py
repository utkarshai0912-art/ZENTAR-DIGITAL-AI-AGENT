"""Implementation of the ``zentar bundles`` CLI subcommand.

Mirrors the structure of ``zentar_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `zentar bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  zentar bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``zentar bundles`` argparse tree.

    Called from ``zentar_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `zentar bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``zentar bundles <subcommand>`` to the right handler."""
