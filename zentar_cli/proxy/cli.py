"""CLI handlers for the ``zentar proxy`` subcommand."""
...
from zentar_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "zentar proxy requires aiohttp. Install one of:\n"
        "  pip install 'zentar-digital-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"zentar auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Zentar proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Zentar proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: zentar proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``zentar proxy <subcommand>``."""
    ...
    print(
        "zentar proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  zentar proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
