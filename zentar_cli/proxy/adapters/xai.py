...
# xAI's public API is OpenAI-compatible for the endpoints Zentar commonly
# uses. The Responses endpoint is included because Zentar' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Zentar-managed OAuth credentials."""

    auth_hint = "zentar auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`zentar auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`zentar auth reset xai-oauth` or re-authenticate with "
                    "`zentar auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `zentar auth add xai-oauth --type oauth`."
            )
