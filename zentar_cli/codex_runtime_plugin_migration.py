"""Migrate Zentar' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Zentar' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Zentar mcp_servers.<n>.command/args/env  → codex stdio transport
  Zentar mcp_servers.<n>.url/headers       → codex streamable_http transport
  Zentar mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Zentar mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Zentar-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Zentar' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Zentar config.")
...
# Zentar keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Zentar' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, zentar_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Zentar MCP server config to the codex inline-table dict
    ...
    if not isinstance(zentar_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(zentar_cfg.get("command"))
    has_url = bool(zentar_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ZENTAR_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Zentar)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Zentar' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Zentar' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Zentar
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="zentar-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ZENTAR_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed zentar-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.zentar`` symlink that happens to live under ``/private/var/folders``
...
def _build_zentar_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Zentar' own
    ...
    so a zentar installed under /opt/, /usr/local/, or a venv all work.
    ZENTAR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ZENTAR_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_zentar_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ZENTAR_HOME its launcher sets
    ...
    # — that would override the launcher's ZENTAR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ZENTAR_HOME", tmp_path) would
    ...
    zentar_home = os.environ.get("ZENTAR_HOME") or ""
    if zentar_home and _looks_like_test_tempdir(zentar_home):
        zentar_home = ""
    if zentar_home:
        env["ZENTAR_HOME"] = zentar_home
    # PYTHONPATH passes through so a worktree-launched zentar finds the
    ...
    env["ZENTAR_QUIET"] = "1"
    env["ZENTAR_REDACT_SECRETS"] = env.get("ZENTAR_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_zentar_tools: bool = True,
) -> MigrationReport:
    """Translate Zentar mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        zentar_config: full ~/.zentar/config.yaml dict
        ...
        expose_zentar_tools: when True (default), register Zentar' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Zentar for tools
    ...
    zentar_servers = (zentar_config or {}).get("mcp_servers") or {}
    if not isinstance(zentar_servers, dict):
        report.errors.append(
            "mcp_servers in Zentar config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in zentar_servers.items():
    ...
    # Inject Zentar' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Zentar for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/zentar_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_zentar_tools:
        translated["zentar-tools"] = _build_zentar_tools_mcp_entry()
        if "zentar-tools" not in report.migrated:
            report.migrated.append("zentar-tools")
