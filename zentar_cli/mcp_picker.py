"""MCP picker — interactive `zentar mcp picker` (also the default `zentar mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``zentar mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `zentar plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from zentar_cli.cli_output import prompt_yes_no
from zentar_cli.curses_ui import curses_single_select
from zentar_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Zentar session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from zentar_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `zentar mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: zentar mcp install <name>    Picker: zentar mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Zentar is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Zentar — run `zentar update` "
...
def show_catalog() -> None:
    """`zentar mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`zentar mcp picker` (and default `zentar mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`zentar mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `zentar mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
