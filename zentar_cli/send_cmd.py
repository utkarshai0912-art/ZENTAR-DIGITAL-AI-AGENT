"""CLI subcommand: ``zentar send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"zentar send: {file_path} is not a text file. --file reads the "
                ...
                f'  zentar send --to telegram "MEDIA:{file_path}"\n'
                f'  zentar send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  zentar send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"zentar send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"zentar send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"zentar send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"zentar send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"zentar send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `zentar gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.zentar/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_zentar_env() -> None:
    """Populate ``os.environ`` from ``~/.zentar/.env`` AND bridge top-level
    ...
    startup that ``zentar send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.zentar/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.zentar/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``zentar send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.zentar/.env and ~/.zentar/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "zentar send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  zentar send --to telegram \"hello\"\n"
            "  zentar send --to discord:#ops --file report.md\n"
            "  zentar send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "zentar send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `zentar send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Zentar "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.zentar/.env + ~/.zentar/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  zentar send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `zentar send --list telegram`.",
    )
