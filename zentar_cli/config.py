"""
Configuration management for ZENTAR DIGITAL AI AGENT.

Config files are stored in ~/.zentar/ for easy access:
- ~/.zentar/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zentar/.env         - API keys and secrets

This module provides:
- zentar config          - Show current configuration
- zentar config edit     - Open config in editor
- zentar config set      - Set a specific value
- zentar config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``zentar config set`` (which rewrites
    ...
    ``config.yaml`` in place: zentar never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.zentar/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``zentar logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  zentar config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Zentar makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Zentar itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``zentar update``, the TUI build.
...
#   on every plugin install / ``zentar update``.
...
# * ``ZENTAR_HOME`` / ``ZENTAR_PROFILE`` / ``ZENTAR_CONFIG`` /
#   ``ZENTAR_ENV`` — Zentar runtime location flags. Writing these into
...
#
# IMPORTANT: ``ZENTAR_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ZENTAR_LANGFUSE_PUBLIC_KEY,
# ZENTAR_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Zentar runtime location — never via dashboard env writer.
    # NOT a HERMES_* blanket: integration credentials (ZENTAR_GEMINI_*,
    # ZENTAR_LANGFUSE_*, ZENTAR_SPOTIFY_*, ...) ARE allowed.
    "ZENTAR_HOME", "ZENTAR_PROFILE", "ZENTAR_CONFIG", "ZENTAR_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Zentar runtime location "
            "(ZENTAR_HOME, ZENTAR_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ZENTAR_TOOL_PROGRESS", "ZENTAR_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `zentar plugins enable
    # observability/langfuse` or `zentar tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ZENTAR_LANGFUSE_ENV",
    "ZENTAR_LANGFUSE_RELEASE",
    "ZENTAR_LANGFUSE_SAMPLE_RATE",
    "ZENTAR_LANGFUSE_MAX_CHARS",
    "ZENTAR_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from zentar_cli.colors import Colors, color
from zentar_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZENTAR_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zentar_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Zentar is running in package-manager-managed mode.

    Two signals: the ZENTAR_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZENTAR_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade zentar-digital-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``zentar_cli/`` — i.e. the git checkout for source
    installs, ``/opt/zentar`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ZENTAR_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Zentar was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ZENTAR_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ZENTAR_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.zentar``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ZENTAR_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.zentar:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ZENTAR_HOME``, a
    ...
    ``docker`` and ``zentar update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/zentar-digital-agent`` image bakes a ``docker``
        stamp into ``/opt/zentar`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ZENTAR_HOME.
    try:
    ...
    #    host install whose shared $ZENTAR_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``zentar update``.
    try:
        method = (
            (get_zentar_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``zentar_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ZENTAR_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/zentar`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Zentar lives in a ``uv tool`` layout.

    ``uv tool install zentar-digital-agent`` places the install at
    ``.../uv/tools/zentar-digital-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``zentar-digital-agent`` happens to be uv-tool-installed on the machine while
    the *active* Zentar is a regular pip/venv install, causing
    ``zentar update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/zentar-digital-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade zentar-digital-agent"
    if method == "docker":
        return "docker pull nousresearch/zentar-digital-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade zentar-digital-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade zentar-digital-agent"
        return "pip install --upgrade zentar-digital-agent"
    return "zentar update"
...
# banner, the TUI/desktop session info panel, and ``zentar update``. NixOS
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``zentar update`` / ``zentar update --check`` so the wording — and the
...
# Long-form text for ``zentar update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# zentar_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Zentar, it doesn't update
...
✗ ``zentar update`` doesn't apply inside the Docker container.

ZENTAR DIGITAL AI AGENT runs as a published image (nousresearch/zentar-digital-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/zentar-digital-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate zentar-digital-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/zentar-digital-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/zentar-digital-agent/tags
  • Your config and session history live under ``$ZENTAR_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``zentar update`` inside Docker.
...
def format_managed_message(action: str = "modify this Zentar installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZENTAR_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Zentar installation is managed by NixOS "
            f"(ZENTAR_MANAGED={env_hint}).\n"
            "Edit services.zentar-digital-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Zentar installation is managed by Homebrew "
            f"(ZENTAR_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zentar-digital-agent"
        )

    return (
        f"Cannot {action}: this Zentar installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Zentar."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZENTAR_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zentar_bin
    or None if container mode is not active, we're already inside the
    container, or ZENTAR_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZENTAR_DEV") == "1":
        return None
    ...
    container_mode_file = get_zentar_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zentar-digital-agent")
    exec_user = info.get("exec_user", "zentar")
    zentar_bin = info.get("zentar_bin", "/data/current-package/bin/zentar")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "zentar_bin": zentar_bin,
    }
...
# Re-export from zentar_constants — canonical definition lives there.
from zentar_constants import get_zentar_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zentar_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_zentar_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_zentar_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ZENTAR_UID / ZENTAR_GID env vars set by Docker deployments.

    Docker containers running Zentar commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ZENTAR_HOME once, but
    subdirectories created at runtime by ``ensure_zentar_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ZENTAR_UID", "").strip()
    gid_str = os.environ.get("ZENTAR_GID", "").strip()
    try:
...
def _chown_to_zentar_uid(path) -> None:
    """Chown ``path`` to ``ZENTAR_UID:ZENTAR_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_zentar_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_zentar_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zentar group can
    share state with the gateway service.

    The mode can be overridden via the ZENTAR_HOME_MODE environment variable
    (e.g. ZENTAR_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZENTAR_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ZENTAR_UID``/``ZENTAR_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZENTAR_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_zentar_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Zentar runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZENTAR_CONTAINER") or os.environ.get("ZENTAR_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ZENTAR_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZENTAR_HOME, upgrading legacy empty templates.
...
def ensure_zentar_home():
    """Ensure ~/.zentar directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``zentar profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ZENTAR_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_zentar_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_zentar_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZENTAR_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Zentar-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Zentar
        ...
        # identity slot (SOUL.md). Empty by default. The ZENTAR_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Zentar adds a coding operating brief
    ...
    "terminal": {
        ...
        #             ZENTAR_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ZENTAR_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Zentar auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Zentar sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Zentar
        ...
        # bundled Zentar image, which drops to the `zentar` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Zentar sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Zentar should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``zentar chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.zentar/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: zentar sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .zentar.md, or .cursorrules before Zentar applies
    ...
    # single read_file page exceeds these limits, Zentar truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Zentar' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); zentar =
                                      # Zentar' compression threshold triggers
    ...
    # See tools/kanban_tools.py and zentar_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``zentar dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``zentar kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``zentar kanban decompose`` and the kanban
        ...
        # ``zentar profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `zentar model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `zentar` (and `zentar chat`) launches by default:
        ...
        # REPL and `--tui` (or ZENTAR_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `zentar --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `zentar -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZENTAR_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `zentar --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to ZENTAR_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/zentar`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `zentar pets`. Disabled until
        ...
        "pet": {
            ...
            # get_zentar_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``ZENTAR_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ZENTAR_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ZENTAR_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ZENTAR_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ZENTAR_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/zentar``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Zentar appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.zentar/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zentar/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.zentar/cache/delegation/
    ...
    # response. If not, Zentar feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Zentar auto-pauses the goal and
    ...
    "moa": {
        ...
        # <zentar_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.zentar/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZENTAR_SKILL_DIR} and ${ZENTAR_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `zentar curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `zentar curator run --consolidate` overrides
        ...
        # normally restored on every `zentar update`, so pruning them only
        ...
        # skipped), snapshot ~/.zentar/skills/ into
        # ~/.zentar/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `zentar curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for zentar-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ZENTAR DIGITAL AI AGENT*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ZENTAR_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Zentar' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.zentar/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZENTAR_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `zentar doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``zentar_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Zentar to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ZENTAR_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ZENTAR_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `zentar -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `zentar` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `zentar kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     zentar-digital-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Zentar tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.zentar/logs/.
    ...
    # update model picker lists without shipping a zentar-digital-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `zentar model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal ZENTAR_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.zentar/sessions/sessions.json entirely.
        ...
        # the ZENTAR_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.zentar.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.zentar/.env,
        ...
        # behavior: files must live under the Zentar cache, under
        ...
        # user. Bridged to ZENTAR_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Zentar
        # cache (~/.zentar/cache/{documents,images,audio,video,screenshots})
        ...
        # to ZENTAR_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ZENTAR_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ZENTAR_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.zentar/state.db.
    ...
    "sessions": {
        ...
        # default of ``zentar sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.zentar/sessions/session_{sid}.json`` on every turn
    ...
    # ``zentar update`` behaviour.
    "updates": {
        # Run a full ``zentar backup``-style zip of ZENTAR_HOME before every
        # ``zentar update``.  Backups land in ``<ZENTAR_HOME>/backups/`` and
        # can be restored with ``zentar import <path>``.  Off by default:
        # zipping a large ZENTAR_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``zentar update
        ...
        "backup_keep": 5,
        # What `zentar update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ZENTAR_HOME>/lsp/bin/`` on first use.
    ...
    # `zentar tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.zentar/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.zentar/.env (or your shell) and never in config.yaml.
            ...
            # ~/.zentar/bin/ on first use.  When False you must install
            ...
            # `zentar secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.zentar/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Zentar disables it for our users unless they
        # explicitly opt in here. When false (default), Zentar sets
    ...
    # Zentar Desktop (Electron app) launch options. These only affect
    # `zentar desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ZENTAR_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Zentar mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'zentar model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Zentar auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Zentar, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zentar:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZENTAR_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #zentar)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: zentar-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zentar-digital-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Zentar API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Zentar API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Zentar API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ZENTAR_TOOL_PROGRESS and ZENTAR_TOOL_PROGRESS_MODE are deprecated —
    ...
    "ZENTAR_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zentar.config`` entries, then checks
    ...
    try:
        ...
        # should never break `zentar update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Zentar' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Zentar versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zentar_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zentar setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Zentar won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zentar doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ZENTAR_HOME", "~/.zentar")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "zentar update / zentar -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZENTAR_TOOL_PROGRESS")
            old_mode = get_env_value("ZENTAR_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZENTAR_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZENTAR_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZENTAR_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZENTAR_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ZENTAR_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_zentar_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `zentar plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `zentar curator status` has no
    ...
    #   3. Creates `~/.zentar/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_zentar_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_zentar_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `zentar config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `zentar config set`"
                    )
    ...
    # behavior set it to true explicitly via `zentar config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from zentar_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from zentar_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: zentar config set <key> <value>")
    ...
    # `zentar update`, but only the version bump is persisted.
    ...
    # metadata.zentar.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_zentar_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``zentar config set``
    ...
    backends) — while display paths (``zentar status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _zentar_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zentar/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zentar/config.yaml.
    ...
    (which change ``ZENTAR_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/zentar/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_zentar_dotenv()
...
#   openai-codex (OAuth — zentar auth) — OpenAI Codex
#   nous         (OAuth — zentar auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.zentar/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Zentar writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zentar/.env.
    ...
    interactive menu render (`zentar tools`, `zentar setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `zentar tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Zentar env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zentar/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zentar/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zentar/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zentar/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Zentar — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Zentar vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zentar/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.zentar/.env`` over ``os.environ``.

    Used for Zentar-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Zentar Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from zentar_cli.auth import get_anthropic_key
    ...
    # Warn on stale ZENTAR_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ZENTAR_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ZENTAR_MAX_ITERATIONS={_env_ghost} "
                f"(run 'zentar doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  zentar config edit     # Edit config file", Colors.DIM))
    print(color("  zentar config set <key> <value>", Colors.DIM))
    print(color("  zentar setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `zentar config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `zentar config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zentar config set <key> <value>")
            print()
            print("Examples:")
            print("  zentar config set model anthropic/claude-sonnet-4")
            print("  zentar config set terminal.backend docker")
            print("  zentar config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zentar config migrate' to add them")
        ...
        print("  zentar config           Show current configuration")
        print("  zentar config edit      Open config in editor")
        print("  zentar config set <key> <value>   Set a config value")
        print("  zentar config check     Check for missing/outdated config")
        print("  zentar config migrate   Update config with new options")
        print("  zentar config path      Show config file path")
        print("  zentar config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``zentar config`` UI so users
