...
Profile-aware location: ``$ZENTAR_HOME/logs/dashboard-auth.log``.
...
from ``zentar_constants`` or other zentar_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ZENTAR_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``zentar_constants.get_zentar_home`` semantics: env var wins,
    else ``~/.zentar``. A local copy avoids an import cycle with the
    middleware which lives below ``zentar_cli``.
    """
    home = os.environ.get("ZENTAR_HOME") or str(Path.home() / ".zentar")
