"""Slash command definitions and autocomplete for the Zentar CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Zentar config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Zentar works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Zentar is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.zentar/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update ZENTAR DIGITAL AI AGENT to the latest version", "Info"),
    CommandDef("version", "Show ZENTAR DIGITAL AI AGENT version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zentar_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/zentar`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Zentar ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Zentar
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``zentar skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``zentar skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``zentar skills list`` and
...
# unrelated command landed. These claim their slots right after /zentar,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /zentar <command>).
...
# native slot, the alias spelling stays reachable via /zentar reset).
...
# ``/zentar <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/zentar" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /zentar credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /zentar billing.
#   - moa: high-cost slash mode, available through /zentar moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /zentar debug on Slack.
_SLACK_VIA_ZENTAR_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/zentar <verb>`` subcommand.
    ...
    can still reach them via ``/zentar <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/zentar`` is always reserved as the first entry so the
    legacy ``/zentar <subcommand>`` form keeps working for anything that
    ...
    # Reserve /zentar as the catch-all top-level command.
    entries.append(("zentar", "Talk to Zentar or run a subcommand", "[subcommand] [args]"))
    seen.add("zentar")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ZENTAR_ONLY:
            # Intentionally Slack-via-/zentar only (see _SLACK_VIA_ZENTAR_ONLY).
            return
    ...
    # everything except /zentar, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://zentar-digital-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /zentar handler.

    Maps both canonical names and aliases so /zentar bg do stuff works
    the same as /zentar background do stuff.

    Plugin-registered slash commands are included so ``/zentar <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zentar_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
