"""
Dump command for zentar CLI.

Outputs a compact, plain-text summary of the user's Zentar setup
...
from zentar_cli.config import get_zentar_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.zentar/.env.
    ...
    exports. ``zentar debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.zentar_build_sha`` by the Dockerfile's
    ``ZENTAR_GIT_SHA`` build-arg (see ``zentar_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``zentar dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from zentar_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(zentar_home: Path) -> int:
...
def _cron_summary(zentar_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zentar_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zentar_home:      {display_zentar_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.zentar/.env: a managed
        ...
        # A credential added via `zentar auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `zentar auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["zentar-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zentar_home)}")
    lines.append(f"  skills:             {_count_skills(zentar_home)}")
