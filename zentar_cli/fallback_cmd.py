"""
zentar fallback — manage the fallback provider chain.
...
Subcommands:
  zentar fallback [list]   Show the current fallback chain (default when no subcommand)
  zentar fallback add      Pick provider + model via the same picker as `zentar model`,
                           then append the selection to the chain
  zentar fallback remove   Pick an entry to delete from the chain
  zentar fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.zentar/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from zentar_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `zentar model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  zentar fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `zentar model`, then append the selection to the chain."""
    from zentar_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `zentar model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `zentar fallback list` to view, or `zentar fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``zentar fallback [subcommand]``."""
