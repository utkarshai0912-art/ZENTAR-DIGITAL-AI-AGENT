#!/usr/bin/env python3
"""
Zentar CLI - Main entry point.

Usage:
    zentar                     # Interactive chat (default)
    zentar chat                # Interactive chat
    zentar gateway             # Run gateway in foreground
    zentar gateway start       # Start gateway as service
    zentar gateway stop        # Stop gateway service
    zentar gateway status      # Show gateway status
    zentar gateway install     # Install gateway service
    zentar gateway uninstall   # Uninstall gateway service
    zentar setup               # Interactive setup wizard
    zentar logout              # Clear stored authentication
    zentar status              # Show status of all components
    zentar cron                # Manage cron jobs
    zentar cron list           # List cron jobs
    zentar cron status         # Check if cron scheduler is running
    zentar doctor              # Check configuration and dependencies
    zentar honcho setup                    # Configure Honcho AI memory integration
    zentar honcho status                   # Show Honcho config and connection status
    zentar honcho sessions                 # List directory → session name mappings
    zentar honcho map <name>               # Map current directory to a session name
    zentar honcho peer                     # Show peer names and dialectic settings
    zentar honcho peer --user NAME         # Set user peer name
    zentar honcho peer --ai NAME           # Set AI peer name
    zentar honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zentar honcho mode                     # Show current memory mode
    zentar honcho mode [hybrid|honcho|local]  # Set memory mode
    zentar honcho tokens                   # Show token budget settings
    zentar honcho tokens --context N       # Set session.context() token cap
    zentar honcho tokens --dialectic N     # Set dialectic result char cap
    zentar honcho identity                 # Show AI peer identity representation
    zentar honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zentar honcho migrate                  # Step-by-step migration guide: OpenClaw native → Zentar + Honcho
    zentar version             Show version
    zentar update              Update to latest version
    zentar uninstall           Uninstall ZENTAR DIGITAL AI AGENT
    zentar acp                 Run as an ACP server for editor integration
    zentar sessions browse     Interactive session picker with search

    zentar claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: zentar_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``zentar_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``zentar update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``zentar_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, zentar crashes on import and the user can't run
# ``zentar update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import zentar_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'zentar' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``zentar tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``zentar.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("zentar")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"zentar", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"zentar")
        # Windows: the .exe name is already ``zentar.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* zentar_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ZENTAR_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".zentar", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ZENTAR_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ZENTAR_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ZENTAR_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ZENTAR_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`zentar --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from zentar_cli import __release_date__, __version__

    print(f"ZENTAR DIGITAL AI AGENT v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``zentar --version`` before config/logging imports on Termux."""
    if os.environ.get("ZENTAR_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from zentar_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from zentar_cli.subcommands.cron import build_cron_parser
from zentar_cli.subcommands.gateway import build_gateway_parser
from zentar_cli.subcommands.profile import build_profile_parser
from zentar_cli.subcommands.model import build_model_parser
from zentar_cli.subcommands.setup import build_setup_parser
from zentar_cli.subcommands.postinstall import build_postinstall_parser
from zentar_cli.subcommands.whatsapp import build_whatsapp_parser
from zentar_cli.subcommands.slack import build_slack_parser
from zentar_cli.subcommands.login import build_login_parser
from zentar_cli.subcommands.logout import build_logout_parser
from zentar_cli.subcommands.auth import build_auth_parser
from zentar_cli.subcommands.status import build_status_parser
from zentar_cli.subcommands.webhook import build_webhook_parser
from zentar_cli.subcommands.hooks import build_hooks_parser
from zentar_cli.subcommands.doctor import build_doctor_parser
from zentar_cli.subcommands.security import build_security_parser
from zentar_cli.subcommands.dump import build_dump_parser
from zentar_cli.subcommands.debug import build_debug_parser
from zentar_cli.subcommands.backup import build_backup_parser
from zentar_cli.subcommands.import_cmd import build_import_cmd_parser
from zentar_cli.subcommands.config import build_config_parser
from zentar_cli.subcommands.console import build_console_parser
from zentar_cli.subcommands.version import build_version_parser
from zentar_cli.subcommands.update import build_update_parser
from zentar_cli.subcommands.uninstall import build_uninstall_parser
from zentar_cli.subcommands.dashboard import build_dashboard_parser
from zentar_cli.subcommands.gui import build_gui_parser
from zentar_cli.subcommands.logs import build_logs_parser
from zentar_cli.subcommands.prompt_size import build_prompt_size_parser
from zentar_cli.subcommands.memory import build_memory_parser
from zentar_cli.subcommands.acp import build_acp_parser
from zentar_cli.subcommands.tools import build_tools_parser
from zentar_cli.subcommands.insights import build_insights_parser
from zentar_cli.subcommands.skills import build_skills_parser
from zentar_cli.subcommands.pairing import build_pairing_parser
from zentar_cli.subcommands.plugins import build_plugins_parser
from zentar_cli.subcommands.mcp import build_mcp_parser
from zentar_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (zentar tools, zentar setup, zentar model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zentar {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any zentar module import.
#
# Many modules cache ZENTAR_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZENTAR_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zentar/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZENTAR_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `zentar mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Zentar' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo zentar -p <name>` against the invoking user's home.
        ...
        candidate = home / ".zentar" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`zentar chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors zentar_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ZENTAR_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.zentar/profiles/coder or
    # /opt/data/profiles/coder).  If ZENTAR_HOME points to the zentar root
    # instead (e.g. systemd hardcodes ZENTAR_HOME=/root/.zentar), we must
    # still read active_profile — the user may have switched profiles via
    # `zentar profile use` and the gateway should honour that choice.
    # See issue #22502.
    zentar_home_env = os.environ.get("ZENTAR_HOME", "")
    if profile_name is None and zentar_home_env:
        if Path(zentar_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the zentar root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ZENTAR_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``zentar gateway run`` to mean
    # "the root ZENTAR_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ZENTAR_S6_SUPERVISED_CHILD"):
        try:
            from zentar_constants import get_default_zentar_root

            active_path = get_default_zentar_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZENTAR_HOME
    if profile_name is not None:
        try:
            from zentar_cli.profiles import resolve_profile_env

            zentar_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            zentar_home = _resolve_sudo_user_profile_env(profile_name)
            if not zentar_home:
            ...
            # A bug in profiles.py must NEVER prevent zentar from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ZENTAR_HOME"] = zentar_home
...
# Load .env from ~/.zentar/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from zentar_cli.config import get_zentar_home
from zentar_cli.env_loader import load_zentar_dotenv

load_zentar_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ZENTAR_REDACT_SECRETS env
# var BEFORE zentar_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_zentar_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from zentar_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ZENTAR_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ZENTAR_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `zentar` subcommands
...
try:
    from zentar_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from zentar_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if zentar_constants not importable yet
...
from zentar_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to zentar_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (zentar_cli.main._model_flow_*) keep resolving unchanged.
from zentar_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_zentar_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ZENTAR_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ZENTAR_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from zentar_cli.config import get_env_path, get_zentar_home, load_config
    from zentar_cli.auth import get_auth_status

    # Determine whether Zentar itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zentar_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zentar_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from zentar_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_zentar_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Zentar has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Zentar to use their tokens.
    if _has_zentar_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from zentar_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zentar_bin
        cli_args: the original CLI arguments (everything after 'zentar')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    zentar_bin = container_info["zentar_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo zentar {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zentar {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  zentar --tui --resume {target}")
    if title:
        print(f'  zentar --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zentar/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@zentar" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/zentar-ink/package.json",
    "packages/zentar-ink/index.js",
    "packages/zentar-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ZENTAR_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ZENTAR_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ZENTAR_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZENTAR_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    zentar_home = os.environ.get("ZENTAR_HOME") or str(Path.home() / ".zentar")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZENTAR_HOME": zentar_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(zentar_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(zentar_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if zentar_cli_dir is None:
        zentar_cli_dir = Path(__file__).parent
    bundled = zentar_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``zentar update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ZENTAR_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Zentar checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `zentar update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Zentar checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `zentar --tui`\n"
        "If the checkout is still inconsistent, run `zentar update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZENTAR_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZENTAR_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from zentar_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ZENTAR_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZENTAR_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZENTAR_TUI_DIR (e.g. `unset ZENTAR_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @zentar/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @zentar/ink resolves through
        # packages/zentar-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "zentar-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zentar_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from zentar_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="zentar-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ZENTAR_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ZENTAR_PYTHON_SRC_ROOT"] = os.environ.get(
        "ZENTAR_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ZENTAR_PYTHON", sys.executable)
    env.setdefault("ZENTAR_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ZENTAR_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ZENTAR_MODEL"] = model
        env["ZENTAR_INFERENCE_MODEL"] = model
    if provider:
        env["ZENTAR_TUI_PROVIDER"] = provider
        env["ZENTAR_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZENTAR_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ZENTAR_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ZENTAR_TUI_SKILLS"] = value
    if query:
        env["ZENTAR_TUI_QUERY"] = query
    if image:
        env["ZENTAR_TUI_IMAGE"] = image
    if checkpoints:
        env["ZENTAR_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ZENTAR_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ZENTAR_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ZENTAR_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ZENTAR_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ZENTAR_ACCEPT_HOOKS"] = "1"
    ...
    # ZENTAR_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `zentar --tui` try to
    ...
    # still set ZENTAR_TUI_RESUME themselves.
    env.pop("ZENTAR_TUI_RESUME", None)
    if resume_session_id:
        env["ZENTAR_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `zentar update` so
    ...
    if code == 42:
        from zentar_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZENTAR_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``zentar kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``zentar kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ZENTAR_KANBAN_BOARD"):
        return
    try:
        from zentar_cli.kanban_db import get_current_board

        os.environ["ZENTAR_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.zentar/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Zentar — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Zentar still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ZENTAR_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ZENTAR_TUI") == "1":
        return True
    try:
        from zentar_cli.config import load_config
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'zentar sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from zentar_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from zentar_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'zentar doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Zentar isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  zentar setup")
        print()

        from zentar_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'zentar setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from zentar_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZENTAR_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.zentar/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ZENTAR_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ZENTAR_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZENTAR_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zentar_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from zentar_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from zentar_cli.config import get_env_value, save_env_value
    from zentar_constants import find_node_executable, with_zentar_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Zentar?")
        print()
    ...
    # has no creds.json.  Every subsequent `zentar gateway` then paid a 30s
    ...
    session_dir = get_zentar_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: zentar gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `zentar gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zentar gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ZENTAR DIGITAL AI AGENT'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zentar gateway")
        ...
        print()
        print("  Or install as a service: zentar gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'zentar whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``zentar whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``zentar_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from zentar_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from zentar_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from zentar_cli.config import stamp_install_method
    from zentar_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Zentar post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from zentar_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zentar model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from zentar_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from zentar_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from zentar_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ZENTAR_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zentar_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'zentar model' for "
                "available providers, or run 'zentar doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in zentar_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.zentar/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zentar/.env if the active provider is not 'custom'.
    ...
    """
    from zentar_cli.config import get_env_value, save_env_value, load_config
...
#
# Zentar uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `zentar model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `zentar model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`zentar_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from zentar_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from zentar_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Zentar only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``zentar model`` flow, then route aux tasks to them here.
    """
    from zentar_cli.config import load_config
    from zentar_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from zentar_cli.auth import _prompt_model_selection
    from zentar_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from zentar_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from zentar_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from zentar_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Zentar URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from zentar_cli.curses_ui import curses_radiolist
...
# downstream call sites read `zentar_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `zentar` invocation — including
# fast paths like `zentar --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from zentar_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``zentar setup`` / ``zentar model``.
    ...
    recover from a malformed paste without editing ``~/.zentar/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from zentar_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from zentar_cli.config import save_env_value
    from zentar_cli.secret_prompt import masked_secret_prompt
    ...
    from zentar_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `zentar setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zentar_constants import display_zentar_home as _dhh_fn

            print(
                f"    Zentar will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Zentar CLI with a provider."""
    from zentar_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zentar_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zentar_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zentar_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zentar_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from zentar_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``zentar slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: zentar slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `zentar slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from zentar_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zentar_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from zentar_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from zentar_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zentar_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `zentar security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from zentar_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from zentar_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zentar_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zentar_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Zentar home directory to a zip file."""
    if getattr(args, "quick", False):
        from zentar_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zentar_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Zentar backup from a zip file."""
    from zentar_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from zentar_cli.config import detect_install_method
    from zentar_cli.banner import format_banner_version_label
    ...
    try:
        from zentar_cli.banner import check_for_updates
        from zentar_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall ZENTAR DIGITAL AI AGENT (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from zentar_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from zentar_cli.uninstall import run_gui_uninstall
    ...
    from zentar_cli.uninstall import run_uninstall
...
# Critical files that Zentar must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "zentar_cli/main.py",
    "zentar_cli/config.py",
    "zentar_cli/__init__.py",
    "zentar_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``zentar`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="zentar-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zentar update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from zentar_constants import get_zentar_home
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``zentar_cli/web_dist/`` (per vite.config.ts
    outDir: "../zentar_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "zentar_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``zentar`` launcher present but ``zentar_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the zentar venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — zentar venv python3, no nix-shell overhead
    ...
    # a zentar venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``zentar update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zentar web``).
    ...
    # build path usable on a stock `py -m zentar_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_zentar_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (zentar web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``zentar update`` can unconditionally call ``zentar desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``zentar desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ZENTAR_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ZENTAR_HOME."""
    from zentar_constants import get_zentar_home
    return get_zentar_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``zentar update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Zentar.app/Contents/MacOS/Zentar"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Zentar.exe",
            release_dir / "win-ia32-unpacked" / "Zentar.exe",
            release_dir / "win-arm64-unpacked" / "Zentar.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "zentar",
            release_dir / "linux-unpacked" / "Zentar",
            release_dir / "linux-arm64-unpacked" / "zentar",
            release_dir / "linux-arm64-unpacked" / "Zentar",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Zentar'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_zentar_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Zentar.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Zentar.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Zentar.exe: Access is
    ...
    an unrelated "Zentar" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Zentar is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Zentar.app/Contents/MacOS/Zentar  ->  app bundle = .../Zentar.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``zentar desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Zentar Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Zentar Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ZENTAR_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from zentar_logging import setup_logging as _setup_logging_gui
    ...
    # with_zentar_node_path() copies os.environ when called with no arg.
    env = with_zentar_node_path()
    if getattr(args, "fake_boot", False):
        env["ZENTAR_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ZENTAR_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "zentar_root", None):
        env["ZENTAR_DESKTOP_ZENTAR_ROOT"] = str(Path(args.zentar_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ZENTAR_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ZENTAR_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ZENTAR_DESKTOP_DISABLE_GPU=... zentar desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ZENTAR_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ZENTAR_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  zentar gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Zentar.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Zentar.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Zentar.exe, close any")
                    print("  running Zentar desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> zentar desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Zentar is
    ...
    if source_mode:
        print("→ Launching Zentar Desktop from source build...")
    ...
    print(f"→ Launching packaged Zentar Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``zentar dashboard`` processes other than ourselves.

    ``zentar dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``zentar update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Zentar Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``zentar serve`` as
    ...
    environment variable ``ZENTAR_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "zentar dashboard",
        "zentar_cli.main dashboard",
        "zentar_cli/main.py dashboard",
        # The headless backend (`zentar serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "zentar serve",
        "zentar_cli.main serve",
        "zentar_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from zentar_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "zentar.*dashboard"`) keeps us consistent
            # with `zentar_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `zentar update`.
    ...
    print("  Preview now:  zentar curator run --dry-run")
    print("  Pause it:     zentar curator pause")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``zentar update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``zentar update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: zentar curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``zentar dashboard`` processes.

    Called at the end of ``zentar update`` (default ``reason``) and also
    from ``zentar dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Zentar Desktop Electron app spawns this dashboard as a
    # backend child, it sets ZENTAR_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ZENTAR_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    zentar dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.zentar-update-staging"
    backup = f"{dst}.zentar-update-old"
...
def _update_via_zip(args):
    """Update ZENTAR DIGITAL AI AGENT by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `zentar update "
            f"--branch {branch}`, or update against main with `zentar update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/hermes-digital-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="zentar-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"zentar-digital-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for zentar-digital-agent itself
        ...
        # GitHub ZIPs extract to zentar-digital-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"zentar-digital-agent-{branch}")
    ...
    from zentar_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: zentar skills list-modified  "
    ...
    try:
        from zentar_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "zentar-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Zentar behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes zentar completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Zentar couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Zentar couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Zentar behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Zentar couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Zentar couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `zentar update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/hermes-digital-agent.git",
    "git@github.com:NousResearch/zentar-digital-agent.git",
    "https://github.com/NousResearch/hermes-digital-agent",
    "git@github.com:NousResearch/zentar-digital-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/hermes-digital-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_zentar_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_zentar_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Zentar repository.")
        print("  This means you may miss updates from NousResearch/zentar-digital-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/hermes-digital-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/hermes-digital-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``zentar update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from zentar_constants import get_default_zentar_root

    default_home = get_default_zentar_root()
...
# Install-scoped breadcrumb dropped right before ``zentar update`` mutates the
...
# survives and the next ``zentar`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ZENTAR_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``zentar update`` left half-done.
    ...
    protocol stream (``zentar acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if zentar.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _zentar_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Zentar is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Zentar from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `zentar update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from zentar_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``zentar update`` is still progressing even if pip/uv itself is silent.
    """
...
def _zentar_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_zentar_instances(
    ...
    The Zentar Desktop Electron app spawns ``zentar.EXE`` as a backend child,
    so during ``zentar update`` the user-invoked process and the desktop's
    ...
    shims (``zentar.exe`` / ``zentar-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``zentar update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``zentar.exe``) is a separate process from the Python
    ...
    for shim in _zentar_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated zentar.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``zentar update``
    ...
    #      zentar.exe sitting *under* a non-Zentar parent (e.g. a Zentar
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "zentar.exe"
    lines = ["✗ Another zentar.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Zentar Desktop, exit any open `zentar` REPLs, and")
    lines.append("  stop the gateway (`zentar gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `zentar update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_zentar_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``zentar.exe``.
    ...
    when ``zentar update`` runs, ``zentar.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``zentar.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next zentar invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Zentar Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Zentar Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _zentar_exe_shims(scripts_dir):
        ...
        print(
            "    Close Zentar Desktop, exit other `zentar` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `zentar update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_zentar_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``zentar.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``zentar.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``zentar`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_zentar_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``zentar.exe.old.*`` left by prior updates.

    Called early on every zentar invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``zentar update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `zentar update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``zentar.exe`` / ``zentar-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_zentar_exe`` for the rationale.
    """
    ...
    # hours later inside ``zentar update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``zentar.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``zentar.exe`` shim is locked during ``zentar update``, or when uv/distlib
    skips a launcher write. The symptom is ``zentar-digital-agent.exe`` and
    ``zentar-acp.exe`` present but ``zentar.exe`` missing, so ``zentar`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `zentar update --force` after "
            "closing other zentar processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m zentar_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``zentar update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``zentar.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``zentar`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `zentar update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `zentar update --force` after closing other zentar processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from zentar_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ZENTAR_HOME/bin/uv``, but on Termux the official
    ...
    """
    from zentar_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `zentar update`, so we install root-only first
    ...
    nixos_env = with_zentar_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `zentar update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``zentar update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.zentar/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``zentar update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``zentar update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.zentar/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``zentar update --gateway``) the update is already
    ...
    try:
        ...
        # zentar_cli.config.get_zentar_home to simulate setup failure.
        from zentar_cli.config import get_zentar_home as _get_zentar_home

        logs_dir = _get_zentar_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.zentar/logs/update.log`` only, never the terminal.

    During ``zentar update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``zentar update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``zentar update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from zentar_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from zentar_cli.config import recommended_update_command
        from zentar_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``zentar update`` without requiring a reinstall.  The
    ...
    ``zentar`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/zentar.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v zentar'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/zentar, code at /usr/local/lib/zentar-digital-agent).
    fhs_link = Path("/usr/local/bin/zentar")
    ...
    path_comment = (
        "# ZENTAR DIGITAL AI AGENT — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ZENTAR_HOME before running the update.
    ...
    ZENTAR_HOME directories.  The ``--backup`` flag on ``zentar update``
    ...
    # ``zentar_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # ZENTAR_HOME can add minutes to every update. Users who want the
    ...
    try:
        from zentar_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_zentar_home so the user sees ~/.zentar/...
    try:
        from zentar_constants import get_zentar_home, display_zentar_home

        home = get_zentar_home()
        try:
            display_path = f"{display_zentar_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  zentar import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``zentar update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``zentar update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run zentar from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.zentar-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".zentar-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The zentar.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m zentar_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``zentar
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m zentar_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "zentar_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Zentar processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Zentar Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Zentar desktop app / other Zentar terminals, then re-run:"
    )
    lines.append("    zentar update")
    lines.append("  (or use `zentar update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    zentar.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from zentar_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from zentar_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Zentar...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m zentar_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: zentar gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``zentar gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from zentar_cli import gateway_windows
        from zentar_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``zentar update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update ZENTAR DIGITAL AI AGENT to the latest version.
    ...
    if is_managed():
        managed_error("update ZENTAR DIGITAL AI AGENT")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `zentar update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Zentar via pip (for PyPI installs)."""
    from zentar_cli import __version__
    from zentar_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "zentar-digital-agent"]
        ...
        cmd = [pipx, "upgrade", "zentar-digital-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "zentar-digital-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "zentar-digital-agent"]
    ...
    print("✓ Update complete! Restart zentar to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating ZENTAR DIGITAL AI AGENT...")
    print()

    # On Windows, abort early if another zentar.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_zentar_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `zentar serve` backend) will keep .pyd
    ...
    # the zentar.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Zentar windows/gateways and re-run: zentar update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $ZENTAR_HOME, this is cheap
        ...
        try:
            from zentar_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in zentar_cli/config.py bricked
        # every user who ran ``zentar update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``zentar
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``zentar update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_zentar_home added to zentar_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``zentar`` launch finishes the
        ...
        # build.  ``zentar desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``zentar desktop`` shouldn't be forced into a full
        # Electron build by ``zentar update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from zentar_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "zentar_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `zentar desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from zentar_constants import display_zentar_home as _dhh
        ...
        # current — so copy it straight over ~/.zentar/cache/model_catalog.json
        ...
        # modules in this process.  Reload zentar_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zentar_home() added since the last release.
        try:
            import importlib
            import zentar_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ZENTAR_HOME so
        # it is not affected by sync_skills()'s module-level ZENTAR_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ZENTAR_HOME env var points at the default or a named profile.
        try:
            from zentar_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from zentar_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'zentar config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: zentar config migrate")
            else:
                print()
                print("Skipped. Run 'zentar config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from zentar_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `zentar curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``zentar update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from zentar_cli.tools_config import install_cua_driver
        ...
        # When running as ``zentar update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_zentar_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... zentar-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... zentar-gateway*)
            ...
            # Discover all zentar-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. zentar-gateway.service or zentar-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `zentar update` restart it automatically, allow\n"
                            ...
                            # path in `zentar gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: zentar gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: zentar gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: zentar -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Zentar gateway unit files are still installed.
        # When both zentar.service (from a pre-rename install) and the
        # current zentar-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `zentar update` surfaces the issue until the user migrates.
        try:
            from zentar_cli.gateway import (
                has_legacy_zentar_units,
                _find_legacy_zentar_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_zentar_units():
                print()
                print("⚠ Legacy Zentar gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_zentar_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (zentar.service) fight the current")
                print("  zentar-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    zentar gateway migrate-legacy")
                print()
        ...
        print("  zentar model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zentar -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from zentar_constants import display_zentar_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `zentar profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zentar_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → zentar -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.zentar)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  zentar profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     zentar -p {name} chat")
                else:
        ...
        from zentar_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from zentar_constants import get_zentar_home as _hh
        ...
        from zentar_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `zentar profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → zentar -p {name}  ({wrapper})")
        print()
        ...
        from zentar_cli.profiles import profile_exists, validate_alias_name
        ...
        from zentar_cli.profiles import rename_profile
        ...
        from zentar_cli.profiles import export_profile
        ...
        from zentar_cli.profiles import import_profile
        ...
        import tempfile
        from zentar_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="zentar_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  zentar -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      zentar -p {plan.manifest.name} chat")
        ...
        from zentar_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `zentar profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"zentar -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from zentar_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("zentar_requires"):
            print(f"Requires:     Zentar {data['zentar_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from zentar_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zentar_requires:
        print(f"  Requires: Zentar {mf.zentar_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``zentar dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``zentar dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No zentar dashboard processes running.")
        return 0

    print(f"{len(pids)} zentar dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``zentar dashboard register`` for OAuth.
    ...
    try:
        from zentar_cli.web_server import should_require_auth
    ...
    try:
        from zentar_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `zentar dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    zentar dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "ZENTAR_DASHBOARD_OAUTH_CLIENT_ID into ~/.zentar/.env for you.\n"
    ...
    try:
        from zentar_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No zentar dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `zentar update`.
    ...
    # launches the dashboard (`worker dashboard` → ZENTAR_HOME points into
    ...
    try:
        from zentar_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ZENTAR_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "zentar_cli.main",
            "-p", "default",
        ...
        # ZENTAR_HOME.  We must resolve the root explicitly instead of just
        # dropping ZENTAR_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ZENTAR_HOME=/opt/data`), so an unset
        # ZENTAR_HOME falls back to $HOME/.zentar = /opt/data/.zentar — an
        ...
        # update-button guard also misfires).  get_default_zentar_root()
        # returns the root for both layouts: ~/.zentar for a standard install
        ...
        try:
            from zentar_constants import get_default_zentar_root
            env["ZENTAR_HOME"] = str(get_default_zentar_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ZENTAR_HOME) rather than block the reroute.
            env.pop("ZENTAR_HOME", None)
    ...
    # the same logs directory as every other Zentar surface.
    try:
        from zentar_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["ZENTAR_SERVE_HEADLESS"] = "1"
    elif "ZENTAR_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ZENTAR_WEB_DIST"])
            if "ZENTAR_WEB_DIST" in os.environ
            else PROJECT_ROOT / "zentar_cli" / "web_dist"
        )
        ...
        # ZENTAR_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ZENTAR_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ZENTAR_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ZENTAR_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ZENTAR_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ZENTAR_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ZENTAR_WEB_DIST: {_dist_root}")
    ...
    try:
        from zentar_cli.mcp_startup import start_background_mcp_discovery
    ...
    from zentar_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from zentar_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from zentar_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from zentar_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from zentar_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Zentar log files."""
    from zentar_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Zentar command console."""
    from zentar_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from zentar_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``zentar -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``zentar_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``zentar -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``zentar`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ZENTAR_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from zentar_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from zentar_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ZENTAR_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ZENTAR_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ZENTAR_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `zentar --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_zentar_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_zentar_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_zentar_home()}/memories/\n")
    else:
        from zentar_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch ZENTAR DIGITAL AI AGENT as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from zentar_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from zentar_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from zentar_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from zentar_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from zentar_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from zentar_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from zentar_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from zentar_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from zentar_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for zentar CLI."""
    # Cosmetic: make the process show up as 'zentar' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from zentar_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``zentar.exe.old.*`` quarantine files left by previous
    # ``zentar update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_zentar_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``zentar update``
    ...
    # ``zentar skills install update``) merely defers recovery one launch;
    # under-matching (missing ``zentar -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in zentar_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from zentar_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from zentar_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `zentar model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.zentar/.env.  Supports Bitwarden "
    ...
    from zentar_cli import secrets_cli as _secrets_cli
    from zentar_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from zentar_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in zentar_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in zentar_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in zentar_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in zentar_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `zentar whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in zentar_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from zentar_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in zentar_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in zentar_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in zentar_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in zentar_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in zentar_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in zentar_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from zentar_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from zentar_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from zentar_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in zentar_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in zentar_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in zentar_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in zentar_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in zentar_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in zentar_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.zentar/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo zentar uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from zentar_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in zentar_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in zentar_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in zentar_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in zentar_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in zentar_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from zentar_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in zentar_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``zentar --help``, ``zentar version``, ``zentar logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from zentar_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zentar_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Zentar shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from zentar_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from zentar_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in zentar_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in zentar_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `zentar computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `zentar tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `zentar computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Zentar app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from zentar_cli.tools_config import _cua_driver_cmd
            # Honor ZENTAR_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from zentar_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: zentar computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: zentar computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: zentar computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: zentar computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: zentar computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: zentar computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in zentar_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <zentar home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from zentar_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from zentar_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from zentar_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_zentar_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from zentar_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_zentar_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_zentar_home() / "sessions"
            ...
            # `zentar sessions prune`: no time window and no filters at all
            ...
            # Launch zentar --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in zentar_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in zentar_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in zentar_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in zentar_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in zentar_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in zentar_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in zentar_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in zentar_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Zentar-Setup.exe success screen tells users to
    # run `zentar desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in zentar_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in zentar_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in zentar_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``zentar -c Pokemon Agent Dev`` → ``zentar -c 'Pokemon Agent Dev'``
    ...
    from zentar_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'zentar -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (zentar hooks list, cron
