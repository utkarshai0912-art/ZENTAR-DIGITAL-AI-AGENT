"""
Zentar Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with zentar-digital-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.zentar/plugins/<name>/``
3. **Project plugins** – ``./.zentar/plugins/<name>/`` (opt-in via
   ``ZENTAR_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``zentar_digital_agent.plugins``
   entry-point group.
...
from zentar_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``ZENTAR_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ZENTAR_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ZENTAR_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.zentar/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ZENTAR_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ZENTAR_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Zentar loggers.
    """
    ...
    logger.debug(
        "ZENTAR_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Zentar' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by zentar_cli.kanban_db when a task
    ...
    # separate `zentar -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `zentar kanban dispatch`),
...
ENTRY_POINTS_GROUP = "zentar_digital_agent.plugins"

_NS_PARENT = "zentar_plugins"
...
class PluginManifest:
    ...
    #              in ~/.zentar/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``zentar plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Zentar profile name (e.g. ``"default"``).

        Derived from ``ZENTAR_HOME`` via
        :func:`zentar_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.zentar/profiles/<name>``, or ``"custom"`` when
        ``ZENTAR_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Zentar core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``zentar honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``zentar <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`zentar_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from zentar_cli.dashboard_auth import (
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_zentar_dotenv()`` startup — after
        ``~/.zentar/.env`` loads, before Zentar reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Zentar' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``zentar model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: zentar_cli.main imports plugins indirectly
        from zentar_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.zentar/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("ZENTAR_SAFE_MODE"):
            logger.info("ZENTAR_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to zentar_cli/. Two layouts are
        ...
        # 2. User plugins (~/.zentar/plugins/)
        user_dir = get_zentar_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.zentar/plugins/)
        if _env_enabled("ZENTAR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".zentar" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ZENTAR_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with zentar and must
            ...
            # all ~20 of them added several seconds to every `zentar`
            # invocation — including plain `zentar chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Zentar ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `zentar plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``zentar plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`zentar plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `zentar plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``zentar_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``zentar_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="zentar-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``zentar tools`` TUI so plugin-provided toolsets appear
