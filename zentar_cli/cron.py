"""
Cron subcommand for zentar CLI.
...
# commands at execution time when ``_ZENTAR_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: zentar gateway install", Colors.DIM))
    print(color("                    sudo zentar gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  zentar cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'zentar cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: zentar gateway restart")
        ...
        print("    zentar gateway install    # Install as a user service")
        print("    sudo zentar gateway install --system  # Linux servers: boot-time system service")
        print("    zentar gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: zentar cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
