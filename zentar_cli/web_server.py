"""
ZENTAR DIGITAL AI AGENT — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m zentar_cli.main web          # Start on http://127.0.0.1:9119
    python -m zentar_cli.main web --port 8080
"""
...
from zentar_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `zentar dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ZENTAR_WEB_DIST"]) if "ZENTAR_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``zentar gateway run`` — but the
    desktop app spawns a ``zentar dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same ZENTAR_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import zentar_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from zentar_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire zentar_cli.gateway import into a background thread so the event
    # loop is not blocked and ZENTAR_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (ZENTAR_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `zentar
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ZENTAR_DESKTOP") == "1":
...
app = FastAPI(title="ZENTAR DIGITAL AI AGENT", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from zentar_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ZENTAR_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ZENTAR_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Zentar-Session-Token"
...
#
# This list is defined in ``zentar_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from zentar_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Zentar-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``zentar-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from zentar_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from zentar_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from zentar_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Zentar (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during zentar update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``zentar model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Zentar provider slug — fine.
    ...
    1. Vendor-name → Zentar-provider mapping: when the provider string is
       not a known Zentar provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from zentar_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from zentar_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from zentar_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another Zentar process is writing to it.
    """
    from zentar_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "ZENTAR_DASHBOARD_FILES_ROOT"
...
# ZENTAR_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to ZENTAR_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Zentar credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no ZENTAR_HOME resolution, so it blocks these trees wherever
...
def _default_zentar_root_is_opt_data() -> bool:
    raw = os.environ.get("ZENTAR_HOME", "").strip()
    if not raw:
        return False
    try:
        from zentar_constants import get_default_zentar_root

        root = get_default_zentar_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``zentar update``.
    ...
    a container — e.g. the zentar-webui image sharing the Zentar source tree),
    the dashboard's ``zentar update`` button is the correct update path and
    ...
    """
    if _default_zentar_root_is_opt_data():
        return True
    try:
        from zentar_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Zentar root is actually /opt/data
    # (the container/hosted layout) or when ZENTAR_DASHBOARD_FILES_ROOT is set.
    if _default_zentar_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``zentar:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.zentar-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``zentar_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from zentar_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from zentar_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_zentar_home() at call time
    ...
    try:
        ...
        # Windows install the first import of zentar_cli.gateway blocks the
        ...
        # and which providers are registered so ``zentar status`` and the
        # SPA's StatusPage can show "OAuth gate ON via ZENTAR DIGITAL" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from zentar_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from zentar_cli.auth import get_nous_session_validity
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Zentar Cloud
        ...
        if not auth_required:
            status.update({
                "zentar_home": str(get_zentar_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the zentar home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_zentar_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_zentar_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from zentar_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from zentar_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_zentar_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_zentar_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_zentar_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from zentar_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.zentar/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_zentar_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "zentar-update": "zentar-update.log",
...
def _spawn_zentar_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``zentar <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``zentar_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "zentar_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ZENTAR_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["zentar", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``zentar gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``zentar gateway restart`` children race each other on the
    ...
    return _spawn_zentar_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``zentar gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ZENTAR_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/zentar/update")
async def update_hermes():
    """Kick off ``zentar update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Zentar updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("zentar-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "zentar-update",
    ...
    try:
        proc = _spawn_zentar_action(["update"], "zentar-update")
    except Exception as exc:
        _log.exception("Failed to spawn zentar update")
...
@app.get("/api/zentar/update/check")
async def check_zentar_update(force: bool = False):
    """Report whether a Zentar update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/zentar/update`` actually runs ``zentar update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Zentar version string
    ...
    try:
        ...
        if force:
            try:
                (get_zentar_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.zentar/config.yaml`` under ``tts.``.
    """
...
def get_profiles_sessions(
    ...
    from zentar_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Zentar surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Zentar supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    zentar_home = get_zentar_home()
    values: Dict[str, Any] = {}

    # Common native provider stores.
    for path in (
        zentar_home / f"{name}.json",
        zentar_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_zentar_home()))
            return
...
# in zentar_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from zentar_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `zentar model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from zentar_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from zentar_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.zentar/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from zentar_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (zentar_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from zentar_cli.nous_subscription import apply_nous_managed_defaults
                from zentar_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``zentar model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from zentar_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``zentar model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from zentar_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from zentar_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `zentar model` provider — would be
        ...
        # Vertex — a `zentar model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `zentar model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Zentar from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Zentar to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Zentar from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Zentar to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Zentar in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Zentar through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Zentar via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Zentar through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Zentar to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Zentar inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Zentar to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Zentar through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Zentar to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Zentar to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Zentar as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in zentar gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in zentar gateway setup",
...
def _whatsapp_session_path() -> Path:
    from zentar_constants import get_zentar_dir

    return get_zentar_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"ZentarDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "ZENTAR DIGITAL AI AGENT").strip() or "ZENTAR DIGITAL AI AGENT"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Zentar appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ZENTAR_HOME.
...
# the canonical ``zentar auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.zentar/.anthropic_oauth.json`` — Zentar-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_zentar_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_zentar_oauth_credentials,
            _get_zentar_oauth_file,
        )
    except ImportError:
        read_zentar_oauth_credentials = None  # type: ignore
        _get_zentar_oauth_file = None  # type: ignore

    zentar_creds = None
    if read_zentar_oauth_credentials:
        try:
            zentar_creds = read_zentar_oauth_credentials()
        except Exception:
            zentar_creds = None
    if zentar_creds and zentar_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "zentar_pkce",
            "source_label": f"Zentar PKCE ({_get_zentar_oauth_file() if _get_zentar_oauth_file else None})",
            "token_preview": _truncate_token(zentar_creds.get("accessToken")),
            "expires_at": zentar_creds.get("expiresAt"),
            "has_refresh_token": bool(zentar_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from zentar_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Zentar even
    when they also have a separate Zentar-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Zentar never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "zentar auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "zentar auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "zentar auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "zentar auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "zentar auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "zentar auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from zentar_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Zentar, so the disconnect
    ...
    user sees exactly what executes, and Zentar then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Zentar — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``zentar model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `zentar model` universe (zero-edit for new plugins).
    try:
        from zentar_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"zentar auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("zentar_pkce", "claude_code", ...)
    ...
    sync with the `zentar model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Zentar-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_zentar_oauth_file
                oauth_file = _get_zentar_oauth_file()
            ...
            try:
                from zentar_cli.auth import clear_provider_auth
        ...
        try:
            from zentar_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.zentar/.anthropic_oauth.json AND credential pool
...
# Guarded so zentar web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Zentar file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``zentar auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_zentar_oauth_file
    oauth_file = _get_zentar_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "zentar-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ZENTAR_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from zentar_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from zentar_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``zentar auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``zentar model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Zentar "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from zentar_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `zentar sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `zentar sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_zentar_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from zentar_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_zentar_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from zentar_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ZENTAR_HOME)."""
    from zentar_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["zentar_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ZENTAR_HOME at import time. The dashboard is a single
    ...
    with _CRON_PROFILE_LOCK:
        ...
        from zentar_constants import (
            reset_zentar_home_override,
            set_zentar_home_override,
        )

        old_cron_dir = cron_jobs.CRON_DIR
        old_jobs_file = cron_jobs.JOBS_FILE
        old_output_dir = cron_jobs.OUTPUT_DIR
        token = set_zentar_home_override(str(home))
        ...
        try:
            ...
            reset_zentar_home_override(token)
...
#
# Wraps the same config data layer the CLI uses (zentar_cli.mcp_config), so
# servers managed here show up under `zentar mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from zentar_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from zentar_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from zentar_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from zentar_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from zentar_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the ZENTAR_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``zentar mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import ZentarTokenStorage, force_interactive_oauth
        ...
        # resolve ZENTAR_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = ZentarTokenStorage(name)
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds ZENTAR_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (zentar_cli.webhook); the webhook
...
async def list_webhooks():
    import zentar_cli.webhook as wh
...
# spawn the real `zentar gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_zentar_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_zentar_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from zentar_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_zentar_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_zentar_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_zentar_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"zentar-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_zentar_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `zentar import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_zentar_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from zentar_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from zentar_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <zentar_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_zentar_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_zentar_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``zentar`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_zentar_action`` tracks one process/log per name, so a shared
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from zentar_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`zentar -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ZENTAR_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_zentar_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "zentar setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ZENTAR_HOME override so the write lands in the target
    ...
    """
    from zentar_constants import set_zentar_home_override, reset_zentar_home_override

    token = set_zentar_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ZENTAR_HOME override (same mechanism as
    ...
    """
    from zentar_constants import set_zentar_home_override, reset_zentar_home_override
    from zentar_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ZENTAR_HOME override. Returns the
    number of skills newly disabled.
    """
    from zentar_constants import set_zentar_home_override, reset_zentar_home_override
    from zentar_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ZENTAR_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``zentar profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ZENTAR_HOME).
    """
    from zentar_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``zentar profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ZENTAR_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``zentar profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from zentar_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_zentar_home()`` at call
       time — the context-local override from ``set_zentar_home_override``
    ...
    retargeted to the *current* ``get_zentar_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ZENTAR_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_zentar_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_zentar_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from zentar_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    from zentar_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``zentar tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``zentar tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `zentar tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``zentar tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.zentar/.env`` via ``save_env_value`` —
    the same store ``zentar tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``zentar tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``zentar -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ZENTAR_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Zentar,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``zentar computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``zentar --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from zentar_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from zentar_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from zentar_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from zentar_cli.dashboard_auth.audit import AuditEvent, audit_log
        from zentar_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``zentar --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ZENTAR_TUI_RESUME`` env var —
    matching what ``zentar_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ZENTAR_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ZENTAR_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ZENTAR_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ZENTAR_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_zentar_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``zentar -p <name>`` performs. The in-process
    ``ZENTAR_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from zentar_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ZENTAR_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ZENTAR_TUI_INLINE", "1")
    ...
    env["ZENTAR_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ZENTAR_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["ZENTAR_TUI_RESUME"] = resume

    if sidecar_url:
        env["ZENTAR_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ZENTAR_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ZENTAR_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``ZENTAR_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("ZENTAR_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from zentar_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="zentar-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Zentar Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Zentar CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="zentar-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_zentar_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from zentar_cli.console_engine import ZentarConsoleEngine

        engine = ZentarConsoleEngine(
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Zentar Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Zentar inside WSL2 to use the dashboard's /chat "
...
# ZENTAR_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`zentar_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from zentar_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/zentar/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /zentar`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ZENTAR_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `zentar serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("ZENTAR_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (zentar serve): web UI disabled — use "
            "`zentar dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/zentar``)
        ...
        ``__ZENTAR_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ZENTAR_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ZENTAR_BASE_PATH__="{prefix}";'
                f"window.__ZENTAR_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ZENTAR_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ZENTAR_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ZENTAR_BASE_PATH__="{prefix}";'
                f"window.__ZENTAR_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/zentar`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Zentar backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Zentar Teal",         "description": "Classic dark teal — the canonical Zentar look"},
    {"name": "default-large", "label": "Zentar Teal (Large)", "description": "Zentar Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.zentar/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.zentar/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_zentar_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.zentar/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as zentar_cli.plugins):
    1. User plugins:    ~/.zentar/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.zentar/plugins/  (only if ZENTAR_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from zentar_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_zentar_home() / "plugins", "user"),
    ...
    # ``zentar_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ZENTAR_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".zentar" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from zentar_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_zentar_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"zentar auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from zentar_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from zentar_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zentar_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zentar_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.zentar/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.zentar/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"zentar_dashboard_plugin_{plugin['name']}"
...
from zentar_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ZENTAR_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ZENTAR_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``ZENTAR_SERVE_HEADLESS``), so
    the banner announces the bind rather than a browser URL.
    """
    import uvicorn

    try:
        from zentar_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the zentar-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ZENTAR_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with ZENTAR DIGITAL AI AGENT exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `zentar dashboard register` (Nous Portal) or "
    ...
    # ZENTAR_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "ZENTAR_BACKEND_READY" if headless else "ZENTAR_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Zentar backend listening on {host}:{actual_port}")
            else:
                print(f"  Zentar Web UI → http://{host}:{actual_port}")
