...
Used by ``zentar profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``zentar_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the ZENTAR DIGITAL AI AGENT kanban board.
...
Rules:
  ...
  - Never write "ZENTAR DIGITAL AI AGENT profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_zentar_home())
        else:
