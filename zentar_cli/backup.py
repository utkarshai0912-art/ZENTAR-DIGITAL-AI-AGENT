"""
Backup and import commands for zentar CLI.

`zentar backup` creates a zip archive of the entire ~/.zentar/ directory
(excluding the zentar-digital-agent repo and transient files).

`zentar import` restores from a backup zip, overlaying onto the current
ZENTAR_HOME root.
"""
...
from zentar_constants import get_default_zentar_root, get_zentar_home, display_zentar_home
...
# ``zentar-digital-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/zentar-digital-agent/``
...
# ZENTAR_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "zentar-digital-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ZENTAR_HOME) —
...
# File names that ``zentar import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ZENTAR_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ZENTAR_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to zentar root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``zentar-digital-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/zentar-digital-agent/`` — must be preserved.
        if part == "zentar-digital-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ZENTAR_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Zentar home directory."""
    zentar_root = get_default_zentar_root()

    if not zentar_root.is_dir():
        print(f"Error: Zentar home directory not found at {zentar_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"zentar-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"zentar-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_zentar_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(zentar_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(zentar_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``zentar-digital-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "zentar-digital-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(zentar_root)
    ...
    # outside ZENTAR_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_zentar_home()}."
        )
    ...
    print(f"\nRestore with: zentar import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Zentar backup.
    ...
    # Look for telltale files that a zentar home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Zentar backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zentar/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a zentar dir name
        if prefix in {".zentar", "zentar"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_zentar_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (zentar_root / "config.yaml").exists()
        has_env = (zentar_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Zentar configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ZENTAR_HOME.
            ...
            target = zentar_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(zentar_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_zentar_home()}."
            )
        ...
        profiles_dir = zentar_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # zentar_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: zentar profile list  (after installing zentar)")

        # Guidance
        print()
        if not (zentar_root / "zentar-digital-agent").is_dir():
            print("Note: The zentar-digital-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: zentar update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  zentar -p {pname} gateway install")

        print("Done. Your Zentar configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and zentar backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ZENTAR_HOME).
...
# — `zentar update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ZENTAR_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ZENTAR_HOME and the entry is silently
...
def _quick_snapshot_root(zentar_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(zentar_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ZENTAR_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``zentar update``.
    ...
    Args:
        ...
        zentar_home: Override for the Zentar home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(zentar_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for zentar backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_zentar_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, zentar_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``zentar_root`` to ``out_path``.
...
def _pre_update_backup_dir(zentar_home: Optional[Path] = None) -> Path:
    home = zentar_home or get_zentar_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZENTAR_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZENTAR_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``zentar update``) should continue even if the backup fails.
    """
    zentar_root = zentar_home or get_default_zentar_root()
    if not zentar_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(zentar_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, zentar_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `zentar claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZENTAR_HOME under ``backups/`` before a
    ``zentar claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``zentar import <archive>``.  Writes to
    ``<ZENTAR_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `zentar import` and the
