"""``zentar debug`` debug tools for ZENTAR DIGITAL AI AGENT.

Currently supports:
    zentar debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.zentar/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[zentar debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.zentar/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``zentar debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``zentar debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `zentar debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "zentar-digital-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``zentar debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.zentar/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``zentar debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zentar-digital-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ZentarDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from zentar_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_zentar_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``zentar dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``zentar dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "zentar-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``zentar debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "zentar debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `zentar debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  zentar debug delete <url>")

    print("\nShare these links with the Zentar team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Zentar version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``zentar debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from zentar_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "zentar debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `zentar debug share --local` to print the report instead, "
            "or `zentar debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: zentar debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'zentar debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``zentar debug`` call.
    ...
    # best-effort — any failure is swallowed so ``zentar debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: zentar debug <command>")
        print()
        print("Commands:")
