...
the tool registry or launching Zentar (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``zentar-cli`` to the non-existent ``zentar``. ``resolve_toolset('zentar')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``zentar-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``zentar`` instead of ``zentar-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `zentar tools` to reconfigure."
        )
    return warnings
