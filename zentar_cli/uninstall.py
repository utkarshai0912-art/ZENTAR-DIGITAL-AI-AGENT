"""
ZENTAR DIGITAL AI AGENT Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.zentar/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Zentar PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing zentar-digital-agent or zentar PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ZENTAR DIGITAL AI AGENT" comment and following line
                if '# ZENTAR DIGITAL AI AGENT' in line or '# zentar-digital-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('zentar' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing zentar
                if 'zentar' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the zentar wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "zentar",
        Path("/usr/local/bin/zentar"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains zentar_cli reference)
                content = wrapper.read_text()
                if 'zentar_cli' in content or 'zentar-digital-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(zentar_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``zentar`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Zentar home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (zentar_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``zentar gateway run`` processes
    ...
    try:
        from zentar_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zentar_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ZENTAR_HOME`` and ``ZENTAR_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\zentar\git\cmd``, ``%LOCALAPPDATA%\zentar\git\bin``,
#      ``%LOCALAPPDATA%\zentar\git\usr\bin``, ``%LOCALAPPDATA%\zentar\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\zentar\git\`` and Node to
#      ``%LOCALAPPDATA%\zentar\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``zentar dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\zentar\gateway-service\`` and sometimes
...
def _zentar_path_markers(zentar_home: Path) -> list[str]:
    """Path-entry substrings that identify Zentar-owned User-PATH entries."""
    root = str(zentar_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare zentar-digital-agent install dir.
    markers = [root + "\\zentar-digital-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ZENTAR_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "zentar".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-zentar as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(zentar_home: Path) -> list[str]:
    """Strip Zentar-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _zentar_path_markers(zentar_home)
            kept: list[str] = []
...
def remove_zentar_env_vars_windows() -> list[str]:
    """Delete ZENTAR_HOME and ZENTAR_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ZENTAR_HOME", "ZENTAR_GIT_BASH_PATH"):
                try:
...
def _is_default_zentar_home(zentar_home: Path) -> bool:
    """Return True when ``zentar_home`` points at the default (non-profile) root."""
    try:
        from zentar_constants import get_default_zentar_root
        return zentar_home.resolve() == get_default_zentar_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZENTAR_HOME directory.

    We shell out to ``zentar -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ZENTAR_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m zentar_cli.main` so we don't depend on a `zentar`
    #    wrapper that may be half-removed mid-uninstall.
    zentar_invocation = [_sys.executable, "-m", "zentar_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zentar_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ZENTAR_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``zentar uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ZENTAR_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from zentar_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(zentar_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Zentar Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Zentar Chat GUI installation was found.")
        print(f"  Checked: {zentar_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The ZENTAR DIGITAL AI AGENT stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(zentar_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The ZENTAR DIGITAL AI AGENT at {zentar_home / 'zentar-digital-agent'}")
        print(f"  • Your config, sessions, and secrets under {zentar_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The ZENTAR DIGITAL AI AGENT is still installed. Run 'zentar' to use the CLI,")
    print("or 'zentar uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.zentar/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.zentar/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ZENTAR_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_zentar_home(zentar_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.zentar data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ZENTAR DIGITAL AI AGENT Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {zentar_home / 'config.yaml'}")
    print(f"  Secrets: {zentar_home / '.env'}")
    print(f"  Data:    {zentar_home / 'cron/'}, {zentar_home / 'sessions/'}, {zentar_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ZENTAR_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Zentar data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Zentar code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, zentar_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Zentar PATH entries from shell configs / Windows User PATH")
    print("  • Zentar wrapper scripts and Zentar-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Zentar config/data: {zentar_home}")
        if _is_default_zentar_home(zentar_home):
        ...
        print(f"  • Keep Zentar config/data: {zentar_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``zentar`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ZENTAR_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in zentar_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\zentar\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(zentar_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Zentar-owned PATH entries in User environment")

        log_info("Removing ZENTAR_HOME / ZENTAR_GIT_BASH_PATH User env vars...")
        removed_env = remove_zentar_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Zentar-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing zentar command...")
    ...
    #     (only when they still point into this Zentar home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Zentar-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(zentar_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Zentar-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(zentar_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ZENTAR_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from zentar_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(zentar_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.zentar/, just remove the zentar-digital-agent subdir
            if zentar_home in project_root.parents or project_root.parent == zentar_home:
    ...
    #     under ZENTAR_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(zentar_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(zentar_home)
    ...
    # 5. Optionally remove ~/.zentar/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZENTAR_HOME dirs live under
        ...
        try:
            if zentar_home.exists():
                shutil.rmtree(zentar_home)
                log_success(f"Removed {zentar_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {zentar_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {zentar_home}")
    ...
    print()
    print("Thank you for using ZENTAR DIGITAL AI AGENT! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m zentar_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import zentar_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``zentar_constants`` + ``zentar_cli.colors``
    (and lazily ``zentar_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m zentar_cli.uninstall")
    parser.add_argument(
        "--mode",
