"""
Zentar Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``zentar desktop`` / ``zentar gui``)
     Built inside the agent checkout under ``$ZENTAR_HOME/zentar-digital-agent/``:
       ...
       - ``$ZENTAR_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Zentar.app`` or ``~/Applications/Zentar.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Zentar`` (NSIS per-user)
...
the app name ("Zentar"), separate from ``$ZENTAR_HOME``:
  - macOS:   ``~/Library/Application Support/Zentar``
  - Windows: ``%APPDATA%\\Zentar``
  - Linux:   ``$XDG_CONFIG_HOME/Zentar`` (default ``~/.config/Zentar``)
...
``zentar uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(zentar_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return zentar_home / "zentar-digital-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Zentar"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Zentar"
...
def source_built_gui_artifacts(zentar_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``zentar desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``zentar-digital-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(zentar_home)
    ...
    return [
        ...
        zentar_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Zentar" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Zentar.app"),
            home / "Applications" / "Zentar.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Zentar).
            local_base / "Programs" / "Zentar",
            # Older / alternate layout some builds used.
            local_base / "zentar-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Zentar")
    else:
        ...
        paths += [
            data_base / "applications" / "zentar.desktop",
            data_base / "applications" / "Zentar.desktop",
        ]
    return paths


def agent_is_installed(zentar_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ZENTAR_HOME.
    ...
    if (agent_root / "zentar_cli").is_dir():
        return True
...
def gui_is_installed(zentar_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(zentar_home):
        if p.exists():
            return True
...
def gui_install_summary(zentar_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = zentar_home if zentar_home is not None else get_zentar_home()
...
def uninstall_gui(zentar_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``zentar-digital-agent/zentar_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ZENTAR_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove zentar' or "
            "'sudo dnf remove zentar'). AppImage builds are a single file you "
