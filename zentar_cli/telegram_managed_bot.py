...
manual BotFather token copy-paste. Zentar talks only to the Nous onboarding
...
DEFAULT_MANAGER_BOT = "ZentarSetupBot"

DEFAULT_BOT_NAME = "ZENTAR DIGITAL AI AGENT"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``zentar_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``zentar_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"zentar_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Zentar Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Zentar Telegram onboarding service.")
