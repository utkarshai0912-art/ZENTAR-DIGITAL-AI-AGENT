...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or zentar -c.",
    ...
    "Ctrl+Z suspends Zentar to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zentar -c resumes your most recent CLI session. zentar -c \"project name\" resumes by title.",
    "zentar -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "zentar -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zentar chat -t web,terminal enables only specific toolsets for a focused session.",
    "zentar chat -s github-pr-workflow preloads a skill at launch.",
    "zentar chat -q \"query\" runs a single non-interactive query and exits.",
    "zentar chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zentar chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zentar --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zentar chat --source telegram tags the session for filtering in zentar sessions list.",
    "zentar -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zentar doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zentar dump outputs a compact setup summary — great for bug reports.",
    "zentar config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zentar config edit opens config.yaml in your default editor.",
    "zentar config check scans for missing or stale configuration options.",
    "zentar sessions browse opens an interactive session picker with search.",
    "zentar sessions stats shows session counts by platform and database size.",
    "zentar sessions prune --older-than 30 cleans up old sessions.",
    "zentar skills search react --source skills-sh searches the skills.sh public directory.",
    "zentar skills check scans installed hub skills for upstream updates.",
    "zentar skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zentar skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zentar mcp add github --command npx adds MCP servers from the command line.",
    "zentar mcp serve runs Zentar itself as an MCP server for other agents.",
    "zentar auth add lets you add multiple API keys for credential pool rotation.",
    "zentar completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zentar logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zentar backup creates a zip backup of your entire Zentar home directory.",
    "zentar profile create coder creates an isolated profile that becomes its own command.",
    "zentar profile create work --clone copies your current config and keys to a new profile.",
    "zentar update syncs new bundled skills to ALL profiles automatically.",
    "zentar gateway install sets up Zentar as a system service (systemd/launchd).",
    "zentar memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zentar webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: zentar tools disables unused tools, zentar skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "zentar models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zentar -w).",
    ...
    "ZENTAR_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zentar tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'zentar profile create coder' creates the 'coder' command.",
    "zentar profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zentar prints a resume command with session ID and stats.",
    "zentar sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zentar -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zentar skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zentar/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zentar cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zentar runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "zentar gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zentar via a local macOS server.",
    ...
    "Context files (.zentar.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Zentar loads project context from .zentar.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "zentar mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "zentar mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "zentar mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — zentar tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.zentar/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zentar chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "zentar plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zentar acp runs Zentar as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZENTAR_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zentar auth add nous or zentar auth add openai-codex sets up OAuth-based providers.",
    ...
    "zentar status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.zentar/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Zentar your own.",
    ...
    "Before a gateway session reset, Zentar auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.zentar/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Zentar state via ZENTAR_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ZENTAR_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZENTAR_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.zentar/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'zentar profile create ls' would be rejected.",
    "zentar profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zentar claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZENTAR_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via zentar tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.zentar/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.zentar/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Zentar auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Zentar config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Zentar is working.',
    ...
    '/reload reloads ~/.zentar/.env into the running session — pick up new API keys without restarting.',
    ...
    "ZENTAR_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.zentar/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.zentar/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'zentar curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "zentar curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'zentar curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'zentar auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ZENTAR_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ZENTAR_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'zentar dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'zentar dashboard embeds the full Zentar TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.zentar/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.zentar/dashboard-plugins/ — no npm build required.',
    ...
    'ZENTAR_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ZENTAR_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ZENTAR_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ZENTAR_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.zentar/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.zentar/skills/ so drop-in skills appear without restarting the session.',
    ...
    'zentar -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'zentar chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'zentar chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'zentar chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "zentar chat --source tool tags programmatic chats so they don't clutter zentar sessions list.",
    'zentar dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'zentar sessions rename <ID> "new title" renames any past session; zentar sessions delete <ID> removes one.',
    'zentar import restores a session export or profile archive produced by sessions export or profile export.',
    'zentar fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'zentar pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'zentar setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'zentar status --deep runs the full health sweep across every component; plain zentar status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ZENTAR_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ZENTAR_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.zentar/plugins/ — trust-gated by design.',
    "ZENTAR_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ZENTAR_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ZENTAR_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ZENTAR_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ZENTAR_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ZENTAR_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ZENTAR_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ZENTAR_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ZENTAR_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ZENTAR_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ZENTAR_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Zentar block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from zentar sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.zentar/dashboard-plugins/.',
]
