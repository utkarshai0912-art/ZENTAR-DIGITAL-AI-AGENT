...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``zentar model`` CLI picker. Adds network calls
    ...
    try:
        from zentar_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from zentar_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Zentar.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `zentar model` to configure ({auth_type})"
        )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`zentar_cli.model_switch.list_picker_providers`) so the
