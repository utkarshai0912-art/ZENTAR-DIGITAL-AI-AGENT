"""Windows ConPTY bridge for the `zentar dashboard` chat tab.

Drop-in counterpart to ``zentar_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``zentar_cli.web_server`` consumes: ``spawn``, ``read``,
