...
Pure display functions with no ZentarCLI state dependency.
"""
import json
import logging
import os
...
from zentar_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built zentar — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/hermes-digital-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/zentar-digital-agent"
...
def _fetch_pypi_latest(package: str = "zentar-digital-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Zentar update is available.

    Two paths: if ``ZENTAR_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = zentar_home / ".update_check"
    embedded_rev = os.environ.get("ZENTAR_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ZENTAR_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `zentar update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/zentar/update/check`
    ...
    try:
        from zentar_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $ZENTAR_HOME/zentar-digital-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = zentar_home / "zentar-digital-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Zentar git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ZENTAR_HOME/zentar-digital-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``zentar_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from zentar_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/hermes-digital-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Zentar checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/zentar-digital-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"ZENTAR DIGITAL AI AGENT v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ZENTAR_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ZENTAR_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]ZENTAR DIGITAL[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]ZENTAR DIGITAL[/]")

    if os.getenv("ZENTAR_YOLO_MODE"):
    ...
    try:
        from zentar_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from zentar_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built zentar; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ZENTAR_AGENT_LOGO
        console.print(_logo)
        console.print()
