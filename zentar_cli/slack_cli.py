"""``zentar slack ...`` CLI subcommands.

Today only ``zentar slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ zentar slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Zentar. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Zentar deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from zentar_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Zentar in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your ZENTAR DIGITAL AI AGENT on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``zentar_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ZENTAR_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Zentar")
    ...
    """
    name = getattr(args, "name", None) or "Zentar"
    description = getattr(args, "description", None) or "Your ZENTAR DIGITAL AI AGENT on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ZENTAR_HOME") or str(Path.home() / ".zentar")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Zentar app\n"
