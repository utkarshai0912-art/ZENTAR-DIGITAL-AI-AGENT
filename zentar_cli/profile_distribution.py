"""Profile distributions — shareable, packaged Zentar profiles via git.

A distribution is a Zentar profile published as a git repository (or
...
* ``zentar profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``zentar skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``zentar profile``, not a parallel tree):

    zentar profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "zentar-digital-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            zentar_requires=str(data.get("zentar_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.zentar_requires:
            out["zentar_requires"] = self.zentar_requires
        if self.author:
...
def check_zentar_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Zentar {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``zentar profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Zentar profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Zentar distribution."
        )

    # Version check up-front so we fail fast
    check_zentar_requires(manifest.zentar_requires, zentar_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.zentar).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="zentar_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `zentar profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`zentar profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="zentar_dist_update_") as tmp:
        plan = plan_install(
