...
_TASK_NAME_DEFAULT = "Zentar_Gateway"
_TASK_DESCRIPTION = "ZENTAR DIGITAL AI AGENT Gateway - Messaging Platform Integration"
...
def _preserve_zentar_home_path(path: str | Path) -> str:
    """Render Zentar-owned paths under the configured ZENTAR_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\zentar`` as a symlink/junction to
    ...
    path lives under ZENTAR_HOME.
    """
    candidate = Path(path)
    try:
        from zentar_cli.config import get_zentar_home

        home = Path(get_zentar_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Zentar profile."""
    from zentar_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "zentar_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ZENTAR_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ZENTAR_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ZENTAR_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ZENTAR_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ZENTAR_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ZENTAR_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ZENTAR_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ZENTAR_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ZENTAR_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Zentar_Gateway``
    Named profile X: ``Zentar_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during zentar_cli boot.
    from zentar_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\zentar\\gateway-service\\<task_name>.cmd``
    (or ``<ZENTAR_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Zentar installs stay self-contained).
    """
    _assert_windows()
    from zentar_cli.config import get_zentar_home

    script_dir = Path(get_zentar_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ZENTAR_HOME`` whenever it
    ...
    source checkout only if ``ZENTAR_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ZENTAR_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m zentar_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "ZENTAR_HOME={zentar_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ZENTAR_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports zentar_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_zentar_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_zentar_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_zentar_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [pythonw_path, "-m", "zentar_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_zentar_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_zentar_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('ZENTAR_HOME')}) = {_quote_vbs_string(zentar_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('ZENTAR_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_zentar_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_zentar_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    zentar_home = str(Path(get_zentar_home()))
    profile_arg = _profile_arg(zentar_home)

    content = _build_gateway_cmd_script(python_path, working_dir, zentar_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, zentar_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Zentar builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_zentar_home_path(get_python_path())
    )
    project_root = _preserve_zentar_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    zentar_home = str(Path(get_zentar_home()))
    profile_arg = _profile_arg(zentar_home)

    argv = [python_exe, "-m", "zentar_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "ZENTAR_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_zentar_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_zentar_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    ``zentar_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m zentar_cli.main
    ...
    from zentar_cli.config import get_zentar_home
    from zentar_cli.gateway import PROJECT_ROOT
    ...
    try:
        zentar_home = str(Path(get_zentar_home()).resolve())
    except Exception:
        zentar_home = ""
    ...
    if zentar_home:
        env_overlay["ZENTAR_HOME"] = zentar_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m zentar_cli.main gateway run``
    ...
    log_dir = Path(get_zentar_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ZENTAR_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ZENTAR_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from zentar_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `zentar -p <profile> gateway install` must be safe.
    ...
    from zentar_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"zentar {profile_arg} gateway start" if profile_arg else "zentar gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: zentar gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Zentar gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: zentar gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: zentar gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from zentar_cli.config import get_zentar_home
        print(f"    type {Path(get_zentar_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_zentar_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from zentar_cli.config import get_zentar_home

    zentar_home = Path(get_zentar_home())
    print()
    print("Next steps:")
    print("  zentar gateway status                      # Check status")
    print(f"  type {zentar_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Zentar gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: zentar gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: zentar gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: zentar gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `zentar gateway status`."
        )
