...
on Japanese locales, etc.).  Zentar's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``zentar_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Zentar is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ZENTAR_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ZENTAR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Zentar-managed Git install directories so
    ...
    # already-running shells don't see the change — which means zentar
    ...
    # install without Zentar-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Zentar's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Zentar's
    ``zentar config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Zentar (User env var in Windows Settings, or
    export in a PowerShell profile) and Zentar picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Zentar-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\zentar\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``zentar`` from right after install) retain their old PATH.

    Any subprocess Zentar spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Zentar-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "zentar", "git", "cmd"),
        os.path.join(local_appdata, "zentar", "git", "bin"),
        os.path.join(local_appdata, "zentar", "git", "usr", "bin"),
        # Zentar venv Scripts directory — host of the zentar.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes zentar, but harmless to include.
        os.path.join(local_appdata, "zentar", "zentar-digital-agent", "venv", "Scripts"),
