"""zentar hooks — inspect and manage shell-script hooks.

Usage::

    zentar hooks list
    zentar hooks test <event> [--for-tool X] [--payload-file F]
    zentar hooks revoke <command>
    zentar hooks doctor

Consent records live under ``~/.zentar/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.zentar/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``zentar hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: zentar hooks {list|test|revoke|doctor}")
        print("Run 'zentar hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.zentar/config.yaml.")
        print("See `zentar hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `zentar hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `zentar hooks test` and `zentar hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Zentar wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `zentar hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `zentar hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `zentar hooks doctor`.")
