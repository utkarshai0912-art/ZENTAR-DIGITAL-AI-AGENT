"""On-demand supply-chain audit for ZENTAR DIGITAL AI AGENT installs.

Scans three surfaces a Zentar user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Zentar venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.zentar/plugins``
...
def _discover_plugins(zentar_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.zentar/plugins``.
...
def run_audit(
    *,
    ...
    home = zentar_home or Path(get_zentar_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, zentar_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(zentar_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `zentar security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, zentar_home=home
    )
    if total == 0:
