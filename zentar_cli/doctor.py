"""
Doctor command for zentar CLI.

Diagnoses issues with ZENTAR DIGITAL AI AGENT setup.
"""
...
from zentar_cli.config import get_project_root, get_zentar_home, get_env_path
from zentar_cli.env_loader import load_zentar_dotenv
from zentar_constants import display_zentar_home
from zentar_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
ZENTAR_HOME = get_zentar_home()
_DHH = display_zentar_home()  # user-facing display path (e.g. ~/.zentar or ~/.zentar/profiles/coder)

# Load environment variables from ~/.zentar/.env so API key checks work
_env_path = get_env_path()
load_zentar_dotenv(zentar_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from zentar_cli.colors import Colors, color
from zentar_cli.models import _ZENTAR_USER_AGENT
from zentar_constants import OPENROUTER_MODELS_URL
...
from zentar_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.zentar/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ZENTAR_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from zentar_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches zentar_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``zentar --version`` reporting a stale version while
    ...
    """
    try:
        from zentar_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != zentar_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'zentar update', or set "
            "zentar_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-zentar and dashboard static services are up
    ...
    for static in ("main-zentar", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `zentar profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from zentar_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ZENTAR_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ZENTAR_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ZENTAR_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `zentar`.
    os.environ.setdefault("ZENTAR_INTERACTIVE", "1")

    # Handle `zentar doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.zentar/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Zentar Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`zentar doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and zentar_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.zentar/.env (primary location for user config)
    env_path = ZENTAR_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'zentar setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'zentar setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'zentar setup' to create one")
                issues.append("Run 'zentar setup' to create .env")
    
    # Check ~/.zentar/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ZENTAR_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from zentar_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from zentar_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'zentar config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from zentar_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.zentar/.env or run 'zentar setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'zentar setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'zentar config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from zentar_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'zentar setup' to migrate config")
                else:
                    issues.append("Run 'zentar doctor --fix' or 'zentar setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'zentar doctor --fix'")
        except Exception:
            pass

        # Detect stale ZENTAR_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ZENTAR_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from zentar_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("ZENTAR_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ZENTAR_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `zentar setup` run)",
                )
                if should_fix:
                    if remove_env_value("ZENTAR_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ZENTAR_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ZENTAR_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ZENTAR_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ZENTAR_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'zentar doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Zentar' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = zentar_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Zentar a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# ZENTAR DIGITAL AI AGENT Persona\n\n"
                "<!-- Edit this file to customize how Zentar communicates. -->\n\n"
                "You are Zentar, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = zentar_home / "memories"
    ...
    state_db_path = zentar_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from zentar_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'zentar doctor --fix' "
                        "(or 'zentar sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from zentar_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'zentar doctor --fix' "
                        "(or 'zentar sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = zentar_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'zentar doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "zentar"
        ...
        _cmd_link = _cmd_link_dir / "zentar"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(zentar not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/zentar → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/zentar points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/zentar → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/zentar — run 'zentar doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/zentar exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/zentar not found",
                    "(zentar command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/zentar → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/zentar symlink — run 'zentar doctor --fix'")
    ...
    try:
        from zentar_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `zentar update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `zentar doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ZENTAR_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Zentar one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'zentar config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``zentar doctor``.
    ...
    # never the legitimate source for `zentar doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'zentar setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ZENTAR_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: zentar skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ZENTAR_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: zentar memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: zentar memory setup",
                    "No Honcho API key — run 'zentar memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run zentar memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: zentar memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: zentar memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from zentar_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "zentar -p" in content:
                            _m = _re.search(r"zentar -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'zentar doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
