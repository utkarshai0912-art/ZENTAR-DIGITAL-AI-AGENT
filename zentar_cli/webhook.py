"""zentar webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.zentar/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _zentar_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: zentar gateway run
"""
...
def webhook_command(args):
    """Entry point for 'zentar webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: zentar webhook {subscribe|list|remove|test}")
        print("Run 'zentar webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (zentar gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: zentar webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from zentar webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (zentar gateway run)")
