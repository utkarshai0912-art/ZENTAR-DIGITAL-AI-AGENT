"""
Profile management for multiple isolated Zentar instances.

Each profile is a fully independent ZENTAR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zentar/profiles/<name>/`` by default.

The "default" profile is ``~/.zentar`` itself — backward compatible,
zero migration needed.

Usage::

    zentar profile create coder          # fresh profile + bundled skills
    zentar profile create coder --clone  # also copy config, .env, SOUL.md, skills
    zentar profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zentar -p coder chat                 # or via flag
    zentar profile use coder             # set as sticky default
    zentar profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zentar_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.zentar``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   zentar-digital-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `zentar backup` archives
...
# Marker file written by `zentar profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `zentar
...
# `zentar skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Zentar infrastructure directories that only the default profile
         (``~/.zentar``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_zentar_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.zentar) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zentar-digital-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ZENTAR_HOME equals the
...
# into the export. The set below identifies the *known Zentar profile
# artifacts* at the root of ZENTAR_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``zentar_constants``.
...
_RESERVED_NAMES = frozenset({
    "zentar", "default", "test", "tmp", "root", "sudo",
})

# Zentar subcommands that cannot be used as profile names/aliases
_ZENTAR_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zentar root, NOT to the current ZENTAR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZENTAR_HOME points outside
    ``~/.zentar``, profiles live under ``ZENTAR_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zentar_home() / "profiles"


def _get_default_zentar_home() -> Path:
    """Return the default (pre-profile) ZENTAR_HOME path.

    In standard deployments this is ``~/.zentar``.
    In Docker/custom deployments where ZENTAR_HOME is outside ``~/.zentar``
    (e.g. ``/opt/data``), returns ZENTAR_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_zentar_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Zentar installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZENTAR_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_zentar_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, zentar subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ZENTAR_SUBCOMMANDS:
        return f"'{canon}' conflicts with a zentar subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            zentar_exe = shutil.which("zentar") or "zentar"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(zentar_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Zentar. If we
    ...
    try:
        ...
        from zentar_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `zentar doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``zentar -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``zentar profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``zentar -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``zentar profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ZENTAR_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Zentar config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zentar_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, zentar_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ZENTAR_HOME.
    ...
    The returned ``zentar_home`` is the path to pass to
    ``set_zentar_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_zentar_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``zentar update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zentar)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.zentar/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `zentar -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Zentar profile.\n"
    ...
    if not soul_path.exists():
        try:
            from zentar_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `zentar update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`zentar profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Zentar (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `zentar profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `zentar -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZENTAR_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``zentar profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZENTAR_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_zentar_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Zentar *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Zentar backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ZENTAR_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `zentar -p <canon> profile
    ...
    zentar_markers = ("zentar_cli.main", "zentar-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Zentar process: either an entrypoint marker in argv, or
            # a resolved executable named `zentar`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_hermes = (
                any(marker in joined for marker in zentar_markers)
                or exe_name == "zentar"
                or exe_name.startswith("zentar")
            )
            if not is_hermes:
                continue
            ...
            # ...or by ZENTAR_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ZENTAR_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zentar).\n"
            "To remove everything, use: zentar uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_zentar_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ZENTAR_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from zentar_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZENTAR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZENTAR_HOME")
    try:
        os.environ["ZENTAR_HOME"] = str(profile_dir)
        from zentar_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZENTAR_HOME"] = old_home
        elif "ZENTAR_HOME" in os.environ:
            del os.environ["ZENTAR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zentar/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: zentar profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZENTAR_HOME.

    Returns ``"default"`` if ZENTAR_HOME is not set or points to ``~/.zentar``.
    Returns the profile name if ZENTAR_HOME points into ``~/.zentar/profiles/<name>``.
    Returns ``"custom"`` if ZENTAR_HOME is set to an unrecognized path.
    """
    from zentar_constants import get_zentar_home
    zentar_home = get_zentar_home()
    resolved = zentar_home.resolve()

    default_resolved = _get_default_zentar_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ZENTAR_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Zentar file the
      user may have lying alongside ZENTAR_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.zentar itself — its parent is ~/ and its
        # directory name is ".zentar", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zentar profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.zentar itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zentar). "
            "Specify a different name: zentar profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zentar_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"zentar_{old_name}"
    legacy_old_host = f"zentar.{old_name}"
    new_host = f"zentar_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_zentar_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("zentar_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZENTAR_HOME path string.

    Called early in the CLI entry point, before any zentar modules
    are imported, to set the ZENTAR_HOME environment variable.
    """
