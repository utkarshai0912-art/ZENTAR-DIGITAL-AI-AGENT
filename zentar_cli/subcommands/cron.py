"""``zentar cron`` subcommand parser.

Extracted verbatim from ``zentar_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.zentar/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.zentar/scripts/. Pass empty string to clear. "
