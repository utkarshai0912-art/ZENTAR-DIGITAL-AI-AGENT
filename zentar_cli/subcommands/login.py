"""``zentar login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``zentar login`` was removed in favor of ``zentar auth`` / ``zentar model``
    (the runtime handler in ``zentar_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``zentar login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``zentar --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``zentar login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``zentar model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `zentar auth` to manage credentials, "
            "`zentar model` to select a provider, or `zentar setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `zentar model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: zentar-cli)"
    )
