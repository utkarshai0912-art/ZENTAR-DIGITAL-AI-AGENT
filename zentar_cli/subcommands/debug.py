"""``zentar debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ZENTAR DIGITAL AI AGENT. Use 'zentar debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zentar debug share              Upload debug report (asks for confirmation)
    zentar debug share --yes        Skip confirmation (for scripts/CI)
    zentar debug share --lines 500  Include more log lines
    zentar debug share --expire 30  Keep paste for 30 days
    zentar debug share --local      Print report locally (no upload)
    zentar debug share --no-redact  Disable upload-time secret redaction
    zentar debug share --nous       Upload to Nous-internal storage (private)
    zentar debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'zentar debug share'",
    )
