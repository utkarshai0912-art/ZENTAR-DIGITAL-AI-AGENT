"""`zentar checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.zentar/checkpoints/``.  Actions:

    zentar checkpoints               # same as `status`
    zentar checkpoints status        # total size, project count, breakdown
    zentar checkpoints list          # per-project checkpoint counts + workdir
    zentar checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    zentar checkpoints clear [-f]    # nuke the entire base (asks first)
    zentar checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    zentar checkpoints
    zentar checkpoints prune --retention-days 3 --max-size-mb 200
    zentar checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: zentar checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``zentar checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `zentar checkpoints` → status
