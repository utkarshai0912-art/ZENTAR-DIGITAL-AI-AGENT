"""``zentar logs`` — view and filter Zentar log files.
...
Usage examples::
    ...
    zentar logs                    # last 50 lines of agent.log
    zentar logs -f                 # follow agent.log in real time
    zentar logs errors             # last 50 lines of errors.log
    zentar logs gateway -n 100    # last 100 lines of gateway.log
    zentar logs gui -f            # follow gui.log (dashboard/pty/ws)
    zentar logs desktop -f        # follow desktop.log (Electron app boot/backend)
    zentar logs --level WARNING    # only WARNING+ lines
    zentar logs --session abc123   # filter by session ID substring
    zentar logs --component tools  # only tool-related lines
    zentar logs --since 1h         # lines from the last hour
    zentar logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zentar_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Zentar runs — try 'zentar chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zentar_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zentar_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zentar_home()}/logs/")
        return

    print(f"Log files in {display_zentar_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zentar chat' to generate logs)")
