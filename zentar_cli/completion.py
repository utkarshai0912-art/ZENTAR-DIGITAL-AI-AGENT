"""Shell completion script generation for zentar CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zentar_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# ZENTAR DIGITAL AI AGENT bash completion
# Add to ~/.bashrc:
#   eval "$(zentar completion bash)"

_zentar_profiles() {{
    local profiles_dir="$HOME/.zentar/profiles"
...
_zentar_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zentar_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zentar_completion zentar
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef zentar
# ZENTAR DIGITAL AI AGENT zsh completion
# Add to ~/.zshrc:
...
_zentar_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zentar/profiles" ]]; then
        profiles+=($HOME/.zentar/profiles/*(N/:t))
    fi
...
_hermes() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zentar_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'zentar command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ZENTAR DIGITAL AI AGENT fish completion",
        "# Add to your config:",
        "#   zentar completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zentar_profiles",
        "    echo default",
        "    if test -d $HOME/.zentar/profiles",
        "        for d in $HOME/.zentar/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c zentar -f -s p -l profile"
        " -d 'Profile name' -xa '(__zentar_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__zentar_profiles)' -d 'Profile name'"
                )
