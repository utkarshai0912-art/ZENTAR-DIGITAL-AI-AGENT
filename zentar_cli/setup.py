"""
Interactive setup wizard for ZENTAR DIGITAL AI AGENT.
...
from zentar_constants import get_optional_skills_dir
...
# display_zentar_home imported lazily at call sites (stale-module safety during zentar update)
...
from zentar_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from zentar_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Zentar Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Zentar using environment variables or config commands:")
    print_info("  zentar config set model.provider custom")
    print_info("  zentar config set model.base_url http://localhost:8080/v1")
    print_info("  zentar config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'zentar setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``ZENTAR_NONINTERACTIVE=1`` (see ``zentar_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ZENTAR_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ZENTAR_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zentar_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'zentar setup')")


def _print_setup_summary(config: dict, zentar_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'zentar setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from zentar_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `zentar tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from zentar_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'zentar setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'zentar setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'zentar setup terminal'"))
    ...
    # Spotify (OAuth via zentar auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'zentar setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {zentar_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('zentar setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('zentar setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('zentar setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('zentar setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('zentar setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('zentar config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('zentar config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('zentar config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('zentar', Colors.GREEN)}              Start chatting")
    print(f"   {color('zentar gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('zentar doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `zentar tools` and `zentar setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``zentar model``)
    ...
    provider added to ``zentar model`` is automatically available here.
    ...
    # Delegate to the shared zentar model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from zentar_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: zentar model")
    ...
    # on demand via `zentar auth add`, `zentar setup` vision, and
    # `zentar setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from zentar_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``zentar model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from zentar_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.zentar/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Zentar auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via zentar setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'zentar setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Zentar runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `zentar setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `zentar setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ZENTAR_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ZENTAR_MAX_ITERATIONS")
    ...
    print_info("  Run `zentar setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ZENTAR_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.zentar/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from zentar_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_zentar_home(Path(get_zentar_home()))
    except Exception:
        pass
...
def _profile_name_from_zentar_home(zentar_home) -> str | None:
    """Return the active profile name when ZENTAR_HOME is a profile dir."""
    if zentar_home.parent.name == "profiles":
        return zentar_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Zentar delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Zentar to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from zentar_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  zentar config edit")
    print_info("   Open config in your editor:  zentar config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from zentar_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Zentar from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'zentar setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     zentar config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_zentar_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: zentar gateway install --system")
                print_info("  Or run in foreground:  zentar gateway")
        else:
            from zentar_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   zentar gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   zentar gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `zentar setup tools` and `zentar tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``zentar_cli.auth`` — lists every supported
    ...
    """
    try:
        from zentar_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zentar_cli.config
    ...
    if section_key == "model":
        ...
        from zentar_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Zentar.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Zentar to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Zentar at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Zentar at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Zentar at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Zentar at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Zentar equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Zentar config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Zentar.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Zentar's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(zentar_home: Path) -> bool:
    ...
    print_info("Zentar can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: zentar claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=zentar_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: zentar claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Zentar configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Zentar config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Zentar (use zentar claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``zentar setup --portal`` and ``zentar portal``. This is the
    ...
    Zentar session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``zentar setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``zentar model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``zentar portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Zentar Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `zentar portal` == quick setup's Nous step.
    try:
        from zentar_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `zentar portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `zentar portal`: %s", exc)
        print()
    ...
    print_info("  Run `zentar portal info` to inspect routing.")
    print_info("  Run `zentar` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      zentar setup           — full or quick (auto-detected)
      zentar setup model     — just model/provider
      zentar setup tts       — just text-to-speech
      zentar setup terminal  — just terminal backend
      zentar setup gateway   — just messaging platforms
      zentar setup tools     — just tool configuration
      zentar setup agent     — just agent settings
    """
    from zentar_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Zentar Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ ZENTAR DIGITAL AI AGENT Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your ZENTAR DIGITAL AI AGENT installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, zentar_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Zentar configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'zentar setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(zentar_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Zentar?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, zentar_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, zentar_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {zentar_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'zentar config edit'")
    ...
    # Tune later with `zentar setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, zentar_home)


def _run_first_time_quick_setup(config: dict, zentar_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``zentar setup <section>``
    or switch providers with ``zentar model``.
    """
    from zentar_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'zentar setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    zentar setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  zentar setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``zentar tools`` (which
    ...
    try:
        ...
        from zentar_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``zentar setup agent`` /
    ``zentar config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, zentar_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `zentar update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        zentar tools")
        print_info("  Seed skills:         zentar skills opt-in --sync")
        print_info("  Add MCP servers:     zentar mcp add")
        print_info("  Enable plugins:      zentar plugins")
        print_info("  Tune agent settings: zentar setup agent")
        print()
        _print_setup_summary(config, zentar_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, zentar_home)


def _blank_slate_walkthrough(config: dict, zentar_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`zentar update` runs from re-injecting them. Opt back in any")
            print_info("time with `zentar skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `zentar tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `zentar plugins list` / `zentar plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `zentar plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `zentar mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `zentar mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   zentar tools")
    print_info("  Seed skills:         zentar skills opt-in --sync")
    print_info("  Add MCP servers:     zentar mcp add")
    print_info("  Tune agent settings: zentar setup agent")
    print()
...
def _run_quick_setup(config: dict, zentar_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'zentar setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Zentar to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'zentar setup gateway'.")
