...
- ``zentar_cli.providers``        -- canonical provider identity + overlays
- ``zentar_cli.model_normalize``  -- per-provider name formatting
...
_ZENTAR_MODEL_WARNING = (
    "ZENTAR DIGITAL Zentar 3 & 4 models are NOT agentic and are not designed "
    "for use with ZENTAR DIGITAL AI AGENT. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real ZENTAR DIGITAL Zentar 3 / Zentar 4 chat families.
# The previous substring check (`"zentar" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``zentar-brain:qwen3-14b-ctx16k`` that just
# happen to carry "zentar" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Zentar-3-Llama-3.1-70B, zentar-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   zentar-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ZENTAR_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])zentar[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_zentar_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Zentar 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ZENTAR_NON_AGENTIC_RE.search(model_name))


def _check_zentar_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Zentar 3/4 chat model."""
    if is_nous_zentar_non_agentic(model_name):
        return _ZENTAR_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``zentar config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zentar_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'zentar model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'zentar doctor' — config issues detected:"
        ...
        from zentar_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from zentar_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from zentar_cli.config import load_config
            from zentar_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from zentar_cli.providers import resolve_user_provider as _ruser
    ...
    # zentar_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from zentar_cli.models import normalize_opencode_base_url
    ...
    zentar_warn = _check_zentar_model_warning(new_model)
    if zentar_warn:
        warnings.append(zentar_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from zentar_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from zentar_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from zentar_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zentar_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by zentar provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://hermes-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Zentar release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from zentar_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from zentar_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Zentar-mapped providers ---
    from zentar_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from zentar_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for zentar_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(zentar_id)
        if (
            _alias_target
            and _alias_target != zentar_id
        ...
        pconfig = PROVIDER_REGISTRY.get(zentar_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ZENTAR_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from zentar_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(zentar_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `zentar model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(zentar_id)
        if not model_ids:
            model_ids = curated.get(zentar_id, [])
            if zentar_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(zentar_id, model_ids)
        total = len(model_ids)
        if zentar_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Zentar-only providers (nous, openai-codex, copilot, opencode-go) ---
    from zentar_cli.providers import ZENTAR_OVERLAYS
    from zentar_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Zentar provider ID.
    # ZENTAR_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Zentar IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZENTAR_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Zentar slug — e.g. "github-copilot" → "copilot"
        zentar_slug = _mdev_to_hermes.get(pid, pid)
        if zentar_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(zentar_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, zentar_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or zentar_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(zentar_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", zentar_slug, exc)
        ...
        # configured.
        if not has_creds and zentar_slug == "anthropic":
            try:
                ...
                if (zentar_creds and zentar_creds.get("accessToken")) or \
        ...
        if zentar_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(zentar_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(zentar_slug)
                model_ids = _ids if _ids else (curated.get(zentar_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(zentar_slug, []) or curated.get(pid, [])
        elif zentar_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `zentar model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from zentar_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(zentar_slug, []) or curated.get(pid, [])
                if zentar_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(zentar_slug, model_ids)
        total = len(model_ids)
        if zentar_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": zentar_slug,
            "name": get_label(zentar_slug),
            "is_current": zentar_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "zentar",
        ...
        seen_slugs.add(zentar_slug.lower())
        _record_builtin_endpoint(zentar_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ZENTAR_OVERLAYS (keeps /model in sync
    # with `zentar model`).
    try:
        from zentar_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Zentar's canonical write key (matches
            ...
            # Zentar writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from zentar_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Zentar's own writer uses; a hyphen variant is accepted
            ...
            # active model. Zentar's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, zentar_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from zentar_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`zentar_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from zentar_cli.models import fetch_openrouter_models
