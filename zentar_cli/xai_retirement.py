...
Pure logic: walks a Zentar config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `zentar doctor` and a future `zentar migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Zentar config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Zentar config and return retirement issues.
...
def apply_migration(
    ...
    from zentar_cli.config import require_readable_config_before_write
