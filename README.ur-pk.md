...
# ہرمیس ایجنٹ ☤ (ZENTAR DIGITAL AI AGENT)
...
```bash
zentar              # انٹرایکٹو CLI — بات چیت شروع کریں
zentar model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
zentar tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
zentar config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
zentar gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
zentar setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
zentar claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
zentar update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
zentar doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
یہ آپ کو OAuth کے ذریعے لاگ ان کرواتا ہے، Nous کو آپ کا پرووائیڈر مقرر کرتا ہے، اور ٹول گیٹ وے کو آن کر دیتا ہے۔ `zentar portal info` کمانڈ استعمال کر کے آپ کسی بھی وقت چیک کر سکتے ہیں کہ کون کون سی سروسز منسلک ہیں۔ مکمل تفصیلات [Tool Gateway دستاویزات کے صفحے](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway) پر موجود ہیں۔
...
ہرمیس کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `zentar` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `zentar`                                      | `zentar gateway setup` اور `zentar gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`zentar setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
zentar claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
zentar claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
zentar claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
zentar claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.zentar/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `zentar claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔
...
معاونین (contributors) کے لیے فوری آغاز — کلون (clone) کریں اور `setup-zentar.sh` چلائیں:
...
./setup-zentar.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/zentar کا سیم لنک (symlink) بناتا ہے
./zentar              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/NousResearch/hermes-digital-agent/issues)
...
- 🔌 [ZentarClaw](https://github.com/AaronWong1999/hermesclaw) — کمیونٹی وی چیٹ (WeChat) برج: ہرمیس ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
...
[نوس ریسرچ (ZENTAR DIGITAL)](https://nousresearch.com) کی جانب سے تیار کردہ۔
