...
Usage (via cron with --no-agent):
    ...
    zentar cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ZENTAR_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Zentar-Watcher/1.0"})
