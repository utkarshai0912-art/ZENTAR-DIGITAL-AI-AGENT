...
Usage (via cron with --no-agent):
    ...
    zentar cron create zentar-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ZENTAR_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name zentar-issues --repo NousResearch/zentar-digital-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Zentar .env file
(``${ZENTAR_HOME:-~/.zentar}/.env``) to avoid the 60 req/hr
