#!/usr/bin/env python3
"""Canvas LMS API CLI for ZENTAR DIGITAL AI AGENT.
...
def _check_config():
    ...
    if missing:
        zentar_env = os.path.join(
            os.environ.get("ZENTAR_HOME", os.path.expanduser("~/.zentar")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {zentar_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for ZENTAR DIGITAL AI AGENT"
    )
