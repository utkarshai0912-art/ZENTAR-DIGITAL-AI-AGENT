...
the actual `setup.sh` that creates the project workspace, configures Zentar
...
2. **Create profiles** — `zentar profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.zentar/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `zentar kanban create` assigned to the director
...
```bash
zentar profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.zentar/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.zentar/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Zentar
...
Each profile gets a `SOUL.md` at `~/.zentar/profiles/<name>/SOUL.md` that
...
```bash
zentar kanban create "Direct production of <video title>" \
    --assignee director \
...
the Zentar `.env` (`${ZENTAR_HOME:-$HOME/.zentar}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _zentar_env="${ZENTAR_HOME:-$HOME/.zentar}/.env"
    if grep -q "^${var}=" "$_zentar_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_zentar_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_zentar_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY zentar ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY zentar OPENROUTER_API_KEY || exit 1
# ...
```
