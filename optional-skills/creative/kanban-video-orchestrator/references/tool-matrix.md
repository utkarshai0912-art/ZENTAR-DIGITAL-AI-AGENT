...
Maps each role archetype to the Zentar skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public zentar-digital-agent
...
## Zentar skills relevant to video production

### Visual / rendering skills (`zentar-digital-agent/skills/creative/`)
...
### Design / pre-production skills (`zentar-digital-agent/skills/creative/`)
...
### Audio / media skills (`zentar-digital-agent/skills/creative/` + `skills/media/`)
...
These are **not** Zentar skills but external CLIs / APIs that profiles invoke.
...
## Built-in Zentar tools for media review

These are native Zentar tools — not invoked via terminal but through their own
...
key is present in `${ZENTAR_HOME:-~/.zentar}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Zentar profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ZENTAR_HOME:-~/.zentar}/.env` → environment variable.
...
When a new Zentar-public video skill ships:
