...
zentar kanban watch --tenant <project-slug>

# Snapshot of the board
zentar kanban list --tenant <project-slug>
zentar kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
zentar kanban stats --tenant <project-slug>

# Visual dashboard (browser)
zentar dashboard

# Inspect a specific task (includes comments + events)
zentar kanban show <task-id>

# Follow a single task's event stream
zentar kanban tail <task-id>
```

Verify available subcommands with `zentar kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `zentar kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `zentar kanban block <id>` then `zentar kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `zentar kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `zentar kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `zentar profile list` (profile exists?), `zentar status` (gateway/dispatcher up?), and verify tenant. |
...
zentar kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
zentar kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(zentar kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
zentar kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
zentar kanban block <task-id>
zentar kanban archive <task-id>

# Diagnose what happened
zentar kanban show <task-id>      # task body, comments, recent events
zentar kanban tail <task-id>      # follow the live event stream
zentar kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `zentar kanban create` for the director
...
while true; do
    clear
    zentar kanban list --tenant <slug>
    echo "---"
    zentar kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `zentar kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `zentar profile list` and start
