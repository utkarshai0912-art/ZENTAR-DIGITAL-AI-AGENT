#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `zentar kanban list` and
...
def zentar_available() -> bool:
    return shutil.which("zentar") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed zentar CLI."""
    try:
        out = subprocess.run(
            ["zentar", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `zentar kanban list`
    out = subprocess.run(
        ["zentar", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["zentar", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not zentar_available():
        print("ERROR: 'zentar' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
