...
## 5. GODMODE FAST — Zentar 4 405B (Zero Refusal)

**Model:** `nousresearch/zentar-4-405b`
**Strategy:** Zentar models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with ZENTAR DIGITAL AI AGENT

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.zentar/config.yaml`:
