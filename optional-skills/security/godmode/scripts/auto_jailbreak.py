...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZENTAR_HOME", os.path.expanduser("~/.zentar")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZENTAR_HOME", Path.home() / ".zentar")) / "skills" / "red-teaming" / "godmode"
...
ZENTAR_HOME = Path(os.getenv("ZENTAR_HOME", Path.home() / ".zentar"))
CONFIG_PATH = ZENTAR_HOME / "config.yaml"
PREFILL_PATH = ZENTAR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Zentar models — already uncensored, just needs clean prompt
    "zentar": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zentar" in model_lower or "nous" in model_lower:
        return "zentar"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zentar config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zentar/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zentar for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zentar for changes to take effect.")
