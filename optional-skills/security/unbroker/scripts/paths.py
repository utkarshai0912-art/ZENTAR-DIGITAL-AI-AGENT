...
All per-subject data lives under PDD_DATA_DIR (default: $ZENTAR_HOME/unbroker),
which is the same trust boundary Zentar uses for .env and OAuth tokens.
"""
...
def zentar_home() -> Path:
    return Path(os.environ.get("ZENTAR_HOME") or (Path.home() / ".zentar"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else zentar_home() / "unbroker"
