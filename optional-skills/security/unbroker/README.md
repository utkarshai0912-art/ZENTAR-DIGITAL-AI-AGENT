...
The design is **Zentar-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
zentar skills install official/security/unbroker
```

Then start a new Zentar session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Zentar session:
...
- **PII handling.** Dossiers live under the Zentar home directory (`0600`, optionally
