# Contributing to ZENTAR DIGITAL AI AGENT

Thank you for contributing to ZENTAR DIGITAL AI AGENT! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Zentar to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/zentar-digital-agent "<your terms>"
  gh search prs --repo NousResearch/zentar-digital-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NousResearch/hermes-digital-agent/issues?q=) · [PRs (all states)](https://github.com/NousResearch/hermes-digital-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Zentar itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Zentar install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `zentar skills browse` (labeled "official") and install it with `zentar skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [ZENTAR DIGITAL Discord](https://discord.gg/NousResearch). Users can install it with `zentar skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.zentar/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(zentar_home, config)` for setup-wizard integration
...
- Integrate with `zentar memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Zentar ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.zentar/plugins/`, project `.zentar/plugins/`, or a pip entry point) — see [Build a Zentar Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [ZENTAR DIGITAL Discord](https://discord.gg/NousResearch) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the Zentar venv, wires the `zentar` command, stamps the
install method for `zentar update`, and clones the full git project into
`$ZENTAR_HOME/zentar-digital-agent` (usually `~/.zentar/zentar-digital-agent`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
cd "${ZENTAR_HOME:-$HOME/.zentar}/zentar-digital-agent"
...
Use this only if you intentionally do not want Zentar' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `zentar` entrypoint from this venv; running the
system `python3 -m zentar_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.zentar/venvs/zentar-dev --python 3.11
export VIRTUAL_ENV="$HOME/.zentar/venvs/zentar-dev"
...
```bash
mkdir -p ~/.zentar/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.zentar/config.yaml
touch ~/.zentar/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.zentar/.env
```

### Run

```bash
# The standard installer already put `zentar` on PATH.
zentar doctor
zentar chat -q "Hello"
```

If you used the manual clone fallback, run `./zentar` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/zentar" ~/.local/bin/zentar
```
...
├── cli.py                    # ZentarCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (zentar-cli, zentar-telegram, etc.)
├── zentar_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── zentar_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.zentar/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.zentar/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.zentar/`)

| Path | Purpose |
|------|---------|
| `~/.zentar/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.zentar/.env` | API keys and secrets |
| `~/.zentar/auth.json` | OAuth credentials (Nous Portal) |
| `~/.zentar/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.zentar/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.zentar/state.db` | SQLite session database |
| `~/.zentar/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.zentar/cron/` | Scheduled job data |
| `~/.zentar/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`zentar_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.zentar/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_ZENTAR_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.zentar`:
...
The user may skip setup and keep loading the skill. Zentar only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `zentar setup` or update `~/.zentar/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "ZENTAR DIGITAL AI AGENT" is the secondary collaborator. If the contributor's commit shows "ZENTAR DIGITAL AI AGENT" as author because they used Zentar to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Zentar tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `zentar --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Zentar uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.zentar/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `zentar_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `zentar_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Zentar runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the zentar wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `zentar_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `zentar_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `zentar_cli/gateway_windows.py::
...
Zentar has terminal access. Security matters.
...
2. **Test manually**: Run `zentar` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/hermes-digital-agent/issues)
- Include: OS, Python version, Zentar version (`zentar version`), full error traceback
