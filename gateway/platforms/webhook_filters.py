...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.zentar to the active profile home."""
    ...
    if raw == "~/.zentar":
        return zentar_home
    if raw.startswith("~/.zentar/"):
        return zentar_home / raw.removeprefix("~/.zentar/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return zentar_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ZENTAR_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from zentar_constants import get_zentar_home

    scripts_root = (get_zentar_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.zentar" or raw_text.startswith("~/.zentar/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__zentar_ignore__") is True
        ):
            return False, None
