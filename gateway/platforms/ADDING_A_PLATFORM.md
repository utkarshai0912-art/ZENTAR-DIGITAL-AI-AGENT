...
There are two ways to add a platform to the Zentar gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zentar/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Zentar code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `zentar gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zentar_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Zentar core.
...
And add it to the `zentar-gateway` composite:

```python
"zentar-gateway": {
    "includes": [..., "zentar-your-platform"]
}
```
...
## 12. Status Display (`zentar_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zentar_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zentar_cli/ toolsets.py \
  --include="*.py" -l | sort -u
