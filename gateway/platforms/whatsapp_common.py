...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *ZENTAR DIGITAL AI AGENT*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ZENTAR_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/zentar), this function
    mirrors the bridge source to a writable ZENTAR_HOME location and returns that
    ...
    # Try ZENTAR_HOME location first
    zentar_home = get_zentar_home()
    zentar_home_bridge = zentar_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ZENTAR_HOME if needed
    if zentar_home_bridge.exists():
        return zentar_home_bridge

    # Mirror the bridge source to ZENTAR_HOME
    try:
        zentar_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            zentar_home_bridge,
            dirs_exist_ok=False,
