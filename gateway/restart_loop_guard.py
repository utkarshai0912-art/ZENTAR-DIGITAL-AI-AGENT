...
Defenses 1 and 2 (the ``_ZENTAR_GATEWAY`` guard on ``zentar gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.zentar.gateway")``,
...
State lives in ``<ZENTAR_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_zentar_home() / "gateway" / "restart_loop.json"
