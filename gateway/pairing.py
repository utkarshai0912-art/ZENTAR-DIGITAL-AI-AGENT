...
from zentar_constants import get_zentar_dir, get_zentar_home
...
PAIRING_DIR = get_zentar_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from zentar_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ZENTAR_HOME}/pairing`` while newer code/docs may
    write ``{ZENTAR_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<ZENTAR_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<ZENTAR_HOME>/pairing/``
    directory (backward-compat for the ``zentar pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ZENTAR_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from zentar_constants import get_zentar_home
            self._dir = get_zentar_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `zentar` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> zentar pairing approve ...` as root, "
                    "re-run with `docker exec -u zentar <container> ...` and "
                    "chown the existing file to the zentar user, or restart the "
