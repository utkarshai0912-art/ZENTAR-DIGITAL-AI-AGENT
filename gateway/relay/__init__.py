"""Relay/connector support package for the Zentar gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``zentar gateway enroll`` writes them to
    ``~/.zentar/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ZENTAR_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``zentar gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``zentar gateway enroll`` CLI. Two modes, in precedence order:
    ...
    if not token_url:
        ...
        from zentar_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.zentar/.env``.
    ...
    sets neither ``ZENTAR_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``zentar gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'zentar'}"
