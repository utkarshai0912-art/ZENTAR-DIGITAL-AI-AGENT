...
# ``ZENTAR_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``ZENTAR_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("ZENTAR_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in zentar_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `zentar tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `zentar tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_zentar_home()}/cron/output/)"
    )
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.zentar/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.zentar/state.db "
                "and are shown by `zentar sessions list` and `/sessions`. "
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                # (zentar_state.find_latest_gateway_session_for_peer) selects
