...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{ZENTAR_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``ZENTAR_HOME`` is a **durable** store — on Zentar
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``zentar gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting ZENTAR_HOME."""
    base = home if home is not None else get_zentar_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable ZENTAR_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (ZENTAR_HOME is a
    durable Fly volume on Zentar Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable ZENTAR_HOME volume is
