"""
Session-scoped context variables for the Zentar gateway.

Replaces the previous ``os.environ``-based session state
(``ZENTAR_SESSION_PLATFORM``, ``ZENTAR_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["ZENTAR_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("ZENTAR_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("ZENTAR_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("ZENTAR_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("ZENTAR_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("ZENTAR_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("ZENTAR_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("ZENTAR_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("ZENTAR_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("ZENTAR_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("ZENTAR_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("ZENTAR_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from ZENTAR_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("ZENTAR_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("ZENTAR_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("ZENTAR_SESSION_PROFILE", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("ZENTAR_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("ZENTAR_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("ZENTAR_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("ZENTAR_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "ZENTAR_SESSION_PLATFORM": _SESSION_PLATFORM,
    "ZENTAR_SESSION_SOURCE": _SESSION_SOURCE,
    "ZENTAR_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "ZENTAR_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "ZENTAR_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "ZENTAR_SESSION_USER_ID": _SESSION_USER_ID,
    "ZENTAR_SESSION_USER_NAME": _SESSION_USER_NAME,
    "ZENTAR_SESSION_KEY": _SESSION_KEY,
    "ZENTAR_SESSION_ID": _SESSION_ID,
    "ZENTAR_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "ZENTAR_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "ZENTAR_SESSION_PROFILE": _SESSION_PROFILE,
    "ZENTAR_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "ZENTAR_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "ZENTAR_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ZENTAR_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("ZENTAR_SESSION_ID")`` with an ``os.environ`` fallback,
...
def reset_session_vars() -> None:
    ...
    *A's* ``ZENTAR_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``ZENTAR_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``ZENTAR_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("ZENTAR_SESSION_*", default)``.
