...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``zentar setup`` display).
    ...
    # built-ins).  Used by ``zentar gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `zentar`
        # invocation -- including plain `zentar chat`, which never touches any
        ...
        # `zentar setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``zentar setup``/``gateway status``, channel
