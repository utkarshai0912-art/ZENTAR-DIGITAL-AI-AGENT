...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``zentar://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``zentar://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"zentar://blueprint/{quote(blueprint.key)}{qs}"
