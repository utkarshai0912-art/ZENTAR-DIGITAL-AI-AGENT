...
``zentar gateway restart`` (or ``launchctl kickstart ai.zentar.gateway``
or ``systemctl restart zentar-gateway``).  When the cron fires, the
...
``zentar cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``zentar gateway``, ``launchctl ... zentar-gateway``,
``systemctl ... zentar-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_ZENTAR_GATEWAY=1``, and
``zentar gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `zentar gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:zentar\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a zentar-gateway label. macOS launchd
    # labels look like `ai.zentar.gateway` / `zentar-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated zentar services (e.g.
    # `launchctl unload ai.zentar.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*zentar_gateway)"
    # Branch C: systemctl ops on a zentar-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*zentar_gateway)"
    # Branch D: pkill / kill targeting the zentar gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<ZENTAR_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.zentar/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_zentar_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `zentar gateway restart` from a shell outside "
