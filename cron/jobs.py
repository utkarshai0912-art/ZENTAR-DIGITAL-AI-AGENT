...
Jobs are stored in ~/.zentar/cron/jobs.json
Output is saved to ~/.zentar/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own ZENTAR_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same ZENTAR_HOME — so a job authored in profile
# `coder` lives in `~/.zentar/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_zentar_home()` (the active profile home), NOT `get_default_zentar_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever ZENTAR_HOME the
...
# `_get_zentar_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
ZENTAR_DIR = get_zentar_home().resolve()
CRON_DIR = ZENTAR_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``zentar cron status`` process share it
...
# threshold in `zentar cron status` (zentar_cli/cron.py), so the two never
# drift apart.
...
# the cron inactivity timeout is disabled (ZENTAR_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. ZENTAR_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``ZENTAR_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("ZENTAR_CRON_TIMEOUT", "").strip()
...
def _jobs_lock():
    ...
    and standalone ``zentar`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Zentar timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `zentar_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                zentar_tz = _zentar_now().tzinfo
                dt = dt.replace(tzinfo=zentar_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _zentar_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Zentar configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Zentar timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _zentar_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Zentar/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall zentar-digital-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `zentar cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``zentar cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `zentar cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _zentar_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_zentar_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.zentar/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `zentar cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _zentar_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _zentar_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``ZENTAR_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("ZENTAR_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from ZENTAR_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        if next_run_dt <= now:
            ...
            # desktop both run in-process 60s tickers on one ZENTAR_HOME)
            ...
            # (_oneshot_run_claim_ttl_seconds, derived from ZENTAR_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`zentar_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``zentar_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _zentar_now().strftime("%Y-%m-%d_%H-%M-%S")
