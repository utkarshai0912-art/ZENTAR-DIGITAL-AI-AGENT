...
#
# Wired into the image as /etc/cont-init.d/01-zentar-setup by the
...
ZENTAR_HOME="${ZENTAR_HOME:-/opt/data}"
INSTALL_DIR="/opt/zentar"

# Drop to zentar via s6-setuidgid, but skip it when already non-root.
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid zentar "$@"; }
...
# starts non-root. The baked install tree under /opt/zentar is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ZENTAR_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ZENTAR_UID/ZENTAR_GID — or the PUID/PGID aliases — which the
...
# `id -u zentar` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# zentar UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u zentar)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-zentar UID).
...
and the baked /opt/zentar install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ZENTAR_UID=\$(id -u) -e ZENTAR_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the zentar user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ZENTAR_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid zentar mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ZENTAR_HOME paths whose parents only
# root can create (e.g. `ZENTAR_HOME=/home/zentar/.zentar` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ZENTAR_UID/ZENTAR_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ZENTAR_UID/
# ZENTAR_GID still win when both are set.  See #15290, salvages #25872.
ZENTAR_UID="${ZENTAR_UID:-${PUID:-}}"
ZENTAR_GID="${ZENTAR_GID:-${PGID:-}}"

if [ -n "${ZENTAR_UID:-}" ] && validate_uid_gid "$ZENTAR_UID" && [ "$ZENTAR_UID" != "$(id -u zentar)" ]; then
    echo "[stage2] Changing zentar UID to $ZENTAR_UID"
    usermod -u "$ZENTAR_UID" zentar
fi
if [ -n "${ZENTAR_GID:-}" ] && validate_uid_gid "$ZENTAR_GID" && [ "$ZENTAR_GID" != "$(id -g zentar)" ]; then
    echo "[stage2] Changing zentar GID to $ZENTAR_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ZENTAR_GID" zentar 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised zentar user (UID 10000)
...
# zentar process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes zentar. Idempotent across container
...
#   - zentar is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G zentar 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] zentar already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" zentar 2>/dev/null; then
        echo "[stage2] Added zentar to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group zentar failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ZENTAR_UID is remapped or the top-level $ZENTAR_HOME isn't owned by
# the runtime zentar UID, restore ownership to zentar — but ONLY for the
# directories zentar actually writes to. The full $ZENTAR_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of zentar-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_zentar_uid=$(id -u zentar)
...
chown_zentar_tree() {
    ...
    chown -R zentar:zentar "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$ZENTAR_HOME" 2>/dev/null)" != "$actual_zentar_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ZENTAR_HOME (targeted) to zentar ($actual_zentar_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ZENTAR_HOME: chown the directory itself (not its contents)
    # so zentar can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ZENTAR_HOME"; then
        :
    else
        chown zentar:zentar "$ZENTAR_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ZENTAR_HOME failed (rootless container?) — continuing"
    fi
    # Zentar-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by zentar (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ZENTAR_HOME/$sub" ]; then
            chown_zentar_tree "$ZENTAR_HOME/$sub"
        fi
    done
fi
...
# zentar user. Hosted/container instances keep mutable state under
# $ZENTAR_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ZENTAR_DISABLE_LAZY_INSTALLS=1. Keeping /opt/zentar root-owned and
...
# $ZENTAR_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ZENTAR_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to zentar
...
# Always reset ownership of $ZENTAR_HOME/profiles to zentar on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> zentar …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as zentar and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ZENTAR_HOME/profiles" ]; then
    chown_zentar_tree "$ZENTAR_HOME/profiles"
fi

# Always reset ownership of $ZENTAR_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged zentar runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$ZENTAR_HOME/cron" ]; then
    chown_zentar_tree "$ZENTAR_HOME/cron"
fi
...
# zentar pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged zentar gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ZENTAR_HOME is
...
# is negligible.
if [ -d "$ZENTAR_HOME/platforms/pairing" ]; then
    chown_zentar_tree "$ZENTAR_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ZENTAR_HOME/pairing" ]; then
    chown_zentar_tree "$ZENTAR_HOME/pairing"
fi

# Reset ownership of zentar-owned top-level state files on every boot.
# The targeted data-volume chown above only covers zentar-owned
# *subdirectories*; loose state files living directly under $ZENTAR_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> zentar …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged zentar runtime then hits
...
# sweep so host-owned files in a bind-mounted $ZENTAR_HOME are never
...
# entries of zentar_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ZENTAR_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ZENTAR_HOME/$f"; then
            :
        else
            chown zentar:zentar "$ZENTAR_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the zentar runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ZENTAR_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZENTAR_HOME/config.yaml"; then
        :
    else
        chown zentar:zentar "$ZENTAR_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ZENTAR_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as zentar user ---
# Run as zentar via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ZENTAR_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_hermes mkdir -p \
    "$ZENTAR_HOME/backups" \
    "$ZENTAR_HOME/cron" \
    "$ZENTAR_HOME/sessions" \
    "$ZENTAR_HOME/logs" \
    "$ZENTAR_HOME/logs/gateways" \
    "$ZENTAR_HOME/hooks" \
    "$ZENTAR_HOME/memories" \
    "$ZENTAR_HOME/skills" \
    "$ZENTAR_HOME/skins" \
    "$ZENTAR_HOME/plans" \
    "$ZENTAR_HOME/workspace" \
    "$ZENTAR_HOME/home" \
    "$ZENTAR_HOME/pairing" \
    "$ZENTAR_HOME/platforms/pairing" \
    "$ZENTAR_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/zentar/.install_method (see Dockerfile), NOT written here into
# $ZENTAR_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ZENTAR_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.zentar:/opt/data) and sometimes shared with a
...
# 'zentar update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ZENTAR_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/zentar stamp, so deleting the data-dir copy is safe).
if [ -f "$ZENTAR_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ZENTAR_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ZENTAR_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ZENTAR_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ZENTAR_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$ZENTAR_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ZENTAR_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZENTAR_HOME/.env"; then
        :
    else
        chown zentar:zentar "$ZENTAR_HOME/.env" 2>/dev/null || true
        chmod 600 "$ZENTAR_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ZENTAR_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `zentar update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ZENTAR_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ZENTAR_HOME/config.yaml" ]; then
    s6-setuidgid zentar "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ZENTAR_HOME/auth.json" ] && [ -n "${ZENTAR_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ZENTAR_HOME/auth.json"; then
        :
    else
        printf '%s' "$ZENTAR_AUTH_JSON_BOOTSTRAP" > "$ZENTAR_HOME/auth.json"
        chown zentar:zentar "$ZENTAR_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ZENTAR_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ZENTAR_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ZENTAR_HOME/auth.json" ] && [ -n "${ZENTAR_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ZENTAR_HOME/auth.json"; then
        :
    else
        s6-setuidgid zentar "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ZENTAR_HOME/auth.json" \
...
# can set ZENTAR_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ZENTAR_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ZENTAR_HOME/gateway_state.json" ] && \
        [ "${ZENTAR_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ZENTAR_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ZENTAR_HOME/gateway_state.json"
        chown zentar:zentar "$ZENTAR_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ZENTAR_HOME/gateway_state.json"
    fi
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/zentar/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Zentar spawns for the
...
# main-wrapper.sh propagates it into the supervised ``zentar`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-zentar, dashboard, etc.).
