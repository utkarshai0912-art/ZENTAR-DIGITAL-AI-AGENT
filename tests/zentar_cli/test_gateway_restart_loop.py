...
Covers:
- Defense 1: gateway stop/restart refuse when _ZENTAR_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zentar gateway stop",
        "zentar  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ZENTAR GATEWAY RESTART",           # uppercase
    ])
    def test_zentar_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.zentar.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.zentar.gateway.plist",
        "launchctl stop ai.zentar.gateway",
        "systemctl restart zentar-gateway",
        "systemctl stop zentar-gateway.service",
        "systemctl start zentar-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill zentar gateway process",
        "pkill -f zentar.*gateway",
        "pkill -f gateway.*zentar",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zentar cron list",
        "zentar update",
        "zentar config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `zentar gateway start` is benign — starting a gateway from inside a
        ...
        "zentar gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway zentar
        # services must not be falsely blocked (the old `.*zentar` matched any
        # zentar token).
        "launchctl unload ai.zentar.update-checker.plist",
        "launchctl restart ai.zentar.daemon",
        "systemctl restart zentar-meta.service",
        "systemctl restart zentar-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_zentar_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade zentar then run zentar gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.zentar.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_zentar_gateway_once.sh). The script must live under
        # ZENTAR_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / ".zentar"))
        scripts_dir = tmp_path / ".zentar" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify zentar gateway stop/restart refuse when _ZENTAR_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_ZENTAR_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ZENTAR_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ZENTAR_GATEWAY=1.

    Issue #37453: systemctl --user restart zentar-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart zentar-gateway",
        "systemctl --user restart zentar-gateway",
        "systemctl stop zentar-gateway.service",
        "zentar gateway restart",
        "launchctl kickstart gui/501/ai.zentar.gateway",
        "pkill -f zentar.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart zentar-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-zentar systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _ZENTAR_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart zentar-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart zentar-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run zentar gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("zentar gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ZENTAR_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.zentar.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    zentar_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run zentar gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / ".zentar"))
        (tmp_path / ".zentar").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run zentar gateway restart nightly",
        ))
