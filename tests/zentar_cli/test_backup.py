"""Tests for zentar backup and import commands."""
...
def _make_zentar_tree(root: Path) -> None:
    """Create a realistic ~/.zentar directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "zentar_state.db").write_bytes(b"fake-state")
    ...
    # zentar-digital-agent repo (should be EXCLUDED)
    (root / "zentar-digital-agent").mkdir(exist_ok=True)
    (root / "zentar-digital-agent" / "run_agent.py").write_text("# big file\n")
    (root / "zentar-digital-agent" / ".git").mkdir()
    (root / "zentar-digital-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_zentar_digital_agent(self):
        from zentar_cli.backup import _should_exclude
        assert _should_exclude(Path("zentar-digital-agent/run_agent.py"))
        assert _should_exclude(Path("zentar-digital-agent/.git/HEAD"))
    ...
    def test_includes_nested_zentar_digital_agent_in_skills(self):
        """skills/autonomous-ai-agents/zentar-digital-agent/ must NOT be excluded —
        only the root-level zentar-digital-agent/ repo is skipped."""
        from zentar_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/zentar-digital-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/zentar-digital-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under ZENTAR_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        zentar_home.mkdir()
        _make_zentar_tree(zentar_home)

        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        # get_default_zentar_root needs this
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import zentar_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = zentar_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, zentar_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_zentar_digital_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include zentar-digital-agent/ directory."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        _make_zentar_tree(zentar_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "zentar-digital-agent" in n]
            assert agent_files == [], f"zentar-digital-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under ZENTAR_HOME must be
        ...
        venv_pkg = zentar_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = zentar_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_zentar_digital_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../zentar-digital-agent/ but NOT root zentar-digital-agent/."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        _make_zentar_tree(zentar_home)

        # Add a nested zentar-digital-agent directory inside skills (like the real layout)
        nested = zentar_home / "skills" / "autonomous-ai-agents" / "zentar-digital-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# ZENTAR DIGITAL AI AGENT Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root zentar-digital-agent must be excluded
            root_agent = [n for n in names if n.startswith("zentar-digital-agent/")]
            assert root_agent == [], f"root zentar-digital-agent leaked: {root_agent}"
            # Nested skill zentar-digital-agent must be included
            assert "skills/autonomous-ai-agents/zentar-digital-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/zentar-digital-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/zentar-backup-*.zip."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("zentar-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ZENTAR_HOME."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        _make_zentar_tree(zentar_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(zentar_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Zentar backup."""
        from zentar_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only zentar_state.db (old wrong name) is rejected."""
        from zentar_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["zentar_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into zentar home."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (zentar_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (zentar_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (zentar_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (zentar_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_zentar_prefix(self, tmp_path, monkeypatch):
        """Import strips .zentar/ prefix if all entries share it."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".zentar/config.yaml": "model: test\n",
            ".zentar/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (zentar_home / "config.yaml").read_text() == "model: test\n"
        assert (zentar_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_zentar_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a zentar backup."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (zentar_home / "config.yaml").exists()
        # traversal file should NOT exist outside zentar home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (zentar_home / "gateway_state.json").write_text(live_state)
        ...
        assert (zentar_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (zentar_home / "config.yaml").exists()
        assert not (zentar_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        (zentar_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (zentar_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (zentar_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            zentar_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (zentar_home / "gateway.pid").write_text("4242")
        (zentar_home / "processes.json").write_text('{"live": true}')
        ...
        assert (zentar_home / "gateway.pid").read_text() == "4242"
        assert (zentar_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (zentar_home / "cron.pid").exists()
        assert not (zentar_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (zentar_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (zentar_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (zentar_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (zentar_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".zentar"
        src_home.mkdir(parents=True)
        _make_zentar_tree(src_home)

        monkeypatch.setenv("ZENTAR_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from zentar_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".zentar"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ZENTAR_HOME", str(dst_home))
        ...
        # zentar-digital-agent should NOT be present
        assert not (dst_home / "zentar-digital-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from zentar_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without zentar markers fails validation."""
        import io
        from zentar_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .zentar/ prefix wrapping all entries."""
        import io
        from zentar_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".zentar/config.yaml", "test")
            zf.writestr(".zentar/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".zentar/"
...
class TestBackupEdgeCases:
    def test_nonexistent_zentar_home(self, tmp_path, monkeypatch):
        """Backup exits when zentar home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".zentar"
        monkeypatch.setenv("ZENTAR_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("zentar-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_zentar_home(self, tmp_path, monkeypatch):
        """Backup handles empty zentar home (no files to back up)."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        # Only excluded dirs, no actual files
        (zentar_home / "__pycache__").mkdir()
        (zentar_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = zentar_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = zentar_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside zentar root."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside zentar home
        out_zip = zentar_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (zentar_home / "config.yaml").exists()
        assert (zentar_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (zentar_home / "profiles" / "coder" / "config.yaml").exists()
        assert (zentar_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "zentar -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from zentar_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def zentar_home(self, tmp_path):
        """Create a fake ZENTAR_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        assert snap_id is not None
        snap_dir = zentar_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", zentar_home=zentar_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        db_copy = zentar_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        assert (zentar_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        copied = zentar_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        with open(zentar_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(zentar_home=empty) is None

    def test_list_snapshots(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", zentar_home=zentar_home)
        id2 = create_quick_snapshot(label="second", zentar_home=zentar_home)

        snaps = list_quick_snapshots(zentar_home=zentar_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", zentar_home=zentar_home)
        snaps = list_quick_snapshots(limit=3, zentar_home=zentar_home)
        assert len(snaps) == 3

    def test_restore_config(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)

        (zentar_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (zentar_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, zentar_home=zentar_home)
        assert result is True
        assert "openrouter" in (zentar_home / "config.yaml").read_text()

    def test_restore_state_db(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)

        conn = sqlite3.connect(str(zentar_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, zentar_home=zentar_home)
    ...
    def test_restore_nonexistent(self, zentar_home):
        from zentar_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", zentar_home=zentar_home) is False

    def test_auto_prune(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", zentar_home=zentar_home)
        snaps = list_quick_snapshots(limit=100, zentar_home=zentar_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", zentar_home=zentar_home)
        deleted = prune_quick_snapshots(keep=3, zentar_home=zentar_home)
        assert deleted == 7
        assert len(list_quick_snapshots(zentar_home=zentar_home)) == 3

    def test_snapshot_includes_pairing_directories(self, zentar_home):
        ...
        (zentar_home / "platforms" / "pairing").mkdir(parents=True)
        (zentar_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (zentar_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (zentar_home / "pairing").mkdir()
        (zentar_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (zentar_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, zentar_home):
        ...
        assert restore_quick_snapshot(snap_id, zentar_home=zentar_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, zentar_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (zentar update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, zentar_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, zentar_home):
        ...
        escape_dst = zentar_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped ZENTAR_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from zentar_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        copy = zentar_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        copy = zentar_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, zentar_home):
        from zentar_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(zentar_home=zentar_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(zentar_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, zentar_home):
        ...
        board_dir = zentar_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        copy = (
            zentar_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, zentar_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(zentar_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, zentar_home):
        ...
        board = zentar_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(zentar_home=zentar_home)
        snap = zentar_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, zentar_home, monkeypatch):
        ...
        import zentar_cli.backup as bk
        from zentar_cli.backup import create_quick_snapshot
        ...
        copy = zentar_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``zentar update``
    runs before touching anything."""

    @pytest.fixture
    def zentar_home(self, tmp_path):
        root = tmp_path / ".zentar"
        root.mkdir()
        _make_zentar_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, zentar_home):
        from zentar_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(zentar_home=zentar_home)
        assert out is not None
        assert out.exists()
        assert out.parent == zentar_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, zentar_home):
        """Pre-update backup should include the same user data that
        ``zentar backup`` would, and should exclude the same directories."""
        from zentar_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(zentar_home=zentar_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # zentar-digital-agent repo excluded
        assert not any(n.startswith("zentar-digital-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, zentar_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from zentar_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(zentar_home=zentar_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(zentar_home=zentar_home)
    ...
    def test_rotation_keeps_only_n(self, zentar_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(zentar_home=zentar_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (zentar_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, zentar_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from zentar_cli.backup import create_pre_update_backup

        (zentar_home / "backups").mkdir(exist_ok=True)
        manual = zentar_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(zentar_home=zentar_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from zentar_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(zentar_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, zentar_home):
        ...
        """
        from zentar_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(zentar_home=zentar_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, zentar_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, zentar_home):
        ...
        first = create_pre_update_backup(zentar_home=zentar_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(zentar_home=zentar_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(zentar_home=zentar_home, keep=0)
    ...
    def test_skips_symlinked_files(self, zentar_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ZENTAR_HOME."""
        from zentar_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def zentar_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".zentar"
        root.mkdir()
        _make_zentar_tree(root)
        # Point ZENTAR_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ZENTAR_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for zentar_cli.config + zentar_constants so they pick up ZENTAR_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("zentar_cli.config") or mod == "zentar_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, zentar_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from zentar_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "zentar import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((zentar_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, zentar_home, capsys):
        """With the default (``pre_update_backup: false``), ``zentar update``
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((zentar_home / "backups").glob("pre-update-*.zip")) \
            if (zentar_home / "backups").exists() else True

    def test_no_backup_flag_skips(self, zentar_home, capsys):
        from zentar_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (zentar_home / "backups").exists() or not list(
            (zentar_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, zentar_home, capsys):
        ...
        import yaml
        (zentar_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("zentar_cli.config"):
    ...
    def test_config_disabled_is_silent(self, zentar_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, zentar_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (zentar claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``zentar claw migrate`` runs before mutating ~/.zentar/."""
    ...
    def test_creates_backup_under_backups_dir(self, zentar_home):
        from zentar_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(zentar_home=zentar_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `zentar
        # import` and the update-backup listing both pick them up.
        assert out.parent == zentar_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, zentar_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``zentar backup`` / ``create_pre_update_backup`` — no drift."""
        from zentar_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(zentar_home=zentar_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_zentar_import(self, zentar_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Zentar
        backup — `zentar import` should accept it."""
        from zentar_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(zentar_home=zentar_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, zentar_home):
        from zentar_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(zentar_home=zentar_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(zentar_home=zentar_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, zentar_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(zentar_home=zentar_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((zentar_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_zentar_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.zentar yet — nothing to back up."""
        from zentar_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(zentar_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, zentar_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from zentar_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(zentar_home=zentar_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(zentar_home=zentar_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`zentar update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, zentar_home: Path, label="pre-update"):
        from zentar_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, zentar_home=zentar_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from zentar_cli.backup import restore_cron_jobs_if_emptied
        zentar_home = tmp_path / ".zentar"
        jobs_path = zentar_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(zentar_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, zentar_home=zentar_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = zentar_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, zentar_home=zentar_home) is None
        assert restore_cron_jobs_if_emptied("", zentar_home=zentar_home) is None
...
# location, NOT under ZENTAR_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, zentar_home: Path) -> None:
        zentar_home.mkdir(parents=True, exist_ok=True)
        (zentar_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (zentar_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (zentar_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        self._make_min_tree(zentar_home)
        # External provider state living OUTSIDE ZENTAR_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ZENTAR_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        zentar_home = dst_home / ".zentar"
        zentar_home.mkdir()
        ...
        # External state did NOT leak into ZENTAR_HOME.
        assert not (zentar_home / "_external").exists()
