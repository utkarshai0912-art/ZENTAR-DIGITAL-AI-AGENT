...
``zentar_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``zentar --help``, ``zentar version``,
``zentar logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``zentar --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from zentar_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["zentar", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["zentar", "-w", "--tui"], None),
        (["zentar", "version"], "version"),
        (["zentar", "--tui", "chat"], "chat"),
        (["zentar", "-w", "logs"], "logs"),
        (["zentar", "chat", "hello world"], "chat"),
        (["zentar", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["zentar", "-m", "gpt5", "chat"], "chat"),
        (["zentar", "--model", "gpt5", "chat", "hi"], "chat"),
        (["zentar", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["zentar", "-z", "hello world"], None),
        (["zentar", "-z", "hello", "chat"], "chat"),
        (["zentar", "--model=gpt5", "chat"], "chat"),     # inline form
        (["zentar", "--", "chat"], "chat"),               # -- terminator
        (["zentar", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["zentar", "some-plugin-cmd"], "some-plugin-cmd"),
        (["zentar", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["zentar"],                          # bare → chat
        ["zentar", "--help"],                # top-level help
        ["zentar", "-h"],
        ["zentar", "version"],               # known built-in
        ["zentar", "logs"],
        ["zentar", "gateway", "run"],
        ["zentar", "--tui"],
        ["zentar", "-w", "--tui"],
        ["zentar", "chat", "hi"],
        ["zentar", "help"],                  # accepted built-in-ish
        ["zentar", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["zentar", "meet", "join"],          # potential google_meet plugin
        ["zentar", "honcho", "status"],      # potential memory plugin
        ["zentar", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``zentar help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"zentar_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
