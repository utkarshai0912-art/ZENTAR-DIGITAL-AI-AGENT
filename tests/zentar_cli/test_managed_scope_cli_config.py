...
zentar_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `zentar config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_zentar_home = get_zentar_home()`` at import time (module
    singleton), so monkeypatching ZENTAR_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ZENTAR_HOME.)
    """
    import cli

    cli._zentar_home = home
