"""Tests for zentar_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ZENTAR_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Zentar-managed node dir.
"""
...
def _make_zentar_node(zentar_home: Path) -> Path:
    """Create a fake $ZENTAR_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = zentar_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_zentar_node(fake_home):
    zentar_home = fake_home / ".zentar"
    node_bin = _make_zentar_node(zentar_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(zentar_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(zentar_home) == []


def test_removes_dangling_symlink_into_zentar_node(fake_home):
    """A link into the Zentar node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Zentar links that exist; ignores the ones that don't."""
    zentar_home = fake_home / ".zentar"
    node_bin = _make_zentar_node(zentar_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Zentar-managed; node is a real user binary.
