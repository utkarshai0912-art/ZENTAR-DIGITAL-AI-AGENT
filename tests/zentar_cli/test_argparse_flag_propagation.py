...
subparser, placing the flag BEFORE the subcommand (e.g. 'zentar --yolo chat')
...
def _build_parser():
    """Build the zentar argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("zentar_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "zentar_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets ZENTAR_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `zentar gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `zentar <argv>` must exit 0 (help) rather than
