"""Unit tests for zentar_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Zentar.
"""
...
# A representative set of real toolset names. `zentar` is deliberately absent —
# that is the corruption #38798 reported (`zentar-cli` rewritten to `zentar`).
_KNOWN = {
    "zentar-cli",
    "zentar-telegram",
    "zentar-discord",
...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["zentar-cli"], "telegram": ["zentar-telegram"]}
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'zentar' instead of 'zentar-cli'.
    warnings = validate_platform_toolsets({"cli": ["zentar"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'zentar'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'zentar-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["zentar-cli"], "discord": ["bogus"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # zentar-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "zentar-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "zentar-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "zentar-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["zentar"], "discord": ["zentar"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_zentar_cli_valid_and_zentar_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `zentar-cli` is the real toolset and `zentar` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("zentar-cli") is True
    assert validate_toolset("zentar") is False
    warnings = validate_platform_toolsets({"cli": ["zentar"]}, validate_toolset)
    assert any("did you mean 'zentar-cli'?" in w for w in warnings)
