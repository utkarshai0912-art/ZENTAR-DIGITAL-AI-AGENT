"""Tests for `zentar chat --safe-mode` isolation."""
...
_VARS = ("ZENTAR_SAFE_MODE", "ZENTAR_IGNORE_USER_CONFIG", "ZENTAR_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["ZENTAR_SAFE_MODE"] == "1"
        assert os.environ["ZENTAR_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["ZENTAR_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("zentar_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "zentar_cli.plugins", plugins)
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("ZENTAR_SAFE_MODE", "1")
