"""Tests for `zentar memory setup [provider]` routing.
...
``zentar memory setup honcho``) without the interactive picker — which matters
because the per-provider ``zentar <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "zentar memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        with patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("zentar_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("zentar_cli.tools_config.subprocess.run", fake_run):
