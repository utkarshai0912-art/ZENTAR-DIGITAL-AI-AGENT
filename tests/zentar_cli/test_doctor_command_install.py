"""Tests for the Command Installation check in zentar doctor."""
...
import zentar_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ZENTAR_HOME + PROJECT_ROOT for doctor tests."""
    ...
    zentar_bin = venv_bin_dir / "zentar"
    zentar_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    zentar_bin.chmod(0o755)
    ...
    return home, project, zentar_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, zentar_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "zentar"
        cmd_link.symlink_to(zentar_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "zentar doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "zentar"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == zentar_bin.resolve()
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        zentar_bin = project / ".venv" / "bin" / "zentar"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "zentar"
        cmd_link.symlink_to(zentar_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/zentar" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/zentar is a regular file (not symlink), accept it."""
        home, project, zentar_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "zentar"
        cmd_link.write_text("#!/bin/sh\nexec python -m zentar_cli.main \"$@\"\n")
