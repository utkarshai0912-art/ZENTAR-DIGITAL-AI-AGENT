...
zentar_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZENTAR_MANAGED_DIR", str(managed))
...
def test_gateway_run_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(gr, "_zentar_home", home, raising=False)
...
def test_gateway_config_loader_honors_managed(homes, monkeypatch):
    ...
    # load_gateway_config resolves home via get_zentar_home() (ZENTAR_HOME env).
...
def test_tui_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(ts, "_zentar_home", home, raising=False)
    monkeypatch.setattr(ts, "_cfg_cache", None, raising=False)
    monkeypatch.setattr(ts, "_cfg_mtime", None, raising=False)
    monkeypatch.setattr(ts, "get_zentar_home_override", lambda: None, raising=False)
    cfg = ts._load_cfg()
...
def test_logging_config_honors_managed(homes, monkeypatch):
    ...
    level, _max, _bk = zentar_logging._read_logging_config()
    assert level == "DEBUG"


def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # zentar_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("ZENTAR_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import zentar_time

    assert zentar_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (ZENTAR_TIMEZONE, ZENTAR_REDACT_SECRETS, ZENTAR_MAX_ITERATIONS,
    ...
    # writing ZENTAR_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
