"""Tests for IRC gateway configuration via `zentar setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Zentar install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Zentar install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#zentar")
            monkeypatch.setenv("IRC_NICKNAME", "zentar-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `zentar setup gateway` experience with IRC present."""
