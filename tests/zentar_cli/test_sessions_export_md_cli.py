...
def test_sessions_export_md_writes_single_session(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(zentar_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_sessions_export_md_rejects_stdout_target(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zentar", "sessions", "export", "--format", "md", "--session-id", "s1", "-"],
    )
...
def test_sessions_export_jsonl_requires_output_path(monkeypatch, capsys):
    ...
    monkeypatch.setattr(zentar_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(sys, "argv", ["zentar", "sessions", "export"])
...
def test_sessions_export_md_bulk_requires_filter(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zentar", "sessions", "export", "--format", "md", str(tmp_path)],
    )
...
def test_sessions_export_md_accepts_duration_age_grammar(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "zentar", "sessions", "export", "--format", "md",
...
def test_sessions_export_jsonl_honors_filters(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zentar", "sessions", "export", "--source", "telegram", str(out)],
    )
...
def test_sessions_export_trace_writes_claude_jsonl(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(zentar_state, "SessionDB", lambda: _trace_fake_db(captured))
    monkeypatch.setattr(
        sys,
        "argv",
        ["zentar", "sessions", "export", "--format", "trace", "--session-id", "s1", str(out)],
    )
...
def test_sessions_export_trace_stdout(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zentar", "sessions", "export", "--format", "trace", "--session-id", "s1", "-"],
    )
