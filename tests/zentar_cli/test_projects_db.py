"""Tests for the per-profile Projects store (zentar_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="ZENTAR DIGITAL AI AGENT", folders=["/tmp/zentar"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "zentar-digital-agent"
    assert proj.name == "ZENTAR DIGITAL AI AGENT"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/zentar"
    assert [f.path for f in proj.folders] == ["/tmp/zentar"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "zentar-digital-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="ZENTAR DIGITAL AI AGENT")
    pdb.create_project(conn, name="ZENTAR DIGITAL AI AGENT")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["zentar-digital-agent", "zentar-digital-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ZENTAR_HOME.
...
def test_db_path_under_zentar_home():
    # Resolves under ZENTAR_HOME (set by the autouse isolation fixture).
