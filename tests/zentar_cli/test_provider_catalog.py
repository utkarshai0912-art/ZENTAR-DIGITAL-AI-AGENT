"""Tests for the unified provider catalog (zentar_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``zentar model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from zentar_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_zentar_model_provider():
    """PARITY CONTRACT: the catalog == the `zentar model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `zentar model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_zentar_model():
    """The catalog must not invent providers `zentar model` doesn't show."""
