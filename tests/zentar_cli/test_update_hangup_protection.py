"""Tests for SIGHUP protection and stdout mirroring in ``zentar update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``zentar_cli/main.py``.  These exist so
that ``zentar update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_zentar_home if present
        import zentar_cli.config as _cfg
        if hasattr(_cfg, "_ZENTAR_HOME_CACHE"):
            _cfg._ZENTAR_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "zentar update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_zentar_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "zentar_cli.config.get_zentar_home", _boom, raising=True
        )
