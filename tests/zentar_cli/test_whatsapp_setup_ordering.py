...
Before the fix, ``zentar whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``zentar gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    zentar = home / ".zentar"
    zentar.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ZENTAR_HOME", str(zentar))
...
def _env_value(zentar_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from zentar_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("zentar_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``zentar whatsapp`` with an existing paired session and
