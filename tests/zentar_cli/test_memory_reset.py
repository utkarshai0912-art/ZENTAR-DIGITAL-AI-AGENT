"""Tests for the `zentar memory reset` CLI command.
...
- Profile-scoped reset (uses ZENTAR_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZENTAR_HOME with memory files."""
    zentar_home = tmp_path / ".zentar"
    memories = zentar_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nZentar repo is at ~/.zentar/zentar-digital-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `zentar memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        (zentar_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ZENTAR_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_zentar_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
