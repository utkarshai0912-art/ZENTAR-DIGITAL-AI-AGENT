...
from zentar_cli import uninstall


def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "zentar-digital-agent"
    ...
    monkeypatch.setattr(uninstall, "_is_default_zentar_home", lambda home: False)
    ...
    assert str(zentar_home) in output
