"""Tests for the ``zentar send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``zentar_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from zentar_cli import send_cmd
...
def test_load_zentar_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``zentar send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``zentar config set``) lives
    ...
    zentar_home.mkdir()
    (zentar_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_zentar_home() to re-resolve under the patched env.
    from importlib import reload

    import zentar_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_zentar_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_zentar_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    zentar_home = tmp_path / ".zentar"
    zentar_home.mkdir()
    (zentar_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_zentar_env_handles_missing_files(tmp_path, monkeypatch):
