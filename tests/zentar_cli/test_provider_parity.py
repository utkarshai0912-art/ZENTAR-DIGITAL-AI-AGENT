...
the SAME provider universe as ``zentar model`` (the CLI/TUI picker).
...
def test_every_zentar_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `zentar model` universe."""
    ...
    assert not missing, (
        "providers shown in `zentar model` but not configurable in the desktop "
