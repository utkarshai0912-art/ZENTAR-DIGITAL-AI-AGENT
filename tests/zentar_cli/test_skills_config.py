"""Tests for zentar_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("zentar_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from zentar_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("zentar_cli.config.load_config")
    ...
    @patch("zentar_cli.config.load_config")
    @patch.dict("os.environ", {"ZENTAR_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ZENTAR_SESSION_PLATFORM should be used when ZENTAR_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_zentar_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ZENTAR_PLATFORM should win over ZENTAR_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from zentar_cli.skills_config import _get_categories
        skills = [
