...
The cache exists to keep `zentar tools` → "All Platforms" fast: every
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from zentar_cli import config as config_mod
    from zentar_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_zentar_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "zentar-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("zentar_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
