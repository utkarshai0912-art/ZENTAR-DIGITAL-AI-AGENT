"""Tests for zentar_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import zentar_cli.gui_uninstall as gu


def _make_agent(zentar_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = zentar_home / "zentar-digital-agent"
    (agent_root / "zentar_cli").mkdir(parents=True)
    (agent_root / "zentar_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(zentar_home: Path) -> None:
    """Create the source-built GUI artifacts a `zentar desktop` run produces."""
    desktop = zentar_home / "zentar-digital-agent" / "apps" / "desktop"
    ...
    (zentar_home / "zentar-digital-agent" / "node_modules").mkdir(parents=True)
    (zentar_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(zentar_home: Path) -> None:
    (zentar_home / "config.yaml").write_text("x: 1\n")
    (zentar_home / ".env").write_text("KEY=secret\n")
    (zentar_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    zentar_home = tmp_path / ".zentar"
    zentar_home.mkdir()
    assert gu.agent_is_installed(zentar_home) is False
    _make_agent(zentar_home)
    assert gu.agent_is_installed(zentar_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    zentar_home = tmp_path / ".zentar"
    (zentar_home / "zentar-digital-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(zentar_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    zentar_home = tmp_path / ".zentar"
    _make_gui_build(zentar_home)
    artifacts = gu.source_built_gui_artifacts(zentar_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(zentar_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(zentar_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    zentar_home = tmp_path / ".zentar"
    agent_root = _make_agent(zentar_home)
    _make_gui_build(zentar_home)
    _make_user_data(zentar_home)
    ...
    removed = gu.uninstall_gui(zentar_home)
    ...
    assert not (zentar_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "zentar_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (zentar_home / "config.yaml").exists()
    assert (zentar_home / ".env").exists()
    assert (zentar_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    zentar_home = tmp_path / ".zentar"
    _make_agent(zentar_home)
    userdata = tmp_path / "Zentar-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(zentar_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(zentar_home)
    ...
    assert summary["zentar_home"] == str(zentar_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Zentar"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Zentar"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Zentar"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Zentar"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    ZENTAR_HOME with all the destructive externals stubbed out.
    """
    import zentar_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_zentar_home", lambda: zentar_home)
    ...
    from zentar_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (zentar_home / "config.yaml").exists()
    assert zentar_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ZENTAR_HOME non-interactively."""
    import zentar_cli.uninstall as uninstall

    zentar_home = tmp_path / ".zentar"
    (zentar_home / "zentar-digital-agent" / "zentar_cli").mkdir(parents=True)
    (zentar_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not zentar_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m zentar_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_zentar_home", lambda: zentar_home)
    ...
    assert not (desktop / "release").exists()
    assert not (zentar_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "zentar_cli").exists()
    assert (zentar_home / "config.yaml").exists()
