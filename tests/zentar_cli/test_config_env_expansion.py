...
import pytest
from zentar_cli.config import _expand_env_vars, load_config
...
class TestLoadConfigExpansion:
    def test_load_config_expands_env_vars(self, tmp_path, monkeypatch):
        ...
        # zentar_cli.config, making string-target monkeypatches hit a different
...
class TestLoadConfigCacheEnvStaleness:
    ...
    environment (#58514): a load before load_zentar_dotenv() runs, or an env
...
class TestLoadCliConfigExpansion:
    ...
    def test_cli_config_expands_auxiliary_api_key(self, tmp_path, monkeypatch):
        ...
        # Patch the zentar home so load_cli_config finds our test config
