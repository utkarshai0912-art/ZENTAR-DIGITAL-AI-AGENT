...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="zentar")
...
# ── deprecated `zentar login` fails gracefully, not with argparse error ────
#
# `zentar login` is a removed command; its handler (`login_command` in
# `zentar_cli/auth.py`) prints a deprecation notice pointing at `zentar auth` /
# `zentar model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `zentar --help`.
