"""Regression test: `zentar dashboard --tui` must not hard-crash.

Older Zentar desktop app shells (<= 0.15.x) spawn the backend as::

    zentar dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Zentar couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real zentar_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "zentar_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
