...
When ``ZENTAR_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.zentar/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.zentar            (has its own auth.json fixture)
    * Profile     -> tmp_path/.zentar/profiles/coder   (active, ZENTAR_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Zentar is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from zentar_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    zentar_home = tmp_path / "classic"
    zentar_home.mkdir()
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))

    _write(zentar_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ZENTAR_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.zentar as the "real home" — which is NOT equal
    # to the ZENTAR_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    zentar_home = tmp_path / "classic"
    ...
    _write(zentar_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from zentar_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ZENTAR_HOME is set to a custom path that is NOT under
    # ~/.zentar/profiles/ — get_default_zentar_root() returns ZENTAR_HOME
