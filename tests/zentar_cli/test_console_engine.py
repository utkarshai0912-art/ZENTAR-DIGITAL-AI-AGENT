...
from zentar_cli.console_engine import ZentarConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/zentar-console-test.zip",
    "import /tmp/zentar-console-test.zip",
...
def test_console_parses_bare_and_zentar_prefixed_commands(_isolate_zentar_home):
    engine = ZentarConsoleEngine()

    bare = engine.execute("config path")
    prefixed = engine.execute("zentar config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    _isolate_zentar_home,
):
    import zentar_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'zentar doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'zentar setup' to configure\x1b[0m")
        print()

    monkeypatch.setattr(status_mod, "show_status", fake_show_status)

    result = ZentarConsoleEngine().execute("status")

    assert result.status == "ok"
    assert "Sessions" in result.output
    assert "Active: 3 session(s)" in result.output
    assert "zentar doctor" not in result.output
    assert "zentar setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'zentar doctor' for detailed diagnostics"))
        print(osc_link("  Run 'zentar setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    help_text = ZentarConsoleEngine().help_text()
    ...
    assert "Run `zentar skills list`" not in help_text
    assert "Run `zentar tools list`" not in help_text
...
def test_console_help_for_command_uses_cli_summary():
    help_text = ZentarConsoleEngine().help_text("skills list")

    assert help_text == "skills list\nList installed skills"


def test_console_registry_covers_non_admin_cli_surface():
    registered = set(ZentarConsoleEngine().commands)
...
def test_hosted_console_registry_exposes_only_hosted_safe_surface():
    engine = ZentarConsoleEngine(context="hosted")
    hosted = {
...
def test_hosted_console_rejects_local_only_or_dangerous_commands(line):
    result = ZentarConsoleEngine(context="hosted").execute(line)
...
def test_console_rejects_destructive_and_shell_like_commands(line):
    result = ZentarConsoleEngine().execute(line)
...
def test_help_lists_supported_commands_and_not_full_cli():
    result = ZentarConsoleEngine().execute("help")
...
def test_config_set_requires_confirmation_then_writes(_isolate_zentar_home):
    engine = ZentarConsoleEngine()
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_zentar_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_zentar_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_zentar_home):
    ...
    assert code == 0
    assert "Zentar Console" in stdout.getvalue()
    assert "zentar>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_repl_refuses_non_interactive_confirmation(_isolate_zentar_home):
...
def test_main_console_subcommand_smoke(_isolate_zentar_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "zentar_cli.main", "console"],
    ...
    assert "Zentar Console" in result.stdout
