...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ZENTAR_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ZENTAR_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ZENTAR_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from zentar_cli.providers import ZENTAR_OVERLAYS
        assert "bedrock" in ZENTAR_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from zentar_cli.providers import ZENTAR_OVERLAYS
        assert ZENTAR_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from zentar_cli.providers import ZENTAR_OVERLAYS
        assert ZENTAR_OVERLAYS["bedrock"].auth_type == "aws_sdk"
