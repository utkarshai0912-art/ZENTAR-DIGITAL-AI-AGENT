"""Tests for the top-level `./zentar` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./zentar` should use `zentar_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "zentar"
    called = []

    fake_main_module = types.ModuleType("zentar_cli.main")

    def fake_main():
        called.append("zentar_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zentar_cli.main", fake_main_module)
