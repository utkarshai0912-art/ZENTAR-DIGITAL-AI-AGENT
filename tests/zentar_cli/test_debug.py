"""Tests for ``zentar debug`` CLI command and debug utilities."""
...
@pytest.fixture
def zentar_home(tmp_path, monkeypatch):
    """Set up an isolated ZENTAR_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO zentar_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from zentar_cli.debug import _upload_paste_rs
        ...
        with patch("zentar_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "zentar_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from zentar_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from zentar_cli.debug import upload_to_pastebin

        with patch("zentar_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("zentar_cli.debug._upload_paste_rs",
             ...
             patch("zentar_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, zentar_home):
        from zentar_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, zentar_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (zentar_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, zentar_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = zentar_home / "logs" / "agent.log"
        from zentar_cli import debug
    ...
    def test_truncates_large_file(self, zentar_home):
        """Files larger than max_bytes get tail-truncated."""
        from zentar_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (zentar_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, zentar_home):
        ...
        (zentar_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, zentar_home):
    ...
    def test_unknown_log_returns_none(self, zentar_home):
        from zentar_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, zentar_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from zentar_cli.debug import _capture_log_snapshot

        logs_dir = zentar_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, zentar_home):
    ...
    def test_falls_back_when_primary_empty(self, zentar_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ZENTAR_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def zentar_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ZENTAR_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".zentar"
        home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(home))
        ...
        monkeypatch.delenv("ZENTAR_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, zentar_home_with_secret):
        from zentar_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, zentar_home_with_secret):
        from zentar_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, zentar_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ZENTAR_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("ZENTAR_REDACT_SECRETS", "false")

        from zentar_cli.debug import _capture_log_snapshot

        assert os.environ.get("ZENTAR_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, zentar_home_with_secret
    ):
        from zentar_cli.debug import _capture_log_snapshot

        log_path = zentar_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, zentar_home_with_secret):
        from zentar_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from zentar_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, zentar_home):
        from zentar_cli.debug import collect_debug_report

        with patch("zentar_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- zentar dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- zentar dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, zentar_home):
        from zentar_cli.debug import collect_debug_report

        with patch("zentar_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, zentar_home):
        from zentar_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, zentar_home):
        from zentar_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, zentar_home):
        from zentar_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, zentar_home):
        from zentar_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, zentar_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from zentar_cli.debug import run_debug_share
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch("zentar_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("zentar_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, zentar_home, capsys):
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch(
                 "zentar_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, zentar_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, zentar_home, capsys):
        ...
        with patch("zentar_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- zentar dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- zentar dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- zentar dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- zentar dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- zentar dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, zentar_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from zentar_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch("zentar_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("zentar_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, zentar_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, zentar_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, zentar_home_with_secret, capsys
    ):
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch("zentar_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("zentar_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "zentar debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, zentar_home):
        from zentar_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from zentar_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from zentar_cli.debug import delete_paste
        ...
        with patch("zentar_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.zentar/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``zentar debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, zentar_home):
        ...
        import ast
        import inspect
        from zentar_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, zentar_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from zentar_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, zentar_home):
    ...
    def test_merges_with_existing_pending(self, zentar_home):
        """Subsequent calls merge into existing pending.json."""
        from zentar_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, zentar_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, zentar_home):
        from zentar_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, zentar_home):
        from zentar_cli.debug import (
        ...
        with patch("zentar_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, zentar_home):
        from zentar_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("zentar_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, zentar_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, zentar_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, zentar_home):
        from zentar_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("zentar_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, zentar_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: zentar debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from zentar_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("zentar_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("zentar_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, zentar_home, capsys):
        from zentar_cli.debug import run_debug_share
        ...
        with patch("zentar_cli.dump.run_dump"), \
             ...
             patch("zentar_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, zentar_home, capsys):
        from zentar_cli.debug import run_debug_share
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch("zentar_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("zentar_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, zentar_home, capsys):
        from zentar_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``zentar debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, zentar_home):
        from zentar_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("zentar_cli.dump.run_dump"), patch(
            "zentar_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("zentar_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, zentar_home):
        from zentar_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (zentar_home / "logs" / "desktop.log").unlink()

        with patch("zentar_cli.dump.run_dump"), patch(
            "zentar_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, zentar_home):
        from zentar_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, zentar_home):
        from zentar_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, zentar_home):
        from zentar_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, zentar_home):
        from zentar_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, zentar_home):
        from zentar_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, zentar_home):
        from zentar_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, zentar_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from zentar_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, zentar_home):
        import gzip
        import json as _json

        from zentar_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "zentar-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, zentar_home, capsys):
        from zentar_cli.debug import run_debug_share
        ...
        with patch("zentar_cli.dump.run_dump"), patch(
            "zentar_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, zentar_home, capsys):
        from zentar_cli.debug import run_debug_share

        with patch("zentar_cli.dump.run_dump"), patch(
            "zentar_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, zentar_home):
        from zentar_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("zentar_cli.dump.run_dump"), patch(
            "zentar_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("zentar_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``ZentarCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("zentar_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`zentar debug share` requires explicit consent before uploading.
    ...
    def test_aborts_on_user_decline(self, zentar_home, capsys, monkeypatch):
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch("zentar_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())

        mock_upload.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_proceeds_on_user_accept(self, zentar_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, zentar_home, capsys, monkeypatch):
    ...
    def test_non_interactive_requires_yes(self, zentar_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, zentar_home, capsys, monkeypatch):
    ...
    def test_nous_path_also_gated(self, zentar_home, capsys, monkeypatch):
        ...
        with patch("zentar_cli.dump.run_dump"), \
             patch("zentar_cli.diagnostics_upload.share_to_nous") as mock_nous:
            run_debug_share(self._args(nous=True))

        mock_nous.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_local_never_prompts(self, zentar_home, capsys, monkeypatch):
