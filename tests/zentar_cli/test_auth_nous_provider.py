...
from zentar_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from zentar_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ZENTAR_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_zentar_ca_bundle_env_falls_back(self, monkeypatch):
        from zentar_cli.auth import _resolve_verify

        monkeypatch.setenv("ZENTAR_CA_BUNDLE", "/nonexistent/zentar-ca.pem")
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    import zentar_cli.auth as auth_mod
...
def test_resolve_nous_runtime_credentials_env_override_wins_live_not_persisted(
    ...
    monkeypatch.setattr("zentar_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
    monkeypatch.setenv("ZENTAR_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="zentar_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from zentar_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    zentar_home = tmp_path / "zentar"
    _setup_nous_auth(zentar_home, access_token="at-123")
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    zentar_home = tmp_path / "zentar"
    _setup_nous_auth(zentar_home, access_token="at-fresh")
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zentar_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from zentar_cli import auth as auth_mod
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    zentar_home = tmp_path / "zentar"
    _setup_nous_auth(zentar_home, refresh_token="refresh-old")
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `zentar model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return zentar_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from zentar_cli.auth import PROVIDER_REGISTRY, _login_nous

        zentar_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `zentar auth add nous`
    ...
    """
    from zentar_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Zentar is not logged into Nous Portal").
    ...
    # doesn't raise "Zentar is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `zentar auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Zentar must surface an
    ...
    re-authenticate" string led users to report this as a Zentar persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from zentar_cli.auth import _refresh_access_token
    ...
    assert "zentar auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ZENTAR_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("ZENTAR_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ZENTAR_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.zentar/shared/`` across CI runs.
    """
    from zentar_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ZENTAR_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ZENTAR_SHARED_AUTH_DIR must redirect the path."""
    from zentar_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ZENTAR_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from zentar_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from zentar_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from zentar_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `zentar auth add nous
    ...
    zentar_home = tmp_path / "zentar"
    zentar_home.mkdir(parents=True, exist_ok=True)
    (zentar_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from zentar_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "zentar-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different ZENTAR_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ZENTAR_HOME", str(profile_a))
    ...
    # Profile B: fresh ZENTAR_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("ZENTAR_HOME", str(profile_b))
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from zentar_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
