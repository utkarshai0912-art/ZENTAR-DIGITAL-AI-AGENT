...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZENTAR_HOME with .container-mode file."""
    zentar_home = tmp_path / ".zentar"
    zentar_home.mkdir()
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
    monkeypatch.delenv("ZENTAR_DEV", raising=False)

    container_mode = zentar_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=zentar-digital-agent\n"
        "exec_user=zentar\n"
        "zentar_bin=/data/current-package/bin/zentar\n"
    )
    return zentar_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zentar_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "zentar-digital-agent"
    assert info["exec_user"] == "zentar"
    assert info["zentar_bin"] == "/data/current-package/bin/zentar"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zentar_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zentar_dev(container_env, monkeypatch):
    """Returns None when ZENTAR_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZENTAR_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zentar_dev_zero(container_env, monkeypatch):
    """ZENTAR_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ZENTAR_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zentar_home = Path(tmpdir) / ".zentar"
        zentar_home.mkdir()
        (zentar_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zentar_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_zentar_home": lambda: zentar_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZENTAR_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zentar-custom\n"
        "exec_user=myuser\n"
        "zentar_bin=/opt/zentar/bin/zentar\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zentar-custom"
    assert info["exec_user"] == "myuser"
    assert info["zentar_bin"] == "/opt/zentar/bin/zentar"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zentar_cli.main import _exec_in_container
