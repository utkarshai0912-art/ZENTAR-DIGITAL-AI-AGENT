...
def zentar_home(tmp_path, monkeypatch):
    ...
    # /etc/zentar on the dev/CI box can't influence the test.
    monkeypatch.setenv("ZENTAR_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(zentar_home, monkeypatch):
    from zentar_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(zentar_home, monkeypatch):
    from zentar_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(zentar_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_zentar_dotenv(zentar_home=str(home))
