"""Tests for the Zentar plugin system (zentar_cli.plugins)."""
...
from zentar_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<zentar_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<zentar_home>/plugins/``; we derive
    ``<zentar_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ZENTAR_HOME>/config.yaml.
        # Config is always read from ZENTAR_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        zentar_home_str = os.environ.get("ZENTAR_HOME")
        if zentar_home_str:
            zentar_home = Path(zentar_home_str)
        else:
            zentar_home = base.parent
        zentar_home.mkdir(parents=True, exist_ok=True)
        cfg_path = zentar_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.zentar/plugins/ are discovered."""
        plugins_dir = tmp_path / "zentar_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / "zentar_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("zentar_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".zentar" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        zentar_home = tmp_path / "zentar_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under zentar_plugins.<name>."""
        plugins_dir = tmp_path / "zentar_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / "zentar_test"))

        # Clean up any prior namespace module
        sys.modules.pop("zentar_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "zentar_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="zentar_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "zentar_cli.plugins.invoke_hook",
...
class TestPreToolCallDirective:
    ...
    def test_approve_directive_returned(self, monkeypatch):
        from zentar_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "zentar_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_block_returns_message(self, monkeypatch):
        from zentar_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "zentar_cli.plugins.invoke_hook",
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from zentar_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "zentar_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("zentar_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("zentar_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from zentar_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from zentar_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("zentar_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import zentar_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `zentar plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("zentar_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / "zentar_test"
        plugins_dir = zentar_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "zentar_test" / "plugins"
        # Set ZENTAR_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("zentar_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("zentar_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("zentar_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("zentar_cli.plugins.PluginContext.dispatch_tool.__module__", "zentar_cli.plugins"):
...
class TestPluginDebugLogging:
    """ZENTAR_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ZENTAR_PLUGINS_DEBUG", raising=False)
        from zentar_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ZENTAR_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ZENTAR_PLUGINS_DEBUG", "1")
        from zentar_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from ZENTAR_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """ZENTAR_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """ZENTAR_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".zentar" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".zentar" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(prof))
        ctx = self._ctx()
