"""Tests for the Kanban DB layer (zentar_cli.kanban_db)."""
...
from zentar_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated ZENTAR_HOME with an empty kanban DB."""
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("ZENTAR_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("ZENTAR_HOME", str(home))
    monkeypatch.delenv("ZENTAR_KANBAN_DB", raising=False)
    monkeypatch.delenv("ZENTAR_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("ZENTAR_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import zentar_cli.kanban_db as _kb
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("ZENTAR_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """ZENTAR_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import zentar_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("ZENTAR_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("ZENTAR_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("ZENTAR_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("ZENTAR_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("ZENTAR_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "ZENTAR_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from zentar_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Zentar profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="zentar-task", assignee="daily")
...
def test_scratch_workspace_created_under_zentar_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import zentar_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Zentar-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``ZENTAR_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("ZENTAR_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Zentar' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Zentar' DB/metadata/logs on task completion.
    """
...
# spawned with `zentar -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's ZENTAR_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's ZENTAR_HOME."""

    def _set_home(self, monkeypatch, tmp_path, zentar_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: ZENTAR_HOME == ~/.zentar, no profile active.
        default_home = tmp_path / ".zentar"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.zentar/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.zentar/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.zentar/kanban.db.
        default_home = tmp_path / ".zentar"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ZENTAR_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `zentar -p coder`).
        monkeypatch.setenv("ZENTAR_HOME", str(profile_home))
    ...
    def test_docker_custom_zentar_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: ZENTAR_HOME points outside ~/.zentar.
        # `get_default_zentar_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".zentar"`.
        custom_root = tmp_path / "opt" / "zentar"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: ZENTAR_HOME=/opt/zentar/profiles/coder;
        # `get_default_zentar_root()` walks up to /opt/zentar because
    ...
    def test_explicit_override_via_zentar_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: ZENTAR_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".zentar"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(profile_home))
        monkeypatch.setenv("ZENTAR_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(default_home))
        monkeypatch.setenv("ZENTAR_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # ZENTAR_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile ZENTAR_HOME and reads.
        monkeypatch.setenv("ZENTAR_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_zentar_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ZENTAR_KANBAN_DB pins the file path directly and beats both
        # ZENTAR_KANBAN_HOME and the `get_default_zentar_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(default_home))
        monkeypatch.setenv("ZENTAR_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ZENTAR_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows ZENTAR_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_zentar_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ZENTAR_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".zentar"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("ZENTAR_HOME", str(default_home))
        monkeypatch.setenv("ZENTAR_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ZENTAR_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows ZENTAR_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(default_home))
        monkeypatch.setenv("ZENTAR_KANBAN_DB", "   ")
        monkeypatch.setenv("ZENTAR_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject ZENTAR_KANBAN_DB
        # and ZENTAR_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites ZENTAR_HOME.
        default_home = tmp_path / ".zentar"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["ZENTAR_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["ZENTAR_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["ZENTAR_KANBAN_TASK"] == "t_dispatch_env"
        assert env["ZENTAR_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see zentar_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    (see zentar-digital-agent issue #22032).
    ...
    with _patch("zentar_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="zentar_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'zentar kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_zentar_argv()
#
# Workers spawned by the dispatcher must use a `zentar` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["zentar", ...]`
...
def test_resolve_zentar_argv_prefers_path_shim(monkeypatch):
    """When `zentar` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import zentar_cli.kanban_db as kb

    monkeypatch.delenv("ZENTAR_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/zentar")
    argv = kb._resolve_zentar_argv()
    assert argv == ["/usr/local/bin/zentar"]


def test_resolve_zentar_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import zentar_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ZENTAR_BIN", ".\\zentar.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_zentar_argv() == [os.path.abspath(".\\zentar.exe")]


def test_resolve_zentar_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "zentar.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("ZENTAR_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_zentar_argv() == [sys.executable, "-m", "zentar_cli.main"]


def test_resolve_zentar_argv_honors_zentar_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like ZENTAR_BIN lets service managers pin the executable."""
    import shutil
    import zentar_cli.kanban_db as kb

    shim = tmp_path / "bin" / "zentar"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("ZENTAR_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_zentar_argv() == [str(shim)]


def test_resolve_zentar_argv_zentar_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare ZENTAR_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import zentar_cli.kanban_db as kb

    cwd_hermes = tmp_path / "zentar"
    cwd_hermes.write_text("wrong\n", encoding="utf-8")
    cwd_hermes.chmod(cwd_hermes.stat().st_mode | stat.S_IXUSR)
    path_hermes = tmp_path / "bin" / "zentar"
    ...
    monkeypatch.setenv("ZENTAR_BIN", "zentar")

    assert kb._resolve_zentar_argv() == [str(path_hermes)]


def test_resolve_zentar_argv_zentar_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare ZENTAR_BIN does not accept current-directory shadow executables."""
    import sys
    import zentar_cli.kanban_db as kb

    (tmp_path / "zentar.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_zentar_argv_zentar_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved ZENTAR_BIN batch shim is not used as worker argv[0]."""
    import sys
    import zentar_cli.kanban_db as kb
...
def test_resolve_zentar_argv_zentar_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved ZENTAR_BIN command names do not delegate cwd search to Popen."""
    import sys
    import zentar_cli.kanban_db as kb
...
def test_resolve_zentar_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m zentar_cli.main`.

    Pins the correct module name (NOT `zentar` — there is no top-level
    `zentar` package). Regression for #23198: the original PR shipped
    `python -m zentar` which fails with `No module named zentar` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_zentar_argv()
    assert argv == [sys.executable, "-m", "zentar_cli.main"]


def test_resolve_zentar_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `zentar_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m zentar_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("ZENTAR_BIN", None)
    ...
    assert "ZENTAR DIGITAL AI AGENT" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="zentar_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from zentar_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("zentar_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from zentar_cli.kanban_db import connect, write_txn
    import zentar_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from zentar_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from zentar_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("zentar_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("zentar_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("zentar_cli.kanban_db.os.name", "nt")
    with patch("zentar_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("zentar_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("zentar_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "zentar_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("zentar_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "zentar_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("zentar_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("zentar_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("zentar_cli.kanban_db._record_worker_exit"):
            with patch("zentar_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
