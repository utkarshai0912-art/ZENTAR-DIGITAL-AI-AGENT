"""Regression tests for #34107 — Docker UID/GID handling in ensure_zentar_home.

When Zentar runs in Docker with ``ZENTAR_UID=1000`` / ``ZENTAR_GID=911``,
the entrypoint chowns the top-level ``ZENTAR_HOME`` once at startup. But
...
The fix is a ``_chown_to_zentar_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_zentar_uid_gid
# ---------------------------------------------------------------------------


class TestResolveZentarUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ZENTAR_UID", "1000")
        monkeypatch.setenv("ZENTAR_GID", "911")
        from zentar_cli.config import _resolve_zentar_uid_gid
        uid, gid = _resolve_zentar_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ZENTAR_UID", raising=False)
        monkeypatch.delenv("ZENTAR_GID", raising=False)
        from zentar_cli.config import _resolve_zentar_uid_gid
        uid, gid = _resolve_zentar_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ZENTAR_UID", "not-a-number")
        monkeypatch.setenv("ZENTAR_GID", "911")
        from zentar_cli.config import _resolve_zentar_uid_gid
        uid, gid = _resolve_zentar_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ZENTAR_UID", "")
        monkeypatch.setenv("ZENTAR_GID", "")
        from zentar_cli.config import _resolve_zentar_uid_gid
        uid, gid = _resolve_zentar_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_zentar_uid
# ---------------------------------------------------------------------------


class TestChownToZentarUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZENTAR_UID", "1000")
        monkeypatch.setenv("ZENTAR_GID", "911")
        from zentar_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_zentar_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_zentar_uid(d)  # must not raise
