"""Tests for the secret-source tracking in ``zentar_cli.env_loader``.

These cover the small public surface that lets `zentar model` / `zentar setup`
...
from zentar_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_zentar_dotenv()`` is called at module-import time from several
    hot modules (cli.py, zentar_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per ZENTAR_HOME per process.
    """
    ...
    # cli.py, zentar_cli/main.py, run_agent.py, trajectory_compressor.py,
