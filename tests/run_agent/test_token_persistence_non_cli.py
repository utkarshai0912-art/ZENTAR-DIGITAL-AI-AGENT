...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    zentar_state = ModuleType("zentar_state")
    zentar_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "zentar_state", zentar_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Zentar"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Zentar"
