...
class TestToolsetConsistency:
    ...
    def test_zentar_platforms_share_core_tools(self):
        """All zentar-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on zentar-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["zentar-cli", "zentar-telegram", "zentar-discord", "zentar-whatsapp", "zentar-slack", "zentar-signal", "zentar-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_zentar_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zentar-whatsapp")

    def test_zentar_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zentar-api-server")
