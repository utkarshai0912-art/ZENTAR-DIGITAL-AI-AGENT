...
profile's ZENTAR_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.zentar/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active ZENTAR_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever ZENTAR_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    ZENTAR_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import zentar_constants

    monkeypatch.setattr(
        zentar_constants, "_get_platform_default_zentar_home", lambda: root
    )
    monkeypatch.setenv("ZENTAR_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile ZENTAR_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "zentar_home"
    ...
    assert zentar_constants.get_zentar_home().resolve() == profile_home.resolve()
    assert zentar_constants.get_default_zentar_root().resolve() == root.resolve()

    # cron/jobs.py computes ZENTAR_DIR from get_zentar_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.ZENTAR_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_zentar_home", None, raising=False)
    assert scheduler._get_zentar_home().resolve() == profile_home.resolve()
    assert scheduler._get_zentar_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (ZENTAR_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.ZENTAR_DIR.resolve() == root.resolve()
