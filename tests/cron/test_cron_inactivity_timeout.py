...
- Unlimited timeout (ZENTAR_CRON_TIMEOUT=0)
- Backward compat: ZENTAR_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """ZENTAR_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """ZENTAR_CRON_TIMEOUT env var is respected."""
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """ZENTAR_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """ZENTAR_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("ZENTAR_CRON_TIMEOUT", "abc")
        raw = os.getenv("ZENTAR_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """ZENTAR_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("ZENTAR_CRON_TIMEOUT", "")
        raw = os.getenv("ZENTAR_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_zentar_time_importable(self):
        """zentar_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _zentar_now
        assert callable(_zentar_now)

    def test_zentar_constants_importable(self):
        """zentar_constants should be importable from cron context."""
        from zentar_constants import get_zentar_home
        assert callable(get_zentar_home)
