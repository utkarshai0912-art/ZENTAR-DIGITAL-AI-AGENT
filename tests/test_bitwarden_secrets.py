...
binary download are exercised manually by `zentar secrets bitwarden
...
def zentar_home(tmp_path, monkeypatch):
    """Point Zentar at an isolated home directory."""
    home = tmp_path / ".zentar"
    home.mkdir()
    monkeypatch.setenv("ZENTAR_HOME", str(home))
    # Some modules cache get_zentar_home; clear if needed.
    import zentar_constants
    if hasattr(zentar_constants, "_ZENTAR_HOME_CACHE"):
        zentar_constants._ZENTAR_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(zentar_home, monkeypatch):
...
def test_install_bws_happy_path(zentar_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(zentar_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(zentar_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from zentar_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under zentar_home/cache/."""
    home = tmp_path / ".zentar"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
