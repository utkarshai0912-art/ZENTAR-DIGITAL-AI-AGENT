...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="zentar/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is ZENTAR_HOME (config/state) must not spawn a
    # phantom project; it falls through to flat Recents (unscoped). A real repo
    # alongside it still groups normally.
    resolve = _resolver(
        {
            "/home/me/.zentar": ("/home/me/.zentar", "/home/me/.zentar"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.zentar", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.zentar"
...
def test_junk_root_is_dropped_from_the_discovered_tier():
    discovered = [{"root": "/home/me/.zentar", "label": ".zentar", "sessions": 0, "last_active": 9}]

    tree = pt.build_tree([], [], discovered, resolve=None, is_junk_root=lambda r: r == "/home/me/.zentar")
