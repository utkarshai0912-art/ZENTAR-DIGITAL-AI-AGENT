...
def server():
    with patch.dict(
        "sys.modules",
        {
            "zentar_constants": MagicMock(
                get_zentar_home=MagicMock(return_value="/tmp/zentar_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'zentar-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'zentar-release' patched"
    }
