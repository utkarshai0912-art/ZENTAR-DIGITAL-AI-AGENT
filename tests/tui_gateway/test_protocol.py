...
def server():
    with patch.dict("sys.modules", {
        "zentar_constants": MagicMock(get_zentar_home=MagicMock(return_value="/tmp/zentar_test")),
        "zentar_cli.env_loader": MagicMock(),
        "zentar_cli.banner": MagicMock(),
        "zentar_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ZENTAR_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("ZENTAR_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ZENTAR_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from zentar_cli.active_sessions import (
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/zentar-digital-agent-dev": {"name": "zentar-digital-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "zentar-digital-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "zentar_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "zentar-digital-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "zentar-digital-agent-dev"
