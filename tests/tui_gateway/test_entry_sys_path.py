...
Zentar's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``zentar_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call zentar_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call zentar_bootstrap.harden_import_path()"
    )
...
def test_entry_does_not_reimplement_guard_inline():
    ...
    assert '{"", "."}' not in source and "{'', '.'}" not in source, (
        "entry.py should delegate to zentar_bootstrap.harden_import_path, "
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Zentar root.  harden_import_path must relocate the
    Zentar root to the front so ``from utils import ...`` resolves to Zentar."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/zentar", "/usr/lib"]
        zentar_bootstrap.harden_import_path(src_root="/opt/zentar")
        assert sys.path[0] == "/opt/zentar"
        assert sys.path.index("/opt/zentar") < sys.path.index(
