...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and zentar-digital-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # zentar-digital-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_zentar_home():
    prev = os.environ.get("ZENTAR_HOME")
    with tempfile.TemporaryDirectory() as d:
        os.environ["ZENTAR_HOME"] = d
        try:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    prev_home = os.environ.get("ZENTAR_HOME")
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("ZENTAR_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
