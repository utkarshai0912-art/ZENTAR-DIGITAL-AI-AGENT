...
Zentar state stays profile-scoped through ZENTAR_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/hermes-digital-agent/issues/25114
See: https://github.com/NousResearch/hermes-digital-agent/issues/36144
See: https://github.com/NousResearch/hermes-digital-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for zentar_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(zentar_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("ZENTAR_REAL_HOME", raising=False)
    ...
    def test_returns_none_when_zentar_home_unset(self, monkeypatch):
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        zentar_home = real_home / ".zentar" / "profiles" / "coder"
        profile_home = zentar_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_REAL_HOME", str(real_home))

        from zentar_constants import get_subprocess_home, get_real_home
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from zentar_constants import get_real_home

        assert get_real_home() != str(profile_home)

    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".zentar" / "profiles"
        ...
        monkeypatch.setenv("ZENTAR_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ZENTAR_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_zentar_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_zentar_home_override(profile)
        try:
            assert get_zentar_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["ZENTAR_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(zentar_home / "home")
        assert result["ZENTAR_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_zentar_home_unset(self, monkeypatch):
        monkeypatch.delenv("ZENTAR_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["ZENTAR_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from zentar_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from zentar_cli.profiles import create_profile
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(zentar_home / "home"), original_home)
