...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"zentar-digital-agent[termux]"' in text
    assert '"zentar-digital-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"zentar-digital-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
