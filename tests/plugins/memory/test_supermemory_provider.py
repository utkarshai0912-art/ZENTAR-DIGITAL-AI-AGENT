...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", zentar_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Zentar"],
        search_results=[{"memory": "Uses Zentar", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Zentar") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Zentar memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    ...
    assert merged["sm_source"] == "zentar"
    ...
    assert merged2["sm_source"] == "zentar"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "zentar-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", zentar_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "zentar_coder"
...
def test_identity_template_default_profile(monkeypatch, tmp_path):
    ...
    _save_supermemory_config({"container_tag": "zentar-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", zentar_home=str(tmp_path), platform="cli")
    assert p._container_tag == "zentar_default"
...
def test_multi_container_enabled_adds_schema_param(monkeypatch, tmp_path):
    ...
    assert p._allowed_containers == ["zentar", "project_alpha", "shared"]
...
def test_format_connection_summary_ok():
    summary = _format_connection_summary({
        "ok": True,
        "container_tag": "zentar_coder",
        "profile_facts": 12,
        "auto_recall": True,
        "auto_capture": False,
    })
    assert "✓ Connected" in summary
    assert "container: zentar_coder" in summary
...
def test_format_connection_summary_single_fact_and_error():
    ...
    assert "container: zentar" in err
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "zentar"
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("zentar_cli.config.save_config", fake_save_config)
