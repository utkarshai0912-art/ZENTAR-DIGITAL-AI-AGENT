...
Run from the PR worktree:
    ...
    cd ~/.zentar/zentar-digital-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.zentar/zentar-digital-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.zentar/zentar-digital-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a zentar-digital-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a zentar-digital-agent checkout"
)
...
# Isolated ZENTAR_HOME for the config write.
