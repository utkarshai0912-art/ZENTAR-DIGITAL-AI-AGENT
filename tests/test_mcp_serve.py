"""
Tests for mcp_serve — Zentar MCP server.
...
def _isolate_zentar_home(tmp_path, monkeypatch):
    """Redirect ZENTAR_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking zentar_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <ZENTAR_HOME>/state.db for its mtime gate; the autouse
        # fixture points ZENTAR_HOME at tmp_path.
        db_path = tmp_path / "state.db"
