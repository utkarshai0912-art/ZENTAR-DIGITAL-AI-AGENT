"""Tests for ZentarCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a ZentarCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "ZENTAR_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), \
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
            ...
            return _cli_mod.ZentarCLI(**kwargs)
...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"ZENTAR_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"ZENTAR_MAX_ITERATIONS": "not-a-number"})
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "nous", "model": "Zentar-4"},
        })
        assert cli._fallback_model == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "nous", "model": "Zentar-4"},
        ]
...
class TestHistoryDisplay:
    ...
    def test_history_shows_recent_sessions_when_current_chat_is_empty(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running ZENTAR DIGITAL AI AGENT",
                "preview": "check running gateways for zentar agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running ZENTAR DIGITAL AI AGENT" in output
    ...
    def test_resume_updates_zentar_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["ZENTAR_SESSION_ID"] = "current_session"
        _VAR_MAP["ZENTAR_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["ZENTAR_SESSION_ID"] == "target_session"
            assert get_session_env("ZENTAR_SESSION_ID") == "target_session"
        finally:
            cli._session_db.close()
            os.environ.pop("ZENTAR_SESSION_ID", None)
            _VAR_MAP["ZENTAR_SESSION_ID"].set(_UNSET)
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running ZENTAR DIGITAL AI AGENT")

        mock_resume.assert_called_once_with(
            "/resume Checking Running ZENTAR DIGITAL AI AGENT"
        )
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        monkeypatch.setattr(cli, "_zentar_home", zentar_home)
    ...
    def test_normalize_model_name_save_roundtrip_migrates_key(self, tmp_path, monkeypatch):
        ...
        import zentar_cli.config as cfgmod
