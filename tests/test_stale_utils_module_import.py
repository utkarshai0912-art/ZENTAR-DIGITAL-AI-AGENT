...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.zentar/zentar-digital-agent/utils.py)
...
2. ``zentar update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``zentar_cli/main.py`` (the ``zentar update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``zentar_constants`` after the
