...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        zentar_home.mkdir()
        (zentar_home / "config.yaml").write_text(content, encoding="utf-8")
        return zentar_home
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        zentar_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        zentar_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        monkeypatch.delenv("TELEGRAM_REPLY_TO_MODE", raising=False)
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/NousResearch/hermes-digital-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Zentar-created DM topic
