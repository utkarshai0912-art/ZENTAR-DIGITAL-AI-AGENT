...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.zentar/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("ZENTAR_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("ZENTAR_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("ZENTAR_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ZENTAR_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ZENTAR_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ZENTAR_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ZENTAR_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ZENTAR_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ZENTAR_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.zentar/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_zentar_credentials(self, tmp_path, monkeypatch):
        """~/.zentar/.env and ~/.zentar/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        zentar_dir = fake_home / ".zentar"
        zentar_dir.mkdir(parents=True)
        env_file = zentar_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._ZENTAR_HOME",
            zentar_dir,
        )
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.zentar/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        zentar_dir = fake_home / ".zentar"
        (zentar_dir / "mcp-tokens").mkdir(parents=True)
        secret = zentar_dir / rel
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._ZENTAR_ROOT",
            zentar_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(secret)) is None

    def test_denylist_blocks_zentar_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        zentar_dir = fake_home / ".zentar"
        zentar_dir.mkdir(parents=True)
        config_file = zentar_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_zentar_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Zentar root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".zentar" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        zentar_root = fake_home / ".zentar"
        config_file = zentar_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the ZENTAR_HOME root (google_token.json)
        ...
        zentar_dir = fake_home / ".zentar"
        zentar_dir.mkdir(parents=True)
        token = zentar_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._ZENTAR_HOME", zentar_dir)
        monkeypatch.setattr("gateway.platforms.base._ZENTAR_ROOT", zentar_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.zentar/pairing/ (platform pairing tokens) are
    ...
    def test_zentar_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        zentar_dir = fake_home / ".zentar"
        cache_dir = zentar_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_zentar_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.zentar
        ...
        zentar_dir = fake_home / ".zentar"
        zentar_dir.mkdir(parents=True)
        artifact = zentar_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting ZENTAR_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``ZENTAR_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("ZENTAR_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_zentar_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.zentar/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose ZENTAR_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.zentar) emits
        ...
        zentar_root = denied_root / ".zentar"
        prof_cache = zentar_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._ZENTAR_ROOT", zentar_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        zentar_root = denied_root / ".zentar"
        prof_dir = zentar_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"ZENTAR_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZENTAR_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"ZENTAR_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "ZENTAR_HUMAN_DELAY_MODE": "natural",
            "ZENTAR_HUMAN_DELAY_MIN_MS": "oops",
            "ZENTAR_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "ZENTAR_HUMAN_DELAY_MODE": "custom",
            "ZENTAR_HUMAN_DELAY_MIN_MS": "100",
            "ZENTAR_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"ZENTAR_MEDIA_DELIVERY_STRICT": "1",
                                     "ZENTAR_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Zentar
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../zentar/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.zentar/cache/media/sensitive_host_path_abc123.bin"
