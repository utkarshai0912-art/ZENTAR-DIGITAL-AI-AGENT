...
closure the PR changed, against a real temp ``ZENTAR_HOME``.
"""
...
def _fake_switch_result():
    ...
    from zentar_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "zentar_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_zentar_home() / config.yaml`` — point it here.
    monkeypatch.setattr("zentar_constants.get_zentar_home", lambda: zentar_home)
    monkeypatch.setattr("zentar_cli.config.get_zentar_home", lambda: zentar_home)
    return cfg_path
