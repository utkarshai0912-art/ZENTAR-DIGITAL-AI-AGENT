...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under ZENTAR_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.zentar/data/watchlist.json",
                            },
                            {
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_zentar_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.zentar/scripts paths must resolve through ZENTAR_HOME for profiles."""
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.zentar/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
