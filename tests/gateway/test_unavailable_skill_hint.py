...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`zentar_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + ZENTAR_HOME so the real user config is untouched."""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `zentar skills config` writes).
    monkeypatch.setattr(
    ...
    assert "zentar skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "zentar_constants.get_optional_skills_dir",
