...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_zentar_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Zentar, summarize this")
        assert adapter._message_matches_mention_patterns("@ZENTAR DIGITAL AI AGENT help")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Zentar what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Zentar, summarize this") == "summarize this"
        assert adapter._clean_mention_text("ZENTAR DIGITAL AI AGENT: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Zentar about this") == "please ask Zentar about this"
