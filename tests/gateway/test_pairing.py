...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_zentar_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <ZENTAR_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``zentar pairing`` CLI continues to work without a profile context."""
        from zentar_constants import get_zentar_home
        monkeypatch.setattr("zentar_constants.get_zentar_home", lambda: tmp_path)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <ZENTAR_HOME>/profiles/yangyang/pairing/."""
        from zentar_constants import get_zentar_home
        monkeypatch.setattr("zentar_constants.get_zentar_home", lambda: tmp_path)
