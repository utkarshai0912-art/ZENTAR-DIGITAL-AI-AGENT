...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_zentar_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("zentar_constants.display_zentar_home", return_value="~/.zentar/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.zentar/profiles/coder/cron/output/" in prompt
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import zentar_state
        monkeypatch.setattr(zentar_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_dm_aliases_share_one_session_key(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_home))
