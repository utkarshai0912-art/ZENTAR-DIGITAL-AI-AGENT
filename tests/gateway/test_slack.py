...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.zentar."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /zentar, /btw, /stop,
        ...
        for expected in ("/zentar", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@zentar`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@zentar"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@zentar")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /zentar subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /zentar btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_zentar_prefix_still_works(self, adapter):
        """Backward compat: /zentar btw foo must still route to /btw foo.

        Old workspace manifests only declared /zentar as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_zentar_freeform_question(self, adapter):
        """/zentar <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/zentar",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_zentar_slash_stashes_context(self, adapter):
        """Legacy /zentar <subcommand> also stashes context."""
        command = {
            "command": "/zentar",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_zentar_question_does_not_stash_context(self, adapter):
        """Free-form /zentar <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/zentar",
