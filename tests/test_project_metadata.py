...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install zentar-digital-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"zentar-digital-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `zentar update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid zentar update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `zentar update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `zentar update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (zentar_cli/dingtalk_auth.py)
...
def test_nemo_relay_extra_uses_official_0_3_distribution():
    ...
    assert not any(
        spec == "zentar-digital-agent[nemo-relay]"
        for spec in optional_dependencies["all"]
    )
