"""Shared fixtures for the zentar-digital-agent test suite.
...
2. **Isolated ZENTAR_HOME.** ZENTAR_HOME points to a per-test tempdir so
   code reading ``~/.zentar/*`` via ``get_zentar_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".zentar"`` instead of the canonical
   ``get_zentar_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZENTAR_SESSION_* inheritance** — the agent's current gateway
...
_ZENTAR_BEHAVIORAL_VARS = frozenset({
    "ZENTAR_YOLO_MODE",
    "ZENTAR_INTERACTIVE",
    "ZENTAR_QUIET",
    "ZENTAR_TOOL_PROGRESS",
    "ZENTAR_TOOL_PROGRESS_MODE",
    "ZENTAR_MAX_ITERATIONS",
    "ZENTAR_SESSION_PLATFORM",
    "ZENTAR_SESSION_CHAT_ID",
    "ZENTAR_SESSION_CHAT_NAME",
    "ZENTAR_SESSION_THREAD_ID",
    "ZENTAR_SESSION_SOURCE",
    "ZENTAR_SESSION_KEY",
    "ZENTAR_GATEWAY_SESSION",
    "ZENTAR_CRON_SESSION",
    "_ZENTAR_GATEWAY",
    "ZENTAR_PLATFORM",
    "ZENTAR_MODEL",
    "ZENTAR_INFERENCE_MODEL",
    "ZENTAR_INFERENCE_PROVIDER",
    "ZENTAR_TUI_PROVIDER",
    "ZENTAR_MANAGED",
    "ZENTAR_MANAGED_DIR",
    "ZENTAR_DEV",
    ...
    "ZENTAR_TIMEZONE",
    "ZENTAR_REDACT_SECRETS",
    "ZENTAR_BACKGROUND_NOTIFICATIONS",
    "ZENTAR_EXEC_ASK",
    "ZENTAR_HOME_MODE",
    "ZENTAR_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.zentar/kanban.db instead of the per-test ZENTAR_HOME.
    "ZENTAR_KANBAN_DB",
    "ZENTAR_KANBAN_BOARD",
    "ZENTAR_KANBAN_HOME",
    "ZENTAR_KANBAN_WORKSPACES_ROOT",
    "ZENTAR_KANBAN_LOGS_ROOT",
    "ZENTAR_KANBAN_TASK",
    "ZENTAR_KANBAN_WORKSPACE",
    "ZENTAR_KANBAN_RUN_ID",
    "ZENTAR_KANBAN_CLAIM_LOCK",
    "ZENTAR_KANBAN_DISPATCH_IN_GATEWAY",
    "ZENTAR_TENANT",
    ...
    "ZENTAR_DASHBOARD_OAUTH_CLIENT_ID",
    "ZENTAR_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZENTAR_HOME to per-test tempdirs so code that
    reads ``~/.zentar/*`` can't touch the real one, and pins TZ/LANG so
    ...
    for name in _ZENTAR_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ZENTAR_HOME to a per-test tempdir. Code that reads
    #    ``~/.zentar/*`` via ``get_zentar_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.zentar/*`` via
    #    ``Path.home() / ".zentar"`` instead of ``get_zentar_home()``
    #    is a bug to fix at the callsite.
    fake_zentar_home = tmp_path / "zentar_test"
    fake_zentar_home.mkdir()
    (fake_zentar_home / "sessions").mkdir()
    (fake_zentar_home / "cron").mkdir()
    (fake_zentar_home / "memories").mkdir()
    (fake_zentar_home / "skills").mkdir()
    monkeypatch.setenv("ZENTAR_HOME", str(fake_zentar_home))
    ...
    #    ~/.zentar/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zentar_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_zentar_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal zentar config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``zentar-gateway`` process
...
# correlated with a ``tests/zentar_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> zentar-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart zentar-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting zentar/python patterns are also blocked.
    """
    ...
    def _matches_zentar_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZENTAR_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zentar_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch zentar-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m zentar_cli.main".
                if (
                    "zentar" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live zentar-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting zentar/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `zentar update` (or the
        # equivalent `python -m zentar_cli.main update`).  These commands
        ...
        if "update" in low and (
            # zentar update / zentar update --gateway / setsid bash -c ... zentar update
            ("zentar" in low and "update" in low.split())
            or
            # python -m zentar_cli.main update --gateway
            ("zentar_cli" in low and "update" in low.split())
            or
            # venv/bin/zentar update  (absolute path variant used in tests)
            (".venv/bin/zentar" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`zentar update` against the real checkout, fetching "
