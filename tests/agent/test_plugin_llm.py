...
""",
        ...
        from zentar_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        from zentar_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from zentar_cli.plugins import PluginContext, PluginManifest, PluginManager
