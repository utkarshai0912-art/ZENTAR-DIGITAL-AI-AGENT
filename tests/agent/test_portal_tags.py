...
def test_zentar_client_tag_includes_current_version():
    """The client tag must reflect zentar_cli.__version__ verbatim."""
    from zentar_cli import __version__
    from agent.portal_tags import zentar_client_tag

    assert zentar_client_tag() == f"client=zentar-client-v{__version__}"


def test_zentar_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import zentar_client_tag

    tag = zentar_client_tag()
    assert tag.startswith("client=zentar-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import zentar_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=zentar-digital-agent" in tags
    assert zentar_client_tag() in tags
    assert len(tags) == 2
