...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_zentar_env(self):
        """Zentar' own .env inside ZENTAR_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: zentar internal .env is in ~/.zentar/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.zentar/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.zentar/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Zentar cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        zentar_home = tmp_path / ".zentar"
        cache = zentar_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._zentar_home_path", return_value=zentar_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Zentar cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        zentar_home = tmp_path / ".zentar"
        hub = zentar_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_zentar_home(self, tmp_path):
        """The env basename guard does not depend on ZENTAR_HOME resolution."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        zentar_home = tmp_path / ".zentar"
        cache = zentar_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
