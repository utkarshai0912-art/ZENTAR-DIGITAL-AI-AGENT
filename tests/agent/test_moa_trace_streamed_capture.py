...
path via ``zentar chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ZENTAR_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ZENTAR_HOME at a temp dir and turn moa.save_traces on."""
    zentar_home = tmp_path / ".zentar"
    zentar_home.mkdir()
    monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))

    # save_moa_turn reads config via zentar_cli.config.load_config; stub it to
