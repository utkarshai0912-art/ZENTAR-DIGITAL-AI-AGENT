...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ZENTAR_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="zentar_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".zentar"))
    prev_home = os.environ.get("ZENTAR_HOME")
    os.environ["ZENTAR_HOME"] = os.path.join(test_home, ".zentar")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("zentar_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["ZENTAR_HOME"] = prev_home
