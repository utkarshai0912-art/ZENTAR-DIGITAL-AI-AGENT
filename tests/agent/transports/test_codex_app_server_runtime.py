...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Zentar passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ZENTAR_HOME", "/users/alice/.zentar/profiles/backend-worker")
        monkeypatch.setenv("ZENTAR_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ZENTAR_KANBAN_DB",
            "/users/alice/.zentar/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.zentar/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through zentar_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Zentar secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "ZENTAR_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "ZENTAR_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through zentar_subprocess_env must not
