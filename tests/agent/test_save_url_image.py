...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ZENTAR_HOME under tmp_path."""
    monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / ".zentar"))
    (tmp_path / ".zentar").mkdir()

    # Force the constants/image cache helpers to re-read ZENTAR_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("zentar_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_zentar_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ZENTAR_HOME — gateway cleanup
