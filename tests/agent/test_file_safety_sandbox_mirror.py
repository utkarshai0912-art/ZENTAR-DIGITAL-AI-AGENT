...
Those paths look like ``…/sandboxes/<backend>/<task>/home/.zentar/…`` and
they accumulate divergent copies of authoritative profile state (SOUL.md,
config.yaml, memories/*.md) because the host Zentar process never reads
...
class TestClassifySandboxMirrorTarget:
    def test_docker_mirror_soul_md_classified(self, tmp_path):
        ...
        target = (
            tmp_path
            / "profiles" / "group1"
            / "sandboxes" / "docker" / "default" / "home" / ".zentar"
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        ...
        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".zentar"
            / Path(inner)
        )
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        """A plain Zentar path is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target

        target = tmp_path / ".zentar" / "profiles" / "group1" / "SOUL.md"
        target.parent.mkdir(parents=True)
        target.write_text("# real SOUL\n")

        assert classify_sandbox_mirror_target(str(target)) is None

    def test_sandboxes_segment_without_home_zentar_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to Zentar-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_zentar_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-zentar`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.zentar/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestGetSandboxMirrorWarning:
    ...
    def test_mirror_warning_names_mirror_root_and_inner_path(self, tmp_path):
        ...
        assert "sandboxes/docker/default/home/.zentar" in warn
    ...
    def test_warning_is_defense_in_depth_not_boundary(self, tmp_path):
        ...
        target = (
            tmp_path
            / "sandboxes" / "docker" / "t" / "home" / ".zentar"
            / "profiles" / "g" / "SOUL.md"
        )
        target.parent.mkdir(parents=True)
        target.write_text("x")
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_zentar_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_zentar_home_path", lambda: tmp_path / "profiles" / "group1")
