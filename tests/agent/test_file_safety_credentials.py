"""Tests for ZENTAR_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/hermes-digital-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``ZENTAR_HOME`` itself,
...
for the credential stores while leaving arbitrary ``ZENTAR_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_zentar_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "zentar_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_zentar_home_path", lambda: home)
    return home
...
def test_arbitrary_zentar_home_file_not_blocked(fake_home):
    """Non-credential files inside ZENTAR_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Zentar cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into ZENTAR_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "zentar_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at ZENTAR_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == ZENTAR_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at ZENTAR_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in ZENTAR_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_zentar_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Zentar-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ZENTAR_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ZENTAR_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ZENTAR_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ZENTAR_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_zentar_home_path", lambda: profile)
