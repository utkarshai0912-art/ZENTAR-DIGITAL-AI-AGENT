...
def isolated_home(monkeypatch):
    """Temp ZENTAR_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="zentar_test_31179_")
    zentar_home = os.path.join(test_home, ".zentar")
    os.makedirs(zentar_home)
...
def _fresh_modules():
    """Drop cached zentar modules so each test reloads against current env."""
