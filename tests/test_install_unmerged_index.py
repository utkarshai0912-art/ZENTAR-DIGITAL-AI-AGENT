...
The ``zentar update`` Python path already clears the conflict with ``git reset``
...
def test_install_sh_clears_unmerged_index_then_stashes(tmp_path: Path) -> None:
    repo = tmp_path / "zentar-digital-agent"
    repo.mkdir()
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m zentar_cli.main gateway run`` — image name
    ``pythonw``, not ``zentar.exe`` — so the ``taskkill /IM zentar.exe`` guard
    ...
    # The zentar.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM zentar.exe' in text
