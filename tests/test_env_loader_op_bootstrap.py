...
1. ``load_zentar_dotenv()`` auto-loads ``~/.zentar/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_zentar_dotenv(zentar_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "zentar_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
