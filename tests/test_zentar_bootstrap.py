"""Tests for zentar_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Zentar entry point
(zentar, zentar-digital-agent, zentar-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Zentar entry point imports zentar_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zentar_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zentar_bootstrap", None)
    import zentar_bootstrap  # noqa: WPS433
    return zentar_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Zentar CLI
...
class TestEntryPointsImportBootstrap:
    """Every Zentar entry point must import zentar_bootstrap as its
    ...
    # Entry points that invoke Zentar as a process.  Each one must
    # import zentar_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zentar_cli/main.py",   # zentar CLI (console_script)
        "run_agent.py",          # zentar-digital-agent (console_script)
        "acp_adapter/entry.py",  # zentar-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import zentar_bootstrap' and that
        ...
        half-finished ``zentar update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zentar_bootstrap`` as a top-level module) leaves zentar
        ...
        """
        # Resolve relative to the zentar-digital-agent repo root.  Tests live
        # at tests/test_zentar_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # zentar start even when zentar_bootstrap hasn't been
        ...
        assert first_import_name == "zentar_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zentar_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import zentar_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Zentar's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ZENTAR_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ZENTAR_PYTHON_SRC_ROOT"] = env
            elif "ZENTAR_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ZENTAR_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/zentar")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZENTAR_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ZENTAR_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/zentar", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Zentar root.  The guard must relocate Zentar to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/zentar"])
        assert result[0] == "/opt/zentar"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Zentar root.
        assert result.index("/opt/zentar") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/zentar", "/opt/zentar", ""])
        assert result.count("/opt/zentar") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ZENTAR_PYTHON_SRC_ROOT"] = "/env/zentar"
            hb.harden_import_path()
            assert sys.path[0] == "/env/zentar"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZENTAR_PYTHON_SRC_ROOT", None)
