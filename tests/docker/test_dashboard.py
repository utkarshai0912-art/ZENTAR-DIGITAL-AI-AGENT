"""Harness: dashboard opt-in via ZENTAR_DASHBOARD.

Today (tini): dashboard starts once when ZENTAR_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``zentar`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without ZENTAR_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "zentar dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without ZENTAR_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without ZENTAR_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `zentar doctor` and any other health check).
    ...
    ZENTAR_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without ZENTAR_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with ZENTAR_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # ZENTAR_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "ZENTAR_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With ZENTAR_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'zentar dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with ZENTAR_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """ZENTAR_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "zentar dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as zentar, so the
    # zentar user can kill it (same UID).
...
# whenever `ZENTAR_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``ZENTAR_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        built_image, container_name,
        "ZENTAR_DASHBOARD=1",
        "ZENTAR_DASHBOARD_HOST=0.0.0.0",
        "ZENTAR_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"ZENTAR_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and ZENTAR_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``ZENTAR_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
