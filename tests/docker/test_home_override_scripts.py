...
def test_dashboard_service_resets_home(
    ...
    container with ZENTAR_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    ZENTAR_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "ZENTAR_DASHBOARD=1", "ZENTAR_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to zentar:zentar on every boot.
    ...
    # Verify files are now owned by zentar.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "zentar" in r.stdout, (
        f"expected zentar-owned files after restart, got: {r.stdout!r} — "
