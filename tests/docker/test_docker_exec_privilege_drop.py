...
The shim (docker/zentar-exec-shim.sh, installed at /opt/zentar/bin/zentar)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> zentar login` would write /opt/data/auth.json as
...
failed: Zentar is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> zentar …`` (defaulting to root) gets dropped to the
   zentar user before the real binary runs.
2. ``docker exec --user zentar <c> zentar …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $ZENTAR_HOME from a ``docker exec`` session land
   as zentar:zentar — the actual user-visible invariant.
4. The ZENTAR_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-zentar-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown zentar:zentar`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to zentar:zentar,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$ZENTAR_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (zentar_cli.container_boot), which s6 runs
    *strictly after* ``01-zentar-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_zentar_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `zentar` with a Python-style `-c` shim equivalent — there's no
    pure-zentar "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `zentar` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `zentar`
    do anything that writes to disk, then check the file's owner.

    Use `zentar config set` which writes config.yaml under ZENTAR_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "zentar", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by zentar, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "zentar:zentar", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected zentar:zentar. "
        "The shim did not drop privileges before invoking zentar."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user zentar already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "zentar", sleep_container,
         "zentar", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user zentar failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up zentar:zentar — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """ZENTAR_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "ZENTAR_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "zentar", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With ZENTAR_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``ZENTAR_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``ZENTAR_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"ZENTAR_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "zentar", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "zentar:zentar", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected zentar:zentar"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/zentar/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid zentar zentar <args>` which resolves `zentar`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> zentar login`` (root) wrote
    ...
    "Zentar is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `zentar config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is zentar-readable.

    Specifically: pretend the operator ran `zentar login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the zentar UID. We use `zentar auth list` since that touches the
    ...
    # `zentar auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `zentar config set` first to
    # provoke a write into ZENTAR_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "zentar", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # ZENTAR_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "zentar", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the zentar user "
