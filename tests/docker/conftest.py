...
Override the image with ``ZENTAR_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ZENTAR_TEST_IMAGE", "zentar-digital-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ZENTAR_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ZENTAR_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged zentar user
# ---------------------------------------------------------------------------
#
# Background: every Zentar runtime path inside the container drops to UID
# 10000 (the ``zentar`` user) via ``s6-setuidgid zentar``. ``docker exec``
...
# fails (PermissionError swallowed) for zentar, so a test that ran as root
...
# helpers below run every probe under ``-u zentar`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: zentar).
