...
Uses a throwaway ZENTAR_HOME so it never touches ~/.zentar. No real names.
"""
...
def _fresh_home():
    """Point ZENTAR_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_zentar_home() at import time)."""
...
# Import AFTER ZENTAR_HOME is set.
...
# a different get_zentar_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ZENTAR_HOME (throwaway): {HOME}")
