"""Tests for zentar_constants module."""
...
from zentar_constants import (
    ...
    find_zentar_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_zentar_root,
    get_zentar_dir,
    get_zentar_home,
    heal_zentar_managed_node,
    zentar_managed_node_tree_present,
    iter_zentar_node_dirs,
    is_container,
    node_tool_runnable,
    parse_reasoning_effort,
    secure_parent_dir,
    with_zentar_node_path,
)


class TestGetDefaultZentarRoot:
    """Tests for get_default_zentar_root() — Docker/custom deployment awareness."""

    def test_no_zentar_home_returns_native(self, tmp_path, monkeypatch):
        """When ZENTAR_HOME is not set, returns ~/.zentar."""
        monkeypatch.delenv("ZENTAR_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_zentar_root() == tmp_path / ".zentar"

    def test_zentar_home_is_native(self, tmp_path, monkeypatch):
        """When ZENTAR_HOME = ~/.zentar, returns ~/.zentar."""
        native = tmp_path / ".zentar"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(native))
        assert get_default_zentar_root() == native

    def test_zentar_home_is_profile(self, tmp_path, monkeypatch):
        """When ZENTAR_HOME is a profile under ~/.zentar, returns ~/.zentar."""
        native = tmp_path / ".zentar"
    ...
    def test_zentar_home_is_docker(self, tmp_path, monkeypatch):
        """When ZENTAR_HOME points outside ~/.zentar (Docker), returns ZENTAR_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(docker_home))
        assert get_default_zentar_root() == docker_home

    def test_zentar_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ZENTAR_HOME outside ~/.zentar is treated as the root."""
        custom = tmp_path / "my-zentar-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(custom))
        assert get_default_zentar_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ZENTAR_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZENTAR_HOME", str(profile))
        assert get_default_zentar_root() == docker_root

    def test_no_zentar_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\zentar, not ~/.zentar."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ZENTAR_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(zentar_constants.sys, "platform", "win32")

        assert get_default_zentar_root() == local_appdata / "zentar"

    def test_no_zentar_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/zentar without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ZENTAR_HOME", raising=False)
        ...
        assert get_default_zentar_root() == home / "AppData" / "Local" / "zentar"


class TestGetZentarHome:
    """Tests for get_zentar_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ZENTAR_HOME is unset on Windows, use %LOCALAPPDATA%\\zentar."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ZENTAR_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(zentar_constants.sys, "platform", "win32")
        monkeypatch.setattr(zentar_constants, "_profile_fallback_warned", False)

        assert get_zentar_home() == local_appdata / "zentar"


class TestZentarManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_zentar_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(zentar_constants.sys, "platform", "win32")
        monkeypatch.setenv("ZENTAR_HOME", str(home))
        monkeypatch.setattr(zentar_constants, "node_tool_runnable", lambda path: True)

        assert find_zentar_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZENTAR_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(zentar_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(zentar_constants, "heal_zentar_managed_node", lambda: False)
        monkeypatch.setattr(
            zentar_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert zentar_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_zentar_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "zentar"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_zentar_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Zentar-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(zentar_constants, "heal_zentar_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_zentar_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(zentar_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(zentar_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".zentar"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import zentar_cli._subprocess_compat as subprocess_compat
...
class TestGetZentarDir:
    """Tests for ``get_zentar_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_zentar_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_zentar_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_zentar_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_zentar_dir("cache/audio", "audio_cache")
