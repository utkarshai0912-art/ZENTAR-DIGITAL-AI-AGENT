"""Tests for ACP Registry metadata shipped with Zentar."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "zentar-digital-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "ZENTAR DIGITAL AI AGENT"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/hermes-digital-agent"
    assert data["website"].startswith("https://hermes-agent.nousresearch.com/")
    assert data["authors"] == ["ZENTAR DIGITAL"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"zentar-digital-agent[acp]=={data['version']}"
    assert uvx["args"] == ["zentar-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"zentar-digital-agent[acp]=={_pyproject_version()}"
    )
