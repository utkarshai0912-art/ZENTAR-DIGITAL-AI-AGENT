...
def test_root_session_no_compression(db):
    ...
    assert prov["currentZentarSessionId"] == "root1"
    assert prov["rootZentarSessionId"] == "root1"
    assert prov["parentZentarSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_zentar_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentZentarSessionId"] == "old"
    assert prov["rootZentarSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousZentarSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootZentarSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootZentarSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_zentar_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousZentarSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"zentar"}
    assert "sessionProvenance" in meta["zentar"]
    assert meta["zentar"]["sessionProvenance"]["currentZentarSessionId"] == "root1"
