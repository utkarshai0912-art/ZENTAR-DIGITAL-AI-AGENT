...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_zentar_home_override(self, tmp_path):
        """Auth lookup should read from ZENTAR_HOME/auth.json, not ~/.zentar/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".zentar").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``zentar tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "zentar_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from zentar_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.zentar/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.zentar/.env), not bare os.getenv. Parametrized over the four
