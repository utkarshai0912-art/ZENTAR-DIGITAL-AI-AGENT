...
def _clean_state():
    ...
    saved = {}
    for k in ("ZENTAR_INTERACTIVE", "ZENTAR_GATEWAY_SESSION", "ZENTAR_EXEC_ASK", "ZENTAR_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZENTAR_INTERACTIVE"] = "1"
        session_key = os.getenv("ZENTAR_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZENTAR_INTERACTIVE or ZENTAR_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
