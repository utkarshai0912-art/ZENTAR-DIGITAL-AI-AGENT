...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_zentar_cli_expands_web(self):
        """zentar-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"zentar-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("zentar-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['zentar-cli'] yields ['web']."""
        parent_toolsets = {"zentar-cli"}
