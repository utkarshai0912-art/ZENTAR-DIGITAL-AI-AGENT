...
def _chown_zentar_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    zentar_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    zentar_home = target if zentar_home is None else zentar_home
    script = (
        "set -eu\n"
        f'ZENTAR_HOME="{zentar_home}"\n'
        f"{_chown_zentar_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_zentar_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "zentar-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        zentar_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked ZENTAR_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_zentar_home_trees(stage2_text: str) -> None:
    assert 'chown_zentar_tree "$ZENTAR_HOME/$sub"' in stage2_text
    assert 'chown_zentar_tree "$ZENTAR_HOME/profiles"' in stage2_text
    assert 'chown_zentar_tree "$ZENTAR_HOME/cron"' in stage2_text
    assert 'chown -R zentar:zentar "$ZENTAR_HOME/$sub"' not in stage2_text
    assert 'chown -R zentar:zentar "$ZENTAR_HOME/profiles"' not in stage2_text
    assert 'chown -R zentar:zentar "$ZENTAR_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_zentar_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$ZENTAR_HOME"' in stage2_text
