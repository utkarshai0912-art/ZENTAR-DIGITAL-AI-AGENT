...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zentar_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(zentar_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(zentar_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zentar_bin(self):
        """After _INSTALL_FAILED, manual install in ZENTAR_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zentar_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zentar_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(zentar_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zentar_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zentar_bin
            assert _tirith_mod._resolved_path == zentar_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ZENTAR_HOME isolation
# ---------------------------------------------------------------------------

class TestZentarHomeIsolation:
    def test_zentar_bin_dir_respects_zentar_home(self):
        """_zentar_bin_dir must use ZENTAR_HOME, not hardcoded ~/.zentar."""
        from tools.tirith_security import _zentar_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZENTAR_HOME": tmpdir}):
            result = _zentar_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zentar_home(self):
        """_failure_marker_path must use ZENTAR_HOME, not hardcoded ~/.zentar."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZENTAR_HOME": "/custom/zentar"}):
            result = _failure_marker_path()
        assert result == "/custom/zentar/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZENTAR_HOME; verify it's active."""
        zentar_home = os.getenv("ZENTAR_HOME")
        assert zentar_home is not None, "ZENTAR_HOME should be set by conftest"
        assert "zentar_test" in zentar_home, "Should point to test temp dir"

    def test_get_zentar_home_fallback(self):
        """Without ZENTAR_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_zentar_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZENTAR_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".zentar")
            result = _get_zentar_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._zentar_bin_dir",
