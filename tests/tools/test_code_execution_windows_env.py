...
``zentar_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``ZENTAR_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        "ZENTAR_HOME": "/home/alice/.zentar",        # allowlisted → kept
        "ZENTAR_PROFILE": "default",                 # allowlisted → kept
        "ZENTAR_INTERACTIVE": "1",                   # not allowlisted → dropped
        "ZENTAR_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "ZENTAR_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``zentar_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``zentar_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_zentar_tools_module
        stub = generate_zentar_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_zentar_tools_module
        import tempfile, ast
        stub = generate_zentar_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_zentar_tools_module(["terminal"], transport="uds")
...
# let the child import zentar_tools, a user script that printed non-ASCII
