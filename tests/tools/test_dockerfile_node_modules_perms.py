...
Older images made /opt/zentar/ui-tui and /opt/zentar/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ZENTAR_TUI_DIR=/opt/zentar/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R zentar:zentar /opt/zentar/ui-tui" not in text
    assert "chown -R zentar:zentar /opt/zentar/node_modules" not in text
