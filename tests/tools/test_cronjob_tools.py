...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `zentar-digital-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.zentar/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.zentar/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because zentar uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("ZENTAR_INTERACTIVE", raising=False)
        monkeypatch.delenv("ZENTAR_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("ZENTAR_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("ZENTAR_INTERACTIVE", false_like_value)
        monkeypatch.delenv("ZENTAR_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ZENTAR_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ZENTAR_INTERACTIVE", "ZENTAR_GATEWAY_SESSION", "ZENTAR_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ZENTAR_INTERACTIVE", "ZENTAR_GATEWAY_SESSION", "ZENTAR_EXEC_ASK"):
...
class TestUnifiedCronjobTool:
    ...
    def _patch_named_legit(monkeypatch):
        import zentar_cli.runtime_provider as rp
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import zentar_cli.runtime_provider as rp_mod
