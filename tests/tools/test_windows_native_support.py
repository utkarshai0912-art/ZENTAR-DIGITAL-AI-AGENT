...
code paths Zentar takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``zentar_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect ZENTAR_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("zentar_cli.stdio", None)
        # Fresh import now; tests import from zentar_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import zentar_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("zentar_cli.stdio", None)

    def test_no_op_on_posix(self):
        from zentar_cli import stdio
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("ZENTAR_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("ZENTAR_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("zentar_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "zentar_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, zentar_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "zentar_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call zentar_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """zentar_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from zentar_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from zentar_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``zentar update``
        (which runs under zentar-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# zentar_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "zentar_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the ZENTAR_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / zentar_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "zentar_cli/tools_config.py",
            "zentar_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_zentar_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_zentar_home" in source
        assert 'cache_dir = get_zentar_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_zentar_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "zentar_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "zentar_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        zentar_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "zentar_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from zentar_cli._subprocess_compat import" in block
    ...
    def test_watcher_rewrites_console_python_to_windowless(self):
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "ZENTAR_HOME) so the windowless base pythonw resolves zentar_cli."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import zentar_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "zentar_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from zentar_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import zentar_cli.config  # noqa: F401
        ...
        # Mock get_zentar_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            ...
            gw, "_stable_gateway_working_dir", return_value="C:/zentar"
        ), mock.patch(
            "zentar_cli.config.get_zentar_home", return_value="C:/zentar"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
