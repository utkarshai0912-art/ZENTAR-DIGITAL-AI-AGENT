...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        zentar_home.mkdir()
        (zentar_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(zentar_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.zentar/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.zentar/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        zentar_home = tmp_path / ".zentar"
        (zentar_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ZENTAR_HOME": str(zentar_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.zentar")

        assert mounts[0]["container_path"] == "/home/user/.zentar/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.zentar/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.zentar/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ZENTAR_HOME."""
        monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / ".zentar"))
        (tmp_path / ".zentar").mkdir()

        # Create a sensitive file one level above zentar_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside zentar_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ZENTAR_HOME must still be registered."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        (zentar_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_zentar_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ZENTAR_HOME must be allowed."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        subdir = zentar_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ZENTAR_HOME pointing outside must be rejected."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))

        # Create a sensitive file outside zentar_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside zentar_home pointing outside
        symlink = zentar_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ZENTAR_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, zentar_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ZENTAR_HOME."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
        ...
        sensitive.write_text("{}")
        self._write_config(zentar_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(zentar_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ZENTAR_HOME via config must still mount."""
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))

        (zentar_home / "oauth.json").write_text("{}")
        self._write_config(zentar_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        (zentar_home / "cache" / "documents").mkdir(parents=True)
        (zentar_home / "cache" / "audio").mkdir(parents=True)
        (zentar_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.zentar/cache/documents" in paths
        assert "/root/.zentar/cache/audio" in paths
        assert "/root/.zentar/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.zentar/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_zentar_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        zentar_home = tmp_path / ".zentar"
        zentar_home.mkdir()
        # Use legacy dir name with content — get_zentar_dir prefers
        # populated old over new.
        legacy_doc = zentar_home / "document_cache"
        legacy_img = zentar_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(zentar_home / "document_cache") in host_paths
        assert str(zentar_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.zentar/cache/documents" in container_paths
        assert "/root/.zentar/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        zentar_home = tmp_path / ".zentar"
        img_dir = zentar_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.zentar/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.zentar")
            == "/home/agent/.zentar/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        zentar_home = tmp_path / ".zentar"
        (zentar_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ZENTAR_HOME", str(zentar_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(zentar_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        doc_dir = zentar_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        zentar_home = tmp_path / ".zentar"
        ss_dir = zentar_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.zentar/cache/screenshots/session_abc/screen1.png"
