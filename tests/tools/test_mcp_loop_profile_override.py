"""Regression tests for ZENTAR_HOME override propagation onto the MCP loop.
...
get_zentar_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: ZentarTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(ZentarTokenStorage("probe-srv")._tokens_path())
