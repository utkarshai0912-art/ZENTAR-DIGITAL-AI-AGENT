...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("zentar_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The zentar-digital-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_zentar_tools(self):
        """Regression: zentar_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from zentar_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
    ...
    def test_strict_mode_can_still_import_zentar_tools(self):
