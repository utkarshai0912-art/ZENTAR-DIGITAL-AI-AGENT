...
def gw_session(monkeypatch):
    """A clean gateway session: ZENTAR_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_zentar_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ZENTAR_HOME": "/h", "ZENTAR_PROFILE": "p",
        "ZENTAR_CONFIG": "/c.yaml", "ZENTAR_ENV": "/e",
        # other HERMES_* → dropped (broad prefix removed)
        "ZENTAR_BASE_URL": "https://x", "ZENTAR_INTERACTIVE": "1",
        "ZENTAR_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ZENTAR_HOME", "ZENTAR_PROFILE", "ZENTAR_CONFIG", "ZENTAR_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ZENTAR_BASE_URL", "ZENTAR_INTERACTIVE", "ZENTAR_KANBAN_DB",
...
def test_env_scrub_logs_dropped_zentar_vars(caplog):
    ...
    env = {
        "ZENTAR_HOME": "/h",          # allowlisted → kept, not logged
        "ZENTAR_BASE_URL": "https://x",   # dropped → logged
        "ZENTAR_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ZENTAR_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ZENTAR_HOME" in out and "PATH" in out
    assert "ZENTAR_BASE_URL" not in out and "ZENTAR_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ZENTAR_BASE_URL" in msgs and "ZENTAR_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ZENTAR_API_KEY" not in msgs
