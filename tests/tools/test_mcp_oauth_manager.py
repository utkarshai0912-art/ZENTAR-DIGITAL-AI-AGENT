...
def test_zentar_provider_subclass_exists():
    """ZentarMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ZENTAR_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ZENTAR_PROVIDER_CLS is not None
    assert issubclass(_ZENTAR_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_zentar_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ZentarMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ZENTAR_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ZENTAR_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ZENTAR_PROVIDER_CLS is not None
    assert isinstance(provider, _ZENTAR_PROVIDER_CLS)
    assert provider._zentar_server_name == "srv"
