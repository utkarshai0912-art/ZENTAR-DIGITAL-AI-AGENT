...
def _restore_tool_modules():
    original_zentar_home = os.environ.get("ZENTAR_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_zentar_home is None:
            os.environ.pop("ZENTAR_HOME", None)
        else:
            os.environ["ZENTAR_HOME"] = original_zentar_home
        _reset_modules(("tools", "zentar_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    zentar_cli = types.ModuleType("zentar_cli")
    zentar_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zentar_cli"] = zentar_cli
    zentar_home = tmp_path / "zentar-home"
    os.environ["ZENTAR_HOME"] = str(zentar_home)
    sys.modules["zentar_cli.config"] = types.SimpleNamespace(
        get_zentar_home=lambda: zentar_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="zentar-digital-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zentar_home / "modal_snapshots.json",
