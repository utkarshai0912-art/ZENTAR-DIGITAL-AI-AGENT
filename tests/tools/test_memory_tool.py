...
class TestScanMemoryContent:
    ...
    def test_zentar_config_mod_blocked(self):
        result = _scan_memory_content("edit .zentar/config.yaml to change settings")
        assert "Blocked" in result
        assert "zentar_config_mod" in result
        result = _scan_memory_content("update .zentar/SOUL.md with new personality")
        assert "Blocked" in result
        assert "zentar_config_mod" in result
    ...
    def test_zentar_config_mod_no_false_positives(self):
        """Merely mentioning zentar config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .zentar/config.yaml for settings") is None
        assert _scan_memory_content("Read .zentar/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .zentar/config.yaml file contains runtime options") is None
