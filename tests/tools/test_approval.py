...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("zentar_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("zentar_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\zentar-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\zentar-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\zentar-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\zentar-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\zentar-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ZENTAR_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_zentar_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.zentar/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_zentar_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ZENTAR_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_zentar_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ZENTAR_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestZentarConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.zentar/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.zentar/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.zentar/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.zentar/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.zentar/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.zentar/config.yaml")
        assert dangerous is True
        assert "zentar config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.zentar/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_zentar_home_config(self):
    ...
    def test_sed_in_place_absolute_zentar_home_env(self):
        env_path = get_zentar_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_zentar_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ZENTAR_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.zentar/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_zentar_home_config(self):
        config_path = get_zentar_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.zentar/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_zentar_home_env(self):
        env_path = get_zentar_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.zentar/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.zentar/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.zentar/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.zentar/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.zentar/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Zentar config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_zentar_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ZENTAR_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.zentar/config.yaml/.env must require approval — the
    ...
    def test_cp_to_zentar_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.zentar/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Zentar-home prefixes must fold to ~/ and
    ~/.zentar/ in dangerous-command detection.
    ...
    SSH, and Zentar config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_zentar_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ZENTAR_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_zentar_home_config_folds(self, monkeypatch):
        # Zentar home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("ZENTAR_HOME", r"C:\Users\tester\.zentar")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.zentar\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.zentar/zentar-digital-agent && source venv/bin/activate && python -m zentar_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m zentar_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m zentar_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "zentar_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart zentar-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_zentar_gateway_stop_detected(self):
        cmd = "zentar gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_zentar_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `zentar` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "zentar -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_zentar_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "zentar --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_zentar_gateway_multiple_flags_detected(self):
        cmd = "zentar -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_zentar_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "zentar -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_zentar_gateway_start_not_flagged(self):
        cmd = "zentar gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_zentar_detected(self):
        """pkill targeting zentar/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_zentar_detected(self):
        cmd = "killall zentar"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Zentar security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.zentar/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep zentar) bypasses the pkill/killall name-matching
    ...
    def test_pkill_zentar_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof zentar)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof zentar_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof zentar`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Zentar service
    label achieves the same effect as `zentar gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_zentar_detected(self):
        cmd = "launchctl stop ai.zentar.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "zentar" in desc.lower()

    def test_launchctl_kickstart_zentar_detected(self):
        cmd = "launchctl kickstart -k system/ai.zentar.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_zentar_detected(self):
        cmd = "launchctl bootout system/ai.zentar.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_zentar_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.zentar.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Zentar label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ZENTAR_GATEWAY_SESSION", "ZENTAR_CRON_SESSION",
                      "ZENTAR_YOLO_MODE",
                      "ZENTAR_SESSION_KEY", "ZENTAR_INTERACTIVE")
        }
        os.environ.pop("ZENTAR_YOLO_MODE", None)
        os.environ.pop("ZENTAR_INTERACTIVE", None)
        # ZENTAR_CRON_SESSION takes priority over ZENTAR_GATEWAY_SESSION in
        ...
        os.environ.pop("ZENTAR_CRON_SESSION", None)
        os.environ["ZENTAR_GATEWAY_SESSION"] = "1"
        os.environ["ZENTAR_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("zentar_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ZENTAR_INTERACTIVE": "1"}, clear=False):
