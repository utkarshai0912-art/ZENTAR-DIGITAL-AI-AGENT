...
class TestWriteDenyExactPaths:
    ...
    def test_zentar_env(self):
        # ``.env`` under the active ZENTAR_HOME (profile-aware, not just
        # ``~/.zentar``) must be write-denied. The hermetic test conftest
        # points ZENTAR_HOME at a tempdir — resolve via get_zentar_home()
        # to match the denylist.
        from zentar_constants import get_zentar_home
        path = str(get_zentar_home() / ".env")
        assert _is_write_denied(path) is True

    def test_zentar_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.zentar/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "zentar_root"
        ...
        # Sanity check: ZENTAR_HOME does point to the profile dir, not the root.
        from zentar_constants import get_zentar_home, get_default_zentar_root
        assert get_zentar_home() == profile_home
        assert get_default_zentar_root() == root
...
class TestWriteAllowed:
    ...
    def test_zentar_control_files_requested_writable(self):
        from zentar_constants import get_zentar_home
