...
from zentar_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(zentar_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <zentar_home>/plugins/<name> and opt it in."""
    plugin_dir = zentar_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZENTAR_HOME", str(tmp_path / "zentar_empty"))
