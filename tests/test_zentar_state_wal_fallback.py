...
(SQLITE_PROTOCOL — typical on NFS/SMB), Zentar must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``ZENTAR_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    zentar_state._set_last_init_error(None)
    yield
    zentar_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    zentar_state._wal_fallback_warned_paths.clear()
    yield
    zentar_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see zentar_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        zentar_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("zentar_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        zentar_state._set_last_init_error("OperationalError: generic SQLite error")
