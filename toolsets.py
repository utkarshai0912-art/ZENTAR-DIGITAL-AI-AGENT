...
_ZENTAR_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on ZENTAR_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (ZENTAR_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `zentar tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ZENTAR_KANBAN_TASK env "
    ...
    # Coding posture (base Zentar — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in zentar_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Zentar toolsets (CLI + messaging platforms)
    #
    ...
    # the `zentar send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "zentar-cli": {
        ...
        "tools": _ZENTAR_CORE_TOOLS,
        "includes": []
    },

    "zentar-cron": {
        # Mirrors zentar-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `zentar tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as zentar-cli; gated by `zentar tools`",
        "tools": _ZENTAR_CORE_TOOLS,
        "includes": []
    },
    ...
    "zentar-email": {
        "description": "Email bot toolset - interact with Zentar via email (IMAP/SMTP)",
        "tools": _ZENTAR_CORE_TOOLS,
        "includes": []
    },
    ...
    "zentar-sms": {
        "description": "SMS bot toolset - interact with Zentar via SMS (Twilio)",
        "tools": _ZENTAR_CORE_TOOLS,
        "includes": []
    },
    ...
    "zentar-webhook": {
        ...
        "tools": _ZENTAR_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "zentar-gateway": {
        ...
        "tools": [],
        "includes": ["zentar-telegram", "zentar-discord", "zentar-whatsapp", "zentar-slack", "zentar-signal", "zentar-bluebubbles", "zentar-homeassistant", "zentar-email", "zentar-sms", "zentar-mattermost", "zentar-matrix", "zentar-dingtalk", "zentar-feishu", "zentar-wecom", "zentar-wecom-callback", "zentar-weixin", "zentar-qqbot", "zentar-webhook", "zentar-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``zentar-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ZENTAR_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``zentar-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (zentar-<name>).
        # Gives them _ZENTAR_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("zentar-"):
            platform_name = name[len("zentar-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ZENTAR_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
