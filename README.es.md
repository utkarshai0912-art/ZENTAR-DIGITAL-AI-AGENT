...
> **Nota:** En Windows nativo, Zentar funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/NousResearch/hermes-digital-agent/issues).
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\zentar\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). Zentar usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://hermes-agent.nousresearch.com/docs/getting-started/termux). En Termux, Zentar instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\zentar`; WSL2 instala en `~/.zentar` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
zentar              # ¡empieza a chatear!
```
...
```bash
zentar              # CLI interactiva — inicia una conversación
zentar model        # Elige tu proveedor y modelo LLM
zentar tools        # Configura qué herramientas están habilitadas
zentar config set   # Establece valores de configuración individuales
zentar gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
zentar setup        # Ejecuta el asistente de configuración completo
zentar claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
zentar update       # Actualiza a la última versión
zentar doctor       # Diagnostica cualquier problema
```
...
Zentar funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `zentar portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway).
...
Zentar tiene dos puntos de entrada: inicia la interfaz de terminal con `zentar`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `zentar`                                      | Ejecuta `zentar gateway setup` + `zentar gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, Zentar puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`zentar setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
zentar claw migrate              # Migración interactiva (preset completo)
zentar claw migrate --dry-run    # Vista previa de qué se migraría
zentar claw migrate --preset user-data   # Migrar sin secretos
zentar claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.zentar/skills/openclaw-imports/`
...
Consulta `zentar claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-zentar.sh`:

```bash
git clone https://github.com/NousResearch/hermes-digital-agent.git
cd zentar-digital-agent
./setup-zentar.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/zentar
./zentar              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para Zentar y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [ZentarClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta ZENTAR DIGITAL AI AGENT y OpenClaw en la misma cuenta de WeChat.
...
Creado por [ZENTAR DIGITAL](https://nousresearch.com).
